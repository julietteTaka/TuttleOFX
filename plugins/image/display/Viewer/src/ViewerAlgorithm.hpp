...
void loadNewTexture(const ImgProperties &properties) {
  // loading texture
  glTexImage2D(GL_TEXTURE_2D, 0, properties.internalFormat, properties.width,
               properties.height, 0, properties.format, properties.type,
               properties.data);
...
void loadNewTexture(const char *data, GLint internalFormat, size_t width,
...
void reloadTexture() { loadNewTexture(img); }
...
void displayChannelTexture(bool &channel, const float red, const float green,
                           const float blue) {
  ImgProperties p = img;
  if (!channel) {
...
void displayHelp() { std::cout << kViewerHelp << std::endl; }
...
void mouse(int button, int state, int x, int y) {
  ...
  if (state == 0 && button == 0) {
    ...
    std::cout << "at " << std::setw(4) << iX << "," << std::setw(4)
              << (int)img.height - iY << ": ";
