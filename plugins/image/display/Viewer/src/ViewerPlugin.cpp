...
ViewerProcessParams<ViewerPlugin::Scalar>
ViewerPlugin::getProcessParams(const OfxPointD &renderScale) const {
  ViewerProcessParams<Scalar> params;
  return params;
}

void ViewerPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                                const std::string &paramName) {}

bool ViewerPlugin::isIdentity(const OFX::RenderArguments &args,
...
void ViewerPlugin::render(const OFX::RenderArguments &args) {
  ...
  case OFX::eBitDepthNone:
    BOOST_THROW_EXCEPTION(
        ...
        << exception::user(
               "Viewer: Unable to compute custom or non bit depth"));
    break;
