...
struct Compute_alpha_pixel {
  ...
  Compute_alpha_pixel(bool isOutputBW, GeodesicForm &dataC, GeodesicForm &dataS)
      : _isOutputBW(isOutputBW), _dataColor(dataC), _dataSpill(dataS) {}
  ...
  template <typename Pixel> Pixel operator()(const Pixel &p) {
    ...
    if (_dataSpill.isIntoBoundingBox(testPoint)) // if current pixel is into
                                                 // spill bounding box
    {
      if (_dataColor.isIntoBoundingBox(testPoint)) // bounding box test (process
                                                   // optimization)
      {
        if (_dataColor.isPointIntoGeodesicForm(
                testPoint)) // if current pixel is into the color geodesic form
        {
      ...
      if (alpha != 1.0 && _dataSpill.isPointIntoGeodesicForm(testPoint)) {
        // intersections test
        if (_dataColor.testIntersection2(testPoint, false,
                                         true)) // normal intersection
        {
          ...
          // compute max vector
          vectMax.x = _dataSpill._intersectionPoint.x -
                      _dataColor._intersectionPoint.x; // x value
          vectMax.y = _dataSpill._intersectionPoint.y -
                      _dataColor._intersectionPoint.y; // x value
          vectMax.z = _dataSpill._intersectionPoint.z -
                      _dataColor._intersectionPoint.z; // x value
          // compute norms
    ...
    } else                       // output is alpha channel
    {
      for (unsigned int i = 0;
           i < boost::gil::num_channels<Pixel>::type::value - 1;
           ++i) // all RGB channels (not alpha)
