 ...
 * @param effect
 */
ColorCubeViewerOverlay::ColorCubeViewerOverlay(OfxInteractHandle handle,
  ...
  _rotateXForm = _rotateYForm =
      0.0; // initialize rotation centered to geodesic form to 0
  _origin.x = _origin.y = _end.x = _end.y = 0; // initialize mouse positions to
                                               // 0
  ...
  _plugin->changedParam(args, kPointCloudDisplay); // call changed parameters
...
void ColorCubeViewerOverlay::prepareOpenGLScene(const OFX::DrawArgs &args) {
  ...
  if (args.time != getData()._time ||
      args.time != getData()._averageColor._time) // if current overlay has not
                                                  // been updated
  {
  ...
  glOrtho(vleft, vright, vbottom, vtop, vnear,
          vfar); // define new frustrum for overlay data
  ...
  glLoadMatrixd(modelViewMatrix); // load modelView matrix (first time is equal
                                  // to identity)
}
...
bool ColorCubeViewerOverlay::draw(const OFX::DrawArgs &args) {
  ...
  if (_plugin->_paramBoolPointCloudDisplay->getValue()) // Is CloudPointData
                                                        // displayed ? (GUI)
  {
    ...
    if (_plugin->_resetViewParameters) // View parameters need to be reseted
    {
      ...
      _rotateXForm = _rotateYForm =
          0; // reset geodesic form center rotation parameters
    ...
    if (getData()._isSelectionVBOBuilt &&
        _plugin->_paramBoolSeeSelection->getValue()) // color selection VBO data
                                                     // is built
    {
    ...
    if (getData()._isSpillSelectionVBOBuilt &&
        _plugin->_paramBoolSeeSpillSelection->getValue()) // spill selection VBO
                                                          // data is built
    {
    ...
    if (_plugin->_paramBoolDisplayGeodesicForm->getValue()) // does user want to
                                                            // display color
                                                            // geodesic form
      getData()._geodesicFormColor.draw(
          false); // draw geodesic form on screen without alpha
    if (_plugin->_paramBoolDisplaySpillGF->getValue()) // does user want to
                                                       // display spill geodesic
                                                       // form
      getData()._geodesicFormSpill.draw(
          true); // draw spill geodesic form on screen with alpha
  ...
  return displaySomethings; // return if overlay has displayed something (y or
                            // n)
}
...
bool ColorCubeViewerOverlay::penDown(const OFX::PenArgs &args) {
  if (!_isPenDown) // is mouse is not already selected
  {
    ...
    _origin.x = _end.x =
        args.penPosition.x; // capture x position of current click
    _origin.y = _end.y =
        args.penPosition.y; // capture y position of current click
...
bool ColorCubeViewerOverlay::penMotion(const OFX::PenArgs &args) {
  if (_isPenDown) // if mouse is already selected
  {
    ...
    int deltaX =
        _end.x - _origin.x; // compute delta for rotation on Y axis (horizontal)
    int deltaY =
        _end.y - _origin.y; // compute delta for rotation on X axis (vertical)
    if (_isCtrlKeyDown) // rotation center is current color selection average
    {
      _rotateXForm =
          (deltaX / args.pixelScale.x) /
          kRotationSpeed; // add delta to geodesic center rotation (X axis)
      _rotateYForm =
          (deltaY / args.pixelScale.y) /
          kRotationSpeed; // add delta to geodesic center rotation (Y axis)

      // update model-View matrix
      Ofx3DPointD rotationCenter =
          getData()._geodesicFormColor._center; // get current rotation center
      updateModelView(rotationCenter); // update model-view
    } else // rotation center is reference center (0.5,0.5,0.5 in cube
           // reference)
    {
      _rotateX = (deltaX / args.pixelScale.x) /
                 kRotationSpeed; // add delta to cube rotation (X axis)
      _rotateY = (deltaY / args.pixelScale.y) /
                 kRotationSpeed; // add delta to cube rotation (Y axis)
...
CloudPointData &ColorCubeViewerOverlay::getData() {
  return _plugin->getCloudPointData(); // return CloudPointData initialized at
                                       // constructor (by scoped pointer)
}
 ...
 */
void
ColorCubeViewerOverlay::updateModelView(const Ofx3DPointD &rotationCenter) {
  ...
  if (rotationCenter.x == 0.5 && rotationCenter.y == 0.5 &&
    ...
    newTransformationMatrix =
        constructRotationMatrix(rotationCenter, _rotateY, _rotateX,
                                0); // construct new transformation matrix
  else // current rotation center is geodesic form center
    newTransformationMatrix =
        constructRotationMatrix(rotationCenter, _rotateYForm, _rotateXForm,
                                0); // construct new transformation matrix
  // X and Y angles are inverted (better to control view)
  // Add current transformation to model-view matrix
  multMatrixBtoMatrixA(_modelViewMatrix,
                       newTransformationMatrix); // add current transformation
 ...
 */
void ColorCubeViewerOverlay::drawWarning(const Ofx3DPointD &centerPoint,
                                         const double ratio) {
