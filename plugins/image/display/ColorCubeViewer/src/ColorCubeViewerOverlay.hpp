...
private:
  /*OpenGL scene*/
  void prepareOpenGLScene(const OFX::DrawArgs &args); // prepare the frustrum
                                                      // and projection settings
                                                      // and initialize first
                                                      // VBO
  void drawAxes(); // draw X, Y and Z axes on screen
  void updateModelView(const Ofx3DPointD &rotationCenter); // update modelView
                                                           // with (by adding a
                                                           // new
                                                           // transformation)
  void drawWarning(const Ofx3DPointD &centerPoint,
                   const double ratio); // draw a warning sign on the openGL
                                        // scene
