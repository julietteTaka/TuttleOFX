...
struct ComputeAverage {
  ...
  CPixel operator()(const View &image) {
    ...
    if (nbPixels != 0) // if any selected pixel has been added
      sum = pixel_divides_scalar_t<CPixel, double>()(
          sum, nbPixels); // compute average average (sum/nbPixels)
...
struct Pixel_extend_GeodesicForm {
  ...
  // Constructor
  Pixel_extend_GeodesicForm(GeodesicForm &data) : _data(data) {}
  // Operator ()
  template <typename Pixel> Pixel operator()(const Pixel &p) {
    ...
    if (p[3] == 1) // current pixel is in the selection
    {
      ...
      // extend geodesicForm
      _data.extendOnePoint(point); // extends geodesic form (point is already
                                   // into geodesic form checked)
    }
    return p;
  }
};
...
public:
  ...
  bool computeAverageSelection(OFX::Clip *clipColor,
                               const OfxPointD &renderScale);

  // Extends a geodesic form with the selection
  void extendGeodesicForm(OFX::Clip *clipColor, const OfxPointD &renderScale,
