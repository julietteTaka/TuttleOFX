...
public:
  ...
  Ofx3DPointD _intersectionPoint; // point of intersection between ray and
                                  // geodesic form
...
public:
  ...
  // Intersection test
  void testIntersection(const Ofx3DPointD &testPoint); // intersection (Moller)
                                                       // test between a ray and
                                                       // the geodesic form
  bool testIntersection2(const Ofx3DPointD &testPoint,
                         const bool &inverse = false,
                         const bool justIntersectionPoint =
                             false); // intersection (Dan Sunday) test between a
                                     // ray and the geodesic form
  bool isPointIntoGeodesicForm(const Ofx3DPointD &testPoint); // return if
                                                              // testPoint is
                                                              // contained into
                                                              // GeodesicForm

  // Extend geodesicForm
  void extendOnePoint(Ofx3DPointD &testPoint); // extend the geodesic form to
                                               // contain testPoint

  // Bounding box
  void updateBoundingBox(); // if points vector has changed : update bounding
                            // box
  bool isIntoBoundingBox(const Ofx3DPointD &testPoint); // is testPoint into
                                                        // bounding box
...
private:
  ...
  void subdiviseOneFace(PyramidTriangle &f,
                        const int divisor); // sub-divise one face
  // create and add point of one subdivised face
  void createPointsOneFace(const PyramidTriangle &f,
                           const int divisor); // create all of needed point on
                                               // one face
  // compute intersection between a point and a triangle
  bool getIntersection(const Ofx3DPointD &point,
                       const Triangle &triangle); // intersection Moller
  bool getIntersection2(const Ofx3DPointD &point,
                        const PyramidTriangle &pyramidTriangle,
                        Ofx3DPointD &intersectionPoint, double &s, double &t,
                        const bool &inverse = false); // intersection Dan Sunday
  // transform one point to sphere
  void currentPointToSphere(Ofx3DPointD &point,
                            const double &radius); // transform double pyramid
                                                   // to sphere
  // Extends one point
  bool extendsOneTrianglePoint(
      Ofx3DPointD &pointToMove, const double *testPointCenterVector,
      const double normTestPointCenterVector); // Extends one point of the
                                               // intersected triangle (call 3
                                               // times - triangle)
  // update Bounding box
  void updateBoundingBox(const Ofx3DPointD &testPoint); // update bounding box
                                                        // (compare with one
                                                        // point)
                                                        // //test overlay
};
