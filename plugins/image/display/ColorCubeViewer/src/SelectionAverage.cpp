...
void SelectionAverage::draw() {
  ...
  glBegin(GL_LINES);
  glColor3f(complementaryColor[0], complementaryColor[1],
            complementaryColor[2]); // color : complementary to average
  glVertex3f(xBefore, _averageValue.y, _averageValue.z);
  glVertex3f(xAfter, _averageValue.y, _averageValue.z); // X axis
  glVertex3f(_averageValue.x, yBefore, _averageValue.z);
  glVertex3f(_averageValue.x, yAfter, _averageValue.z); // Y axis
  glVertex3f(_averageValue.x, _averageValue.y, zBefore);
  glVertex3f(_averageValue.x, _averageValue.y, zAfter); // Z axis
  glEnd();
}
 ...
 */
bool SelectionAverage::computeAverageSelection(OFX::Clip *clipColor,
  ...
  SView colorView = tuttle::plugin::getGilView<SView>(
      src.get(), srcPixelRod,
      eImageOrientationIndependant); // get current view from color clip
  ComputeAverage<SView>::CPixel average =
      ComputeAverage<SView>()(colorView); // compute color clip average
 ...
 */
void SelectionAverage::extendGeodesicForm(OFX::Clip *clipColor,
                                          const OfxPointD &renderScale,
  ...
  // TUTTLE_TLOG_VAR( TUTTLE_INFO, clipColor->getCanonicalRod(_time,
  // renderScale));
  ...
  SView colorView = tuttle::plugin::getGilView<SView>(
      src.get(), srcPixelRod,
      eImageOrientationIndependant); // get current view from color clip
  Pixel_extend_GeodesicForm funct(geodesicForm); // functor declaration
                                                 // //initialize functor
  terry::algorithm::transform_pixels(colorView,
                                     funct); // with functor reference;
}
}
}
}
