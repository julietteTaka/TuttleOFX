 ...
 */
void ColorCubeViewerPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  // describe the plugin
  desc.setLabels("TuttleColorCubeViewer", "ColorCubeViewer", "ColorCubeViewer");
  ...
  // Don't declare the plugin as fully safe: It's not a safety problem, but we
  // need to compute common data.
  ...
  desc.setOverlayInteractDescriptor(
      new OFX::DefaultEffectOverlayWrap<ColorCubeViewerOverlayDescriptor>());
}
 ...
 */
void ColorCubeViewerPluginFactory::describeInContext(
  ...
  OFX::ClipDescriptor *strongSelectionClip =
      desc.defineClip(kClipColorSelection);
  ...
  OFX::ClipDescriptor *spillSelectionClip =
      desc.defineClip(kClipSpillSelection);
  ...
  // Group display
  OFX::GroupParamDescriptor *groupDisplay =
      desc.defineGroupParam(kGroupDisplay);
  groupDisplay->setLabel(kGroupDisplayLabel);
  groupDisplay->setAsTab(); // current group is a tab

  // Group settings
  OFX::GroupParamDescriptor *groupSettings =
      desc.defineGroupParam(kGroupSettings);
  groupSettings->setLabel(kGroupSettingsLabel); // add label
  groupSettings->setAsTab(); // current group is a tab

  // Group process
  OFX::GroupParamDescriptor *groupProcess =
      desc.defineGroupParam(kGroupProcess);
  groupProcess->setLabel(kGroupProcessLabel); // add label
  groupProcess->setAsTab(); // current group is a tab

  // Check-box display
  OFX::BooleanParamDescriptor *pointCloudDisplay =
      desc.defineBooleanParam(kPointCloudDisplay);
  ...
  OFX::BooleanParamDescriptor *pointCloudDiscretization =
      desc.defineBooleanParam(kBoolDiscretizationDisplay);
  ...
  pointCloudDiscretization->setLayoutHint(
      OFX::eLayoutHintNoNewLine); // line is not finished
  pointCloudDiscretization->setEnabled(
      false); // Disabled by default (display cloud point is not selected)
  pointCloudDiscretization->setHint("Activate discretization point cloud.");
  pointCloudDiscretization->setEvaluateOnChange(false); // Don't render on
                                                        // change
  pointCloudDiscretization->setParent(
      groupDisplay); // add component to group Display

  // Discretization int range
  OFX::IntParamDescriptor *discretizationDisplay =
      desc.defineIntParam(kIntDiscretizationDisplay);
  ...
  discretizationDisplay->setEnabled(
      false); // Disabled by default (display cloud point is not selected)
  ...
  discretizationDisplay->setParent(
      groupDisplay); // add component to group display
  ...
  resetTransformationParameters->setLabel(
      kPushButtonResetTransformationParametersLabel); // label
  resetTransformationParameters->setHint("Reset view parameters"); // help
  resetTransformationParameters->setParent(
      groupDisplay); // add component to group display

  // Number of divison Geodesic form
  OFX::IntParamDescriptor *nbDivisionsGeodesicForm =
      desc.defineIntParam(kIntNumberOfDivisonGeodesicForm);
  nbDivisionsGeodesicForm->setLabel(
      kIntNumberOfDivisonGeodesicFormLabel); // label
  ...
  nbDivisionsGeodesicForm->setHint(
      "Change precision of treatment (can make process slower)"); // help
  nbDivisionsGeodesicForm->setParent(
      groupSettings); // add component to settings group

  // Selection average mode
  OFX::ChoiceParamDescriptor *averageMode =
      desc.defineChoiceParam(kColorAverageMode);
  averageMode->setLabel(kColorAverageModeLabel);
  averageMode->setHint("Average mode :\n -automatic : average of color "
                       "selection \n -manual : choose average separately from "
                       "selection");
  ...
  OFX::RGBAParamDescriptor *colorAverage =
      desc.defineRGBAParam(kColorAverageSelection);
  ...
  colorAverage->setLayoutHint(OFX::eLayoutHintNoNewLine); // line is not
                                                          // finished
  colorAverage->setParent(groupSettings); // add to settings group

  // Selection average compute
  OFX::PushButtonParamDescriptor *colorAverageCompute =
      desc.definePushButtonParam(kColorAverageComputing);
  ...
  OFX::BooleanParamDescriptor *onlySelectionColor =
      desc.defineBooleanParam(kBoolOnlySelection);
  ...
  displaySpillGF->setEvaluateOnChange(
      false); // don't need to recompute on change
  ...
  OFX::BooleanParamDescriptor *seeSelection =
      desc.defineBooleanParam(kBoolColorSelectionDisplay);
  ...
  seeSelection->setLayoutHint(OFX::eLayoutHintNoNewLine); // line is not
                                                          // finished
  seeSelection->setParent(groupDisplay); // add to display group

  // Check box see spill selection
  OFX::BooleanParamDescriptor *seeSpillSelection =
      desc.defineBooleanParam(kBoolSpillSelectionDisplay);
  ...
  seeSpillSelection->setEvaluateOnChange(
      false); // don't need to recompute on change
  ...
  OFX::DoubleParamDescriptor *scaleGF =
      desc.defineDoubleParam(kDoubleScaleGeodesicForm);
 ...
 */
OFX::ImageEffect *
ColorCubeViewerPluginFactory::createInstance(OfxImageEffectHandle handle,
                                             OFX::EContext context) {
  return new ColorCubeViewerPlugin(handle);
}
}
}
}
