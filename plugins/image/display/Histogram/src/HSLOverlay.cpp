...
HSLOverlay::HSLOverlay(OfxInteractHandle handle, OFX::ImageEffect*
...
/*Destructor*/
HSLOverlay::~HSLOverlay() { _plugin->releaseOverlayData(); }
...
bool HSLOverlay::draw(const OFX::DrawArgs &args) {
  ...
  if (!(_plugin->_paramDisplayTypeSelection->getValue() == 1)) {
    ...
    const Number maxH =
        *(std::max_element(getOverlayData()._data._bufferHue.begin(),
                           getOverlayData()._data._bufferHue.end()));
    const Number maxS =
        *(std::max_element(getOverlayData()._data._bufferSaturation.begin(),
                           getOverlayData()._data._bufferSaturation.end()));
    const Number maxL =
        *(std::max_element(getOverlayData()._data._bufferLightness.begin(),
  ...
  float selectionMultiplier =
      (float)_plugin->_paramSelectionMultiplierSelection->getValue();
  ...
  if (_plugin->_paramOverlayHSelection->getValue()) // HUE CHANNEL
  {
    if (!hasGridBeenDisplayed) // if grid has not been displayed yet
    {
      ...
      hasGridBeenDisplayed = true; // set grid has already been displayed to
                                   // true
    }
    displayASpecificHistogram(getOverlayData()._data._bufferHue,
                              getOverlayData()._selectionData._bufferHue, step,
                              heightH, size.x, redHisto, selectionMultiplier);
    if (getOnlyChannelSelectedHSL() == eSelectedChannelH) {
      displaySelectionPoints(getOverlayData()._selectionData._bufferHue, step,
                             size.x, redHisto); // selection points
      displayAverageBar(getOverlayData()._averageData._averageHue, averageHisto,
  ...
  if (_plugin->_paramOverlaySSelection->getValue()) // SATURATION CHANNEL
  {
    ...
    displayASpecificHistogram(getOverlayData()._data._bufferSaturation,
                              getOverlayData()._selectionData._bufferSaturation,
                              step, heightS, size.x, greenHisto,
                              selectionMultiplier);
    if (getOnlyChannelSelectedHSL() == eSelectedChannelS) {
      displaySelectionPoints(getOverlayData()._selectionData._bufferSaturation,
                             step, size.x, greenHisto); // selection points
      displayAverageBar(getOverlayData()._averageData._averageSaturation,
  ...
  if (_plugin->_paramOverlayLSelection->getValue()) // LIGHTNESS CHANNEL
  {
    ...
    displayASpecificHistogram(getOverlayData()._data._bufferLightness,
                              getOverlayData()._selectionData._bufferLightness,
                              step, heightL, size.x, blueHisto,
                              selectionMultiplier);
    if (getOnlyChannelSelectedHSL() == eSelectedChannelL) {
      displaySelectionPoints(getOverlayData()._selectionData._bufferLightness,
                             step, size.x, blueHisto); // selection points
      displayAverageBar(getOverlayData()._averageData._averageLightness,
                        averageHisto, size.x, size.y, step); // average bar
  ...
  if (_plugin->_paramOverlayHSelection->getValue())
    displayASpecificHistogramBorder(getOverlayData()._data._bufferHue, step,
                                    heightH, size.x, redHisto); // H
  if (_plugin->_paramOverlaySSelection->getValue())
    displayASpecificHistogramBorder(getOverlayData()._data._bufferSaturation,
                                    step, heightS, size.x, greenHisto); // S
  if (_plugin->_paramOverlayLSelection->getValue())
    displayASpecificHistogramBorder(getOverlayData()._data._bufferLightness,
                                    step, heightL, size.x, blueHisto); // L
  return true;
}
 ...
 * @return
 */
OverlayData &HSLOverlay::getOverlayData() { return _plugin->getOverlayData(); }
