...
public:
  ...
  bool draw(const OFX::DrawArgs &args); // draw all of the component (main
                                        // display)
  ...
  void displaySelectedAreas(const OfxPointI &fullImgSize,
                            const OfxPointI &imgSize,
                            const OfxRectI &pixelRoD); // display the selected
                                                       // areas (texture)
  void displaySelectionZone(); // display the current selection zone (white
                               // square)
...
private:
  void drawWarning(const Ofx3DPointD &centerPoint,
                   const double ratio); // draw a warning signal
};
