#include "HistogramAlgorithm.hpp"
...
template <class View>
HistogramProcess<View>::HistogramProcess(HistogramPlugin &effect)
    : ImageGilFilterProcessor<View>(effect, eImageOrientationIndependant),
      _plugin(effect) {}

template <class View>
void HistogramProcess<View>::setup(const OFX::RenderArguments &args) {
 ...
 */
template <class View>
void HistogramProcess<View>::multiThreadProcessImages(
  ...
  funct._isOutputBW = (_params._paramOutputSetting->getValue() ==
                       1); // is output black and white (or alpha channel)
