...
OverlayData::OverlayData(const OfxPointI &size, const int nbSteps,
    ...
    : _currentTime(0), _vNbStep(nbSteps),
      _vNbStepCurveFromSelection(nbStepsCurvesFromSelection),
      _isComputing(false), _isDataInvalid(true), _size(size) {
  clearAll(size);
}
 ...
 */
void OverlayData::computeHistogramBufferData(HistogramBufferData &data,
                                             SView &srcView, const OfxTime time,
  ...
  Pixel_compute_histograms funct(_imgBool, data,
                                 isSelection); // functor declaration
 ...
 */
void OverlayData::resetVectortoZero(HistogramVector &v,
                                    const std::size_t numberOfStep) const {
 ...
 */
void
OverlayData::correctHistogramBufferData(HistogramBufferData &toCorrect) const {
  // RGB
...
void OverlayData::computeAverages() {
  // RGB
  this->_averageData._averageRed =
      computeAnAverage(this->_selectionData._bufferRed); // R
  this->_averageData._averageBlue =
      computeAnAverage(this->_selectionData._bufferBlue); // G
  this->_averageData._averageGreen =
      computeAnAverage(this->_selectionData._bufferGreen); // B
  // HSL
  this->_averageData._averageHue =
      computeAnAverage(this->_selectionData._bufferHue); // H
  this->_averageData._averageSaturation =
      computeAnAverage(this->_selectionData._bufferSaturation); // S
  this->_averageData._averageLightness =
      computeAnAverage(this->_selectionData._bufferLightness); // L
}
 ...
 */
void OverlayData::computeFullData(OFX::Clip *clipSrc, const OfxTime time,
  ...
  // TUTTLE_TLOG_INFOS;
  // TUTTLE_TLOG_VAR( TUTTLE_INFO, "computeHistogramBufferData - fetchImage " <<
  // time );
  boost::scoped_ptr<OFX::Image> src(clipSrc->fetchImage(
      time,
      clipSrc->getCanonicalRod(time))); // scoped pointer of current source clip
  // TUTTLE_TLOG_INFOS;
  ...
  OfxRectI srcPixelRod =
      clipSrc->getPixelRod(time, renderScale); // get current RoD
...
bool OverlayData::isImageSizeModified(const OfxPointI &imgSize) const {
  return (_size.x != imgSize.x || _size.y != imgSize.y);
}
...
/**
 * Compute only curve from selection data (averages,histograms buffer and
 * selection buffer)
 * @param clipSrc	source of the plugin
 * @param time	current time
 * @param renderScale	current renderScale
 */
void OverlayData::computeCurveFromSelectionData(OFX::Clip *clipSrc,
                                                const OfxTime time,
                                                const OfxPointD &renderScale) {
  _isComputing = true;
  ...
  boost::scoped_ptr<OFX::Image> src(clipSrc->fetchImage(
      _currentTime, clipSrc->getCanonicalRod(_currentTime))); // scoped pointer
                                                              // of current
                                                              // source clip
  ...
  OfxRectI srcPixelRod =
      clipSrc->getPixelRod(_currentTime, renderScale); // get current RoD
  if ((clipSrc->getPixelDepth() != OFX::eBitDepthFloat) ||
      (!clipSrc->getPixelComponents())) {
    BOOST_THROW_EXCEPTION(exception::Unsupported()
                          << exception::user() + "Can't compute histogram data "
  ...
  // Compute histogram buffer
  Pixel_compute_histograms funct(_imgBool, _curveFromSelection,
                                 true); // functor declaration
  terry::algorithm::transform_pixels(srcView, funct); //(USED functor reference)

  this->correctHistogramBufferData(_curveFromSelection); // correct Histogram
                                                         // data to make up for
                                                         // discretization
                                                         // (average)
}
