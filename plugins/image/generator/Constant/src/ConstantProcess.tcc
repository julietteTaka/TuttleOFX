...
template <class View> ConstantParams<View> ConstantProcess<View>::getParams() {
...
template <class View>
void
ConstantProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  using namespace boost::gil;
  ...
  terry::algorithm::transform_pixels_progress(
      dst, terry::numeric::pixel_assigns_color_t<Pixel>(_params._color),
      this->getOfxProgress());
}
}
}
