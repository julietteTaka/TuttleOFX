...
void ColorBarsProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  ColorBarsVirtualView colorbars(
      ...
      Locator(Point(0, 0), Point(1, 1),
              ColorBarsFunctorT(dims.x, params.mode == eColorBarsLevels75)));
  // create a subview depending on the image ratio
  _srcView = subimage_view<>(colorbars, 0, 0, boost::numeric_cast<int>(dims.x),
 ...
 */
template <class View>
void ColorBarsProcess<View>::multiThreadProcessImages(
  ...
  for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
    typename ColorBarsVirtualView::x_iterator src_it =
