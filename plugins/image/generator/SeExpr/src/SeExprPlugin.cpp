...
SeExprProcessParams<SeExprPlugin::Scalar>
SeExprPlugin::getProcessParams(const OfxPointD &renderScale) const {
  ...
  case eParamChooseInputFile: {
    try {
      ...
      // BOOST_THROW_EXCEPTION( exception::File() << "unable to read file " +
      // _paramFile->getValue() );
    }
    break;
  }
...
void SeExprPlugin::changedParam(const OFX::InstanceChangedArgs &args,
