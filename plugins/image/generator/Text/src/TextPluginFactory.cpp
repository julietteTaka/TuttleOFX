...
void TextPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::BooleanParamDescriptor *isExpression =
      desc.defineBooleanParam(kParamIsExpression);
  isExpression->setLabel("Expression");
  isExpression->setHint(
      "If you check this parameter the text must be a python code.\n"
  ...
  fontFile->setHint("When a font file path is fill, font & bold and italic "
  ...
  FcChar8 *s;
  FcPattern *p = FcPatternBuild(NULL, FC_WEIGHT, FcTypeInteger, FC_WEIGHT_BOLD,
                                FC_SLANT, FcTypeInteger, FC_SLANT_ITALIC, NULL);
  ...
  /*
          OFX::DoubleParamDescriptor* ratio = desc.defineDoubleParam(
     kParamRatio );
  ...
  */
  ...
  OFX::RGBAParamDescriptor *backgroundColor =
      desc.defineRGBAParam(kParamBackgroundColor);
  backgroundColor->setLabel("Background Color");
  backgroundColor->setDefault(0.0, 0.0, 0.0, 0.0);

  OFX::Double2DParamDescriptor *position =
      desc.defineDouble2DParam(kParamPosition);
  position->setLabel("Position");
  position->setDefault(0.0, 0.0);

  OFX::DoubleParamDescriptor *letterSpacing =
  ...
  OFX::BooleanParamDescriptor *verticalFlip =
      desc.defineBooleanParam(kParamVerticalFlip);
  verticalFlip->setLabel("Vertical flip");
  verticalFlip->setDefault(false);
  verticalFlip->setAnimates(false);
  verticalFlip->setHint("Some hosts use inverted images, so you can correct "
                        "this problem using this flag.");
  ...
  OFX::ChoiceParamDescriptor *mergeFunction =
      desc.defineChoiceParam(kParamFunction);
  mergeFunction->setLabels(kParamFunctionLabel, kParamFunctionLabel,
                           kParamFunctionLabel);
  mergeFunction->appendOption("atop", "atop: Ab+B(1-a)");
  mergeFunction->appendOption("average", "average: (A+B)/2");
  mergeFunction->appendOption(
      "color", "color: hue from B, saturation from B, lightness from A");
  mergeFunction->appendOption("color-burn", "color-burn: darken B towards A");
  mergeFunction->appendOption("color dodge inversed",
                              "color dodge inversed: brighten B towards A");
  mergeFunction->appendOption("conjoint-over",
  ...
  mergeFunction->appendOption("disjoint-over",
  ...
  mergeFunction->appendOption(
      "hard-light", "hard-light: multiply if A < 0.5, screen if A > 0.5");
  mergeFunction->appendOption("hypot", "hypot: sqrt(A*A+B*B)");
  mergeFunction->appendOption("in", "in: Ab");
  mergeFunction->appendOption(
      "interpolated", "interpolated: (like average but better and slower)");
  mergeFunction->appendOption("mask", "mask: Ba");
  mergeFunction->appendOption("matte",
  ...
  mergeFunction->appendOption("overlay",
