...
RampProcessParams<View> RampProcess<View>::getProcessParams() const {
  ...
  color_convert(rgba32f_pixel_t(cStart.r, cStart.g, cStart.b, cStart.a),
                params.colorStart);
  OfxRGBAColourD cEnd = _plugin._colorEnd->getValue();
  color_convert(rgba32f_pixel_t(cEnd.r, cEnd.g, cEnd.b, cEnd.a),
                params.colorEnd);
...
void RampProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  // create a ramp view
  RampVirtualView checker(
      dims, Locator(Point(0, 0), Point(1, 1),
                    RampFunctorT(dims, params.colorStart, params.colorEnd,
                                 params.direction, params.color)));
  // create a subview depending on the image ratio
  _srcView =
      subimage_view<>(checker, 0.0, 0.0, boost::numeric_cast<int>(dims.x),
...
template <class View>
void
RampProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  ...
  for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
    typename RampVirtualView::x_iterator src_it =
