...
public:
  LineEquation() {}
  LineEquation(const point_t &A, const point_t &B) : _AB(B - A) {
  ...
  template <class Point> Point pointOrthogonalProjection(const Point &p) const {
    // compute coordinates of M, the orthogonal projection of p on current line
    Point res;

    res.y = (-(_a * p.x) - (_a * _AB.y * p.y / _AB.x) - _c) /
            (_b - (_a * _AB.y / _AB.x));
...
public:
  ...
  ColorGrandient1DLinearFunctor(const point_t &size,
                                const std::vector<point_t> &points,
                                const std::vector<value_type> &colors)
      : _points(points), _colors(colors) {
  ...
  double distance(const point_t &a, const point_t &b) const {
    return std::sqrt(boost::math::pow<2>(b.x - a.x) +
                     boost::math::pow<2>(b.y - a.y));
  }
  ...
  result_type operator()(const point_t &pA) const {
    ...
    return pixel_plus_t<result_type, result_type, result_type>()(
        pixel_multiplies_scalar_t<result_type, double, result_type>()(
            _colors[0], distB),
        pixel_multiplies_scalar_t<result_type, double, result_type>()(
            _colors[1], distC));
  }
