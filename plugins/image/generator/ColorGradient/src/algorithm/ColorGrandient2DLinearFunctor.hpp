...
public:
  ...
  ColorGrandient2DLinearFunctor(const point_t &size,
...
private:
  ...
  void computeDistances(const point_t &p) const {
    double sum = 0;
    DistancesVector::iterator it_dist = _distances.begin(),
                              itEnd_dist = _distances.end();
...
public:
  result_type operator()(const point_t &p) const {
    ...
    typename ColorsVector::const_iterator it_color = _colors.begin(),
                                          itEnd_color = _colors.end();
    ...
    for (; it_color != itEnd_color; ++it_color, ++it_dist) {
      ...
      pixel_plus_assign_t<result_type, result_type>()(
          pixel_multiplies_scalar_t<result_type, Scalar, result_type>()(
              *it_color, *it_dist),
          outputColor);
    }
    return outputColor;
