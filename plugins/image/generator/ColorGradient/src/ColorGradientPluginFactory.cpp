 ...
 */
void
ColorGradientPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
                                              OFX::EContext context) {
  describeGeneratorParamsInContext(desc, context);

  OFX::ChoiceParamDescriptor *gradientType =
      desc.defineChoiceParam(kGradientType);
  ...
  for (unsigned int i = 0; i < kMaxNbPoints; ++i) {
    OFX::Double2DParamDescriptor *point =
        desc.defineDouble2DParam(getPointParamName(i));
    point->setLabel(getPointParamName(i));
    //		point->setIsSecret( true );
    point->setDoubleType(OFX::eDoubleTypeNormalisedXYAbsolute);
    OFX::RGBAParamDescriptor *color =
        desc.defineRGBAParam(getColorParamName(i));
 ...
 */
OFX::ImageEffect *
ColorGradientPluginFactory::createInstance(OfxImageEffectHandle handle,
                                           OFX::EContext context) {
