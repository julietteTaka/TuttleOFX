...
ColorGradientPlugin::ColorGradientPlugin(OfxImageEffectHandle handle)
  ...
  changedParam(args,
               kNbPoints); // init IsSecret property for each point/color param
}
...
ColorGradientProcessParams<View> ColorGradientPlugin::getProcessParams() const {
  ...
  unsigned int nbPoints =
      boost::numeric_cast<unsigned int>(_nbPoints->getValue());
...
void ColorGradientPlugin::render(const OFX::RenderArguments &args) {
  ...
  case eGradientType1DRadial: {
    renderFunctor<ColorGrandient1DLinearFunctor>(
        args); /// @todo tuttle: not implemented yet
    return;
  }
...
void ColorGradientPlugin::getClipPreferences(
    OFX::ClipPreferencesSetter &clipPreferences) {
  GeneratorPlugin::getClipPreferences(clipPreferences);
}

void ColorGradientPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kNbPoints) {
    ...
    for (unsigned int i = nbPoints; i < kMaxNbPoints;
         ++i, ++it_point, ++it_color) {
  ...
  else if( boost::starts_with( paramName, kPoint ) )
  {
          try
          {
                  unsigned int n            = boost::lexical_cast<unsigned int>(
  paramName.c_str() + kPoint.size() );
