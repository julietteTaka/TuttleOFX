...
ColorGradientProcess<View, ColorGradientFunctor>::ColorGradientProcess(
    ColorGradientPlugin &instance)
    : ImageGilProcessor<View>(instance, eImageOrientationFromBottomToTop),
      _plugin(instance) {}

template <class View, template <typename> class ColorGradientFunctor>
void ColorGradientProcess<View, ColorGradientFunctor>::setup(
  ...
  ColorGradientVirtualView colorGradient(
      Point(dims.x, dims.x),
      Locator(Point(0, 0), Point(1, 1),
              ColorGradientFunctorT(dims, params._points, params._colors)));
  // create a subview depending on the image ratio
  _srcView = subimage_view<>(colorGradient, 0, yshift,
 ...
 */
template <class View, template <typename> class ColorGradientFunctor>
void ColorGradientProcess<View, ColorGradientFunctor>::multiThreadProcessImages(
  ...
  for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
    typename ColorGradientVirtualView::x_iterator src_it =
