...
void ColorCubeProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  gil_function_requires<
      StepIteratorConcept<typename ColorCubeLocator::x_iterator> >();
  ...
  ColorCubeVirtualView colorCube(
      ColorCubePoint(size, size),
      ColorCubeLocator(ColorCubePoint(0, 0), ColorCubePoint(1, 1),
                       ColorCubeFunctorT(tileSize, _params.step)));
  // create a subview depending on the image ratio
  _srcColorCubeView = subimage_view<>(colorCube, xshift, yshift,
 ...
 */
template <class View>
void ColorCubeProcess<View>::multiThreadProcessImages(
  ...
  for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
    typename ColorCubeVirtualView::x_iterator src_it =
        this->_srcColorCubeView.x_at(procWindowOutput.x1, y);
