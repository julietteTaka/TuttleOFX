...
void CheckerboardProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  CheckerboardVirtualView checker(
      ...
      Locator(Point(0, 0), Point(1, 1),
              CheckerboardFunctorT(tileSize, params._color1, params._color2)));
  // create a subview depending on the image ratio
  _srcView = subimage_view<>(checker, 0, 0, boost::numeric_cast<int>(dims.x),
 ...
 */
template <class View>
void CheckerboardProcess<View>::multiThreadProcessImages(
  ...
  for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
    typename CheckerboardVirtualView::x_iterator src_it =
