...
void ColorWheelProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  if (_params.mode == eColorWheelModeRainbow) {
    ...
    gil_function_requires<
        StepIteratorConcept<typename RainbowLocator::x_iterator> >();

    RainbowPoint dims(rod.x2 - rod.x1, rod.y2 - rod.y1);

    // create a squared color wheel
    RainbowVirtualView rainbow(dims, RainbowLocator(RainbowPoint(0, 0),
                                                    RainbowPoint(1, 1),
                                                    RainbowFunctorT(dims)));
    // create a subview depending on the image ratio
    _srcRainbowView =
        subimage_view<>(rainbow, 0, 0, boost::numeric_cast<int>(dims.x),
                        boost::numeric_cast<int>(dims.y));
  } else {
    boost::function_requires<PixelLocatorConcept<ColorWheelLocator> >();
    gil_function_requires<
        StepIteratorConcept<typename ColorWheelLocator::x_iterator> >();
    ...
    ColorWheelVirtualView colorWheel(
        ...
        ColorWheelLocator(ColorWheelPoint(0, 0), ColorWheelPoint(1, 1),
                          ColorWheelFunctorT(
                              tileSize, _params.mode == eColorWheelModeBlack)));
    // create a subview depending on the image ratio
    _srcColorWheelView = subimage_view<>(colorWheel, xshift, yshift,
                                         boost::numeric_cast<int>(dims.x),
                                         boost::numeric_cast<int>(dims.y));
  }
}
 ...
 */
template <class View>
void ColorWheelProcess<View>::multiThreadProcessImages(
  ...
  if (_params.mode == eColorWheelModeRainbow) {
    for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
      typename RainbowVirtualView::x_iterator src_it =
          this->_srcRainbowView.x_at(procWindowOutput.x1, y);
    ...
    for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
      typename ColorWheelVirtualView::x_iterator src_it =
          this->_srcColorWheelView.x_at(procWindowOutput.x1, y);
      typename View::x_iterator dst_it =
          this->_dstView.x_at(procWindowOutput.x1, y);
      for (int x = procWindowOutput.x1; x < procWindowOutput.x2;
           ++x, ++src_it, ++dst_it) {
