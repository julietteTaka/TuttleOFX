...
ResizeProcessParams<ResizePlugin::Scalar>
ResizePlugin::getProcessParams(const OfxPointD &renderScale) const {
...
void ResizePlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamMode) {
    ...
    case eParamModeSize: {
      ...
      _paramSizeWidth->setIsSecretAndDisabled(
          !keepRatio || orientation != eParamSizeOrientationX);
      _paramSizeHeight->setIsSecretAndDisabled(
          !keepRatio || orientation != eParamSizeOrientationY);
    ...
    std::size_t width = 0;
    std::size_t height = 0;
    getFormatResolution(static_cast<EParamFormat>(_paramFormat->getValue()),
                        width, height);
  ...
  } else if (paramName == kParamSizeKeepRatio &&
    ...
    _paramSizeOrientation->setIsSecretAndDisabled(!keepRatio);
    _paramSize->setIsSecretAndDisabled(keepRatio);
  } else if (paramName == kParamSizeOrientation &&
             args.reason == OFX::eChangeUserEdit) {
    const bool keepRatio = _paramSizeKeepRatio->getValue();
    const EParamSizeOrientation orientation =
        static_cast<EParamSizeOrientation>(_paramSizeOrientation->getValue());
    ...
    _paramSizeHeight->setIsSecretAndDisabled(
        !keepRatio || orientation != eParamSizeOrientationY);
  } else if (paramName == kParamSizeWidth &&
             args.reason == OFX::eChangeUserEdit) {
    _paramMode->setValue(eParamModeSize);
  ...
  } else if (paramName == kParamSizeHeight &&
             args.reason == OFX::eChangeUserEdit) {
    _paramMode->setValue(eParamModeSize);
    _paramSizeKeepRatio->setValue(true);
    _paramSizeOrientation->setValue(eParamSizeOrientationY);

    _paramSize->setValue(_paramSize->getValue().x,
                         _paramSizeHeight->getValue());
...
bool ResizePlugin::getRegionOfDefinition(
  ...
  //	TUTTLE_LOG_INFO( rod.x1 << ", " << rod.y1 << " || " << rod.x2 << ", " <<
  //rod.y2 );
  ...
  case eParamModeScale: {
    /*
    const Point2 inA( srcRodInDstFrame.x1, srcRodInDstFrame.y1 ); // top left
    corner
    const Point2 inB( srcRodInDstFrame.x2, srcRodInDstFrame.y2 ); // down right
    corner
...
void
ResizePlugin::getRegionsOfInterest(const OFX::RegionsOfInterestArguments &args,
...
bool ResizePlugin::isIdentity(const OFX::RenderArguments &args,
