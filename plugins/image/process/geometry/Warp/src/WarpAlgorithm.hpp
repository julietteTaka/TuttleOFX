...
inline boost::gil::point2<F>
transform(const tuttle::plugin::warp::TPS_Morpher<F> &op,
...
template <typename Pixel> struct pixel_merge_t {
  const double _ratioA;
  const double _ratioB;

  pixel_merge_t(const double ratio) : _ratioA(1.0 - ratio), _ratioB(ratio) {}
  ...
  Pixel operator()(const Pixel &srcA, const Pixel &srcB) const {
    ...
    pixel_assigns_t<Pixel, Pixel>()(
        pixel_plus_t<Pixel, Pixel, Pixel>()(
            pixel_multiplies_scalar_t<Pixel, double, Pixel>()(srcA, _ratioA),
