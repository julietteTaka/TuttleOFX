...
/**
 * @brief Radial basis function: Thin plate spline (a special polyharmonic
 *spline).
 *
 * @f[
 * r^2 * log(r)
...
template <typename SCALAR> TPS_Morpher<SCALAR>::TPS_Morpher() {}
...
void TPS_Morpher<SCALAR>::setup(const std::vector<Point2> pIn,
                                const std::vector<Point2> pOut,
                                const double regularization,
                                const bool applyWarp, const std::size_t width,
  ...
  for (std::size_t y = 0; y < _nbPoints; ++y) {
    ...
    for (std::size_t x = 0; x < _nbPoints; ++x) {
      if (y == x) {
        ...
        //				_mat_L( i, i ) = _mat_K( i, i ) =
        // regularization /** (a*a)*/;
      } else {
        const Point2 &point_j = _pIn[x];
        const Scalar sum =
            pow<2>(point_i.x - point_j.x) + pow<2>(point_i.y - point_j.y);
  ...
  _mat_V(_nbPoints + 0, 0) = _mat_V(_nbPoints + 1, 0) =
      _mat_V(_nbPoints + 2, 0) = 0.0;
  _mat_V(_nbPoints + 0, 1) = _mat_V(_nbPoints + 1, 1) =
      _mat_V(_nbPoints + 2, 1) = 0.0;
...
typename TPS_Morpher<SCALAR>::Point2 TPS_Morpher<SCALAR>::
operator()(const point2<S2> &pt) const {
  using boost::math::pow;
  ...
  double dx =
      _mat_V(m + 0, 0) + _mat_V(m + 1, 0) * npt.x + _mat_V(m + 2, 0) * npt.y;
  double dy =
      _mat_V(m + 0, 1) + _mat_V(m + 1, 1) * npt.x + _mat_V(m + 2, 1) * npt.y;
  ...
  for (std::size_t i = 0; i < m; ++i, ++it_out, ++it_Vx, ++it_Vy) {
    const double d =
        base_func(pow<2>(it_out->x - npt.x) + pow<2>(it_out->y - npt.y));
    dx += (*it_Vx) * d;
    dy += (*it_Vy) * d;
  }
  dx *= _transition;
