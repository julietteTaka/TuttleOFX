...
void WarpPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  ...
  desc.setOverlayInteractDescriptor(
      new OFX::DefaultEffectOverlayWrap<WarpEffectOverlayDescriptor>());
}
...
void WarpPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::PushButtonParamDescriptor *reset =
      desc.definePushButtonParam(kParamReset);
  reset->setLabel("Reset");

  OFX::PushButtonParamDescriptor *setKey =
      desc.definePushButtonParam(kParamSetKey);
  setKey->setLabel("SetKey");

  OFX::PushButtonParamDescriptor *nextCurve =
      desc.definePushButtonParam(kParamNextCurve);
  nextCurve->setLabel("NextCurve");

  OFX::DoubleParamDescriptor *transition =
      desc.defineDoubleParam(kParamTransition);
  ...
  // Settings
  {
    OFX::GroupParamDescriptor *groupSettings =
        desc.defineGroupParam(kParamGroupSettings);
    ...
    for (std::size_t cptCBegin = 0; cptCBegin < kMaxNbPoints; ++cptCBegin) {
      ...
      curveBegin[cptCBegin] =
          desc.defineBooleanParam(kParamCurveBegin + resultCBegin);
      curveBegin[cptCBegin]->setDefault(false);
      curveBegin[cptCBegin]->setParent(groupSettings);
      curveBegin[cptCBegin]->setIsSecret(true);
    }

    OFX::BooleanParamDescriptor *inverse =
        desc.defineBooleanParam(kParamInverse);
    inverse->setLabel("Inverse");
    inverse->setDefault(false);
    inverse->setParent(groupSettings);

    OFX::DoubleParamDescriptor *rigidity =
        desc.defineDoubleParam(kParamRigiditeTPS);
    ...
    OFX::IntParamDescriptor *nbPointsBezier =
        desc.defineIntParam(kParamNbPointsBezier);
    ...
    OFX::GroupParamDescriptor *groupOverlay =
        desc.defineGroupParam(kParamGroupOverlay);
    groupOverlay->setLabel("Overlay points et tangentes");

    OFX::BooleanParamDescriptor *overlay =
        desc.defineBooleanParam(kParamOverlay);
    ...
    OFX::BooleanParamDescriptor *overlayTgtIn =
        desc.defineBooleanParam(kParamOverlayTgtIn);
    overlayTgtIn->setLabel("Bezier et tangentes In");
    overlayTgtIn->setHint("Affiche la courbe de Bezier et ses tangentes en "
                          "fonction des points In");
    ...
    OFX::BooleanParamDescriptor *overlayTgtOut =
        desc.defineBooleanParam(kParamOverlayTgtOut);
    overlayTgtOut->setLabel("Bezier et tangentes Out");
    overlayTgtOut->setHint("Affiche la courbe de Bezier et ses tangentes en "
                           "fonction des points Out");
  ...
  //////////////////// TGT Points IN////////////////////
  {
    OFX::GroupParamDescriptor *groupTgtIn =
        desc.defineGroupParam(kParamGroupTgtIn);
    groupTgtIn->setLabel("Tangente points In");
    groupTgtIn->setOpen(false);

    OFX::RGBParamDescriptor *ouverlayTgtInColor =
        desc.defineRGBParam(kParamOverlayTgtInColor);
    ...
    for (std::size_t cptTgtIn = 0; cptTgtIn < kMaxNbPoints * 2; ++cptTgtIn) {
      ...
      pTgtIn[cptTgtIn] =
          desc.defineDouble2DParam(kParamPointTgtIn + resultTgtIn);
    ...
    OFX::RGBParamDescriptor *ouverlayOutColor =
  ...
  //////////////////// TGT Points Out////////////////////
  {
    OFX::GroupParamDescriptor *groupTgtOut =
        desc.defineGroupParam(kParamGroupTgtOut);
    groupTgtOut->setLabel("Tangente points Out");
    groupTgtOut->setOpen(false);

    OFX::RGBParamDescriptor *ouverlayTgtOutColor =
        desc.defineRGBParam(kParamOverlayTgtOutColor);
    ...
    for (std::size_t cptTgtOut = 0; cptTgtOut < kMaxNbPoints * 2; ++cptTgtOut) {
      ...
      pTgtOut[cptTgtOut] =
          desc.defineDouble2DParam(kParamPointTgtOut + resultTgtOut);
