...
WarpPlugin::WarpPlugin(OfxImageEffectHandle handle) : ImageEffect(handle) {
  ...
  for (std::size_t cptCBegin = 0; cptCBegin < kMaxNbPoints; ++cptCBegin) {
    _paramCurveBegin[cptCBegin] = fetchBooleanParam(
        kParamCurveBegin + boost::lexical_cast<std::string>(cptCBegin));
  ...
  for (std::size_t cptIn = 0; cptIn < kMaxNbPoints; ++cptIn) {
    _paramPointIn[cptIn] = fetchDouble2DParam(
        kParamPointIn + boost::lexical_cast<std::string>(cptIn));
  }
  ...
  for (std::size_t cptOut = 0; cptOut < kMaxNbPoints; ++cptOut) {
    _paramPointOut[cptOut] = fetchDouble2DParam(
        kParamPointOut + boost::lexical_cast<std::string>(cptOut));
  }
  ...
  for (std::size_t cptTgtIn = 0; cptTgtIn < kMaxNbPoints; ++cptTgtIn) {
    _paramPointTgtIn[2 * cptTgtIn] = fetchDouble2DParam(
        kParamPointTgtIn + boost::lexical_cast<std::string>(2 * cptTgtIn));
    _paramPointTgtIn[2 * cptTgtIn + 1] = fetchDouble2DParam(
        kParamPointTgtIn + boost::lexical_cast<std::string>(2 * cptTgtIn + 1));
  }
  ...
  for (std::size_t cptTgtOut = 0; cptTgtOut < kMaxNbPoints; ++cptTgtOut) {
    _paramPointTgtOut[2 * cptTgtOut] = fetchDouble2DParam(
        kParamPointTgtOut + boost::lexical_cast<std::string>(2 * cptTgtOut));
    _paramPointTgtOut[2 * cptTgtOut + 1] =
        fetchDouble2DParam(kParamPointTgtOut +
                           boost::lexical_cast<std::string>(2 * cptTgtOut + 1));
  }
  ...
  changedParam(_instanceChangedArgs, kParamNbPoints); // init IsSecret property
                                                      // for each pair of points
                                                      // parameter
}

WarpProcessParams<WarpPlugin::Scalar>
WarpPlugin::getProcessParams(const OfxPointD &renderScale) const {
...
void WarpPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (boost::starts_with(paramName, kParamPointIn) ||
    ...
    case eParamMethodDelete: { break; }
    case eParamMethodMove: { break; }
    }
  }
  ...
  // Si le mode est MultiCurve
    ...
    TUTTLE_LOG_DEBUG(TUTTLE_INFO, _paramCurveBegin[(_paramNbPoints->getValue())]
                                      ->getValue());
  } else if (paramName == kParamSetKey) {
    // @todo adrien: c'est peut-etre mieux de le faire uniquement sur les points
    // cree ? _paramNbPoints->getValue()
    for (std::size_t i = 0; i < kMaxNbPoints; ++i) {
      _paramPointIn[i]->setValueAtTime(args.time, _paramPointIn[i]->getValue());
      _paramPointOut[i]
          ->setValueAtTime(args.time, _paramPointOut[i]->getValue());

      _paramPointTgtIn[2 * i]
          ->setValueAtTime(args.time, _paramPointTgtIn[2 * i]->getValue());
      _paramPointTgtIn[(2 * i) + 1]->setValueAtTime(
          args.time, _paramPointTgtIn[(2 * i) + 1]->getValue());

      _paramPointTgtOut[2 * i]
          ->setValueAtTime(args.time, _paramPointTgtOut[2 * i]->getValue());
      _paramPointTgtOut[(2 * i) + 1]->setValueAtTime(
          args.time, _paramPointTgtOut[(2 * i) + 1]->getValue());
    }
  }
}

bool WarpPlugin::isIdentity(const OFX::RenderArguments &args,
...
void WarpPlugin::render(const OFX::RenderArguments &args) {
  ...
  if (dstComponents == OFX::ePixelComponentRGBA) {
    ...
    TUTTLE_LOG_ERROR("Pixel components ("
                     << mapPixelComponentEnumToString(dstComponents)
