...
double barycentre(const double a, const double b, const double c,
                  const double d, const double t) {
  using boost::math::pow;
  // B(t) = P0(1-t)^3 + 3 P1 t (1-t)^2 + 3 P2 t^2 (1-t) + P3 t^3;
  return (a * pow<3>(1 - t)) + (3 * b * t * pow<2>(1 - t)) +
         (3 * c * pow<2>(t) * (1 - t)) + (d * pow<3>(t));
}

point2<double> barycentre(const std::vector<point2<double> > &pts,
                          const double t) {
  point2<double> p;
...
void drawBezier(const std::vector<point2<double> > &tabPts,
                const std::size_t nbBezier, const double r, const double v,
                const double b) {
...
void bezierSubdivide(const std::vector<point2<double> > &pts,
