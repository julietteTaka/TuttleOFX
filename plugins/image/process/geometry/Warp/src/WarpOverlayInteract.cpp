...
WarpOverlayInteract::WarpOverlayInteract(OfxInteractHandle handle,
  ...
  for (std::size_t i = 0; i < kMaxNbPoints; ++i) {
    //		interact::PointInteract* point = new
    //interact::ParamPoint<interact::FrameClip, eCoordinateSystemXY > ( _infos,
    //_plugin->_paramPointOut[i], _plugin->_clipSrc );

    typedef interact::ParamPoint<interact::FrameClip, eCoordinateSystemXY>
    PPoint;
    typedef interact::ParamTangent<interact::FrameClip, eCoordinateSystemXY>
    PTangent;
    ...
    activeIn->push_back(
        new interact::IsNotSecretParamFunctor<>(_plugin->_paramPointIn[i]));

    _interactScene.push_back(
        new PTangent(_infos, _plugin->_paramPointIn[i],
    ...
    // Points out
    interact::AndActiveFunctor<> *activeOut =
        new interact::AndActiveFunctor<>();
    activeOut->push_back(
        new interact::IsActiveBooleanParamFunctor<>(_plugin->_paramOverlayOut));
    activeOut->push_back(
        new interact::IsNotSecretParamFunctor<>(_plugin->_paramPointOut[i]));

    _interactScene.push_back(
        new PTangent(_infos, _plugin->_paramPointOut[i],
    ...
    for (std::size_t c = 0; c < nbPoints - 1; ++c) {
      ...
      if (longDown == false) {
        ...
        if (_plugin->_paramOverlayTgtIn->getValue())
          bezier::drawBezier(tabPtsIn,
                             _plugin->_paramNbPointsBezier->getValue(), rIn,
                             vIn, bIn);
        ...
        if (_plugin->_paramOverlayTgtOut->getValue())
          bezier::drawBezier(tabPtsOut,
                             _plugin->_paramNbPointsBezier->getValue(), rOut,
                             vOut, bOut);
      }
    }
  }
...
bool WarpOverlayInteract::penMotion(const OFX::PenArgs &args) {
  ...
  if (_plugin->_paramMethod->getValue() == eParamMethodCreation) {
    if (longDown == true) {
      // Tangente In
      // Point courant
      const point2<double> ptCurIn(
          ofxToGil(_plugin->_paramPointIn[numPt - 1]->getValue()));
      ...
      _plugin->_paramPointTgtIn[2 * (numPt - 1)]
          ->setValue(args.penPosition.x, args.penPosition.y);
      // Point numéro 2 de la tangente
      _plugin->_paramPointTgtIn[2 * (numPt - 1) + 1]
          ->setIsSecretAndDisabled(false);
      _plugin->_paramPointTgtIn[2 * (numPt - 1) + 1]
          ->setValue(2 * ptCurIn.x - args.penPosition.x,
                     2 * ptCurIn.y - args.penPosition.y);

      // Tangente Out
      // Point courant
      const point2<double> ptCurOut(
          ofxToGil(_plugin->_paramPointOut[numPt - 1]->getValue()));
      // Point numéro 1 de la tangente
      _plugin->_paramPointTgtOut[2 * (numPt - 1)]
          ->setIsSecretAndDisabled(false);
      _plugin->_paramPointTgtOut[2 * (numPt - 1)]
          ->setValue(args.penPosition.x, args.penPosition.y);
      // Point numéro 2 de la tangente
      _plugin->_paramPointTgtOut[2 * (numPt - 1) + 1]
          ->setIsSecretAndDisabled(false);
      _plugin->_paramPointTgtOut[2 * (numPt - 1) + 1]
          ->setValue(2 * ptCurOut.x - args.penPosition.x,
                     2 * ptCurOut.y - args.penPosition.y);
    }
    return _interactScene.penUp(args);
  } else if ((numPt < kMaxNbPoints) &&
             (_plugin->_paramMethod->getValue() == eParamMethodMove))
...
bool WarpOverlayInteract::penDown(const OFX::PenArgs &args) {
  ...
  // Si le mode est delete
    ...
    for (unsigned int i = 0; i < nbPoints; ++i) {
      if ((abs(args.penPosition.x) - (_plugin->_paramPointIn[i]->getValue().x) <
               seuil &&
           abs((args.penPosition.y) -
               (_plugin->_paramPointIn[i]->getValue().y)) < seuil) ||
          (abs(args.penPosition.x) -
                   (_plugin->_paramPointOut[i]->getValue().x) <
               seuil &&
           abs((args.penPosition.y) -
               (_plugin->_paramPointOut[i]->getValue().y)) < seuil)) {
        if (i < nbPoints - 1) {
          for (std::size_t ptSuivants = i; ptSuivants < nbPoints;
               ++ptSuivants) {
            _plugin->_paramPointIn[ptSuivants]
                ->setValue(_plugin->_paramPointIn[ptSuivants + 1]->getValue());
            _plugin->_paramPointOut[ptSuivants]
                ->setValue(_plugin->_paramPointOut[ptSuivants + 1]->getValue());
        ...
        _plugin->_paramPointTgtIn[2 * nbPoints - 2]
            ->setIsSecretAndDisabled(true);
        _plugin->_paramPointTgtIn[2 * nbPoints - 1]
            ->setIsSecretAndDisabled(true);
        _plugin->_paramPointTgtOut[2 * nbPoints - 2]
            ->setIsSecretAndDisabled(true);
        _plugin->_paramPointTgtOut[2 * nbPoints - 1]
            ->setIsSecretAndDisabled(true);
...
bool WarpOverlayInteract::keyDown(const OFX::KeyArgs &args) {
  if (args.keySymbol == kOfxKey_space)
    TUTTLE_LOG_DEBUG(TUTTLE_INFO, "test");
  else
    TUTTLE_LOG_DEBUG(TUTTLE_INFO, "test failed");
  return false;
}
