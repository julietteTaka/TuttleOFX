...
PinningProcessParams<PinningPlugin::Scalar>
PinningPlugin::getProcessParams(const OfxTime time,
...
void PinningPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamSetToCornersIn) {
    ...
    case eParamMethodAffine: {
      ...
      subrange(A, 3, 6, 0, 3) = subrange(A, 0, 3, 3, 6) =
          zero_matrix<double>(n);
      subrange(A, 0, 3, 2, 3) = subrange(A, 3, 6, 5, 6) =
          scalar_matrix<double>(3, 1, 1);
    ...
    case eParamMethodBilinear: {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        A(2 * i + 1, 0) = A(2 * i + 1, 1) = A(2 * i + 1, 2) = A(2 * i + 1, 3) =
            0;
...
bool PinningPlugin::isIdentity(const OFX::RenderArguments &args,
                               OFX::Clip *&identityClip, double &identityTime) {
  return false; /// @todo remove this.
  using namespace boost::numeric::ublas;
  PinningProcessParams<Scalar> params =
      getProcessParams(args.time, args.renderScale);
  bool identity = false;
  ...
  case eParamMethodAffine:
  case eParamMethodPerspective: {
    if (norm_inf(params._perspective._matrix - identity_matrix<Scalar>(3)) ==
        0) {
      identity = true;
    }
    break;
