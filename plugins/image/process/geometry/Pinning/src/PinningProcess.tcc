...
template <class View>
void
PinningProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::sampler;

  const terry::Rect<std::ssize_t> procWindowOutput =
      ofxToGil(this->translateRoWToOutputClipCoordinates(procWindowRoW));

  switch (_params._samplerProcessParams._filter) {
  case eParamFilterNearest: {
    resample< ::terry::sampler::nearest_neighbor_sampler>(
        this->_srcView, this->_dstView, procWindowOutput);
    return;
  }
  case eParamFilterBilinear: {
    resample< ::terry::sampler::bilinear_sampler>(
        this->_srcView, this->_dstView, procWindowOutput);
    return;
  }
  case eParamFilterBC: {
  ...
  case eParamFilterMitchell: {
    resample<mitchell_sampler>(this->_srcView, this->_dstView,
                               procWindowOutput);
    return;
  }
  ...
  case eParamFilterLanczos3: {
    resample<lanczos3_sampler>(this->_srcView, this->_dstView,
                               procWindowOutput);
    return;
  }
  case eParamFilterLanczos4: {
    resample<lanczos4_sampler>(this->_srcView, this->_dstView,
                               procWindowOutput);
    return;
  }
  case eParamFilterLanczos6: {
    resample<lanczos6_sampler>(this->_srcView, this->_dstView,
                               procWindowOutput);
    return;
  }
  case eParamFilterLanczos12: {
    resample<lanczos12_sampler>(this->_srcView, this->_dstView,
                                procWindowOutput);
    return;
  }
  case eParamFilterGaussian: {
    ...
    resample<gaussian_sampler>(this->_srcView, this->_dstView,
                               procWindowOutput);
    return;
  }
  }
...
void PinningProcess<View>::resample(View &srcView, View &dstView,
  ...
  case eParamMethodPerspective:
    terry::sampler::resample_pixels_progress<Sampler>(
        srcView, dstView, _params._perspective, procWindow,
        _params._samplerProcessParams._outOfImageProcess,
        this->getOfxProgress(), sampler);
    return;
  case eParamMethodBilinear:
    terry::sampler::resample_pixels_progress<Sampler>(
        srcView, dstView, _params._bilinear, procWindow,
        _params._samplerProcessParams._outOfImageProcess,
        this->getOfxProgress(), sampler);
    return;
  }
