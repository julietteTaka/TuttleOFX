...
PushPixelProcessParams<PushPixelPlugin::Scalar>
PushPixelPlugin::getProcessParams(const OfxPointD &renderScale) const {
  ...
  params._interpolation =
      static_cast<EParamInterpolation>(_paramInterpolation->getValue());
  ...
  params._kernelGaussianDerivative =
      buildGaussianDerivative1DKernel<Scalar>(params._size, normalizedKernel);
  params._kernelGaussian =
      buildGaussian1DKernel<Scalar>(params._size, normalizedKernel);

  return params;
}

bool PushPixelPlugin::isIdentity(const OFX::RenderArguments &args,
...
void PushPixelPlugin::getRegionsOfInterest(
  ...
  if (_clipMask->isConnected()) {
    ...
    double maxKernelTL = std::max(params._kernelGaussian.left_size(),
                                  params._kernelGaussianDerivative.left_size());
    double maxKernelBR =
        std::max(params._kernelGaussian.right_size(),
                 params._kernelGaussianDerivative.right_size());
    OfxRectD maskRoi;
    maskRoi.x1 = args.regionOfInterest.x1 -
                 (maxKernelTL * _clipMask->getPixelAspectRatio());
    maskRoi.y1 = args.regionOfInterest.y1 - maxKernelTL;
    maskRoi.x2 = args.regionOfInterest.x2 +
                 (maxKernelBR * _clipMask->getPixelAspectRatio());
...
void PushPixelPlugin::render(const OFX::RenderArguments &args) {
  ...
  BOOST_ASSERT(_clipDst->getPixelComponents() ==
               _clipSrc->getPixelComponents());
