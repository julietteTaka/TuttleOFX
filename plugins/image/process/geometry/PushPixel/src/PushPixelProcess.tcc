...
void PushPixelProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  if (_clipMaskConnected) {
    ...
    this->_maskView = this->getView(
        this->_mask.get(), _clipMask->getPixelRod(args.time, args.renderScale));
 ...
 */
template <class View>
void PushPixelProcess<View>::multiThreadProcessImages(
  ...
  typedef typename boost::mpl::if_<boost::is_floating_point<Channel>, Channel,
                                   bits32f>::type ChannelFloat;
  ...
  const OfxRectI imgGradientWin =
      rectanglesIntersection(procWindowRoW, usedMaskPixelRod);
  const bool partialGradient =
      (imgGradientWin != procWindowRoW); // if true we can only calculate a
                                         // gradient for a subpart of the
                                         // procWindow
  ...
  if ((imgGradientWin.y2 - imgGradientWin.y1 == 0) ||
      (imgGradientWin.x2 - imgGradientWin.x1 == 0)) {
    ...
    case eParamOutputPushPixel: {
      ...
      View dst = subimage_view(this->_dstView, procWindowOutput.x1,
                               procWindowOutput.y1, procWindowSize.x,
                               procWindowSize.y);
      copy_pixels(src, dst);
      return;
    }
  ...
  const OfxPointI imgGradientSize = { imgGradientWin.x2 - imgGradientWin.x1,
                                      imgGradientWin.y2 - imgGradientWin.y1 };
  ...
  Point proc_mask_tl(imgGradientWin.x1 - usedMaskPixelRod.x1,
                     imgGradientWin.y1 - usedMaskPixelRod.y1);
  ...
  */
  ...
  if (_clipMaskConnected) {
    ...
    typedef typename boost::mpl::if_<contains_color<MaskPixel, alpha_t>,
                                     alpha_t, gray_t>::type MaskColorChannel;
    typedef channel_view_type<MaskColorChannel, MaskView> KthChannelView;
    typename KthChannelView::type channelMaskView =
        KthChannelView::make(this->_maskView); // gray or alpha channel

    if (correlateMotionVectors<OfxAllocator, ViewGray,
                               typename KthChannelView::type, Point, Scalar>(
            xGradientView, yGradientView, channelMaskView, proc_mask_tl,
            _params._kernelGaussianDerivative, _params._kernelGaussian,
            _params._boundary_option, this->getOfxProgress()))
      return;
  } else {
    typedef channel_view_type<alpha_t, View> KthChannelView;
    typename KthChannelView::type channelMaskView =
        KthChannelView::make(this->_srcView); // alpha channel
  ...
  if (modifyVectors(xGradientView, yGradientView, this->_params._angle,
                    _params._intensity, this->getOfxProgress()))
    return;

  // apply motion vectors
  switch (_params._output) {
  case eParamOutputMotionVectors: {
    const OfxRectI gradientRodInSrc =
        translateRegion(imgGradientWin, this->_srcPixelRod);
    // subdst is the part of dst for which we have motion vectors information
    View subdst =
        subimage_view(this->_dstView, gradientRodInSrc.x1, gradientRodInSrc.y1,
                      imgGradientSize.x, imgGradientSize.y);
    copy_and_convert_pixels(xGradientView,
                            kth_channel_view<0>(subdst)); // put x vectors
    if (this->progressForward(procWindowSize.y * 0.5))
      return;
    copy_and_convert_pixels(yGradientView,
                            kth_channel_view<1>(subdst)); // put y vectors
    if (this->progressForward(procWindowSize.y * 0.5))
      return;
    return;
  }
  case eParamOutputPushPixel: {
    ...
    case eParamInterpolationNearest:
      motionvectors_resample_pixels<sampler::nearest_neighbor_sampler>(
          this->_srcView, this->_srcPixelRod, xGradientView, yGradientView,
          imgGradientWin, this->_dstView, this->_dstPixelRod, procWindowRoW,
          sampler::eParamFilterOutBlack, this->getOfxProgress());
      return;
