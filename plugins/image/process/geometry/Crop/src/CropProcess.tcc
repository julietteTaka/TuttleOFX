...
template <class View>
void
CropProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  using namespace boost::gil;
  // Currently, it's not possible to place the crop depending on the RoW and not
  // on image source RoD.
  ...
  const OfxRectI procCropRoW = rectanglesIntersection(
      rectanglesIntersection(_params._cropRegion, procWindowRoW),
      this->_srcPixelRod);

  const OfxRectI procCropOutput =
      translateRegion(procCropRoW, this->_dstPixelRod);
