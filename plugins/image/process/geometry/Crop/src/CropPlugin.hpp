...
template <class Pixel> struct CropProcessParams {
  EParamMode _mode;
...
public:
  ...
  CropProcessParams<Pixel> getProcessParams(const OfxTime time,
                                            const OfxPointD &renderScale =
                                                OFX::kNoRenderScale) const;

  OfxRectI getCropRegionValue() const;
  OfxRectI computeCropRegion(const OfxTime time,
                             const bool fromRatio = false) const;
...
template <class Pixel>
CropProcessParams<Pixel>
CropPlugin::getProcessParams(const OfxTime time,
