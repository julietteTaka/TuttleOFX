...
OfxRectI CropPlugin::computeCropRegion(const OfxTime time,
                                       const bool fromRatio) const {
  ...
  const EParamSymmetric symmetric =
      static_cast<EParamSymmetric>(_paramSymmetric->getValue());
  switch (axis) {
  case eParamAxisXY: {
...
void CropPlugin::changedClip(const OFX::InstanceChangedArgs &args,
...
void CropPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamXMin || paramName == kParamYMin ||
      paramName == kParamXMax || paramName == kParamYMax ||
...
CropPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args,
  ...
  CropProcessParams<rgba32f_pixel_t> params =
      getProcessParams<rgba32f_pixel_t>(args.time);
  ...
  case eParamModeFillColor: { break; }
  }
  return false;
}
