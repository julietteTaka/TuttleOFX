...
inline point2<F>
transform(const ::tuttle::plugin::lens::NormalLensDistortParams<F> &params,
          const point2<F2> &src) {
  BOOST_ASSERT(params._distort);
  BOOST_ASSERT(params._coef1 >= 0);
  point2<F> pc(
      params.pixelToLensCenterNormalized(src)); // centered normalized space
...
template <typename F, typename F2>
inline point2<F>
transform(const ::tuttle::plugin::lens::NormalLensUndistortParams<F> &params,
          const point2<F2> &src) {
  BOOST_ASSERT(!params._distort);
  BOOST_ASSERT(params._coef1 >= 0);
  ...
  if (delta > 0) {
    ...
    BOOST_ASSERT(cQ >= 0);
    BOOST_ASSERT(cR / (sqrt(cQ * cQ * cQ)) <= 1 &&
                 cR / (sqrt(cQ * cQ * cQ)) >= -1);
    t = std::acos(cR / (sqrt(cQ * cQ * cQ)));
    coef = -2.0 * std::sqrt(cQ) *
           std::cos((t - (boost::math::constants::pi<F>() / 2.0)) / 3.0);
  } else {
...
template <typename F, typename F2>
inline point2<F>
transform(const ::tuttle::plugin::lens::FisheyeLensDistortParams<F> &params,
          const point2<F2> &src) {
  point2<F> pc(params.pixelToLensCenterNormalized(src));
  pc *= params._postScale;

  // F r2 = pc.x * pc.x + pc.y * pc.y; // distance to center
  // F coef = params._postScale * ( 1.0 + r2 * params._coef1 + ( r2 * r2 ) *
  // params._coef2 ); // distortion coef for current pixel
  // pc.x *= coef;
  // pc.y *= coef;
  F r = std::sqrt(pc.x * pc.x + pc.y * pc.y); // distance to center
  if (r == 0) {
  ...
  F coef =
      0.5 * std::tan(r * params._coef1) / (std::tan(0.5 * params._coef1) * r);
...
template <typename F, typename F2>
inline point2<F>
transform(const ::tuttle::plugin::lens::FisheyeLensUndistortParams<F> &params,
          const point2<F2> &src) {
  point2<F> pc(params.pixelToLensCenterNormalized(src));
  pc *= params._postScale;
...
template <typename F, typename F2>
inline point2<F>
transform(const ::tuttle::plugin::lens::AdvancedLensDistortParams<F> &params,
          const point2<F2> &src) {
  point2<F> pc(params.pixelToLensCenterNormalized(src));
  pc *= params._postScale;
...
typename Params::Point2 transformValues(const Params &params,
...
OfxRectD transformValues(const Params &params, const OfxRectD &rec) {
  ...
  if (params._lensCenterDst.x > rec.x1 && params._lensCenterDst.x < rec.x2) {
    points.push_back(
        terry::transform(params, Point2(params._lensCenterDst.x, rec.y1)));
    points.push_back(
        terry::transform(params, Point2(params._lensCenterDst.x, rec.y2)));
  }
  if (params._lensCenterDst.y > rec.y1 && params._lensCenterDst.y < rec.y2) {
    points.push_back(
        terry::transform(params, Point2(rec.x1, params._lensCenterDst.y)));
    points.push_back(
        terry::transform(params, Point2(rec.x2, params._lensCenterDst.y)));
  }
 ...
 */
template <class Obj>
inline Obj transformValues(const EParamLensType lensType,
  ...
  BOOST_THROW_EXCEPTION(exception::Unsupported()
                        << exception::user(
                               "Outside of the plugin fonctionnalities."));
  return obj;
}
...
inline void transformValuesApply(const Params &params,
                                 std::vector<boost::gil::point2<F2> > &vec) {
  for (typename std::vector<boost::gil::point2<F2> >::iterator
           it = vec.begin(),
           itEnd = vec.end();
       it != itEnd; ++it) {
...
template <class Params, typename F2>
inline void
transformValuesApply(const Params &params,
                     std::vector<std::vector<boost::gil::point2<F2> > > &vec) {
  for (typename std::vector<std::vector<boost::gil::point2<F2> > >::iterator
           it = vec.begin(),
           itEnd = vec.end();
       it != itEnd; ++it) {
...
template <class Obj>
inline void transformValuesApply(const EParamLensType lensType,
                                 const LensDistortProcessParams<double> &params,
                                 Obj &obj) {
  switch (lensType) {
  case eParamLensTypeStandard: {
    if (params._distort) {
      return transformValuesApply<NormalLensDistortParams<double> >(params,
                                                                    obj);
    } else {
      return transformValuesApply<NormalLensUndistortParams<double> >(params,
                                                                      obj);
    }
  }
  case eParamLensTypeFisheye: {
    if (params._distort)
      return transformValuesApply<FisheyeLensDistortParams<double> >(params,
                                                                     obj);
    else
      return transformValuesApply<FisheyeLensUndistortParams<double> >(params,
                                                                       obj);
  }
  case eParamLensTypeAdvanced: {
    if (params._distort)
      return transformValuesApply<AdvancedLensDistortParams<double> >(params,
                                                                      obj);
    else
      return transformValuesApply<AdvancedLensUndistortParams<double> >(params,
                                                                        obj);
  }
  }
}
