...
void LensDistortPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  } else if (paramName == kParamGridOverlay || paramName == kParamGridCenter ||
             paramName == kParamGridScale) {
    redrawOverlays();
  }
}

bool LensDistortPlugin::isIdentity(const OFX::RenderArguments &args,
  ...
  if (_displaySource->getValue()) {
    isIdentity = true;
             ...
             (!_coef2->getIsEnable() ||
              _coef2->getValue() == _coef2->getDefault()) &&
             (!_squeeze->getIsEnable() ||
              _squeeze->getValue() == _squeeze->getDefault()) &&
             (!_asymmetric->getIsEnable() ||
              _asymmetric->getValue() == _asymmetric->getDefault())) {
    isIdentity = true;
  }
  if (isIdentity) {
...
bool LensDistortPlugin::getRegionOfDefinition(
  ...
  LensDistortProcessParams<Scalar> params(
      getProcessParams(srcRod, srcRod, _clipDst->getPixelAspectRatio(), true));
  ...
  case eParamResizeRodMin: {
    Point2 tl = transformValues(
        getLensType(), params,
        Point2(srcRodInDstFrame.x1, srcRodInDstFrame.y1)); // top left
    Point2 tr = transformValues(
        getLensType(), params,
        Point2(srcRodInDstFrame.x2, srcRodInDstFrame.y1)); // top right
    Point2 bl = transformValues(
        getLensType(), params,
        Point2(srcRodInDstFrame.x1, srcRodInDstFrame.y2)); // bottom left
    Point2 br = transformValues(
        getLensType(), params,
        Point2(srcRodInDstFrame.x2, srcRodInDstFrame.y2)); // bottom right
    ...
    if (params._lensCenterSrc.x > srcRodInDstFrame.x1 &&
        params._lensCenterSrc.x < srcRodInDstFrame.x2) {
      Point2 t =
          transformValues(getLensType(), params,
                          Point2(params._lensCenterSrc.x, srcRodInDstFrame.y1));
      pMin.y = std::max(pMin.y, t.y);
      Point2 b =
          transformValues(getLensType(), params,
                          Point2(params._lensCenterSrc.x, srcRodInDstFrame.y2));
      pMax.y = std::min(pMax.y, b.y);
    }
    if (params._lensCenterSrc.y > srcRodInDstFrame.y1 &&
        params._lensCenterSrc.y < srcRodInDstFrame.y2) {
      Point2 l =
          transformValues(getLensType(), params,
                          Point2(srcRodInDstFrame.x1, params._lensCenterSrc.y));
      pMin.x = std::max(pMin.x, l.x);
      Point2 r =
          transformValues(getLensType(), params,
                          Point2(srcRodInDstFrame.x2, params._lensCenterSrc.y));
  ...
  case eParamResizeRodManual: {
    ...
    point2<double> pMin(srcRodInDstFrame.x1,
                        srcRodInDstFrame.y1); // top left corner
    point2<double> pMax(srcRodInDstFrame.x2,
                        srcRodInDstFrame.y2); // down right corner
...
void LensDistortPlugin::getRegionsOfInterest(
  ...
  if (_srcRefClip->isConnected()) {
    ...
    params = getProcessParams(srcRod, dstRod, srcRefRod,
  ...
  //    srcRoi.x1 += 2; // if we remove 2 pixels to the needed RoI the plugin
  // crash, because it tries to access to this pixels
  //    srcRoi.y1 += 2; // so the calculation of the RoI has a precision of one
  // pixel
  //    srcRoi.x2 -= 2;
  //    srcRoi.y2 -= 2;
...
LensDistortProcessParams<LensDistortPlugin::Scalar>
LensDistortPlugin::getProcessParams(const OfxRectD &inputRod,
                                    const OfxRectD &outputRod,
                                    const OfxRectD &optionalInputRod,
                                    const double pixelAspectRatio,
                                    const bool reverse) const {
  const bool useOptionalInputRod =
      (optionalInputRod.x1 != optionalInputRod.x2) &&
      (optionalInputRod.y1 != optionalInputRod.y2);
  const OfxRectD &choosedInputRod =
      useOptionalInputRod ? optionalInputRod : inputRod;
  ...
  Point2 imgShift =
      Point2(inputRod.x1 - outputRod.x1,
             inputRod.y1 - outputRod.y1); // translate output -> source
  params._imgSizeSrc = Point2(choosedInputRod.x2 - choosedInputRod.x1,
                              choosedInputRod.y2 - choosedInputRod.y1);
  params._imgCenterSrc =
      Point2(params._imgSizeSrc.x * 0.5, params._imgSizeSrc.y * 0.5);
  params._imgCenterDst = params._imgCenterSrc + imgShift;
  params._imgHalfDiagonal =
      std::sqrt(params._imgCenterSrc.x * params._imgCenterSrc.x *
                    pixelAspectRatio * pixelAspectRatio +
                params._imgCenterSrc.y * params._imgCenterSrc.y);
  params._pixelRatio = pixelAspectRatio;

  switch (getCenterType()) {
  case eParamCenterTypeSource: {
    params._lensCenterSrc = pointNormalizedXXcToCanonicalXY(
        ofxToGil(_center->getValue()), params._imgSizeSrc);
    if (useOptionalInputRod) {
      Point2 imgShiftBetweenInputs =
          Point2(optionalInputRod.x1 - inputRod.x1,
                 optionalInputRod.y1 -
                     inputRod.y1); // translate inputRod -> optionalInputRod
...
LensDistortParams LensDistortPlugin::getProcessParams() const {
  ...
  lensDistortParams._lensType =
      (tuttle::plugin::lens::EParamLensType)_lensType->getValue();
  lensDistortParams._centerType =
      (tuttle::plugin::lens::EParamCenterType)_centerType->getValue();
