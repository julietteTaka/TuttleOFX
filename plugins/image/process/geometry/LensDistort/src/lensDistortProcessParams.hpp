 ...
 *     -----------------------------
 * 0.0, 0.0
 *  * centered normalized coordinate system (custom, only depending on image
 * width)
 *     ----------------------------- 0.5, 0.5
 *     |                           |
 *     |---------------------------|
 ...
 * -0.5, -0.5
 */
template <typename F> class CoordonatesSystemParams {
public:
  typedef F Float;
...
public:
  ...
  F _imgHalfDiagonal; ///< half diagonal of the image (to work between -0.5 and
                      ///0.5)
  F _pixelRatio;
...
template <typename F> CoordonatesSystemParams<F>::~CoordonatesSystemParams() {}
...
public:
  ...
  Point2 _lensCenterDst; ///< center of the lens ditortion in the dest image
                         ///coordinates (in pixels)
  Point2 _lensCenterSrc; ///< center of the lens ditortion in the source image
                         ///coordinates (in pixels)
  Point2 _postScale;
  Point2 _preScale;
...
public:
  ...
  template <typename F2>
  inline Point2
  pixelToLensCenterNormalized(const boost::gil::point2<F2> &p) const {
    Point2 pp(p.x, p.y);
