...
createGrid(const OfxRectD &rod, const unsigned int nbTiles, const Point2 &scale,
  ...
  const double yShift = (rodSize.x - rodSize.y) /
                        2.0; // vertical shift (because it's a squared grid)
  ...
  for (unsigned int i = 0; i < size; ++i) {
    ...
    for (unsigned int j = 0; j <= steps; ++j) {
      ...
      Point2 diagonal(si * scale.x + halfSize + pixCenter.x,
                      si * scale.y + halfSize + pixCenter.y - yShift);
      double sj = (j * stepSize) - halfSize;
      horizontal.push_back(
          Point2(sj * scale.x + halfSize + pixCenter.x, diagonal.y));
      vertical.push_back(
          Point2(diagonal.x, sj * scale.y + halfSize + pixCenter.y - yShift));
    }
...
void shiftGrid(std::vector<std::vector<Point2> > &grid, const Point2 &shift) {
  for (typename std::vector<std::vector<Point2> >::iterator itv = grid.begin(),
                                                            itvEnd = grid.end();
       itv != itvEnd; ++itv) {
    for (typename std::vector<Point2>::iterator it = itv->begin(),
                                                itEnd = itv->end();
         it != itEnd; ++it) {
      *it += shift;
    }
...
LensDistortOverlayInteract::LensDistortOverlayInteract(OfxInteractHandle handle,
  ...
  const interact::FrameOptionalClip frame(_plugin->_srcRefClip,
                                          _plugin->_clipSrc);
  interact::PointInteract *centerPoint = new interact::ParamPoint<
      interact::FrameOptionalClip, eCoordinateSystemXXcn>(
      _infos, _plugin->_center, frame);
  _interactScene.push_back(
      new interact::ParamPointRelativePoint<interact::FrameOptionalClip,
  ...
  _interactScene.push_back(
      centerPoint,
      new interact::IsActiveBooleanParamFunctor<>(_plugin->_centerOverlay));
}
...
bool LensDistortOverlayInteract::draw(const OFX::DrawArgs &args) {
  ...
  if (_plugin->_gridOverlay->getValue() && _plugin->_clipSrc->isConnected()) {
    ...
    //		const OfxRectD outputRod = _plugin->_clipDst->getCanonicalRod(
    //args.time );
    // parameters
    const Point2 gridCenter(ofxToGil(_plugin->_gridCenter->getValue()));
    const Point2 gridScale(ofxToGil(_plugin->_gridScale->getValue()));
    std::vector<std::vector<Point2> > grid =
        createGrid(srcRod, steps, gridScale, gridCenter);
    if (!_plugin->_displaySource->getValue()) {
      LensDistortProcessParams<Scalar> params;
      static const double pixelAspectRatio = 1.0; // here the pixel aspect ratio
                                                  // is 1.0 because we work in
                                                  // canonical coordinates
      params =
          _plugin->getProcessParams(srcRod, srcRod, pixelAspectRatio, true);
