...
public:
  ...
  OFX::DoubleParam *_squeeze; ///< squeeze coefficient horizontally/vertically
                              ///(not implemented yet)
  OFX::Double2DParam *_asymmetric;   ///< lens distortion is asymmetric
                                     ///horizontally/vertically (not implemented
                                     ///yet)
  OFX::Double2DParam *_center;       ///< center coordonnates
  OFX::BooleanParam *_centerOverlay; ///< lens center overlay
  OFX::ChoiceParam *_centerType; ///< centered the lens distortion on source RoD
                                 ///or image size (not implemented yet)
  ...
  OFX::ChoiceParam *_resizeRod;  ///< Choice how to resize the RoD (default 'no'
                                 ///resize)
  OFX::DoubleParam *_resizeRodManualScale; ///< scale the output RoD

  OFX::GroupParam *_groupDisplayParams; ///< group of all overlay options (don't
...
public:
   ...
   * @todo pixelRatio !
   */
  LensDistortProcessParams<Scalar>
  getProcessParams(const OfxRectD &inputRod, const OfxRectD &outputRod,
  ...
  getProcessParams(const OfxRectD &inputRod, const OfxRectD &outputRod,
    ...
    return getProcessParams(inputRod, outputRod, noOptionalInputRod,
                            pixelAspectRatio, reverse);
  }

  LensDistortParams getProcessParams() const;

  const EParamLensType getLensType() const {
    return static_cast<EParamLensType>(_lensType->getValue());
  }
  const EParamCenterType getCenterType() const {
    return static_cast<EParamCenterType>(_centerType->getValue());
  }
  const EParamResizeRod getResizeRod() const {
    return static_cast<EParamResizeRod>(_resizeRod->getValue());
  }
