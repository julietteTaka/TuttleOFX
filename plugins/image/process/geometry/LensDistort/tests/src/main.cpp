...
int main(int argc, char **argv) {
  if (argc <= 2) {
    TUTTLE_LOG_FATAL(
        "need one argument (image filename) : " << argv[0] << " image.jpg 0.5");
    return 1;
  }
  ...
  std::stringstream ss(
      argv[2]); // Could of course also have done ss("1234") directly.
  double coef1;
  ...
  // png_write_view( "output_crop.png", color_converted_view<rgb8_pixel_t>(
  // subimage_view(const_view(img_src), -250, -558, img_src.dimensions().x+800,
  // img_src.dimensions().y+1500 ) ) );

  fill_pixels(view(img_dst), rgb32f_pixel_t(0.8, 0.0, 0.0));

  tuttle::plugin::lens::AdvancedLensDistortParams<double> _p;
  /*
     point2<int> imgDataSize = point2<int>( img_src.dimensions().x,
     img_src.dimensions().y );
     ...
     _p._imgDiagonal = std::sqrt( _p._imgSize.x * _p._imgSize.x + _p._imgSize.y
     * _p._imgSize.y );
     _p._pixelRatio = 1.0;
  ...
  _p._imgDiagonal = std::sqrt(_p._imgCenterSrc.x * _p._imgCenterSrc.x +
                              _p._imgCenterSrc.y * _p._imgCenterSrc.y);
  _p._pixelRatio = 1;
  ...
  tuttle::plugin::lens::resample_pixels<bilinear_sampler>(
      const_view(img_src), view(img_dst),
      static_cast<tuttle::plugin::lens::NormalLensDistortParams<double> *>(&_p),
      procWindow, &progress);

  png_write_view("output_src.png",
                 color_converted_view<rgb8_pixel_t>(const_view(img_src)));
  png_write_view("output_dst.png",
                 color_converted_view<rgb8_pixel_t>(const_view(img_dst)));
  tiff_write_view("output_dst.tiff",
                  color_converted_view<rgb8_pixel_t>(const_view(img_dst)));
}
