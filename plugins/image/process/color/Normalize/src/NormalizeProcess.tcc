...
void NormalizeProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  for (std::ssize_t n = 0; n < boost::gil::num_channels<Pixel>::type::value;
       ++n) {
    _sMin[n] = smin[n];
    ...
    if ((smax[n] - smin[n]) == 0)
      ...
      _ratio[n] = channel_traits<tChannel>::max_value() *
                  (_params._dstMaxColor[n] - _params._dstMinColor[n]) /
                  (smax[n] - smin[n]);
      r[n] = 1.f * channel_traits<tChannel>::max_value() *
             (_params._dstMaxColor[n] - _params._dstMinColor[n]) /
             (smax[n] - smin[n]);
    }
  }
...
void NormalizeProcess<View>::processChannel(View &src, View &dst, bool process,
                                            Pixel32f &ratio, Pixel32f &sMin,
                                            Pixel32f &dMin) {
  using namespace terry;
  using namespace terry::numeric;
  ...
  if (process) {
    ...
    copy_pixels(channel_view<LocalChannel>(src),
                channel_view<LocalChannel>(dst));
  }
}
 ...
 */
template <class View>
void NormalizeProcess<View>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
  const OfxRectI procWindowOutput =
      this->translateRoWToOutputClipCoordinates(procWindowRoW);
  const OfxRectI procWindowSrc =
      translateRegion(procWindowRoW, this->_srcPixelRod);
  ...
  if (_params._processR && _params._processG && _params._processB &&
    ...
    transform_pixels_progress(
        src, dst, pixel_scale_t<Pixel, rgba32f_pixel_t>(_ratio, _sMin, _dMin),
        *this);
  } else {
    // by channel:
    // red
    {
      processChannel<View, red_t>(src, dst, _params._processR, _ratio, _sMin,
                                  _dMin);
    }
    // green
    {
      processChannel<View, green_t>(src, dst, _params._processG, _ratio, _sMin,
                                    _dMin);
    }
    // blue
    {
      processChannel<View, blue_t>(src, dst, _params._processB, _ratio, _sMin,
                                   _dMin);
    }
    // alpha
    {
      processChannel<View, alpha_t>(src, dst, _params._processA, _ratio, _sMin,
                                    _dMin);
    }
  }
}
...
template <>
void NormalizeProcess<boost::gil::rgb32f_view_t>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
  ...
  const OfxPointI procWindowSize = { procWindowRoW.x2 - procWindowRoW.x1,
                                     procWindowRoW.y2 - procWindowRoW.y1 };
  rgb32f_view_t src =
      subimage_view(this->_srcView, procWindowSrc.x1, procWindowSrc.y1,
                    procWindowSize.x, procWindowSize.y);
  rgb32f_view_t dst =
      subimage_view(this->_dstView, procWindowOutput.x1, procWindowOutput.y1,
                    procWindowSize.x, procWindowSize.y);
  ...
  if (_params._processR && _params._processG && _params._processB) {
    transform_pixels_progress(
        src, dst, pixel_scale_t<Pixel, Pixel>(ratio, sMin, dMin), *this);
  } else {
    // by channel:
    // red
    {
      processChannel<rgb32f_view_t, red_t>(src, dst, _params._processR, ratio,
                                           sMin, dMin);
    }
    // green
    {
      processChannel<rgb32f_view_t, green_t>(src, dst, _params._processG, ratio,
                                             sMin, dMin);
    }
    // blue
    {
      processChannel<rgb32f_view_t, blue_t>(src, dst, _params._processB, ratio,
                                            sMin, dMin);
    }
  }
}
...
template <>
void NormalizeProcess<boost::gil::rgb16_view_t>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
  ...
  if (_params._processR && _params._processG && _params._processB) {
    transform_pixels_progress(
        src, dst, pixel_scale_t<Pixel, rgb32f_pixel_t>(ratio, sMin, dMin),
        *this);
  } else {
    // by channel:
    // red
    {
      processChannel<rgb16_view_t, red_t>(src, dst, _params._processR, ratio,
                                          sMin, dMin);
    }
    // green
    {
      processChannel<rgb16_view_t, green_t>(src, dst, _params._processG, ratio,
                                            sMin, dMin);
    }
    // blue
    {
      processChannel<rgb16_view_t, blue_t>(src, dst, _params._processB, ratio,
                                           sMin, dMin);
    }
  }
}
...
template <>
void NormalizeProcess<boost::gil::rgb8_view_t>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
  ...
  if (_params._processR && _params._processG && _params._processB) {
    ...
    // red
    {
      processChannel<rgb8_view_t, red_t>(src, dst, _params._processR, ratio,
                                         sMin, dMin);
    }
    // green
    {
      processChannel<rgb8_view_t, green_t>(src, dst, _params._processG, ratio,
                                           sMin, dMin);
    }
    // blue
    {
      processChannel<rgb8_view_t, blue_t>(src, dst, _params._processB, ratio,
                                          sMin, dMin);
    }
  }
}
...
template <>
void NormalizeProcess<boost::gil::gray32f_view_t>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
...
template <>
void NormalizeProcess<boost::gil::gray16_view_t>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
  ...
  if (_params._processA) {
    transform_pixels_progress(
        src, dst, pixel_scale_t<Pixel, gray32f_pixel_t>(ratio, sMin, dMin),
        *this);
  }
}

template <>
void NormalizeProcess<boost::gil::gray8_view_t>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  using namespace terry::algorithm;
