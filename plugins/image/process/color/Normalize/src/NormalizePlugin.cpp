...
NormalizeProcessParams<NormalizePlugin::Scalar>
NormalizePlugin::getProcessParams(const OfxPointD &renderScale) const {
  using namespace boost::gil;
  NormalizeProcessParams<Scalar> params;

  params._mode = static_cast<EParamMode>(_mode->getValue());
  params._analyseMode =
      static_cast<EParamAnalyseMode>(_analyseMode->getValue());
...
void NormalizePlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamMode) {
    ...
    case OFX::ePixelComponentRGBA: {
      ...
      case OFX::eBitDepthFloat: {
        ...
        View srcView = getGilView<View>(src.get(), srcPixelRod,
...
void NormalizePlugin::getRegionsOfInterest(
...
bool NormalizePlugin::isIdentity(const OFX::RenderArguments &args,
