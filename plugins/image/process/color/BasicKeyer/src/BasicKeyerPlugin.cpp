...
BasicKeyerProcessParams<View> BasicKeyerPlugin::getProcessParams() const {
  ...
  unsigned int nbPoints =
      boost::numeric_cast<unsigned int>(_paramNbPoints->getValue());
...
void BasicKeyerPlugin::render(const OFX::RenderArguments &args) {
  ...
  switch (components) {
  case OFX::ePixelComponentRGBA: {
    doGilRender<BasicKeyerProcess, false, boost::gil::rgba_layout_t>(
        *this, args, bitDepth);
    return;
  }
...
void BasicKeyerPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  paramName.c_str() + kParamPoint.size() );
