...
void ColorTransferPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  ...
  desc.setDescription(
      "ColorTransfer\n"
      "\n"
      "The standard usage of this node is to impose one image's color "
      ...
      "\n"
      "Technically, the node modify the statistics of the image in a "
      "particular colorspace lambda-alpha-beta.\n"
      "This colorspace, created by Rederman and al., is closed to the human "
      "perception and minimizes "
      "the correlation between channels for many natural scenes.\n"
      "However, a choice parameter allows the user to choose in which "
      "colorspace the color transfer should be processed: "
      "LMS, lambda-alpha-beta or original/source colorspace.  \n"
      "According to the chosen colorspace, result varies (more or less "
      "colorfull, or dense...).\n\n"
      "For more details see the article:\n"
      "'Color Transfert between images' by Erik Reinhard, Michael Ashikhmin, "
      "Bruce Gooch and Peter Shirley. University of Utah.\n");
 ...
 */
void
ColorTransferPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *colorspace =
      desc.defineChoiceParam(kParamColorspace);
  colorspace->setLabel("Transformation colorspace");
  colorspace->setHint("Select colorspace in which to apply the transformation");
  colorspace->appendOption(kParamColorspaceNone,
                           "without colorspace transformation");
  colorspace->appendOption(kParamColorspaceLMS,
                           "apply correction in LMS colorspace");
  colorspace->appendOption(kParamColorspaceLab,
                           "apply correction in L(alpha)(beta) colorspace");
  colorspace->setDefault(eColorspaceLab);

  OFX::DoubleParamDescriptor *averageCoef =
      desc.defineDoubleParam(kParamAverageCoef);
  ...
  OFX::DoubleParamDescriptor *dynamicCoef =
      desc.defineDoubleParam(kParamDynamicCoef);
  ...
  //	// output region
  //	OFX::Double2DParamDescriptor* regionA = desc.defineDouble2DParam(
  //kParamRegionA );
  //	regionA->setLabel( "Region" );
  //	regionA->setDefault( -0.5, -0.5 );
  //	regionA->setIsSecret( true ); ///< @todo
  //	OFX::Double2DParamDescriptor* regionB = desc.defineDouble2DParam(
  //kParamRegionB );
  //	regionB->setLabel( "" );
  ...
  //
  //	// same region
  //	OFX::BooleanParamDescriptor* sameRegion = desc.defineBooleanParam(
  //kParamSameRegion );
  //	sameRegion->setDefault( true );
  //	sameRegion->setIsSecret( true ); ///< @todo
  //
  //	// input region
  //	OFX::Double2DParamDescriptor* inputRegionA = desc.defineDouble2DParam(
  //kParamInputRegionA );
  //	inputRegionA->setLabel( "Input region" );
  //	inputRegionA->setDefault( -0.5, -0.5 );
  //	inputRegionA->setIsSecret( true ); ///< @todo
  //	OFX::Double2DParamDescriptor* inputRegionB = desc.defineDouble2DParam(
  //kParamInputRegionB );
 ...
 */
OFX::ImageEffect *
ColorTransferPluginFactory::createInstance(OfxImageEffectHandle handle,
                                           OFX::EContext context) {
