...
CPixel getPixel(const Pixel &p, const EColorspace eColorspace) {
  ...
  case eColorspaceLMS: { return pixel_rgb_to_lms_t<Pixel, CPixel>()(p); }
  case eColorspaceLab: { return pixel_rgb_to_lab_t<Pixel, CPixel>()(p); }
  }
  BOOST_ASSERT(false);
  CPixel cp;
  pixel_assigns_t<Pixel, CPixel>()(p, cp);
...
CPixel setPixel(const Pixel &p, const EColorspace eColorspace) {
  ...
  case eColorspaceLMS: { return pixel_lms_to_rgb_t<Pixel, CPixel>()(p); }
  case eColorspaceLab: { return pixel_lab_to_rgb_t<Pixel, CPixel>()(p); }
  }
  BOOST_ASSERT(false);
  CPixel cp;
  pixel_assigns_t<Pixel, CPixel>()(p, cp);
...
template <class View> struct ColorParams {
  typedef typename View::value_type Pixel;
  Pixel _srcAverage, _dstAverage, _deviationRatio;
  EColorspace _eColorspace;

  ColorParams(const Pixel &srcAverage, const Pixel &dstAverage,
              const Pixel &deviationRatio, const EColorspace eColorspace) {
...
template <class View>
void
ColorTransferProcess<View>::computeAverage(const View &image, Pixel &average,
                                           Pixel &deviation,
  ...
  //	average = pixel_divides_scalar_t<CPixel, double, Pixel>()( sumAverage,
  // nbPixels );
  ...
  for (std::ssize_t y = 0; y < image.height(); ++y) {
    ...
    pixel_plus_assign_t<CPixel>()(pixel_divides_scalar_t<CPixel, double>()(
                                      sumDeviationLine, image.width()),
                                  cDeviation);
  }
...
void ColorTransferProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  _params =
      _plugin.getProcessParams(args.renderScale); // Retrieve plugin parameters
  ...
  if (_plugin._clipSrcRef->isConnected()) {
    ...
    if (OFX::getImageEffectHostDescription()->hostName ==
      ...
      _srcRefPixelRod =
          _plugin._clipSrcRef->getPixelRod(args.time, args.renderScale);
    } else {
  ...
  if (OFX::getImageEffectHostDescription()->hostName ==
    ...
    // bug in nuke, getRegionOfDefinition() on OFX::Image returns bounds
    _dstRefPixelRod =
        _plugin._clipDstRef->getPixelRod(args.time, args.renderScale);
  } else {
  ...
  computeAverage(this->_dstRefView, _dstRefAverage, dstRefDeviation,
                 _params._colorspace);
  // TUTTLE_LOG_VAR4( TUTTLE_INFO, _srcRefAverage[0], _srcRefDeviation[0],
  // _dstRefAverage[0], _dstRefDeviation[0]);
  ...
  _deviationRatio =
      pixel_divides_t<Pixel, Pixel, Pixel>()(dstRefDeviation, srcRefDeviation);
  ...
  if (_params._averageCoef != 1.0) {
    ...
    Pixel vecAverage =
        pixel_minus_t<Pixel, Pixel, Pixel>()(_dstRefAverage, _srcRefAverage);
    pixel_multiplies_scalar_assign_t<double, Pixel>()(_params._averageCoef,
                                                      vecAverage);
    // - apply this new vector to srcAverage
    _dstRefAverage =
        pixel_plus_t<Pixel, Pixel, Pixel>()(_srcRefAverage, vecAverage);
  }
  if (_params._dynamicCoef != 1.0) {
    ...
    pixel_multiplies_scalar_assign_t<double, Pixel>()(_params._dynamicCoef,
                                                      _deviationRatio);
 ...
 */
template <class View>
void ColorTransferProcess<View>::multiThreadProcessImages(
  ...
  terry::algorithm::transform_pixels_progress(
      src, dst, ColorParams<View>(_srcRefAverage, _dstRefAverage,
                                  _deviationRatio, _params._colorspace),
      *this);
}
}
}
