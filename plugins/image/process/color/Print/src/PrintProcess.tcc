...
template <class SView> CacaImage load_cacaimage_from_view(const SView sView) {
...
template <>
CacaImage load_cacaimage_from_view<boost::gil::gray8_view_t>(
    const boost::gil::gray8_view_t sView) {
  typedef boost::gil::channel_mapping_type<boost::gil::gray8_view_t>::type
  Channel;
  ...
  bmask = 0x0000ff00;
  amask = 0x00000000;
  bpp = boost::gil::num_channels<boost::gil::gray8_view_t>::value *
        sizeof(ChannelBaseType) * 8;
...
struct channel_cout_t : public std::unary_function<Channel, Channel> {
  GIL_FORCEINLINE
  Channel operator()(
      typename boost::gil::channel_traits<Channel>::const_reference ch) const {
    std::cout << ch << " ";
    return ch;
  }
};

template <template <class> class Func> struct call_pixel_by_channel_t {
...
template <class View> PrintProcess<View>::~PrintProcess() {}
...
template <class View>
void
PrintProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
      ...
      try {
        ...
        if (!cv) {
          BOOST_THROW_EXCEPTION(exception::Failed()
                                << exception::user(
                                       "Print: unable to initialise libcaca"));
        }
        switch (_params._colorMode) {
        case eParamColorMono:
         ...
         *  - \c "fullgray": use black, white and two shades of gray for both
         * the
        ...
        if (caca_set_dither_algorithm(cacaImg.dither,
                                      dither ? dither : "fstein")) {
          BOOST_THROW_EXCEPTION(exception::Unknown()
                                << exception::dev() +
                                       "Can't dither image with algorithm " +
                                       dither);
        }
        ...
        caca_dither_bitmap(cv, 0, 0, cols, lines, cacaImg.dither,
                           cacaImg.pixels);
        ...
        if (_params._openGlViewer == true) {
          ...
          caca_get_event(dp, CACA_EVENT_KEY_PRESS | CACA_EVENT_QUIT |
