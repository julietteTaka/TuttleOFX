...
template <typename Scalar> struct GammaProcessParams {
  double iRGamma, iGGamma, iBGamma, iAGamma;
};
...
public:
  ...
  GammaProcessParams<Scalar> getProcessParams(const OfxPointD &renderScale =
...
public:
  ...
  EGammaType getGammaType() const {
    return static_cast<EGammaType>(_gammaType->getValue());
  }
};
}
}
