 ...
 */
template <typename Scalar> struct HistogramKeyerProcessParams {
  ...
  OFX::BooleanParam *_boolClampCurveValues; // clamp curve values (Advanced
                                            // group)
};
...
public:
  ...
  bool _isNbStepChanged; // do we have to recompute selection histograms (nbStep
                         // has changed)
  bool _isHistogramRefresh; // do we have to recompute selection histograms
                            // (other reason)
  ...
  OFX::ChoiceParam *_paramDisplayTypeSelection; // display option list
                                                // global/adapted to channel
                                                // (Histogram overlay group)
  OFX::PushButtonParam *_paramResetAll; // clear all button (Histogram overlay
                                        // group)

  OFX::BooleanParam *_paramDisplaySelection; // display selection on clip source
                                             // (Selection group)
  OFX::IntParam *_paramSelectionFromCurve; // precision of selection to curve
                                           // (Selection group)
  OFX::ChoiceParam *_paramSelectionMode; // selection mode
                                         // unique/additive/subtractive
                                         // (Selection group)

  OFX::IntParam *_paramNbStepSelection; // step selection (Advanced group)
  OFX::DoubleParam *_paramSelectionMultiplierSelection; // selection multiplier
                                                        // (Advanced group)
  OFX::PushButtonParam *_paramRefreshOverlaySelection; // refresh overlay button
                                                       // (Advanced group)
  OFX::BooleanParam *_paramClampCurveValues; // clamp curve values (Advanced
                                             // group)

  OFX::ChoiceParam *_paramOutputSettingSelection; // output display list
                                                  // (BW/Alpha)
  OFX::BooleanParam *_paramReverseMaskSelection; // revert mask check box

  OFX::PushButtonParam *_paramButtonAppendToSelectionHSL; // append to selection
                                                          // HSL - Push button
  OFX::PushButtonParam *_paramButtonAppendToSelectionRGB; // append to selection
                                                          // RGB - Push button

  bool _isRendering; // is plugin rendering ? (if rendering don't modify data)

  /*Overlay data parameters*/
  boost::scoped_ptr<OverlayData> _overlayData; // scoped pointer points the
                                               // overlay data (or NULL)
  std::size_t _overlayDataCount; // count (class calling scoped pointer)

  /*Creators*/
  HistogramKeyerPlugin(OfxImageEffectHandle handle);
  HistogramKeyerProcessParams<Scalar>
  getProcessParams(const OfxTime time,
