...
RGBOverlay::RGBOverlay(OfxInteractHandle handle, OFX::ImageEffect*
effect):OFX::OverlayInteract( handle ),_infos( effect )
{
...
/*Destructor*/
RGBOverlay::~RGBOverlay() { _plugin->releaseOverlayData(); }
...
bool RGBOverlay::draw(const OFX::DrawArgs &args) {
  ...
  if (!(_plugin->_paramDisplayTypeSelection->getValue() == 1)) {
    ...
    Number maxR = *(std::max_element(getOverlayData()._data._bufferRed.begin(),
                                     getOverlayData()._data._bufferRed.end()));
    Number maxG =
        *(std::max_element(getOverlayData()._data._bufferGreen.begin(),
                           getOverlayData()._data._bufferGreen.end()));
    Number maxB = *(std::max_element(getOverlayData()._data._bufferBlue.begin(),
                                     getOverlayData()._data._bufferBlue.end()));
  ...
  float selectionMultiplier =
      (float)_plugin->_paramSelectionMultiplierSelection
          ->getValue(); // get selection multiplier from plugin
  ...
  if (_plugin->_paramOverlayRSelection->getValue()) // RED CHANNEL
  {
    ...
    displayASpecificHistogram(getOverlayData()._data._bufferRed,
                              getOverlayData()._selectionData._bufferRed, step,
                              heightR, size.x, redHisto, selectionMultiplier);
    if (getOnlyChannelSelectedRGB() == eSelectedChannelR) {
      displaySelectionPoints(getOverlayData()._selectionData._bufferRed, step,
                             size.x, redHisto); // selection points
      displayAverageBar(getOverlayData()._averageData._averageRed, averageHisto,
                        size.x, size.y, step); // average bar
  ...
  if (_plugin->_paramOverlayGSelection->getValue()) // GREEN CHANNEL
  {
    ...
    displayASpecificHistogram(getOverlayData()._data._bufferGreen,
                              getOverlayData()._selectionData._bufferGreen,
                              step, heightG, size.x, greenHisto,
                              selectionMultiplier);
    if (getOnlyChannelSelectedRGB() == eSelectedChannelG) {
      displaySelectionPoints(getOverlayData()._selectionData._bufferGreen, step,
                             size.x, greenHisto); // selection points
      displayAverageBar(getOverlayData()._averageData._averageGreen,
                        averageHisto, size.x, size.y, step); // average bar
  ...
  if (_plugin->_paramOverlayBSelection->getValue()) // BLUE CHANNEL
  {
    ...
    displayASpecificHistogram(getOverlayData()._data._bufferBlue,
                              getOverlayData()._selectionData._bufferBlue, step,
                              heightB, size.x, blueHisto, selectionMultiplier);
    if (getOnlyChannelSelectedRGB() == eSelectedChannelB) {
      displaySelectionPoints(getOverlayData()._selectionData._bufferBlue, step,
                             size.x, blueHisto); // selection points
      displayAverageBar(getOverlayData()._averageData._averageBlue,
                        averageHisto, size.x, size.y, step); // average bar
  ...
  if (_plugin->_paramOverlayRSelection->getValue())
    displayASpecificHistogramBorder(getOverlayData()._data._bufferRed, step,
                                    heightR, size.x, redHisto); // R
  if (_plugin->_paramOverlayGSelection->getValue())
    displayASpecificHistogramBorder(getOverlayData()._data._bufferGreen, step,
                                    heightG, size.x, greenHisto); // G
  if (_plugin->_paramOverlayBSelection->getValue())
    displayASpecificHistogramBorder(getOverlayData()._data._bufferBlue, step,
                                    heightB, size.x, blueHisto); // B
  return true;
}

/**
 * Permits to know if there is only one RGB channel selected
 * @return "R", "G", "B" if there is only one channel selected,  "none" or
 * "more" else
 */
 ...
 * @return
 */
OverlayData &RGBOverlay::getOverlayData() { return _plugin->getOverlayData(); }
