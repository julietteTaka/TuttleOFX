...
/*
 * structure which contains selection average for each channel to display
 * average bar
 */
 ...
 */
typedef boost::multi_array<unsigned char, 2, OfxAllocator<unsigned char> >
bool_2d;
...
struct Pixel_compute_histograms {
  ...
  std::ssize_t _y, _x; // position of the current pixel (functor needs to know
                       // which pixel is it)
  bool _isSelectionMode; // do we work on all of the pixels (normal histograms)
                         // or only on selection

  Pixel_compute_histograms(bool_2d &selection, HistogramBufferData &data,
                           const bool isSelectionMode)
      : _data(data), _imgBool(selection), _height(_imgBool.shape()[0]),
        _width(_imgBool.shape()[1]), _y(0), _x(0),
  ...
  template <typename Pixel> Pixel operator()(const Pixel &p) {
    ...
    if (ok) // if current pixel is selected
    {
      ...
      // HLS
      for (int v = 0; v < boost::gil::num_channels<hsl32f_pixel_t>::type::value;
           ++v) {
        val = hsl_pix[v];
        if (val >= 0 && val <= 1) {
          double inter = round(val * (_data._step - 1));
          ...
          if (v == 0)
            ...
            _data._bufferLightness.at(indice) +=
                1; // increments saturation buffer
          else if (v == 1)
            _data._bufferSaturation.at(indice) +=
                1; // increments lightness buffer
        }
      }
    }
...
public:
  ...
  /**
   * Image size checker
   * @warning HACK changeClip method doesn't work in nuke when time of source
   * clip is changed so we have to check size of imgBool all the time
   */
   ...
   * Histogram computing
   */
  void computeFullData(OFX::Clip *clipSrc, const OfxTime time,
                       const OfxPointD &renderScale,
                       const bool selectionOnly =
                           false); // compute full data
                                   // (average/selection/histograms)
  void computeCurveFromSelectionData(OFX::Clip *clipSrc, const OfxTime time,
                                     const OfxPointD &renderScale); // compute
                                                                    // only
                                                                    // selection
                                                                    // to curve
                                                                    // data
...
private:
  ...
  void computeHistogramBufferData(HistogramBufferData &data, SView &srcView,
                                  const OfxTime time,
                                  const bool isSelection =
                                      false); // compute a HisogramBufferData
  void correctHistogramBufferData(HistogramBufferData &toCorrect)
      const; // correct a complete HistogramBufferData
  void resetHistogramBufferData(HistogramBufferData &toReset)
      const; // reset a complete HistogramBufferData
  ...
  void resetVectortoZero(HistogramVector &v, const std::size_t size)
      const; // reset a specific channel buffer
  int computeAnAverage(const HistogramVector &selection_v) const; // compute
                                                                  // average of
                                                                  // a specific
                                                                  // channel
...
public:
  ...
  HistogramBufferData _curveFromSelection; // curve from selection histogram
                                           // data
  ...
  std::size_t _vNbStepCurveFromSelection; // nbStep for curve to selection
                                          // buffers
  bool _isComputing;
