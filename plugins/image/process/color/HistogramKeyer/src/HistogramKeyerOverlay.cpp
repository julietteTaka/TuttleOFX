...
HistogramKeyerOverlay::HistogramKeyerOverlay(OfxInteractHandle handle,
                                             OFX::ImageEffect *effect)
    : OFX::OverlayInteract(handle),
      _plugin(static_cast<HistogramKeyerPlugin *>(_effect)), _infos(effect),
      _hslParam(_plugin), _rgbParam(_plugin) {
...
bool HistogramKeyerOverlay::draw(const OFX::DrawArgs &args) {
  ...
  const OfxPointI imgSize =
      _plugin->_clipSrc->getPixelRodSize(args.time, args.renderScale);
  const OfxRectI pixelRegionOfDefinition =
      _plugin->_clipSrc->getPixelRod(args.time);

  // Global display option
  if (_plugin->_paramGlobalDisplaySelection->getValue() == false)
    return false;

  if (_isFirstTime || getOverlayData()._isDataInvalid ||
      getOverlayData().isCurrentTimeModified(args.time) ||
      getOverlayData().isImageSizeModified(imgSize) ///< HACK changeClip method
                                                    ///doesn't work in nuke when
                                                    ///source clip is changed so
                                                    ///we have to check size of
                                                    ///imgBool all of the time
      ) {
    ...
    if (!_plugin->_isRendering) {
      ...
      getOverlayData().computeFullData(_plugin->_clipSrc, args.time,
                                       args.renderScale);
    } else // Data is not updated : draw warning signal
    {
      ...
      const double ratio = (viewport[2] - viewport[0]) /
                           (double)(viewport[3] - viewport[1]); // compute ratio
...
bool HistogramKeyerOverlay::penMotion(const OFX::PenArgs &args) {
  if (_penDown &&
      !_keyDown) // the mouse is moving but there is not Ctrl key pressed
  {
  ...
  {
    const OfxRectI pixelRegionOfDefinition = _plugin->_clipSrc->getPixelRod(
        args.time, args.renderScale); // pixel region of definition
    ...
    if (_plugin->_paramSelectionMode->getValue() ==
        2) // selection mode is subtractive mode
      getOverlayData()._imgBool[y][x] =
          0; // current pixel is no more marked as selected
    else
      getOverlayData()._imgBool[y][x] =
          255; // current pixel is marked as selected
    return true; // event captured
  }
...
bool HistogramKeyerOverlay::penDown(const OFX::PenArgs &args) {
  const OfxPointI fullsize =
      _plugin->_clipSrc->getPixelRodSize(args.time); // full size
  const OfxPointI imgSize = _plugin->_clipSrc->getPixelRodSize(
      args.time, args.renderScale); // size with renderscale

  if (!_penDown && !_keyDown &&
      _plugin->_paramDisplaySelection->getValue()) // mouse is already used and
                                                   // there is not Ctrl key
                                                   // pressed
  {
    _penDown = true;
    if (args.penPosition.y < fullsize.y &&
        args.penPosition.y > 0) // mouse Y is into the image
      _origin.y = args.penPosition.y * args.renderScale.y;
    else {
      if (args.penPosition.y >
          fullsize.y) // clamp the selected Y pixel to the image borders
    ...
    if (args.penPosition.x < fullsize.x &&
        args.penPosition.x > 0) // mouse X is on the image
      _origin.x = args.penPosition.x * args.renderScale.x;
    else {
      if (args.penPosition.x >
          fullsize.x) // clamp the selected X pixel to the image borders
        _origin.x = imgSize.x; // click is on the right of the image
      else
        _origin.x = 0; // click is on the left of the image
    }
    _end.x = args.penPosition.x * args.renderScale.x; // set X end of the
                                                      // selection square at the
                                                      // origin (initialization)
    _end.y = args.penPosition.y * args.renderScale.y; // set Y end of the
                                                      // selection square at the
                                                      // origin (initialization)

    _squareBegin.x = _squareEnd.x =
        args.penPosition.x; // copy x value to square position
    _squareBegin.y = _squareEnd.y =
        args.penPosition.y; // copy v value to square position
...
bool HistogramKeyerOverlay::penUp(const OFX::PenArgs &args) {
  const OfxPointI fullSize =
      _plugin->_clipSrc->getPixelRodSize(args.time); // full image size
  const OfxPointI imgSize = _plugin->_clipSrc->getPixelRodSize(
      args.time, args.renderScale); // size with renderscale
  const OfxRectI pixelRegionOfDefinition = _plugin->_clipSrc->getPixelRod(
      args.time, args.renderScale); // pixel region of definition
  ...
  // clamp selection
  _end.x = args.penPosition.x * args.renderScale.x; // attach the end of the
                                                    // selection square to the
                                                    // current pixel X
  _end.y = args.penPosition.y * args.renderScale.y; // attach the end of the
                                                    // selection square to the
                                                    // current pixel Y
        ...
        args.penPosition.y > 0)) // if click is not on the image
  {
    ...
    if (args.penPosition.y < 0.0 ||
        args.penPosition.y > fullSize.y) // problem with Y axis
    {
  ...
  if (_penDown && !_keyDown) // if there is not Ctrl key pressed
  {
    ...
    BOOST_ASSERT(endY >= 0);
    BOOST_ASSERT(endX >= 0);
    BOOST_ASSERT(getOverlayData()._imgBool.shape()[0] >=
                 std::size_t(endY + step_y));
    BOOST_ASSERT(getOverlayData()._imgBool.shape()[1] >=
                 std::size_t(endX + step_x));

    unsigned char fillValue;
    if (_plugin->_paramSelectionMode->getValue() ==
        2) // selection mode is subtractive
    ...
    for (int val_y = 0; val_y < step_y; ++val_y) {
      for (int val_x = 0; val_x < step_x; ++val_x) {
        const int y =
            endY + val_y -
            pixelRegionOfDefinition.y1; // y in img bool size (reformat)
        const int x =
            endX + val_x -
            pixelRegionOfDefinition.x1; // x in img bool size (reformat)
...
bool HistogramKeyerOverlay::keyDown(const OFX::KeyArgs &args) {
  if (args.keySymbol == kOfxKey_Control_L ||
      args.keySymbol ==
          kOfxKey_Control_R) // if the pressed key is Ctrl key (left or right)
  {
...
bool HistogramKeyerOverlay::keyUp(const OFX::KeyArgs &args) {
  if ((args.keySymbol == kOfxKey_Control_L ||
       args.keySymbol == kOfxKey_Control_R) &&
 ...
 */
void HistogramKeyerOverlay::displaySelectedAreas(const OfxPointI &fullImgSize,
                                                 const OfxPointI &imgSize,
  ...
  glBegin(GL_QUADS);
  glTexCoord2d(0, 1);
  glVertex2i(pixelRoD.x1, pixelRoD.y2); // glVertex2d(0,fullImgSize.y);
                                        // //Top Left
  glTexCoord2d(1, 1);
  glVertex2i(
      pixelRoD.x2,
      pixelRoD.y2); // glVertex2d(fullImgSize.x,fullImgSize.y);	//Top Right
  glTexCoord2d(1, 0);
  glVertex2i(pixelRoD.x2, pixelRoD.y1); // glVertex2d(fullImgSize.x,0);
                                        // //Bottom Right
  glTexCoord2d(0, 0);
  glVertex2i(pixelRoD.x1, pixelRoD.y1); // glVertex2d(0,0);
                                        // //Bottom Left
  glEnd();
 ...
 */
void HistogramKeyerOverlay::drawWarning(const Ofx3DPointD &centerPoint,
                                        const double ratio) {
  ...
  glVertex2d((centerPoint.x - 0.025 * ratio),
             centerPoint.y); // first point of triangle
  glVertex2d((centerPoint.x + 0.025 * ratio),
             centerPoint.y); // second point of triangle
  glVertex2d(centerPoint.x,
             (centerPoint.y + 0.085 * ratio)); // third point of triangle
  glVertex2d((centerPoint.x - 0.025 * ratio),
             centerPoint.y); // first point of triangle (boucle)
