...
void CTLPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  ...
  desc.setDescription(
      ...
      "language that has been designed to serve as a building "
      "block for digital color management systems. CTL allows users to "
      "describe color transforms in a concise and unambiguous way "
      "by expressing them as programs."
      "\n"
      "Color transforms can be shared by distributing CTL programs. Two "
      "parties with the same CTL program can apply the same "
      "transform to an image. "
      "In addition to the original image, a CTL program can have input "
      "parameters whose settings affect how the input image will be "
      "transformed. For example, a transform may have an 'exposure' parameter, "
      "such that changing the exposure makes the image "
      "brighter or darker. In order to guarantee identical results, parties "
      "that have agreed to use a particular transform must also agree "
      "on the settings for the transform's parameters.\n"
      "A domain-specific programming language such as CTL can be designed to "
      "allow only the kinds of operations that are needed to"
      "describe color transforms. This improves the portability of programs, "
      "protects users against application software crashes and"
...
void CTLPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *chooseInput =
      desc.defineChoiceParam(kParamChooseInput);
  ...
  OFX::PushButtonParamDescriptor *updateRender =
      desc.definePushButtonParam(kParamChooseInputCodeUpdate);
  updateRender->setLabel("Update");
  updateRender->setHint("Rendering the CTL code");

  OFX::StringParamDescriptor *file =
