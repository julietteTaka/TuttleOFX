 ...
 */
void loadModule(Ctl::Interpreter &interpreter, const std::string &moduleName,
                const std::string &code);
void loadModuleRecursive(Ctl::Interpreter &interpreter,
...
void ctlMessageOutput(const std::string &message) {
  if (ctlPlugin) {
    ctlPlugin->sendMessage(OFX::Message::eMessageMessage, "CTL message",
                           message);
  }
}

template <class Type>
void fillInputArg(Ctl::FunctionArgPtr &arg, const std::string &argStr,
                  const Type &v, const std::size_t n) {
  if (!arg ||
      //		!arg->type().cast<half>() ||
      !arg->isVarying()) {
    ...
    BOOST_THROW_EXCEPTION(
        Iex::ArgExc(std::string("Cannot set value of argument ") + argStr));
  }
...
template <class Type>
void retrieveOutputArg(const Ctl::FunctionArgPtr &arg,
                       const std::string &argStr, Type &v,
                       const std::size_t n) {
  if (!arg ||
      //		!arg->type( ).cast<half>() ||
      !arg->isVarying()) {
...
template <class Type>
void callCtlChunk(Ctl::FunctionCallPtr call, const std::size_t n, Type &rOut,
                  Type &gOut, Type &bOut, Type &aOut, const Type &r,
                  const Type &g, const Type &b, const Type &a) {
...
template <class Type>
void callCtl(Ctl::Interpreter &interp, Ctl::FunctionCallPtr call,
             const std::size_t size, Type *rOut, Type *gOut, Type *bOut,
             Type *aOut, const Type *r, const Type *g, const Type *b,
             const Type *a) {
  std::size_t n = size;
  while (n > 0) {
...
void CTLProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  ...
  for (int y = procWindowRoW.y1; y < procWindowRoW.y2; ++y) {
    View srcLineV =
        subimage_view(this->_srcView, procWindowRoW.x1, y, procWindowSize.x, 1);
    View dstLineV =
        subimage_view(this->_dstView, procWindowRoW.x1, y, procWindowSize.x, 1);
    ...
    callCtl<float>(_interpreter, call, procWindowSize.x, rOut, gOut, bOut, aOut,
                   r, g, b, a);
