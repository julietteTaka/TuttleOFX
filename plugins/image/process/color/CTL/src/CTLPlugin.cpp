...
CTLProcessParams<CTLPlugin::Scalar>
CTLPlugin::getProcessParams(const OfxPointD &renderScale) const {
...
void CTLPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                             const std::string &paramName) {
  if (paramName == kParamChooseInput) {
    EParamChooseInput input =
        static_cast<EParamChooseInput>(_paramInput->getValue());
    switch (input) {
    case eParamChooseInputCode: {
...
bool
CTLPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args,
                                 OfxRectD &rod) {
...
bool CTLPlugin::isIdentity(const OFX::RenderArguments &args,
...
void CTLPlugin::render(const OFX::RenderArguments &args) {
  ...
  case OFX::ePixelComponentRGBA: {
    doGilRender<CTLProcess, false, boost::gil::rgba_layout_t>(*this, args,
                                                              bitDepth);
    return;
  }
