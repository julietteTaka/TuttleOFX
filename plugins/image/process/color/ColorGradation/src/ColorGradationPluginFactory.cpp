...
void ColorGradationPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  ...
  // For the moment, this node is float only to handle clamping pb. TODO fix
  // correctly this
 ...
 */
void
ColorGradationPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::BooleanParamDescriptor *alpha =
      desc.defineBooleanParam(kParamProcessAlpha);
  alpha->setLabel("Process alpha");
  alpha->setHint("Apply the conversion on alpha channel.");
  alpha->setDefault(false);

  OFX::PushButtonParamDescriptor *invert =
      desc.definePushButtonParam(kParamInvert);
 ...
 */
OFX::ImageEffect *
ColorGradationPluginFactory::createInstance(OfxImageEffectHandle handle,
                                            OFX::EContext context) {
