...
ColorTransformPlugin::getProcessParams(const OfxPointD &renderScale) const {
...
void ColorTransformPlugin::changedParam(const OFX::InstanceChangedArgs &args,
...
bool ColorTransformPlugin::isIdentity(const OFX::RenderArguments &args,
...
void ColorTransformPlugin::render(const OFX::RenderArguments &args) {
  ...
  /*
          // instantiate the render code based on the pixel depth of the dst
     clip
          ...
          switch( components )
          {
                  ...
                  {
                          doGilRender<ColorTransformProcess, false,
     ...
     mapPixelComponentEnumToString(components) + ") not supported by the
     plugin." );
                  }
          }
