...
Matrix5x5 colorTransformMatrix(const ColorTransformParams &params) {
  ...
  matrix = prec_prod(
      matrix, terry::color::hueRotateMatrix<Matrix5x5>(params._hueShift));
  std::cout << __LINE__ << " matrix: " << matrix << std::endl;
  matrix = prec_prod(
      matrix, terry::color::saturationMatrix<Matrix5x5>(params._saturation));
  std::cout << __LINE__ << " matrix: " << matrix << std::endl;

  matrix = prec_prod(matrix, terry::color::scaleMatrix<Matrix5x5>(
  ...
  matrix = prec_prod(matrix, terry::color::offsetMatrix<Matrix5x5>( // - Pivot
  ...
  matrix =
      prec_prod(matrix, terry::color::contrastMatrix<Matrix5x5>( // Contrast
...
template <class ChannelColor, class Pixel>
typename boost::gil::channel_type<Pixel>::type get_color_if_exist(
    const Pixel &p,
    const typename boost::gil::channel_type<Pixel>::type &elseValue) {
  using namespace boost::gil;
  typedef typename contains_color<Pixel, ChannelColor>::type hasChannel;

  return get_color_if_exist_impl<ChannelColor, Pixel>(p, elseValue,
                                                      hasChannel());
}

template <std::size_t y, class Matrix5x5, class Pixel>
inline typename boost::gil::channel_type<Pixel>::type
...
inline void matrixProdChannelAssignImpl(Pixel &dst, const Matrix5x5 &matrix,
                                        const Pixel &src,
                                        const boost::mpl::true_) {
  using namespace boost::gil;
...
inline void matrixProdChannelAssignImpl(Pixel &dst, const Matrix5x5 &matrix,
                                        ...
                                        const boost::mpl::false_) {}
template <class ChannelColor, std::size_t y, class Matrix5x5, class Pixel>
inline void matrixProdChannelAssign(Pixel &dst, const Matrix5x5 &matrix,
                                    const Pixel &src) {
  using namespace boost::gil;
  typedef typename contains_color<Pixel, ChannelColor>::type hasChannel;
...
template <class Matrix5x5> struct MatrixProdPixel {
  Matrix5x5 _matrix;
  MatrixProdPixel(const Matrix5x5 &matrix) : _matrix(matrix) {}

  template <typename Pixel> Pixel operator()(const Pixel &src) const {
    using namespace boost::gil;
