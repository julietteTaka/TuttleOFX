...
IdKeyerProcessParams<View> IdKeyerPlugin::getProcessParams() const {
  ...
  for (size_t i = 0; i < nbPoints; ++i, ++it_color) {
    ...
    // TUTTLE_LOG_WARNING( "key color " <<  c.r << ", " <<  c.g << ", " <<  c.b
    // << ", " << c.a );
  }
  return params;
}
...
void IdKeyerPlugin::render(const OFX::RenderArguments &args) {
  ...
  case OFX::ePixelComponentRGBA: {
    doGilRender<IdKeyerProcess, false, boost::gil::rgba_layout_t>(*this, args,
                                                                  bitDepth);
    return;
  }
...
void IdKeyerPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                                 const std::string &paramName) {
  if (paramName == kParamNbPoints) {
    const size_t nbPoints =
        boost::numeric_cast<size_t>(_paramNbPoints->getValue());
