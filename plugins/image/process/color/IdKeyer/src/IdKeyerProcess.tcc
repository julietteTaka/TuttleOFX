...
IdKeyerProcess<View>::IdKeyerProcess(IdKeyerPlugin &instance)
    : Parent(instance, eImageOrientationIndependant), _plugin(instance) {}
...
struct id_keyer_t {
  ...
  id_keyer_t(PixelList colors, double tolerance, bool useAlpha)
      : _colors(colors), _tolerance(tolerance), _useAlpha(useAlpha) {}

  template <class Pixel> Pixel operator()(const Pixel &src) const {
    ...
    for (PixelList::const_iterator color = _colors.begin();
         color != _colors.end(); ++color) {
      // TUTTLE_LOG_WARNING( get_color( res, red_t()   ) << "\t" << get_color(
      // *color, red_t()   ) );
      P min(*color);
      P max(*color);
      double minFactor = 1.0 - _tolerance;
      double maxFactor = 1.0 + _tolerance;
      numeric::pixel_multiplies_scalar_assign_t<double, P>()(
          minFactor, min); // min *= minFactor
      numeric::pixel_multiplies_scalar_assign_t<double, P>()(
          maxFactor, max); // max *= maxFactor
...
template <class View>
void
IdKeyerProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  using namespace terry;
