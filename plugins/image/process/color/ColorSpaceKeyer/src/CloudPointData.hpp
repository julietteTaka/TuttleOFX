...
struct pixel_is_lesser_t {
  ...
  bool operator()(const PixelRef1 &p1, const PixelRef2 &p2) const {
    for (int v = 0; v < 3 /*boost::gil::num_channels<Pixel>::type::value*/;
         ++v) {
      if (p1[v] < p2[v])
        return true;
      if (p1[v] > p2[v])
...
struct Pixel_copy {
  ...
  bool _isSelection; // is current operation on selection (only take good alpha
                     // pixels)

  // Constructor
  Pixel_copy(DataVector &data, bool isSelection = false)
      : _data(data), _isSelection(isSelection) {}
  // Operator ()
  template <typename Pixel> Pixel operator()(const Pixel &p) {
    ...
    for (int v = 0; v < 3 /*boost::gil::num_channels<Pixel>::type::value*/;
         ++v) {
      const float val = boost::gil::channel_convert<boost::gil::bits32f>(
          p[v]); // capt channel (red, green or blue)
...
template <typename Pixel> struct Pixel_copy_discretization {
  typedef std::set<SPixel, tuttle::plugin::colorSpaceKeyer::pixel_is_lesser_t<
                               SPixel, SPixel> > PixelSet;
  ...
  Pixel_copy_discretization(DataVector &data, int nbStep,
      ...
      : _data(data), _nbStep(nbStep), _isSelection(isSelection) {
  ...
  Pixel operator()(const Pixel &p) {
    ...
    // recopy channels
    for (int v = 0; v < 3 /*boost::gil::num_channels<Pixel>::type::value*/;
         ++v) // We don't want work with alpha channel
    {
    ...
    _setData.insert(
        add); // insert current pixel into the set (doubles are not allowed)
    return p;
  }
...
protected:
  ...
  public:
    ...
    VBO(const void *data, int dataSize, GLenum usage)
        : _id(0), _size(0), _color(0), _colorDifferent(0) // Constructor
    {
  ...
  public:
    ...
    void genBuffer(unsigned int &id, const void *data, int size, GLenum target,
                   GLenum usage); // generate VBO data
    void genBufferColor(unsigned int &idColor, const void *data, int size,
                        GLenum target, GLenum usage); // generate VBO data
...
public:
  ...
  bool _isSelectionVBOBuilt; // if color selection VBO is not built don't draw
                             // it on overlay
  bool _isSpillSelectionVBOBuilt; // if spill selection VBO is not built don't
                                  // draw it on overlay

  // VBO to draw
  VBO _imgVBO; // VBO to display on overlay (cloud point VBO)
  VBO _selectionColorVBO; // VBO to display on overlay (color selection in
                          // white)
  VBO _selectionSpillVBO; // VBO to display on overlay (spill selection in
                          // silver)
...
public:
  ...
  // VBO management
  bool generateVBOData(OFX::Clip *clipSrc, const OfxPointD &renderScale,
                       bool vboWithDiscretization,
                       int discretizationStep); // create new VBO data (fill up
                                                // buffer)
  bool generateColorSelectionVBO(OFX::Clip *clipColor,
                                 const OfxPointD &renderScale,
                                 bool vboWithDiscretization,
                                 int discretizationStep); // Change color of
                                                          // selected pixel
                                                          // (color clip)
  bool generateSpillSelectionVBO(OFX::Clip *clipSpill,
                                 const OfxPointD &renderScale,
                                 bool vboWithDiscretization,
                                 int discretizationStep); // Change color of
                                                          // selected pixel
...
private:
  // VBO data management
  int generateAllPointsVBOData(SView srcView); // generate a VBO with all of the
                                               // pixels
  int generateDiscretizedVBOData(
      SView srcView,
      const int &discretizationStep); // generate a  VBO with discretization
  // selection VBO data management
  int generateAllPointsSelectionVBOData(SView srcView); // generate a VBO (and
                                                        // color) with all of
                                                        // the selected pixels
  int generateAllPointsSpillVBOData(SView srcView); // generate a VBO (and
                                                    // color) with all of the
                                                    // selected pixels
};
}
}
