...
ColorSpaceKeyerPlugin::ColorSpaceKeyerPlugin(OfxImageEffectHandle handle)
  ...
  _paramBoolPointCloudDisplay = fetchBooleanParam(
      kPointCloudDisplay); // Is CloudPointData displayed (boolean parameter)
  _cloudPointDataCount = 0;

  // Associate intern parameters pointers to GUI components
  _paramBoolDiscretizationActive =
      fetchBooleanParam(kBoolDiscretizationDisplay); // is discretization active
                                                     // on point cloud - check
                                                     // box
  _paramIntDiscretization = fetchIntParam(
      kIntDiscretizationDisplay); // discretization step - Int parameter
  _paramIntNbOfDivisionsGF =
      fetchIntParam(kIntNumberOfDivisonGeodesicForm); // number of divisions
                                                      // geodesic form - Int
                                                      // parameter
  _paramBoolDisplayGeodesicForm = fetchBooleanParam(
      kBoolOnlySelection); // display geodesic form - check box
  _paramChoiceAverageMode = fetchChoiceParam(
      kColorAverageMode); // average color mode - Choice parameter
  _paramRGBAColorSelection = fetchRGBAParam(
      kColorAverageSelection); // average color selection - RGBA parameter
  _paramPushButtonAverageCompute = fetchPushButtonParam(
      kColorAverageComputing); // average color computing - Push button
  _paramBoolSeeSelection = fetchBooleanParam(
      kBoolColorSelectionDisplay); // see selection on overlay - check box
  _paramDoubleScaleGF = fetchDoubleParam(
      kDoubleScaleGeodesicForm); // scale geodesic form - double parameter
  _paramBoolSeeSpillSelection = fetchBooleanParam(
      kBoolSpillSelectionDisplay); // see spill selection - check box
  _paramBoolDisplaySpillGF = fetchBooleanParam(
      kBoolDisplaySpillGF); // see spill geodesic form - check box

  // verify display Discrete enable value
  if (_paramBoolPointCloudDisplay->getValue()) // called default value
  {
    _paramBoolDiscretizationActive->setEnabled(
        true); // Enable discretization check box
    _paramIntDiscretization->setEnabled(
        true); // Enable discretization int parameter
    _paramBoolDisplayGeodesicForm->setEnabled(
        true); // Enable see color geodesic form display
    _paramBoolSeeSelection->setEnabled(
        true); // Enable see color selection display
    _paramBoolDisplaySpillGF->setEnabled(
        true); // Enable see spill geodesic form display
    _paramBoolSeeSpillSelection->setEnabled(
        true); // Enable see spill selection display
  }
  // verify choice average enable value
  if (_paramChoiceAverageMode->getValue() == 1) {
    _paramRGBAColorSelection->setEnabled(
        true); // Enable color average selection (RGBA parameter)
    _paramPushButtonAverageCompute->setEnabled(
        true); // Enable color average computing (Push button)
  }
...
ColorSpaceKeyerPlugin::getProcessParams(const OfxPointD &renderScale) const {
  ColorSpaceKeyerProcessParams<Scalar> params; // create parameters container
                                               // object
 ...
 */
void ColorSpaceKeyerPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                                         const std::string &paramName) {
  if (paramName ==
      kPointCloudDisplay) // display point cloud check box value has changed
  {
    if (_paramBoolPointCloudDisplay->getValue() &&
        hasCloudPointData()) // display point cloud is selected
    {
      ...
      _paramIntDiscretization->setEnabled(
          true); // Enable discretization int parameter
      _paramBoolDisplayGeodesicForm->setEnabled(
          true); // Enable geodesic form display
      ...
      getCloudPointData().generateVBOData( // create a VBO
          ...
          _paramBoolDiscretizationActive->getValue(), // is discretization mode
                                                      // active
          _paramIntDiscretization->getValue()); // get discretization step
      // generate color selection VBO data (object created in overlay)
      getCloudPointData().generateColorSelectionVBO( // create a color selection
                                                     // VBO
      ...
      getCloudPointData().generateSpillSelectionVBO( // create a spill selection
                                                     // VBO
    ...
    } else // display point cloud is not selected
    {
      ...
      _paramBoolDiscretizationActive->setEnabled(
          false); // Disable discretization treatment
      _paramIntDiscretization->setEnabled(
          false); // Disable discretization choice
      _paramBoolDisplayGeodesicForm->setEnabled(
          false); // Disable geodesic form display
      _paramBoolSeeSelection->setEnabled(false); // Disable geodesic form
                                                 // display
      _paramBoolDisplaySpillGF->setEnabled(
          false); // Disable see spill geodesic form display
      _paramBoolSeeSpillSelection->setEnabled(
          false); // Disable see spill selection display
    }
  }
  if (paramName ==
      kBoolDiscretizationDisplay) // discretization active check box changed
  {
  ...
  if (paramName ==
      kIntDiscretizationDisplay) // discretization value has changed (int range)
  {
    if (hasCloudPointData() &&
        _paramBoolDiscretizationActive->getValue()) // it is not batch mode
    {
      if (_paramBoolDiscretizationActive->getValue()) // discretization is
                                                      // active
      {
        getCloudPointData()._time = args.time; // update time
        getCloudPointData().generateVBOData( // create a VBO data with
                                             // discretization (and the new
                                             // discretization step)
            _clipSrc, // source clip
            args.renderScale, // current render scale
  ...
  if (paramName == kIntNumberOfDivisonGeodesicForm) // number of divisions
                                                    // geodesic form has changed
                                                    // (int range)
  {
    if (hasCloudPointData()) // if there is overlay data
    {
    ...
    _renderAttributes.recomputeGeodesicForm =
        true; // recompute process attributes
  }
  ...
  if (paramName == kColorAverageMode &&
      hasCloudPointData()) // Average mode : choice list
  {
  ...
  if (paramName == kColorAverageSelection &&
      hasCloudPointData()) // selection average RGBA component
  {
    updateGeodesicForms(args);
  }
  if (paramName == kColorAverageComputing &&
      hasCloudPointData()) // push button : average computing (selection =>
                           // manual)
  {
    // update automatic average
    getCloudPointData()._averageColor._time = args.time; // change current time
    getCloudPointData()._averageColor.computeAverageSelection(
        _clipColor, args.renderScale); // compute automatic average
    ...
    changedParam(args, kColorAverageSelection); // call changedParam function
 ...
 * If clip has changed
 */
void ColorSpaceKeyerPlugin::changedClip(const OFX::InstanceChangedArgs &args,
                                        const std::string &clipName) {
  if (clipName ==
      kOfxImageEffectSimpleSourceClipName) // if source clip has changed
  {
    if (this->hasCloudPointData()) // it is not batch mode
    {
      ...
      if (test->isConnected()) {
        getCloudPointData()._time = args.time; // update time
        getCloudPointData().generateVBOData( // create a VBO data with or
                                             // without discretization
            _clipSrc, // source clip
            args.renderScale, // current render scale
            _paramBoolDiscretizationActive->getValue(), // is discretization
                                                        // used to generate VBO
  ...
  } else if (clipName == kClipSpillSelection) // if color clip has changed
  {
    ...
    if (this->hasCloudPointData()) // it is not batch mode
    {
      ...
      if (test->isConnected()) {
        ...
        getCloudPointData()._time = args.time; // update time
        getCloudPointData().generateSpillSelectionVBO( // create a spill
                                                       // selection VBO data
                                                       // with or without
                                                       // discretization
            _clipSpill, // spill clip
...
bool ColorSpaceKeyerPlugin::isIdentity(const OFX::RenderArguments &args,
...
void ColorSpaceKeyerPlugin::render(const OFX::RenderArguments &args) {
  if (OFX::getImageEffectHostDescription()->hostName ==
          "uk.co.thefoundry.nuke" && /// HACK: Nuke doesn't call changeClip
                                     /// function when time is changed
      hasCloudPointData())           // if there is overlay data
  {
    if (args.time != getCloudPointData()._time) // different time between
                                                // overlay and VBO data
    {
      // update VBO data
      getCloudPointData()._time =
          args.time; // change computing time in cloud point data
      getCloudPointData().generateVBOData( // create a VBO data (with
    ...
    if (args.time != getCloudPointData()._averageColor._time) // different time
                                                              // between overlay
                                                              // and average
                                                              // data
    {
      ...
      getCloudPointData()._averageColor.computeAverageSelection(
          _clipColor, args.renderScale); // update average value
      // color geodesic form
      getCloudPointData()._geodesicFormColor._scale =
          _paramDoubleScaleGF->getValue(); // set scale value
      // spill geodesic form
      getCloudPointData()._geodesicFormSpill._scale =
          _paramDoubleScaleGF->getValue(); // set scale value
      ...
      getCloudPointData()._averageColor.extendGeodesicForm(
          _clipColor, args.renderScale,
          getCloudPointData()._geodesicFormColor); // extends geodesic form
                                                   // (color)
      getCloudPointData()._geodesicFormSpill.copyGeodesicForm(
          getCloudPointData()._geodesicFormColor); // extends geodesic form
                                                   // (spill)
      getCloudPointData()._averageColor.extendGeodesicForm(
          _clipSpill, args.renderScale,
          getCloudPointData()._geodesicFormSpill); // extends geodesic form
                                                   // (spill)
    }
  }
  ...
  if (_renderAttributes.recomputeGeodesicForm ||
      _renderAttributes.time != args.time)
...
/*
 *Update geodesic forms for overlay (and call update function for process
 *parameters)
 * @param args
 */
void ColorSpaceKeyerPlugin::updateGeodesicForms(
  ...
  getCloudPointData()._averageColor._time =
      args.time; // set current time into average
  getCloudPointData()._averageColor.computeAverageSelection(
      _clipColor, args.renderScale); // update automatic average
  if (_paramChoiceAverageMode->getValue() == 1) // average mode is manual
  {
    // get current average selection
    OfxRGBAColourD selectionColor =
        _paramRGBAColorSelection->getValue(); // get current average selection
  ...
  getCloudPointData()._averageColor.extendGeodesicForm(
      _clipColor, args.renderScale,
      getCloudPointData()._geodesicFormColor); // extends color geodesic form
  getCloudPointData()._geodesicFormSpill.copyGeodesicForm(
      getCloudPointData()._geodesicFormColor); // extends spill geodesic form
                                               // (color clip)
  getCloudPointData()._averageColor.extendGeodesicForm(
      _clipSpill, args.renderScale,
      getCloudPointData()._geodesicFormSpill); // extends spill geodesic form
                                               // (spill clip)
  // scale geodesic form
  getCloudPointData()._geodesicFormColor.scaleGeodesicForm(
      _paramDoubleScaleGF->getValue());
  getCloudPointData()._geodesicFormSpill.scaleGeodesicForm(
      _paramDoubleScaleGF->getValue());
  _updateVBO = true; // update VBO in overlay
  _renderAttributes.recomputeGeodesicForm = true; // recompute process
                                                  // attributes
  this->redrawOverlays(); // redraw scene
}
 ...
 * @param args
 */
void ColorSpaceKeyerPlugin::updateProcessGeodesicForms(
    const OFX::RenderArguments &args) {
  // update time
  _renderAttributes.time = args.time; // update time
  // Create geodesic form
  SelectionAverage selectionAverage =
      SelectionAverage(_time); // create selection
  // update color geodesic form
  _renderAttributes.geodesicFormColor._scale =
      _paramDoubleScaleGF->getValue(); // set good scale to geodesic form
  // update spill geodesic form
  _renderAttributes.geodesicFormSpill._scale =
      _paramDoubleScaleGF->getValue(); // set good scale to geodesic forms

  if (_paramChoiceAverageMode->getValue() == 0) // average mode is automatic
  {
    selectionAverage.computeAverageSelection(
        _clipColor, _renderScale); // compute average selection
    // subdivise geodesic forms
    _renderAttributes.geodesicFormColor.subdiviseFaces(
        selectionAverage._averageValue,
        _paramIntDiscretization->getValue()); // create geodesic form
    _renderAttributes.geodesicFormSpill.subdiviseFaces(
        selectionAverage._averageValue,
        _paramIntDiscretization->getValue()); // create geodesic form
  } else // average mode is manual
  {
    Ofx3DPointD selectedAverage; // initialize average
    OfxRGBAColourD colorSelected =
        _paramRGBAColorSelection->getValue(); // get selected color
  ...
  selectionAverage.extendGeodesicForm(
      _clipColor, _renderScale,
      _renderAttributes.geodesicFormColor); // extends geodesic form color
  _renderAttributes.geodesicFormSpill.copyGeodesicForm(
      _renderAttributes.geodesicFormColor); // extends geodesic form spill
                                            // (color clip)
  selectionAverage.extendGeodesicForm(
      _clipSpill, _renderScale,
      _renderAttributes.geodesicFormSpill); // extends geodesic form spill
                                            // (spill takes account of spill
                                            // clip)
