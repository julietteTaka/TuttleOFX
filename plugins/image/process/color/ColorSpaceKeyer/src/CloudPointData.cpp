...
CloudPointData::CloudPointData(const OfxPointI &size, OfxTime time)
    : _averageColor(
          time) // Selection average constructor (current time is needed)
{
  ...
  // selection buffer
  _selectionCopy.reserve(imgSize *
                         0.5); // reserve memory for color selection buffer
  _spillCopy.reserve(imgSize * 0.5); // reserve memory for spill selection
                                     // buffer
 ...
 */
bool CloudPointData::generateVBOData(OFX::Clip *clipSrc,
                                     const OfxPointD &renderScale,
                                     const bool vboWithDiscretization,
  ...
  boost::scoped_ptr<OFX::Image> src(clipSrc->fetchImage(
  ...
  const OfxRectI srcPixelRod =
      clipSrc->getPixelRod(_time, renderScale); // get current RoD
  if ((clipSrc->getPixelDepth() != OFX::eBitDepthFloat) ||
      (clipSrc->getPixelComponents() == OFX::ePixelComponentNone)) {
    BOOST_THROW_EXCEPTION(exception::Unsupported()
                          << exception::user() + "Can't compute histogram data "
                                                 "with the actual input clip "
                                                 "format.");
    return false;
  }
  ...
  if (srcPixelRod != src->getBounds()) // the host does bad things !
  {
    ...
    TUTTLE_LOG_WARNING("Image RoD and image bounds are not the same (rod="
                       << srcPixelRod << " , bounds:" << src->getBounds()
                       << ").");
    return false;
  }
  // Compute if source is OK
  SView srcView = tuttle::plugin::getGilView<SView>(
      src.get(), srcPixelRod,
  ...
  {
    generateDiscretizedVBOData(
        srcView, discretizationStep); // create data and return buffer size
  } else {
...
void CloudPointData::updateVBO() {
  // point cloud VBO
  _imgVBO.createVBO(&(_imgCopy.front()),
                    _imgCopy.size() / 3); // generate VBO to draw
  _imgVBO._color = true; // activate color for VBO
  // color selection VBO
  _selectionColorVBO._colorDifferent =
      true; // color buffer is not the same than vertex buffer
  _selectionColorVBO._color = false; // disable color for VBO
  _selectionColorVBO.createVBO(
      &(_selectionCopy.front()), _selectionCopy.size() / 3, GL_STATIC_DRAW,
      &(_selectionCopy.front())); // generate color selection VBO to draw
  // spill selection VBO
  _selectionSpillVBO._colorDifferent =
      true; // color buffer is not the same than vertex buffer
  _selectionSpillVBO._color = false; // disable color for VBO
  _selectionSpillVBO.createVBO(
      &(_spillCopy.front()), _spillCopy.size() / 3, GL_STATIC_DRAW,
...
int CloudPointData::generateAllPointsVBOData(SView srcView) {
  // compute buffer size
  int size = (int)(srcView.height() *
                   srcView.width()); // return size : full image here

  // copy full image into buffer
  Pixel_copy funct(_imgCopy); // functor declaration
  // treatment
  terry::algorithm::transform_pixels(
      srcView, funct); // transform pixel did with functor reference
  return size;
}
 ...
 */
int CloudPointData::generateDiscretizedVBOData(SView srcView,
                                               const int &discretizationStep) {
  // compute buffer size
  int size = (int)(srcView.height() *
                   srcView.width()); // return size : full image here
  ...
  Pixel_copy_discretization<SPixel> funct(
      _imgCopy, discretizationStep); // functor declaration
 ...
 *
 */
bool CloudPointData::generateColorSelectionVBO(OFX::Clip *clipColor,
                                               const OfxPointD &renderScale,
                                               bool vboWithDiscretization,
  ...
  boost::scoped_ptr<OFX::Image> src(clipColor->fetchImage(
      _time, clipColor->getCanonicalRod(_time))); // scoped pointer of current
                                                  // color clip
  ...
  const OfxRectI srcPixelRod =
      clipColor->getPixelRod(_time, renderScale); // get current RoD
  ...
  generateAllPointsSelectionVBOData(
      srcView); // generate a selection VBO without discretization
 ...
 *
 */
bool CloudPointData::generateSpillSelectionVBO(OFX::Clip *clipSpill,
                                               const OfxPointD &renderScale,
                                               bool vboWithDiscretization,
                                               int discretizationStep) {
  ...
  boost::scoped_ptr<OFX::Image> src(clipSpill->fetchImage(
      _time, clipSpill->getCanonicalRod(_time))); // scoped pointer of current
                                                  // color clip
  ...
  const OfxRectI srcPixelRod =
      clipSpill->getPixelRod(_time, renderScale); // get current RoD
...
void CloudPointData::VBO::deleteVBO() {
  if (_id != 0) // if VBO exists
  {
    ...
    _id = 0; // 0 is reserved, glGenBuffersARB() will return non-zero id if
             // success
  }
  if (_idColor != 0) {
 ...
 */
void CloudPointData::VBO::genBuffer(unsigned int &id, const void *data,
  ...
  if (dataSize !=
      bufferSize) // mismatch between data size and input array (control)
  {
    deleteVBO();
    std::cout << "[createVBO()] Mismatch between Data size and input array"
              << std::endl;
  }
}

void CloudPointData::VBO::genBufferColor(unsigned int &idColor,
...
void CloudPointData::VBO::draw() {
  if (_id && _size) {
    ...
    if (_color) // draw vector using color
    {
      ...
      glColorPointer(3, GL_FLOAT, 0, 0); // point buffer which is used for
                                         // colors (same than vertex or not)
