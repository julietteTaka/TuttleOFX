...
void GeodesicForm::createGeodesicForm(const Ofx3DPointD &center) {
  ...
  // double kRadiusFromCenter = .5f;                        // radius of
  // GeodesicForm
  double altitude =
      sqrt((double)3.0) / 2 *
      kRadiusFromCenter; // altitude of an equilateral triangle is sqrt(3)/2*a
  ...
  */
  ...
  // base1
  base1.x = center.x - (squareDiag / 2.0); // X axis (use square diagonal to
                                           // translate on x axis)
...
void GeodesicForm::draw(bool alpha) {
  ...
  for (unsigned int i = 0; i < _faces.size(); ++i) {
    ...
    if (!_faces[i].triangles.empty()) // if there are triangles (subdivision) in
                                      // the current face
    {
      glBegin(GL_TRIANGLES);
      for (unsigned int j = 0; j < _faces[i].triangles.size();
           ++j) // for each triangle of the current face
      {
        if (alpha)
        ...
        Ofx3DPointD tr1 =
            *(_faces[i].triangles[j].point1); // get point1 of current triangle
        glVertex3d(tr1.x, tr1.y, tr1.z);      // draw point
        Ofx3DPointD tr2 =
            *(_faces[i].triangles[j].point2); // get point2 of current triangle
        glVertex3d(tr2.x, tr2.y, tr2.z);      // draw point
        Ofx3DPointD tr3 =
            *(_faces[i].triangles[j].point3); // get point3 of current triangle
  ...
  // draw points
  for (unsigned int i = 0; i < _points.size();
       ++i) // for each points in point vector
  ...
  if (_hasIntersection) {
    ...
    glVertex3d(_intersection.point1->x, _intersection.point1->y,
               _intersection.point1->z); // point 1
    glVertex3d(_intersection.point2->x, _intersection.point2->y,
               _intersection.point2->z); // point 2
    glVertex3d(_intersection.point3->x, _intersection.point3->y,
               _intersection.point3->z); // point 3
    glEnd();

    // draw intersection point
    glBegin(GL_POINTS);
    glColor3f(0.0f, 1.0f, 1.0f); // color cyan
    glVertex3d(_intersectionPoint.x, _intersectionPoint.y,
               _intersectionPoint.z); // intersection point
    glEnd();
  }
}
...
void GeodesicForm::testOnePointFunction() {
  ...
  normTestPointCenter +=
      testPointCenterVector[0] * testPointCenterVector[0]; // add x*x value
  normTestPointCenter +=
      testPointCenterVector[1] * testPointCenterVector[1]; // add y*y value
  normTestPointCenter +=
      testPointCenterVector[2] * testPointCenterVector[2]; // add z*z value
  normTestPointCenter = sqrt(normTestPointCenter);         // compute norm

  std::cout << "norm Intersection normal : " << normTestPointCenter
            << std::endl; // display norm
 ...
 */
void GeodesicForm::subdiviseFaces(const Ofx3DPointD &center,
                                  const int divisor) {
  ...
  for (unsigned int i = 0; i < _points.size(); ++i)
    currentPointToSphere(_points[i],
                         kRadiusFromCenter); // translate current point
...
void GeodesicForm::subdiviseOneFace(PyramidTriangle &f, const int divisor) {
  ...
  for (unsigned int y = divisor; y > 0; --y) // Y axis
  {
    ...
    for (unsigned int x = 0; x < y; ++x) // X axis
    {
      ...
      {
        if (_points[pt2].x == pX.x && _points[pt2].y == pX.y &&
            _points[pt2].z == pX.z) //_points[i] == point2
      ...
      {
        if (_points[pt3].x == pX2.x && _points[pt3].y == pX2.y &&
            _points[pt3].z == pX2.z) //_points[i] == point3
      ...
      {
        if (fabs(up.x - _points[pt1].x) < epsilon &&
            fabs(up.y - _points[pt1].y) < epsilon &&
      ...
      {
        if (fabs(bottom.x - _points[pt1].x) < epsilon &&
            fabs(bottom.y - _points[pt1].y) < epsilon &&
 ...
 */
void GeodesicForm::createPointsOneFace(const PyramidTriangle &f,
                                       const int divisor) {
  ...
  for (unsigned int y = divisor; y > 0; --y) // Y axis
  {
    ...
    for (unsigned int x = 0; x <= y; ++x) // X axis
    {
      ...
      if (pX.x == facePoint1.x && pX.y == facePoint1.y &&
          pX.z == facePoint1.z) { // current point is f.point1
      } else if (pX.x == facePoint2.x && pX.y == facePoint2.y &&
                 pX.z == facePoint2.z) { // current point is f.point2
      } else if (pX.x == facePoint3.x && pX.y == facePoint3.y &&
                 pX.z == facePoint3.z) { // current point is f.point3
      } else {
 ...
 */
bool GeodesicForm::getIntersection(const Ofx3DPointD &point,
  ...
  double directionVectorTriangle1[3]; // initialize first direction vector of
                                      // the triangle
  double directionVectorTriangle2[3]; // initialize second direction vector of
                                      // the triangle
  // compute values of first direction vector of the triangle
  directionVectorTriangle1[0] =
      triangle.point2->x - triangle.point1->x; // X value
  directionVectorTriangle1[1] =
      triangle.point2->y - triangle.point1->y; // Y value
  directionVectorTriangle1[2] =
      triangle.point2->z - triangle.point1->z; // Z value
  // compute values of second direction vector of the triangle
  directionVectorTriangle2[0] =
      triangle.point3->x - triangle.point1->x; // X value
  directionVectorTriangle2[1] =
      triangle.point3->y - triangle.point1->y; // Y value
  directionVectorTriangle2[2] =
      triangle.point3->z - triangle.point1->z; // Z value

  // Multiply directionVectorTriangle2 by directionVector
  double p[3]; // initialization
  // vector product
  p[0] = directionVector[1] * directionVectorTriangle2[2] -
         directionVector[2] * directionVectorTriangle2[1]; // X axis
  p[1] = directionVector[2] * directionVectorTriangle2[0] -
         directionVector[0] * directionVectorTriangle2[2]; // Y axis
  p[2] = directionVector[0] * directionVectorTriangle2[1] -
         directionVector[1] * directionVectorTriangle2[0]; // Z axis
  ...
  if (fabs(firstCheck) < epsilon) // if firstCheck == 0
  {
    ...
    q[0] =
        s[1] * directionVectorTriangle1[2] - s[2] * directionVectorTriangle1[1];
    q[1] =
        s[2] * directionVectorTriangle1[0] - s[0] * directionVectorTriangle1[2];
    q[2] =
        s[0] * directionVectorTriangle1[1] - s[1] * directionVectorTriangle1[0];
    ...
    _intersectionPoint.x = (1 - u - v) * triangle.point1->x +
                           u * triangle.point2->x +
                           v * triangle.point3->x; // X value
    _intersectionPoint.y = (1 - u - v) * triangle.point1->y +
                           u * triangle.point2->y +
                           v * triangle.point3->y; // Y value
    _intersectionPoint.z = (1 - u - v) * triangle.point1->z +
                           u * triangle.point2->z +
                           v * triangle.point3->z; // Z value
...
void GeodesicForm::testIntersection(const Ofx3DPointD &testPoint) {
  ...
  pointChangeReference.x =
      testPoint.x - _center.x; // X value in changed reference
  pointChangeReference.y =
      testPoint.y - _center.y; // Y value in changed reference
  pointChangeReference.z =
      testPoint.z - _center.z; // Z value in changed reference
...
bool GeodesicForm::isPointIntoGeodesicForm(const Ofx3DPointD &testPoint) {
  ...
  double normFirstVector =
      testPointVector[0] * testPointVector[0]; // initialize to x*x
  ...
  // test inverse intersection (if inverse intersection work point is into
  // extended geodesic form)
  Ofx3DPointD pointChangeReference; // testPoint is geodesicForm reference
  pointChangeReference.x =
      testPoint.x - _center.x; // X value in changed reference
  pointChangeReference.y =
  ...
  while (i < 8 && !intersectionFound) {
    ...
    while (j < _faces[i].triangles.size() &&
           !intersectionFound) // while there are triangle on current faces and
                               // there is not intersection
    {
      ...
      double s, t;
      if (getIntersection2(testPoint, tr, _intersectionPoint, s, t,
                           true)) // test inverse intersection
      {
...
void GeodesicForm::extendOnePoint(Ofx3DPointD &testPoint) {
  if (isPointIntoGeodesicForm(testPoint)) // current point is already into
                                          // geodesic form (do not extends)
  {
    return; // end of function
  }
  if (!testIntersection2(testPoint)) // current point is not into geodesic form
                                     // and there is not intersection
  {
  ...
  normTestPointCenterVector +=
      testPointCenterVector[0] * testPointCenterVector[0]; // add x*x
  normTestPointCenterVector +=
      testPointCenterVector[1] * testPointCenterVector[1]; // add y*y
  normTestPointCenterVector +=
      testPointCenterVector[2] * testPointCenterVector[2]; // add z*z
  ...
  if (extendsOneTrianglePoint(*(_intersection.point1), testPointCenterVector,
                              normTestPointCenterVector)) // try to extends
    updateBoundingBox(*(_intersection.point1)); //_intersection.point1 has been
                                                //changed => update bounding box
  // Extends triangle.point2
  if (extendsOneTrianglePoint(*(_intersection.point2), testPointCenterVector,
                              normTestPointCenterVector)) // try to extends
    updateBoundingBox(*(_intersection.point2)); //_intersection.point2 has been
                                                //changed => update bounding box
  // Extends triangle.point3
  if (extendsOneTrianglePoint(*(_intersection.point3), testPointCenterVector,
                              normTestPointCenterVector)) // try to extends
    updateBoundingBox(*(_intersection.point3)); //_intersection.point3 has been
                                                //changed => update bounding box
}
 ...
 */
bool
GeodesicForm::extendsOneTrianglePoint(Ofx3DPointD &pointToMove,
                                      const double *testPointCenterVector,
  ...
  normVectorCenterPointToMove +=
      vectorCenterPointToMove[0] * vectorCenterPointToMove[0]; // add x*x
  normVectorCenterPointToMove +=
      vectorCenterPointToMove[1] * vectorCenterPointToMove[1]; // add y*y
  normVectorCenterPointToMove +=
      vectorCenterPointToMove[2] * vectorCenterPointToMove[2]; // add z*z
  normVectorCenterPointToMove =
      sqrt(normVectorCenterPointToMove); // compute norm
  // normalize center-point to move vector
  vectorCenterPointToMove[0] /= normVectorCenterPointToMove; // normalize X
                                                             // value
  vectorCenterPointToMove[1] /= normVectorCenterPointToMove; // normalize Y
                                                             // value
  vectorCenterPointToMove[2] /= normVectorCenterPointToMove; // normalize Z
                                                             // value

  // compute translation triangle.point3
  double cosAngleIntersectionPointToMove =
      DOT(vectorCenterPointToMove, testPointCenterVector);
  // compute new norm triangle.point3
  double hypothenusCenterPointToMove =
      normTestPointCenterVector /
      cosAngleIntersectionPointToMove; // compute new norm
  if (normVectorCenterPointToMove >
      hypothenusCenterPointToMove) // compare current norm with existing norm
  {
    // nothing to do point to move is enough far of center
    return false; // point has not been changed
  } else {
    hypothenusCenterPointToMove -=
        normVectorCenterPointToMove; // compute value to add
    // compute translation
    vectorCenterPointToMove[0] *=
        hypothenusCenterPointToMove; // get coordinate X
    vectorCenterPointToMove[1] *=
        hypothenusCenterPointToMove; // get coordinate Y
    vectorCenterPointToMove[2] *=
        hypothenusCenterPointToMove; // get coordinate Z
 ...
 */
void GeodesicForm::currentPointToSphere(Ofx3DPointD &point,
                                        const double &radius) {
  ...
  normVectorCenterPoint +=
      vectorCenterPoint[0] * vectorCenterPoint[0]; // add x*x
  normVectorCenterPoint +=
      vectorCenterPoint[1] * vectorCenterPoint[1]; // add y*y
  normVectorCenterPoint +=
      vectorCenterPoint[2] * vectorCenterPoint[2]; // add z*z
 ...
 */
bool GeodesicForm::getIntersection2(const Ofx3DPointD &point,
                                    const PyramidTriangle &pyramidTriangle,
                                    Ofx3DPointD &intersectionPoint, double &s,
                                    double &t, const bool &inverse) {
  ...
  u[0] =
      pyramidTriangle.point2->x - pyramidTriangle.point1->x; // compute x value
  u[1] =
      pyramidTriangle.point2->y - pyramidTriangle.point1->y; // compute y value
  u[2] =
      pyramidTriangle.point2->z - pyramidTriangle.point1->z; // compute z value
  // Compute second edge
  double v[3]; // initialize
  v[0] =
      pyramidTriangle.point3->x - pyramidTriangle.point1->x; // compute x value
  v[1] =
      pyramidTriangle.point3->y - pyramidTriangle.point1->y; // compute y value
  v[2] =
      pyramidTriangle.point3->z - pyramidTriangle.point1->z; // compute z value
 ...
 */
bool GeodesicForm::testIntersection2(const Ofx3DPointD &testPoint,
                                     const bool &inverse,
                                     const bool justIntersectionPoint) {
  ...
  while (i < 8 && !intersectionFound) {
    ...
    if (getIntersection2(testPoint, _faces[i], _intersectionPoint, s, t,
                         inverse)) // test intersection with current face (stock
                                   // parametric coordinates)
    {
      if (justIntersectionPoint) // if we just want to know if there is an
                                 // intersection
        return true; // there is an intersection (no more computing)

      // compute pyramid side step
      double pyramidStep =
          (double)1 / (double)_nbDivisions; // compute pyramid side step
      // compute parametric coordinate axis X
      double parametricCoordinateX =
          floor(s / pyramidStep); // define parametric X axis value
      // compute parametric coordinate axis Y
      double parametricCoordinateY =
          floor(t / pyramidStep); // define parametric Y axis value

      // define triangle indice
      int indiceTr = 0;
      if (parametricCoordinateX ==
          0) // if intersected triangle is on the first line
      {
        indiceTr = 2 * parametricCoordinateY; // define intersected triangle
                                              // indice on first line
      } else // intersected triangle indice is not on first line
      {
        ...
        {
          indiceTr += (((int)_nbDivisions - indice) * 2) -
                      1; // increments triangle indice
      ...
      int indiceIntersected =
          0; // check intersection triangles (2 possibilities)
      while (!hasIntersected && indiceIntersected < 3) {
        ...
        hasIntersected = getIntersection2(
            testPoint, pyTr, _intersectionPoint, s, t,
      ...
      if (!hasIntersected) // intersection has not been found yet (test hard
                           // method)
      {
        unsigned int j =
            0; // declare indice to ride through triangle vector of current face
        while (j < _faces[i].triangles.size() &&
               !intersectionFound) // test for each triangle of current face
        {
          ...
          if (getIntersection2(testPoint, test, _intersectionPoint, sT, tT)) {
            ...
            _idFaceIntersection = i; // Keep intersection face
            _hasIntersection = true; // An intersection has been found
            _intersection = _faces[i].triangles[j]; // keep intersection
                                                    // triangle
          }
          ++j; // increments indice
        }
...
/*
 * update bounding box (if geodesic form point vector has changed cause of an
 * extend)
 */
void GeodesicForm::updateBoundingBox() {
  ...
  for (unsigned int i = 0; i < _points.size(); ++i) {
    ...
    if (_points[i].x > _boundingBox.max.x ||
        _points[i].x < _boundingBox.min.x) {
      if (_points[i].x > _boundingBox.max.x)
    ...
    if (_points[i].y > _boundingBox.max.y ||
        _points[i].y < _boundingBox.min.y) {
      if (_points[i].y > _boundingBox.max.y)
    ...
    if (_points[i].z > _boundingBox.max.z ||
        _points[i].z < _boundingBox.min.z) {
      if (_points[i].z > _boundingBox.max.z)
...
bool GeodesicForm::isIntoBoundingBox(const Ofx3DPointD &testPoint) {
  if (testPoint.x >= _boundingBox.min.x &&
      testPoint.x <= _boundingBox.max.x) // test X axis
  {
    if (testPoint.y >= _boundingBox.min.y &&
        testPoint.y <= _boundingBox.max.y) // test Y axis
    {
      if (testPoint.z >= _boundingBox.min.z &&
          testPoint.z <= _boundingBox.max.z) // test Z axis
