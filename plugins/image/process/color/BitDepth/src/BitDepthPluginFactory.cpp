...
void BitDepthPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  ...
  desc.setDescription("Convert channels bit depth."
                      "\n"
                      "\n"
                      "In computer graphics, color depth or bit depth is the "
                      "number of bits used to "
                      "represent the color of a single pixel in a bitmapped "
                      "image or video frame "
                      "buffer. This concept is also known as bits per pixel "
                      "(bpp), particularly when "
                      "specified along with the number of bits used. Higher "
                      "color depth gives a "
...
void BitDepthPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *outBitDepth =
      desc.defineChoiceParam(kParamOutputBitDepth);
 ...
 */
OFX::ImageEffect *
BitDepthPluginFactory::createInstance(OfxImageEffectHandle handle,
                                      OFX::EContext context) {
