...
template <class View>
OCIOColorSpaceProcess<View>::OCIOColorSpaceProcess(
    OCIOColorSpacePlugin &instance)
...
void OCIOColorSpaceProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  try {
    ...
    _config->getProcessor(_params._inputSpace.c_str(),
                          _params._outputSpace.c_str());
  }
  catch (OCIO::Exception &exception) {
    BOOST_THROW_EXCEPTION(exception::File()
                          << exception::user(exception.what()));
  }
}
...
void OCIOColorSpaceProcess<View>::applyLut(View &dst, View &src) {
  ...
  try {
    OCIO::ConstProcessorRcPtr processor = _config->getProcessor(
        _params._inputSpace.c_str(), _params._outputSpace.c_str());
    ...
    if (is_planar<View>::value) {
      ...
      for (std::size_t y = 0; y < (unsigned int)dst.height(); ++y) {
        ...
        OCIO::PackedImageDesc imageDesc(
            (float *)&(dst(0, y)[0]), dst.width(), 1,
            num_channels<View>::type::value, OCIO::AutoStride,
            dst.pixels().pixel_size(), dst.pixels().row_size());
