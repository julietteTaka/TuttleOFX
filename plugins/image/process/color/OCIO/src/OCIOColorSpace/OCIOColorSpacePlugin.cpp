...
void OCIOColorSpacePlugin::render(const OFX::RenderArguments &args) {
  if (!_wasOCIOVarFund) {
    BOOST_THROW_EXCEPTION(exception::Unsupported()
                          << exception::user() + "Tuttle.ocio.colorspace : an "
                                                 "OCIO environnement variable "
                                                 "must be set with a path to "
  ...
  case OFX::ePixelComponentRGBA: {
    ...
    case OFX::eBitDepthFloat: {
      doGilRender<OCIOColorSpaceProcess, false, rgba_layout_t, bits32f>(*this,
                                                                        args);
      return;
    }
  ...
  case OFX::ePixelComponentRGB: {
    switch (bitDepth) {
    case OFX::eBitDepthFloat: {
      doGilRender<OCIOColorSpaceProcess, false, rgb_layout_t, bits32f>(*this,
                                                                       args);
      return;
    }
    case OFX::eBitDepthUByte:
...
OCIOColorSpacePlugin::getProcessParams(const OfxPointD &renderScale) const {
