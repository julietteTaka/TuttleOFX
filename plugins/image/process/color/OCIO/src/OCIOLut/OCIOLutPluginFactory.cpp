...
void OCIOLutPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  ...
  desc.setDescription("OCIO Lut\n"
                      "\n"
                      "Lut input filename (3dl, ccc, cc, csp, cub, cube, hdl, "
 ...
 */
void OCIOLutPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
                                             OFX::EContext context) {
  OFX::ClipDescriptor *srcClip =
      desc.defineClip(kOfxImageEffectSimpleSourceClipName);
  srcClip->addSupportedComponent(
      OFX::ePixelComponentRGBA); //@todo mfe, test this
  // srcClip->addSupportedComponent( OFX::ePixelComponentAlpha );//@todo mfe,
  // add and test this (cf OCIOPlugin.cpp, render)
  ...
  OFX::ChoiceParamDescriptor *interpolationType =
      desc.defineChoiceParam(kParamInterpolationType);
 ...
 */
OFX::ImageEffect *
OCIOLutPluginFactory::createInstance(OfxImageEffectHandle handle,
                                     OFX::EContext context) {
