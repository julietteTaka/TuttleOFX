...
OCIOLutProcessParams
OCIOLutPlugin::getProcessParams(const OfxPointD &renderScale) const {
  using namespace boost::filesystem;

  OCIOLutProcessParams params;
  params._interpolationType = getOCIOInterpolationType(
  ...
  if (!bfs::exists(params._filename)) {
    BOOST_THROW_EXCEPTION(exception::FileNotExist()
                          << exception::filename(params._filename));
  }
