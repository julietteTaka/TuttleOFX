...
void OCIOLutProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  try {
    ...
    BOOST_THROW_EXCEPTION(exception::File() << exception::user() +
                                                   "OCIO Error: " +
                                                   exception.what());
  }
}
...
template <class View>
void
OCIOLutProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
...
void OCIOLutProcess<View>::applyLut(View &dst, View &src) {
  ...
  try {
    ...
    BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() +
                                                     "OCIO Error: " +
                                                     exception.what());
  }
}
