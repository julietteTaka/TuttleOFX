...
struct FunctorFade
  ...
  FunctorFade(const double t) : _t(t) {}
...
template <typename Pixel> struct FunctorFadeToColor {
  const Pixel _color;
  const double _t;
  const double _invt; // 1.0 - _t

  FunctorFadeToColor(const Pixel &color, const double t)
      : _color(color), _t(t), _invt(1.0 - t) {}
...
void FadeProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  if (_plugin._clipSrcFrom->isConnected()) {
    ...
    if (OFX::getImageEffectHostDescription()->hostName ==
      ...
      _srcPixelRodA =
          _plugin._clipSrcFrom->getPixelRod(args.time, args.renderScale);
    } else {
  ...
  if (_plugin._clipSrcTo->isConnected()) {
    ...
    if (OFX::getImageEffectHostDescription()->hostName ==
      ...
      // bug in nuke, getRegionOfDefinition() on OFX::Image returns bounds
      _srcPixelRodB =
          _plugin._clipSrcTo->getPixelRod(args.time, args.renderScale);
    } else {
  ...
  if (_plugin._clipSrcFrom->isConnected() &&
      _plugin._clipSrcTo->isConnected()) {
...
template <class View>
void
FadeProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  ...
  const OfxRectI procWindowOutput =
      translateRegion(procWindowRoW, this->_dstPixelRod);
  const OfxRectI srcRodA =
      _srcPixelRodA; // translateRegion( _srcPixelRodA, _params._offsetA );
  const OfxRectI srcRodB =
      _srcPixelRodB; // translateRegion( _srcPixelRodB, _params._offsetB );
  ...
  if (fromConnected && toConnected) {
    ...
    case eParamRodUnion: {
      ...
      }
      {
        ...
        View src =
            subimage_view(this->_srcViewA, procWindowSrc.x1, procWindowSrc.y1,
      ...
      }
      {
        ...
        View src =
            subimage_view(this->_srcViewB, procWindowSrc.x1, procWindowSrc.y1,
                          procWindowSize.x, procWindowSize.y);
        View dst = subimage_view(this->_dstView, procWindowOutput.x1,
                                 procWindowOutput.y1, procWindowSize.x,
                                 procWindowSize.y);
    ...
    View srcViewA_inter =
        subimage_view(this->_srcViewA, procIntersect.x1 - srcRodA.x1,
                      procIntersect.y1 - srcRodA.y1, procIntersectSize.x,
                      procIntersectSize.y);
    View srcViewB_inter =
        subimage_view(this->_srcViewB, procIntersect.x1 - srcRodB.x1,
                      procIntersect.y1 - srcRodB.y1, procIntersectSize.x,
                      procIntersectSize.y);
    ...
    merge_views(srcViewA_inter, srcViewB_inter, dstView_inter,
                FunctorFade<Pixel>(_params._transition));
    ...
    const OfxRectI procWindowSrc =
        translateRegion(procWindowRoW, fromConnected ? srcRodA : srcRodB);
    ...
    terry::algorithm::transform_pixels_progress(
        src, dst, FunctorFadeToColor<Pixel>(c, _params._transition),
        this->getOfxProgress());
  } else {
    ...
    terry::algorithm::transform_pixels_progress(
        dst, pixel_assigns_color_t<Pixel>(c), this->getOfxProgress());
  }
}
}
}
