...
GIL_FORCEINLINE void fill_pixels(const View &dstView, const Value &val,
                                 const OfxRectI &region) {
  View dst = subimage_view(dstView, region.x1, region.y1, region.x2 - region.x1,
                           region.y2 - region.y1);
...
GIL_FORCEINLINE void copy_pixels(const View &src, const OfxRectI &srcRegion,
                                 const View &dst, const OfxRectI &dstRegion) {
  const OfxPointI regionSize = { srcRegion.x2 - srcRegion.x1,
  ...
  View srcCrop = subimage_view(src, srcRegion.x1, srcRegion.y1, regionSize.x,
                               regionSize.y);
  View dstCrop = subimage_view(dst, dstRegion.x1, dstRegion.y1, regionSize.x,
                               regionSize.y);
...
template <class View>
void fillAroundIntersection(const View &viewA, const OfxRectI &srcAPixelRod,
  ...
  if (!rectangleContainsAnother(srcAPixelRod, srcBPixelRod)) {
    ...
    const OfxRectI procWindowOutput =
        translateRegion(procWindowRoW, dstPixelRod);

    fill_pixels(dstView, pixelZero, procWindowOutput);
  }
  {
    // fill with A
    const OfxRectI intersectRegionA =
        rectanglesIntersection(procWindowRoW, srcAPixelRod);
    const OfxRectI procWindowSrcA =
        translateRegion(intersectRegionA, srcAPixelRod);
    const OfxRectI procWindowOutputA =
        translateRegion(intersectRegionA, dstPixelRod);
  ...
  {
    // fill with B
    const OfxRectI intersectRegionB =
        rectanglesIntersection(procWindowRoW, srcBPixelRod);
    const OfxRectI procWindowSrcB =
        translateRegion(intersectRegionB, srcBPixelRod);
    const OfxRectI procWindowOutputB =
        translateRegion(intersectRegionB, dstPixelRod);
...
template <class View>
void fillAroundIntersection(const View &viewA, const OfxRectI &srcAPixelRod,
                            const OfxRectI &srcBPixelRod, const View &dstView,
 ...
 */
template <class View, class Functor>
void MergeProcess<View, Functor>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace terry;
  using namespace terry::numeric;
  ...
  const OfxRectI procIntersect =
      rectanglesIntersection(procWindowRoW, intersect);
  const OfxPointI procIntersectSize = { procIntersect.x2 - procIntersect.x1,
  ...
  case eParamRodUnion: {
    fillAroundIntersection(this->_srcViewA, srcRodA, this->_srcViewB, srcRodB,
                           this->_dstView, this->_dstPixelRod, procWindowRoW);
    break;
  }
  case eParamRodA: {
    fillAroundIntersection(this->_srcViewA, srcRodA, srcRodB, this->_dstView,
                           this->_dstPixelRod, procWindowRoW);
    break;
  }
  case eParamRodB: {
    fillAroundIntersection(this->_srcViewB, srcRodB, srcRodA, this->_dstView,
                           this->_dstPixelRod, procWindowRoW);
    break;
  }
  }

  View srcViewA_inter = subimage_view(
      this->_srcViewA, procIntersect.x1 - srcRodA.x1,
      procIntersect.y1 - srcRodA.y1, procIntersectSize.x, procIntersectSize.y);
  View srcViewB_inter = subimage_view(
      this->_srcViewB, procIntersect.x1 - srcRodB.x1,
      procIntersect.y1 - srcRodB.y1, procIntersectSize.x, procIntersectSize.y);

  View dstView_inter =
      subimage_view(this->_dstView, procIntersect.x1 - this->_dstPixelRod.x1,
