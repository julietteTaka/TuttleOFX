...
MergeProcessParams<MergePlugin::Scalar>
MergePlugin::getProcessParams(const OfxPointD &renderScale) const {
...
// void MergePlugin::changedParam( const OFX::InstanceChangedArgs& args, const
// std::string& paramName )
//{
//}

bool
MergePlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args,
                                   OfxRectD &rod) {
  MergeProcessParams<Scalar> params = getProcessParams();

  OfxRectD srcRodA =
      translateRegion(_clipSrcA->getCanonicalRod(args.time), params._offsetA);
  OfxRectD srcRodB =
      translateRegion(_clipSrcB->getCanonicalRod(args.time), params._offsetB);
...
void MergePlugin::render(const OFX::RenderArguments &args) {
  ...
  //			case eParamMergeXOR:
  //				TUTTLE_LOG_FATAL( "Need an alpha channel for this Merge
  //operation." );
  //				return;
  //			default:
  //				break;
...
void MergePlugin::render_if(const OFX::RenderArguments &args,
                            boost::mpl::true_) {
  typedef typename View::value_type Pixel;
...
void MergePlugin::render(const OFX::RenderArguments &args) {
  typedef typename View::value_type Pixel;
  typedef typename boost::gil::contains_color<
      typename View::value_type, boost::gil::alpha_t>::type has_alpha_t;
  typedef typename boost::is_same<typename Functor<Pixel>::operating_mode_t,
                                  terry::merge_per_channel_with_alpha>::type
  merge_need_alpha_t;
  typedef typename boost::mpl::if_<merge_need_alpha_t, has_alpha_t,
                                   boost::mpl::true_>::type render_condition_t;
