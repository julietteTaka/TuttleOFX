...
template <class View>
void
ThinningProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  ...
  //	                           View,
  //							   typename
  //kth_channel_view_type<0,View>::type
  //>::type CView;
  typedef View CView;
  typedef typename terry::image_from_view<
      CView, OfxAllocator<unsigned char> >::type CImage;
  ...
  const OfxRectI procWindowRoWCrop1 =
      rectanglesIntersection(rectangleGrow(procWindowRoW, border), srcRodCrop1);
  const OfxRectI procWindowRoWCrop2 =
      rectanglesIntersection(procWindowRoW, srcRodCrop2);
  ...
  terry::algorithm::transform_pixels_locator_progress(
      this->_srcView, ofxToGil(this->_srcPixelRod), view_tmp,
      ofxToGil(procWindowRoWCrop1), ofxToGil(procWindowRoWCrop1),
      terry::filter::thinning::pixel_locator_thinning_t<View, CView>(
          this->_srcView, terry::filter::thinning::lutthin1),
      this->getOfxProgress());
  terry::algorithm::transform_pixels_locator_progress(
      view_tmp, ofxToGil(procWindowRoWCrop1), // srcRodCrop1,
      this->_dstView, ofxToGil(this->_dstPixelRod),
      ofxToGil(procWindowRoWCrop2),
      terry::filter::thinning::pixel_locator_thinning_t<CView, View>(
          view_tmp, terry::filter::thinning::lutthin2),
      this->getOfxProgress());
}
}
}
