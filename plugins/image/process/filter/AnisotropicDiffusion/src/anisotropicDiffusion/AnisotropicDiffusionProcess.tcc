...
template <class View>
AnisotropicDiffusionProcess<View>::AnisotropicDiffusionProcess(
    AnisotropicDiffusionPlugin &instance)
...
template <class View>
void
AnisotropicDiffusionProcess<View>::setup(const OFX::RenderArguments &args) {
  // Fetch output image
  this->_dst.reset(_plugin._clipDst->fetchImage(args.time));
  if (!this->_dst.get())
    BOOST_THROW_EXCEPTION(exception::ImageNotReady()
                          << exception::user("Output"));
  ...
  if (_srcTensor->getRowDistanceBytes() < 0 ||
    ...
    BOOST_THROW_EXCEPTION(exception::InOutMismatch()
                          << exception::user("Incompatible row bytes !"));
  }
  ...
  _srcTensorView =
      interleaved_view(_upScaledSrcBounds.x2 - _upScaledSrcBounds.x1,
                       _upScaledSrcBounds.y2 - _upScaledSrcBounds.y1,
                       static_cast<Pixel *>(_srcTensor->getPixelData()),
                       _srcTensor->getRowDistanceBytes());

  this->_dstView =
      interleaved_view(_dBounds.x2 - _dBounds.x1, _dBounds.y2 - _dBounds.y1,
 ...
 */
template <class View>
void AnisotropicDiffusionProcess<View>::multiThreadProcessImages(
  ...
  View src = subimage_view(_srcView, srcRect.x1 - _upScaledSrcBounds.x1,
                           srcRect.y1 - _upScaledSrcBounds.y1,
                           srcRect.x2 - srcRect.x1, srcRect.y2 - srcRect.y1);

  View srct = subimage_view(_srcTensorView, srcRect.x1 - _upScaledSrcBounds.x1,
                            srcRect.y1 - _upScaledSrcBounds.y1,
                            srcRect.x2 - srcRect.x1, srcRect.y2 - srcRect.y1);
  ...
  blur_anisotropic(dst, src, srct, dregion, amplitude_rgb,
                   _fast_approx->getValue());
}
 ...
 */
template <class View>
void AnisotropicDiffusionProcess<View>::blur_anisotropic(
    View &dst, View &src, View &G, const region_t &dregion,
  ...
  if (amplitude.r > 0.0f || amplitude.g > 0.0f || amplitude.b > 0.0f) {
    if (src.num_channels() < 3 || dst.num_channels() < 3) {
      TUTTLE_LOG_ERROR("blur_anisotropic() : Specified tensor field ("
                       << (int)G.width() << ", " << (int)G.height() << ", "
                       << (int)G.num_channels() << ") is not valid.");
      return;
    }
    ...
    const unsigned int dx1 = src.width() - 1, dy1 = src.height() - 1,
    ...
    this->progressBegin((int)((360.0f - (360 % (int)da) / 2.0f) / da) * h,
                        "PDE Denoiser algorithm in progress");
    fill_black(
        tmpv /*subimage_view(tmpv, dregion.dox, dregion.doy, dregion.dw, dregion.dh)*/);

    // 2D version of the algorithm
    for (float theta = (360 % (int)da) / 2.0f; theta < 360.0f;
         (theta += da), ++N) {
      const float thetar = theta * boost::math::constants::pi<float>() / 180.0f,
                  vx = std::cos(thetar), vy = std::sin(thetar);
      pW = &sW[0];
      for (unsigned int yg = 0; yg < h; ++yg) {
        ...
        for (unsigned int xg = 0; xg < w; ++xg) {
          ...
          const float u = a * vx + b * vy, v = b * vx + c * vy,
      ...
      for (unsigned int y = 0; y < h; ++y) {
        ...
        for (unsigned int x = 0; x < w; ++x) {
          ...
          const float cu = pW[0], cv = pW[1], n = pW[2],
          ...
          const float lrgba[] = {
            length_r, length_g, length_b, length_r + length_g + length_b / 3.0f
          };

          const float srgba[] = { fsigma2_r,
                                  fsigma2_g,
                                  fsigma2_b,
            ...
            if (fast_approx) {
              for (unsigned int i = 0; i < nc; ++i) {
                ...
                if (amplitude_tab[i] > 0)
                  for (float l = 0.0f; l < lrgba[i] && X >= 0.0f && X <= dx1 &&
                                           Y >= 0.0f && Y <= dy1;
                       l += dl) {
                    const int cx = (int)(X + 0.5f), cy = (int)(Y + 0.5f);
                    float u = sW[(cy * w + cx) * nc + 0],
                          v = sW[(cy * w + cx) * nc + 1];
                    if ((pu * u + pv * v) < 0.0f) {
                      u = -u;
                      v = -v;
    ...
    copy_pixels(
        subimage_view(src, dregion.dox, dregion.doy, dregion.dw, dregion.dh),
