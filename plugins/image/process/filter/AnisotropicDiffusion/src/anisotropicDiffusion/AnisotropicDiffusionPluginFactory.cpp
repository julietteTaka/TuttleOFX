...
void
AnisotropicDiffusionPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  // basic labels
  desc.setLabels("TuttleAnisotropicDiffusion", "AnisotropicDiffusion",
                 "Anisotropic diffusion");
  desc.setPluginGrouping("tuttle/image/process/filter");

  desc.setDescription(
      "Anisotropic diffusion is the ability to blur along edges directions, "
      "thus it remove noise while preserving important details in the image.\n"
      "It's based on PDE (Partial Derivated Equations) and creates liquify "
      "effects.\n"
      "\n"
      "Inputs:\n"
...
void AnisotropicDiffusionPluginFactory::describeInContext(
  ...
  tensorsDstClip->setLabels("Input tensors", "Input tensors",
                            "User tensor image input.");
  ...
  OFX::GroupParamDescriptor *groupParamsPDE =
      desc.defineGroupParam(kParamGroupPDEAlgorithm);
  groupParamsPDE->setLabel("PDE Based Algorithm Parameters");

  OFX::BooleanParamDescriptor *fast_approx =
      desc.defineBooleanParam(kParamFastApproximation);
  ...
  amplitude->setDefault(kDefaultAmplitudeValue, kDefaultAmplitudeValue,
                        kDefaultAmplitudeValue);
 ...
 */
OFX::ImageEffect *
AnisotropicDiffusionPluginFactory::createInstance(OfxImageEffectHandle handle,
