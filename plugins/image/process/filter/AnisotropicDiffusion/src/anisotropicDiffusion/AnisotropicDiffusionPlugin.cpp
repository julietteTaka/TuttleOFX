...
AnisotropicDiffusionPlugin::AnisotropicDiffusionPlugin(
...
int AnisotropicDiffusionPlugin::getMargin() {
  ...
  return (int)std::ceil(std::sqrt(2.0f) *
                        std::ceil(std::max(std::max(std::sqrt(2.0f * color.r),
                                                    std::sqrt(2.0f * color.g)),
                                           std::sqrt(2.0f * color.b))));
}

void AnisotropicDiffusionPlugin::getRegionsOfInterest(
