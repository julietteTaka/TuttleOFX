 ...
 *                                            1 = kind of blurred edge
 *detection,
 *                                            2 = kind of very blurred edge
 *detection)
...
void deriche(const S_VIEW &src, D_VIEW &dst, const float sigma,
             const int order = 0, const char axe = 'x',
  ...
  if (sigma >= 0.1f || order) {
    const float nsigma = sigma < 0.1f ? 0.1f : sigma, alpha = 1.695f / nsigma,
                ema = expf(-alpha), ema2 = expf(-2.0f * alpha),
                b1 = -2.0f * ema, b2 = ema2;
    float a0 = 0.0f, a1 = 0.0f, a2 = 0.0f, a3 = 0.0f, coefp = 0.0f,
          coefn = 0.0f;
    switch (order) {
    case 0: {
      const float k =
    ...
    case 2: {
      const float ea = expf(-alpha), k = -(ema2 - 1) / (2 * alpha * ema),
                  kn = (-2.0f *
                        (-1.0f + 3.0f * ea - 3.0f * ea * ea + ea * ea * ea) /
    ...
    default: {
      std::cerr << "Given filter order (order = " << order
                << ") must be 0, 1 or 2." << std::endl;
      return;
    }
    }
    typedef typename bgil::channel_type<S_VIEW>::type sPix_t;
    typedef typename bgil::channel_type<D_VIEW>::type dPix_t;
    const int nc = src.num_channels() > dst.num_channels() ? dst.num_channels()
                                                           : src.num_channels();
      ...
      for (int v = 0; v < nc; ++v) {
        for (int y = 0; y < h; ++y) {
          ...
          for (int m = 0; m < N; ++m) {
            ...
            iterSrcX++;
            const sPix_t yc = *(ptrY++) =
                (sPix_t)(a0 * xc + a1 * xp - b1 * yp - b2 * yb);
            xp = xc;
            yb = yp;
            yp = yc;
          }
 ...
 */
template <typename S_VIEW, typename D_VIEW>
void dericheFilter(const S_VIEW &src, D_VIEW &dst, const float sigma,
                   int order = 0, float threshold = 0.0f) {
  if (order > 0) {
    if (src.num_channels() < 3) {
      std::cerr << "Error: destination must have at least 3 channels"
                << std::endl;
      return;
    }
    int w = src.width();
