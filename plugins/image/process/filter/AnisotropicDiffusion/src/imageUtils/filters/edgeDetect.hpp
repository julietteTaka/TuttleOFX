...
/**
 * @brief Apply precise forward/backward finite differences to get a gradient
 *image
 *
...
D_VIEW &simple_structure_tensor(const S_VIEW &src, D_VIEW &final_dst,
                                float threshold) {
...
void harris(const S_VIEW &src, D_VIEW &dst, float threshold) {
  ...
  const int nc = dst.num_channels() < src.num_channels() ? dst.num_channels()
                                                         : src.num_channels();
  // Concept checking
  bgil::gil_function_requires<bgil::ColorSpacesCompatibleConcept<
      typename bgil::color_space_type<S_VIEW>::type,
...
void symmetric_eigen(ViewIt &tensorXY, std::vector<double> &val,
  ...
  const double a = (*tensorXY)[0], b = (*tensorXY)[1], c = (*tensorXY)[1],
               d = (*tensorXY)[2], e = a + d;
