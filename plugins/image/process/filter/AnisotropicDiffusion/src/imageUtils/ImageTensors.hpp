 ...
 */
template <typename TensorView> struct tensor_t {

  tensor_t()
      : dox(0), doy(0), dw(0), dh(0), algorithm(false), stAlgo(0), alpha(0.0),
        sigma(0.0), sharpness(0.0), anisotropy(0.0), geom_fact(0.0),
        threshold(0.0) {};
  int dox, doy, dw, dh;
 ...
 */
template <class View> class ImageTensors : public View {
private:
  template <class D_ITER>
  void compute_vectors(D_ITER &iter_dst, const std::vector<double> &val,
                       const std::vector<double> &vec, const double power1,
                       const double power2);
...
public:
  ...
  void anisotropic_gradient(const View &src,
...
template <class View>
void
ImageTensors<View>::process(const View &srcView, E_TensorsAlgorithm tensAlgo,
                            tuttle::plugin::IProgress *progress, void *args) {
  switch (tensAlgo) {
...
template <class View>
template <class D_ITER>
void ImageTensors<View>::compute_vectors(D_ITER &iter_dst,
...
template <class View>
void
ImageTensors<View>::anisotropic_gradient(const View &src,
                                         tuttle::plugin::IProgress *progress,
                                         tensor_t<View> *args) {
  ...
  if (tensorAlgo == false) {
    ...
    copy_pixels(
        subimage_view(tmpv2, args->dox, args->doy, dst.width(), dst.height()),
        dst);
  }
}
}
