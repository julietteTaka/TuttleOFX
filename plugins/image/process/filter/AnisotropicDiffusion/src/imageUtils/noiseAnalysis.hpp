...
D_VIEW &pseudo_residual(S_VIEW &src, D_VIEW &dst) {
  ...
  gil_function_requires<
      ColorSpacesCompatibleConcept<typename color_space_type<S_VIEW>::type,
 ...
 */
template <class S_VIEW, typename t> double variance_mean(S_VIEW &src, t &mean) {
...
/**
 * @brief Robustly estimatate the variance of a the noise using the
 * pseudo-residuals.
 * @see variance_estimation()
 */
template <class S_VIEW> double noise_variance(S_VIEW &src) {
  double mean;
