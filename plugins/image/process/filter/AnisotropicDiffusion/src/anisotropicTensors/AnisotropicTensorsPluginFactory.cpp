 ...
 */
void
AnisotropicTensorsPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  // basic labels
  desc.setLabels("TuttleAnisotropicTensors", "AnisotropicTensors",
                 "Generates structure tensors image");
  desc.setPluginGrouping("tuttle/image/process/filter");

  desc.setDescription(
      "Generates structure tensors image, to use as input of the "
      "AnisotropicDiffusion.\n"
      "\n"
      "\n"
      "Tensors are used to get geographic information (edges and directions). "
      "In this map, you will see horizontal edges shown with red color, "
      "vertical edges shown in blue and finally, when the blur must be "
      "isotropic, "
      "the color is pink (blur along X and Y directions). The main goal is to "
      "get pink on uniform noisy area:"
      "Structure tensors is just used to see how edges are detected, "
      "diffuse tensors map is the final tensor map used by the "
      "AnisotropicDiffusion.\n"
      "\n"
      "Inputs:\n"
      "- Source: Source image.\n"
      "\n"
      "Parameters:\n"
      "- Display effect margin: Margin used by the algorithm to avoid border "
      "effect.\n"
      "- Alpha: pre-blurring of the edge detection. Can be interpreted as the "
      "noise scale. "
      "If too high, it detects noise as edges, if too low, it removes tiny "
      "edges. "
      "Play this parameter with geometry factor and thresholding "
      "quantization.\n"
      "- Sigma: post-blurring of the edge detection. Interpreted as regularity "
      "of the tensor valued geometry.\n"
      "- Geometry factor: tends to transform noise to pink when falling near "
      "0.\n"
      "- Thresholding quantization: Quantization of the structure tensor, to "
      "eliminate noise effect.\n"
      "- Sharpness: set how much you want to preserve edges.\n"
      "- Anisotropy: anisotropy factor. Increase the value to get more fluid "
      "blurring.\n");
  ...
  desc.setOverlayInteractDescriptor(
      new OFX::DefaultEffectOverlayWrap<TensorsMarginOverlay>());
}
 ...
 */
void AnisotropicTensorsPluginFactory::describeInContext(
  ...
  // Controls
  OFX::BooleanParamDescriptor *algo =
      desc.defineBooleanParam(kParamDisplayStructureTensors);
  algo->setLabel("Display Structure Tensors");
  algo->setDefault(false);

  OFX::BooleanParamDescriptor *margin =
      desc.defineBooleanParam(kParamDisplayEffectMargin);
  margin->setLabel("Display Effect Margin");
  margin->setDefault(false);
  margin->setEvaluateOnChange(false);

  OFX::GroupParamDescriptor *groupParamsStrTensors =
      desc.defineGroupParam(kParamGroupStructureTensors);
  ...
  OFX::DoubleParamDescriptor *geom_fact =
      desc.defineDoubleParam(kParamGeometryFactor);
  ...
  OFX::DoubleParamDescriptor *threshold =
      desc.defineDoubleParam(kParamThresholdingQuantization);
  ...
  threshold->setHint(
      "Tresholding quantization to remove noise on uniform areas");

  OFX::GroupParamDescriptor *groupParamsDifTensors =
      desc.defineGroupParam(kParamGroupDiffuseTensors);
  groupParamsDifTensors->setLabel("Diffuse tensors parameters");

  OFX::ChoiceParamDescriptor *stAlgo =
      desc.defineChoiceParam(kParamEdgeDetectAlgo);
  ...
  OFX::DoubleParamDescriptor *sharpness =
      desc.defineDoubleParam(kParamSharpness);
  ...
  OFX::DoubleParamDescriptor *anisotropy =
      desc.defineDoubleParam(kParamAnisotropy);
 ...
 */
OFX::ImageEffect *
AnisotropicTensorsPluginFactory::createInstance(OfxImageEffectHandle handle,
