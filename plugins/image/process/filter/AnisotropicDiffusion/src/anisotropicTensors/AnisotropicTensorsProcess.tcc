 ...
 */
template <class View>
AnisotropicTensorsProcess<View>::AnisotropicTensorsProcess(
    TensorsPlugin &instance)
...
void AnisotropicTensorsProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  this->_srcView = interleaved_view(
      _upScaledSrcBounds.x2 - _upScaledSrcBounds.x1,
      _upScaledSrcBounds.y2 - _upScaledSrcBounds.y1,
      (Pixel *)this->_src->getPixelData(), this->_src->getRowDistanceBytes());

  this->_dstView = interleaved_view(
      _dBounds.x2 - _dBounds.x1, _dBounds.y2 - _dBounds.y1,
 ...
 */
template <class View>
void AnisotropicTensorsProcess<View>::multiThreadProcessImages(
  ...
  View src = subimage_view(this->_srcView, srcRect.x1 - _upScaledSrcBounds.x1,
  ...
  dv.process(src, imageUtils::ImageTensors<View>::eAnisotGradient, this,
             &params);
}
}
}
