...
BlurProcessParams<BlurPlugin::Scalar>
BlurPlugin::getProcessParams(const OfxPointD &renderScale) const {
  ...
  params._gilKernelX = buildGaussian1DKernel<Scalar>(
      params._size.x, normalizedKernel, kernelEpsilon);
  params._gilKernelY = buildGaussian1DKernel<Scalar>(
...
/*
   void BlurPlugin::changedParam( const OFX::InstanceChangedArgs &args, const
...
bool
BlurPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args,
                                  OfxRectD &rod) {
...
void
BlurPlugin::getRegionsOfInterest(const OFX::RegionsOfInterestArguments &args,
...
bool BlurPlugin::isIdentity(const OFX::RenderArguments &args,
