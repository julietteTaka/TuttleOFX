...
void SobelProcess<SView, DView>::preProcess() {
  this->progressBegin(5 /* steps */ * this->_renderWindowSize.x *
                      this->_renderWindowSize.y);
}
 ...
 */
template <class SView, class DView>
void SobelProcess<SView, DView>::multiThreadProcessImages(
  ...
  //	TUTTLE_LOG_INFO( "Sobel X: " <<
  //_params._xKernelGaussianDerivative.size() << "x" <<
  //_params._xKernelGaussian.size() );
  //	TUTTLE_LOG_INFO( "Sobel Y: " <<
  //_params._yKernelGaussianDerivative.size() << "x" <<
  //_params._yKernelGaussian.size() );
  ...
  const Point proc_tl(procWindowRoW.x1 - this->_srcPixelRod.x1,
                      procWindowRoW.y1 - this->_srcPixelRod.y1);

  if (_params._xKernelGaussianDerivative.size() == 0 ||
      (!_params._unidimensional && _params._xKernelGaussian.size() == 0)) {
    ...
    if (_params._unidimensional) {
      ...
      case eParamPassFull: {
        correlate_rows_cols_auto<DPixelGray, OfxAllocator>(
            color_converted_view<DPixelGray>(this->_srcView),
            _params._xKernelGaussianDerivative, _params._xKernelGaussian,
      ...
      case eParamPass1: {
        correlate_rows_auto<DPixelGray>(
            color_converted_view<DPixelGray>(this->_srcView),
            _params._xKernelGaussianDerivative, kth_channel_view<0>(dst),
            proc_tl, _params._boundary_option);
        break;
      }
      case eParamPass2: {
        correlate_cols_auto<DPixelGray>(
            kth_channel_view<0>(this->_srcView), _params._xKernelGaussian,
            kth_channel_view<0>(dst), proc_tl, _params._boundary_option);
        break;
      }
      }
  ...
  if (_params._yKernelGaussianDerivative.size() == 0 ||
      (!_params._unidimensional && _params._yKernelGaussian.size() == 0)) {
    ...
    if (_params._unidimensional) {
      ...
      case eParamPassFull: {
        correlate_rows_cols_auto<DPixelGray, OfxAllocator>(
            color_converted_view<DPixelGray>(this->_srcView),
            _params._yKernelGaussian, _params._yKernelGaussianDerivative,
            kth_channel_view<1>(dst), proc_tl, _params._boundary_option);
        break;
      }
      case eParamPass1: {
        correlate_rows_auto<DPixelGray>(
  ...
  computeGradientDirection(
      dst, boost::mpl::bool_<(boost::gil::num_channels<DView>::value >= 4)>());
}

template <class SView, class DView>
void SobelProcess<SView, DView>::computeGradientDirection(DView &dst,
                                                          boost::mpl::true_) {
  using namespace boost;
  using namespace terry;
  using namespace terry::algorithm;
  ...
  if (!_params._computeGradientDirection) {
    ...
    if (_params._gradientDirectionAbs) {
      transform_pixels_progress(
          ...
          transform_pixel_by_channel_t<
              terry::color::channel_gradientDirectionAbs_t>(),
          this->getOfxProgress());
    } else {
      transform_pixels_progress(
          kth_channel_view<0>(dst), // srcX
          ...
          kth_channel_view<3>(dst), // dst: gradient direction
          transform_pixel_by_channel_t<
              terry::color::channel_gradientDirection_t>(),
          this->getOfxProgress());
    }
  }
}
