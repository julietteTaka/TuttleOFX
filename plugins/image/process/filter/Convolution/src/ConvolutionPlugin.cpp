...
//		unsigned int yy = y * params._size.x;
          for( unsigned int x = 0; x < params._size.x; ++x )
          {
                  ...
                  TUTTLE_LOG_INFO( "coef[" << y << "][" << x << "] = " <<
params._convMatrix(x, y) );
          }
  }
  */
  return params;
...
bool ConvolutionPlugin::getRegionOfDefinition(
...
void ConvolutionPlugin::getRegionsOfInterest(
...
bool ConvolutionPlugin::isIdentity(const OFX::RenderArguments &args,
...
void ConvolutionPlugin::changedParam(const OFX::InstanceChangedArgs &args,
