 ...
 */
template <class View>
void ConvolutionProcess<View>::multiThreadProcessImages(
  ...
  Point proc_tl(procWindowRoW.x1 - this->_srcPixelRod.x1,
  ...
  if (_params._size.x == 0)
    ...
    correlate_rows_auto<Pixel>(this->_srcView, _params._convX, dst, proc_tl,
                               _params._boundary_option);
  else
    correlate_rows_cols_auto<Pixel, OfxAllocator>(
        this->_srcView, _params._convX, _params._convY, dst, proc_tl,
        _params._boundary_option);
  /*
          break;
  }
