...
void FloodFillProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  if (_params._relativeMinMax) {
    ...
    _lowerThres =
        (_params._lowerThres * (minmax.max[0] - minmax.min[0])) + minmax.min[0];
    _upperThres =
        (_params._upperThres * (minmax.max[0] - minmax.min[0])) + minmax.min[0];
  } else {
 ...
 */
template <class View>
void FloodFillProcess<View>::multiThreadProcessImages(
  ...
  const OfxRectI procWindowRoWCrop =
      rectanglesIntersection(procWindowRoW, srcRodCrop);

  terry::draw::fill_pixels(this->_dstView, ofxToGil(procWindowOutput),
                           get_black<Pixel>());
  ...
  case eParamMethod4: {
    flood_fill<Connexity4, IsUpper<Scalar>, IsUpper<Scalar>, View, View,
               OfxAllocator>(
        this->_srcView, ofxToGil(this->_srcPixelRod), this->_dstView,
        ofxToGil(this->_dstPixelRod), ofxToGil(procWindowRoWCrop),
        IsUpper<Scalar>(_upperThres), IsUpper<Scalar>(_lowerThres));
    break;
  }
  case eParamMethod8: {
  ...
  {
    //			flood_fill_bruteForce<IsUpper<Scalar>, IsUpper<Scalar>,
    //View,
    // View>(
