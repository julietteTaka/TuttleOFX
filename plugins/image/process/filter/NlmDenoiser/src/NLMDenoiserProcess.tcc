...
template <class View> NLMDenoiserProcess<View>::~NLMDenoiserProcess() {}

template <class View>
void NLMDenoiserProcess<View>::addFrame(const OfxRectI &dBounds,
  ...
  // Build views
  _srcViews.push_back(bgil::interleaved_view(w, h, (Pixel *)img->getPixelData(),
...
void NLMDenoiserProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  this->_dstView = bgil::interleaved_view(
      dBounds.x2 - dBounds.x1, dBounds.y2 - dBounds.y1,
...
template <class View> void NLMDenoiserProcess<View>::preProcess() {
  // Initialize progress bar
  if (_paramOptimized->getValue()) {
    const int min_xpi = std::min((int)_paramRegionRadius->getValue() * 2,
                                 (int)_srcViews[0].width()),
              min_ypi = std::min((int)_paramRegionRadius->getValue() * 2,
                                 (int)_srcViews[0].height());
    std::stringstream msg;
    msg << "NL-Means algorithm in progress (automatic bandwidth = "
        << computeBandwidth() << ").";
    this->progressBegin(_paramDepth->getValue() * min_xpi * min_ypi, msg.str());
  } else {
    this->progressBegin(
        (int)((_srcViews[0].width() * this->_renderArgs.renderScale.x) *
 ...
 */
template <class View>
void NLMDenoiserProcess<View>::multiThreadProcessImages(
  ...
  View subDst = bgil::subimage_view(
      this->_dstView, procWindowRoW.x1 - this->_renderArgs.renderWindow.x1,
...
template <class View> double NLMDenoiserProcess<View>::computeBandwidth() {
  double bandwidth = 0.0;
  // Assume that we work in RGB
  const int nc = 3;
  double np = (2 * _paramPatchRadius->getValue()) *
              (2 * _paramPatchRadius->getValue()) * nc;
  if (np < 100)
    bandwidth = ((((((1.1785e-12 * np - 5.1827e-10) * np + 9.5946e-08) * np -
                    9.7798e-06) *
                       np +
                   6.0756e-04) *
                      np -
 ...
 */
template <class View>
void NLMDenoiserProcess<View>::nlMeans(View &dst, const OfxRectI &procWindow,
  ...
  upscaledProcWindow =
      rectanglesIntersection(upscaledProcWindow, _upScaledBounds);
  ...
  for (it = _srcViews.begin(); it != _srcViews.end(); ++it) {
    subSrcViews.push_back(
        subimage_view(*it, tUpscaledProcWindow.x1, tUpscaledProcWindow.y1,
                      tUpscaledProcWindow.x2 - tUpscaledProcWindow.x1,
                      tUpscaledProcWindow.y2 - tUpscaledProcWindow.y1));
  }

  // Allocate average buffers (assimilate this two buffers as 2D float buffers,
  // not images !)
  rgba32f_image_t weight_cumul(
      w, h); /// @todo: use memory allocated by host using memorySuite
  ...
  if (!_plugin.abort()) {
    View procView =
        subimage_view(subSrcViews[0], nProcWindow.x1, nProcWindow.y1, w, h);
    for (int yj = 0; yj < h; ++yj) {
      ...
      for (int xj = 0; xj < w; ++xj) {
        // Final estimate
        for (int v = 0; v < nc; ++v) {
          (*dst_it)[v] = (((*wcIter)[v] * mix[v] + 1.0f * (*src_it)[v]) /
                          ((*wnIter)[v] * mix[v] + 1.0f));
        }
...
void NLMDenoiserProcess<View>::computeWeights(const std::vector<View> &srcViews,
  ...
  static const int nc =
      boost::mpl::min<boost::mpl::int_<3>,
                      typename bgil::num_channels<Pixel>::type>::type::value;
      ...
      for (int xi = -min_xpi; xi <= min_xpi; ++xi) {
        ...
        if (xi != 0 || yi != 0) {
          ...
          for (int yj = yl; yj < yh; ++yj) {
            ...
            if (yi >= 0) {
              lbound = -patchRadius + yj < 0 ? std::max(-yj, -patchRadius)
                                             : -patchRadius;
              hbound = lbound + (patchRadius * 2 + 1);
              if (hbound + j > hi)
                hbound = std::min(hi - j, patchRadius);
            } else {
              hbound = patchRadius + yj > hi ? std::min(patchRadius, (hi - yj))
                                             : patchRadius;
            ...
            for (int xj = xl; xj < xh; ++xj) {
              ...
              if (xj - patchRadius - 1 >= 0 && i - patchRadius - 1 >= 0) {
                for (int k = lbound; k < hbound; ++k) {
                  for (int v = 0; v < nc; ++v) {
                    e = loc1(-patchRadius - 1, k)[v] -
                        loc2(-patchRadius - 1, k)[v];
                    eucl_dist -= e * e;
                  }
                }
              }
              ...
              bool w1Pass =
                  (zi == 0 && i >= procWindow.x1 && i < procWindow.x2 &&
