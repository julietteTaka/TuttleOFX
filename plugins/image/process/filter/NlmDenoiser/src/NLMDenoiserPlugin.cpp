...
NLMDenoiserPlugin::NLMDenoiserPlugin(OfxImageEffectHandle handle)
    : OFX::ImageEffect(handle), _clipDst(0), _clipSrc(0) {
 ...
 */
void NLMDenoiserPlugin::getFramesNeeded(const OFX::FramesNeededArguments &args,
  ...
  OfxRangeD realRange;
  realRange.min =
      clamp(requestedRange.min, clipFullRange.min, clipFullRange.max);
  realRange.max =
      clamp(requestedRange.max, clipFullRange.min, clipFullRange.max);
  //	TUTTLE_TLOG_VAR2( TUTTLE_INFO, realRange.min, realRange.max );

  frames.setFramesNeeded(*_clipSrc, realRange);
  //	TUTTLE_TLOG( TUTTLE_INFO, "NLMDenoiserPlugin::getFramesNeeded timerange
  //min:" << realRange.min << ", max:" << realRange.max << " for time:" <<
  //args.time );
}

void NLMDenoiserPlugin::getRegionsOfInterest(
    const OFX::RegionsOfInterestArguments &args,
    OFX::RegionOfInterestSetter &rois) {
  const double margin =
      (_paramRegionRadius->getValue() + _paramPatchRadius->getValue()) *
