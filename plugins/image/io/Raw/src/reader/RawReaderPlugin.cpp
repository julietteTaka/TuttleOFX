...
RawReaderProcessParams<RawReaderPlugin::Scalar>
RawReaderPlugin::getProcessParams(const OfxTime time) {
  ...
  params._interpolation =
      static_cast<EInterpolation>(_paramInterpolation->getValue());
  ...
  params._whiteBalance =
      static_cast<EWhiteBalance>(_paramWhiteBalance->getValue());
...
void RawReaderPlugin::updateInfos(const OfxTime time) {
  ...
  ss << "Color sources /Legend: (U)nknown, (I)nit, (CO)nstant, (L)oaded, "
        "(CA)lculated/:"
     << "\n";
  ss << "Cam->XYZ matrix:"
     << "\n";
  for (int i = 0; i < 4; ++i)
    ss << color.cam_xyz[i][0] << "\t" << color.cam_xyz[i][1] << "\t"
       << color.cam_xyz[i][2] << "\n"; // %6.4f

  TUTTLE_LOG_DEBUG(TUTTLE_INFO, ss);
}

void RawReaderPlugin::changedParam(const OFX::InstanceChangedArgs &args,
...
void RawReaderPlugin::getClipPreferences(
