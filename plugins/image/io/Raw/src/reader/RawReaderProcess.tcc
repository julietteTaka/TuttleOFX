...
template <class View>
static int progressCallback(void *data, LibRaw_progress p, int iteration,
                            int expected) {
  typedef RawReaderProcess<View> PluginProcess;
  // PluginProcess* process = reinterpret_cast<PluginProcess*>( data );
  TUTTLE_LOG_DEBUG(TUTTLE_INFO, "Callback: " << libraw_strprogress(p)
                                             << "  pass " << iteration << " of "
                                             << expected);
...
// typedef void (*data_callback)(void *callback_data,const char *file, const int
// offset);
// void LibRaw::set_dataerror_handler(data_callback func, void *callback_data);

// typedef void (* memory_callback)(void *callback_data,const char *file, const
// char *where);
...
RawReaderProcess<View>::RawReaderProcess(RawReaderPlugin &instance)
    : ImageGilProcessor<View>(instance, eImageOrientationFromTopToBottom),
      _plugin(instance), _p1(_rawProcessor.imgdata.idata),
      _size(_rawProcessor.imgdata.sizes), _color(_rawProcessor.imgdata.color),
      _thumbnail(_rawProcessor.imgdata.thumbnail),
      _p2(_rawProcessor.imgdata.other), _out(_rawProcessor.imgdata.params) {
  this->setNoMultiThreading();

  _rawProcessor.set_progress_handler(progressCallback<View>,
                                     reinterpret_cast<void *>(this));
  //	_rawProcessor.set_memerror_handler( xxxCallback,
  // reinterpret_cast<void*>(this) );
  //	_rawProcessor.set_dataerror_handler( xxxCallback,
  // reinterpret_cast<void*>(this) );
}
...
template <class View> void RawReaderProcess<View>::preProcess() {
 ...
 */
template <class View>
void RawReaderProcess<View>::multiThreadProcessImages(
  ...
  try {
    ...
    _out.exp_correc = 1; // every time correct exposure (use default parameters
                         // to don't change)
...
*/
    ...
    if (const int ret = _rawProcessor.open_file(_params._filepath.c_str())) {
      BOOST_THROW_EXCEPTION(exception::Unknown()
                            << exception::user() + "Cannot open file: " +
                                   libraw_strerror(ret)
    ...
    if (const int ret = _rawProcessor.unpack()) {
      BOOST_THROW_EXCEPTION(exception::Unknown()
                            << exception::user() + "Cannot unpack file: " +
                                   libraw_strerror(ret)
                            << exception::filename(_params._filepath));
    }
    ...
    if (LIBRAW_SUCCESS != ret) {
      if (LIBRAW_FATAL_ERROR(ret)) {
        BOOST_THROW_EXCEPTION(exception::Unknown()
                              << exception::user() +
                                     "Cannot do postprocessing: " +
                                     libraw_strerror(ret)
                              << exception::filename(_params._filepath));
      } else {
        TUTTLE_LOG_ERROR("Error on postprocessing ("
                         << quotes(_params._filepath)
                         << "): " << libraw_strerror(ret));
        TUTTLE_LOG_ERROR("Try to continue...");
      }
    }
    //		libraw_processed_image_t* image =
    //_rawProcessor.dcraw_make_mem_image(
    //&ret );
    //		if( ! image )
    //		{
    //			TUTTLE_LOG_ERROR( "Cannot unpack " << filepath << " to
    //memory
    // buffer: " << libraw_strerror( ret ) );
    //			return dst;
    //		}
    ...
    //		if( LIBRAW_SUCCESS != ret )
    //		{
    //			TUTTLE_LOG_ERROR( "Cannot do document_mode_processing on "
    //<<
    ...
    TUTTLE_TLOG(TUTTLE_INFO,
                "Image size: " << _rawProcessor.imgdata.sizes.width << ", "
                               << _rawProcessor.imgdata.sizes.height);

    //		TUTTLE_LOG_VAR2( image->width, image->height );
    TUTTLE_TLOG_VAR2(TUTTLE_INFO, _rawProcessor.imgdata.sizes.width,
                     _rawProcessor.imgdata.sizes.height);

    //		// And let us print its dump; the data are accessible through
    //data
    // fields of the class
    //		for( int i = 0;
    //			i < _rawProcessor.imgdata.sizes.iwidth *
    //_rawProcessor.imgdata.sizes.iheight;
    //			++i )
    //				TUTTLE_LOG( "i=" << i <<
    //					  " R=" <<
    //_rawProcessor.imgdata.image[i][0]
    //<<
    //					  " G=" <<
    //_rawProcessor.imgdata.image[i][1]
    //<<
    //					  " B=" <<
    //_rawProcessor.imgdata.image[i][2]
    //<<
    //					  " G2=" <<
    //_rawProcessor.imgdata.image[i][3]
    //				);

    typedef boost::gil::rgba16c_view_t RawView;
    typedef RawView::value_type RawPixel;
    RawView imageView = interleaved_view(
        _size.width, _size.height, // image->width, image->height,
    ...
    TUTTLE_LOG_VAR2(TUTTLE_INFO, imageView.dimensions().x,
                    imageView.dimensions().y);
