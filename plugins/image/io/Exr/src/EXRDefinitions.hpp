...
static const std::string kParamCompressionHintRLE =
    ...
    "This "
    "method is fast, and works well for images with large flat areas, but for "
    "photographic "
    "images, the compressed file size is usually between 60 and 75 percent of "
    "the "
    "uncompressed size.";
static const std::string kParamCompressionHintZIPS =
    "Lossless, zlib compression, one scan line at a time.\n"
    "Uses the open-source zlib library for compression. Like ZIP compression, "
    "but "
...
static const std::string kParamCompressionHintZIP =
    ...
    "open-"
    "source zlib library. ZIP decompression is faster than PIZ decompression, "
    "but ZIP "
    "compression is significantly slower. Photographic images tend to shrink "
    "to between "
    "45 and 55 percent of their uncompressed size.\n"
    "Multi-resolution files are often used as texture maps for 3D renderers. "
    "For this "
    "application, fast read accesses are usually more important than fast "
    "writes, or "
    "maximum compression. For texture maps, ZIP is probably the best "
    "compression "
    "method.\n"
    "In scan-line based files, 16 rows of pixels are accumulated and "
    "compressed "
...
static const std::string kParamCompressionHintPIZ =
    ...
    "Huffman-"
    "encoded. This scheme tends to provide the best compression ratio for the "
    "types of "
    "images that are typically processed at Industrial Light & Magic. Files "
    "are "
    "compressed and decompressed at roughly the same speed. For photographic "
    "images with film grain, the files are reduced to between 35 and 55 "
    "percent of their "
    "uncompressed size.\n"
    "PIZ compression works well for scan-line based files, and also for tiled "
    "files with "
    "large tiles, but small tiles do not shrink much. (PIZ-compressed data "
    "start with a "
    "relatively long header; if the input to the compressor is short, adding "
    "the header "
...
static const std::string kParamCompressionHintPXR24 =
    ...
    "After reducing 32-bit floating-point data to 24 bits by rounding (while "
    "leaving 16-bit "
    "floating-point data unchanged), differences between horizontally adjacent "
    "pixels "
    "are compressed with zlib, similar to ZIP. PXR24 compression preserves "
    "image "
    "channels of type HALF and UINT exactly, but the relative error of FLOAT "
    "data "
    ...
    "large, but "
    "where full 32-bit floating-point accuracy is not necessary. Rounding "
    "improves "
    "compression significantly by eliminating the pixels' 8 least significant "
    "bits, which "
...
static const std::string kParamCompressionHintB44 =
    ...
    "bytes. Each "
    "block is then packed into 14 bytes, reducing the data to 44 percent of "
    "their "
    ...
    "compressed pixels is about 22 percent of the size of the original RGB "
    "data. "
    ...
    "image, "
    "but not on the data in the pixels. All images with the same resolution "
    "and the same "
    "set of channels have the same size. This can be advantageous for systems "
    "that "
    "support real-time playback of image sequences; the predictable file size "
    "makes it "
...
static const std::string kParamCompressionHintB44A =
    ...
    "the same "
    "value, which are packed into 3 instead of 14 bytes. For images with large "
    "uniform "
