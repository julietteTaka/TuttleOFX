...
void EXRReaderPlugin::changedParam(const OFX::InstanceChangedArgs &args,
...
void EXRReaderPlugin::updateCombos() {
  ...
  const std::string &firstChannelName =
      _channelNames[_paramsChannelChoice[0]->getValue()];
  Imf::PixelType firstChannelType =
      cl.find(firstChannelName.c_str()).channel().type;
...
void EXRReaderPlugin::getClipPreferences(
  ...
  if (getExplicitChannelConversion() == eParamReaderChannelAuto) {
    ...
    default: {
      BOOST_THROW_EXCEPTION(exception::FileNotExist()
                            << exception::user() + "EXR: doesn't support " +
                                   _channelNames.size() + " channels.");
      break;
    }
    }
  }
...
bool EXRReaderPlugin::getRegionOfDefinition(
  ...
  try {
    ...
    if (_paramOutputData->getValue() == 0) {
      ...
      //			TUTTLE_TLOG_INFO( "ExrReaderPlugin:
      //displayWindow: " << displayWindow.min.x << ", " << displayWindow.min.y
      //<< ", " << displayWindow.max.x << ", " << displayWindow.max.y );
      //			TUTTLE_TLOG_INFO( "ExrReaderPlugin: dataWindow:
      //" << h.dataWindow().min.x << ", " << h.dataWindow().min.y << ", " <<
