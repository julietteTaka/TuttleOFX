...
// type
template <typename T>
struct channel_traits_impl<const T, false> : public channel_traits_impl<T,
                                                                        false> {
...
/// Models: ChannelValueConcept
template <
    typename BaseChannelValue, // base channel (models ChannelValueConcept)
    typename MinVal, typename MaxVal>
// classes with a static apply() function returning the minimum/maximum channel
// values
...
class packed_channel_reference<
    ...
    false> : public detail::
                 packed_channel_reference_base<
                     packed_channel_reference<BitField, FirstBit, NumBits,
                                              false>,
...
// (Visual Studio 8) STL calls swap qualified.
// swap with 'left bias':
...
struct is_integral<gil::packed_channel_reference<
    BitField, FirstBit, NumBits, IsMutable> > : public mpl::true_ {};

template <typename BitField, int NumBits, bool IsMutable>
struct is_integral<gil::packed_dynamic_channel_reference<
    BitField, NumBits, IsMutable> > : public mpl::true_ {};

template <typename BaseChannelValue, typename MinVal, typename MaxVal>
struct is_integral<gil::scoped_channel_value<
    BaseChannelValue, MinVal,
    MaxVal> > : public is_integral<BaseChannelValue> {};
}
