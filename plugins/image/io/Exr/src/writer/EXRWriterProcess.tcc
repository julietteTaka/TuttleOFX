 ...
 */
template <class View>
void EXRWriterProcess<View>::multiThreadProcessImages(
      ...
      case eTuttlePluginComponentsAuto: {
        ...
        default:
          BOOST_THROW_EXCEPTION(exception::Unsupported()
                                << exception::user(
                                       "Exr Writer: components not supported"));
          break;
        }
        break;
      }
...
void EXRWriterProcess<View>::writeGrayImage(View &src, std::string &filepath,
  ...
  default:
    BOOST_THROW_EXCEPTION(exception::ImageFormat()
                          << exception::user(
                                 "ExrWriter: incompatible image type"));
    break;
  }
  ...
  case 1: {
    ...
    frameBuffer.insert("Y", Imf::Slice(pixType, pixelsY, bitsTypeSize,
                                       bitsTypeSize * src.width()));
    break;
  }
  // RGB
  // RGBA
...
template <class View>
template <class WPixel>
void EXRWriterProcess<View>::writeImage(View &src, std::string &filepath,
                                        Imf::PixelType pixType) {
  using namespace terry;
  ...
  copy_and_convert_pixels(src, dvw);
  Imf::Header header(src.width(), src.height(),
                     (float)_plugin._clipSrc->getPixelAspectRatio());
  ...
  default:
    BOOST_THROW_EXCEPTION(
        exception::Bug()
        << exception::dev(
               "EXRWriter: unrecognized compression parameter value."));
    break;
  }
  ...
  case 3: {
    ...
    frameBuffer.insert("R", Imf::Slice(pixType, pixelsR, bitsTypeSize,
                                       bitsTypeSize * src.width()));
    frameBuffer.insert("G", Imf::Slice(pixType, pixelsG, bitsTypeSize,
                                       bitsTypeSize * src.width()));
    frameBuffer.insert("B", Imf::Slice(pixType, pixelsB, bitsTypeSize,
                                       bitsTypeSize * src.width()));
    break;
  }
  // RGBA
  case 4: {
    ...
    frameBuffer.insert("B", Imf::Slice(pixType, pixelsB, bitsTypeSize,
                                       bitsTypeSize * src.width()));
    frameBuffer.insert("A", Imf::Slice(pixType, pixelsA, bitsTypeSize,
                                       bitsTypeSize * src.width()));
    break;
  }
  // Gray
