...
void convertGrayToRGB(DataVector &dataVec, size_t width, size_t height,
                      int pixelSize) {
...
void convertRGBAToRGB(DataVector &dataVec, size_t width, size_t height,
                      int pixelSize) {
  size_t size = width * height;
...
void convertRGBAToABGR(DataVector &dataVec, size_t width, size_t height,
                       int pixelSize) {
  ...
  char shortMask[16] = {
    0x06, 0x07, 0x04, 0x05, 0x02, 0x03, 0x00, 0x01, // pixel 1
  ...
  case 4: {
    ...
    for (; i < size - 3; i += 4) {
      ...
      __m128i result = _mm_srl_epi32(_mm_and_si128(data, maskD),
                                     _mm_set_epi32(24, 24, 24, 24));
      result = _mm_or_si128(
          _mm_srl_epi32(_mm_and_si128(data, maskC), _mm_set_epi32(8, 8, 8, 8)),
          result);
      result = _mm_or_si128(
          _mm_sll_epi32(_mm_and_si128(data, maskB), _mm_set_epi32(8, 8, 8, 8)),
          result);
      result = _mm_or_si128(_mm_sll_epi32(_mm_and_si128(data, maskA),
                                          _mm_set_epi32(24, 24, 24, 24)),
                            result);
...
#ifndef __SSSE3__
    __m128i maskA =
        _mm_set_epi32(0x00000000, 0x0000ffff, 0x00000000, 0x0000ffff);
    __m128i maskB = _mm_sll_epi32(maskA, _mm_set_epi32(16, 16, 16, 16));
    __m128i maskC =
        _mm_set_epi32(0x0000ffff, 0x00000000, 0x0000ffff, 0x00000000);
...
#else
      __m128i result = _mm_srl_epi32(_mm_and_si128(data, maskD),
                                     _mm_set_epi32(48, 48, 48, 48));
      result = _mm_or_si128(_mm_srl_epi32(_mm_and_si128(data, maskC),
                                          _mm_set_epi32(16, 16, 16, 16)),
                            result);
      result = _mm_or_si128(_mm_sll_epi32(_mm_and_si128(data, maskB),
                                          _mm_set_epi32(16, 16, 16, 16)),
                            result);
      result = _mm_or_si128(_mm_sll_epi32(_mm_and_si128(data, maskA),
                                          _mm_set_epi32(48, 48, 48, 48)),
...
void selectChannelInRGBA(void *dataSrcPtr) {
  // boost::gil::rgb32f_view_t vw = boost::gil::interleaved_view (  size.x,
  // size.y, Iterator pixels, std::ptrdiff_t rowsize_in_bytes);
}
