...
template <class View> DPXReaderProcess<View>::~DPXReaderProcess() {}
 ...
 */
template <class View>
void DPXReaderProcess<View>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  using namespace boost::gil;
  readImage(this->_dstView);
}

template <class View> View &DPXReaderProcess<View>::readImage(View &dst) {
  ...
  case tuttle::io::DpxImage::eCompTypeR8G8B8: {
    ...
    rgb8c_view_t src = interleaved_view(
        _dpxImage.width(), _dpxImage.height(),
        (const rgb8_pixel_t *)(_dpxImage.data()), _dpxImage.width() * 3);
    copy_and_convert_pixels(src, dst);
    break;
  }
  case tuttle::io::DpxImage::eCompTypeR8G8B8A8: {
    // Tests passed: fill, non fill, big endian, little endian
    rgba8c_view_t src = interleaved_view(
        _dpxImage.width(), _dpxImage.height(),
        (const rgba8_pixel_t *)(_dpxImage.data()), _dpxImage.width() * 4);
  ...
  case tuttle::io::DpxImage::eCompTypeA8B8G8R8: {
    ...
    abgr8c_view_t src = interleaved_view(
        _dpxImage.width(), _dpxImage.height(),
        (const abgr8_pixel_t *)(_dpxImage.data()), _dpxImage.width() * 4);
  ...
  case tuttle::io::DpxImage::eCompTypeR10G10B10: {
    ...
    default: {
      ...
      gray32_view_t src = interleaved_view(
          width, height, (gray32_pixel_t *)(_dpxImage.rawData()),
  ...
  case tuttle::io::DpxImage::eCompTypeR10G10B10A10: {
    ...
    default: {
      BOOST_THROW_EXCEPTION(
          exception::Failed()
          << exception::dev() +
                 "Unsupported dpx file format (RGBA10 byte packed). ");
    }
    }
    break;
  }
  case tuttle::io::DpxImage::eCompTypeA10B10G10R10: {
    ...
    case 2: {
      BOOST_THROW_EXCEPTION(
          exception::Failed()
          << exception::dev() +
                 "Unsupported dpx file format (ABGR10 byte packed). ");
    }
    }
    break;
  }
  case tuttle::io::DpxImage::eCompTypeR12G12B12: {
    ...
    default: {
      int width = _dpxImage.width();
      int height = _dpxImage.height();
      rgb12_packed_view_t src = interleaved_view(
          width, height, (rgb12_packed_pixel_t *)(_dpxImage.data()),
          _dpxImage.width() * 6);
  ...
  case tuttle::io::DpxImage::eCompTypeR12G12B12A12: {
    // Tests passed: fill big endian or little endian, non fill little endian or
    // big endian
    // Interpret pixels according to its bit packing
    switch (_dpxImage.packing()) {
    // bit stream
    ...
    default: {
      BOOST_THROW_EXCEPTION(
          exception::Failed()
          << exception::dev() +
                 "Unsupported dpx file format (RGBA12 byte packed). ");
      rgba12_packed_view_t vw =
          interleaved_view(_dpxImage.width(), _dpxImage.height(),
  ...
  case tuttle::io::DpxImage::eCompTypeA12B12G12R12: {
    ...
    default: {
      BOOST_THROW_EXCEPTION(
          exception::Failed()
          << exception::dev() +
                 "Unsupported dpx file format (ABGR12 byte packed). ");
      /*
                                              abgr12_packed_view_t vw =
         interleaved_view( _dpxImage.width(), _dpxImage.height(),
  ...
  case tuttle::io::DpxImage::eCompTypeUnknown: {
    BOOST_THROW_EXCEPTION(exception::Failed()
                          << exception::dev() +
                                 "Unrecognized image type, bit depth is " +
                                 _dpxImage.getHeader().bitSize() +
                                 ", component type is: " +
                                 _dpxImage.getHeader().descriptor() + ".\n" +
                                 "DPX HEADER:\n" + _dpxImage.getHeader());
    break;
  }
  }
...
void DPXReaderProcess<View>::bitStreamToView(DST_V &dst, const int nc,
