...
/// example 16-bit RGB in '565' format
...
/// PixelValueConcept, PixelBasedConcept
...
template <typename BitField, // A type that holds the bits of the pixel.
          ...
          typename Layout>
// Layout defining the color space and ordering of the channels. Example value:
// rgb_layout_t
struct packed_pixel {
  BitField _bitfield;
...
struct kth_element_type<packed_pixel<BitField, ChannelRefVec, Layout>,
...
at_c(const packed_pixel<P, C, L> &p) {
  return typename kth_element_const_reference_type<packed_pixel<P, C, L>,
                                                   K>::type(&p._bitfield);
}
...
struct is_pixel<
    packed_pixel<BitField, ChannelRefVec, Layout> > : public mpl::true_ {};
...
/// packed_pixel<P,CR,Layout>
/// \ingroup PixelIteratorModel
/// \brief Iterators over interleaved pixels.
/// The pointer packed_pixel<P,CR,Layout>* is used as an iterator over
...
struct iterator_is_mutable<packed_pixel<
    P, C, L> *> : public mpl::bool_<packed_pixel<P, C, L>::is_mutable> {};
template <typename P, typename C, typename L>
...
struct has_trivial_constructor<
    gil::packed_pixel<P, C, L> > : public has_trivial_constructor<P> {};
}
#endif
