...
void DPXReaderPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  desc.setLabels("TuttleDpxReader", "DpxReader", "Dpx file reader");
...
void DPXReaderPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::PushButtonParamDescriptor *displayHeader =
      desc.definePushButtonParam(kParamDisplayHeader);
  displayHeader->setLabel("See Header");
  displayHeader->setHint(
      "See the file header without formating (debug purpose only).");
}
