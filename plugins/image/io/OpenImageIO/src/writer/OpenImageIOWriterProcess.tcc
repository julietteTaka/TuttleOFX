...
template <class View>
OpenImageIOWriterProcess<View>::OpenImageIOWriterProcess(
    OpenImageIOWriterPlugin &instance)
 ...
 */
template <class View>
ETuttlePluginBitDepth OpenImageIOWriterProcess<View>::getDefaultBitDepth(
    const std::string &filepath, const ETuttlePluginBitDepth &bitDepth) {
  if (bitDepth == eTuttlePluginBitDepthAuto) {
    ...
    if (format.find("exr") != std::string::npos ||
        format.find("hdr") != std::string::npos ||
        format.find("rgbe") != std::string::npos) {
      return eTuttlePluginBitDepth32f;
    } else if (format.find("jpg") != std::string::npos ||
               format.find("jpeg") != std::string::npos ||
               format.find("bmp") != std::string::npos ||
               format.find("dds") != std::string::npos ||
               format.find("ico") != std::string::npos ||
               format.find("jfi") != std::string::npos ||
               format.find("pgm") != std::string::npos ||
               format.find("pnm") != std::string::npos ||
               format.find("ppm") != std::string::npos ||
               format.find("pbm") != std::string::npos ||
               format.find("pic") != std::string::npos) {
      return eTuttlePluginBitDepth8;
    } else {
      // bmp, cin, dpx, fits, j2k, j2c, jp2, jpe, png, sgi, tga, tif, tiff,
      // tpic, webp
 ...
 */
template <class View>
void OpenImageIOWriterProcess<View>::multiThreadProcessImages(
  ...
  ETuttlePluginBitDepth finalBitDepth =
      getDefaultBitDepth(params._filepath, params._bitDepth);
  ...
  try {
    ...
    case eTuttlePluginBitDepthAuto: {
      ...
      if (ext == ".cin") {
        ...
        case OFX::ePixelComponentRGB:
          writeImage<rgb16_image_t>(this->_srcView, params._filepath,
                                    eTuttlePluginBitDepth10);
          break;
      ...
      if (ext == ".jpg" || ext == ".jpeg" || ext == ".png" || ext == ".tga") {
        writeImage<rgb8_image_t>(this->_srcView, params._filepath,
      ...
      case OFX::eBitDepthUByte: {
        ...
        case eTuttlePluginComponentsAuto: {
          switch (_plugin._clipSrc->getPixelComponents()) {
          case OFX::ePixelComponentAlpha:
            writeImage<gray8_image_t>(this->_srcView, params._filepath,
                                      eTuttlePluginBitDepth8);
            break;
          case OFX::ePixelComponentRGB:
            writeImage<rgb8_image_t>(this->_srcView, params._filepath,
            ...
            break;
          case OFX::ePixelComponentRGBA:
            writeImage<rgba8_image_t>(this->_srcView, params._filepath,
                                      eTuttlePluginBitDepth8);
            break;
          default:
            BOOST_THROW_EXCEPTION(
      ...
      case OFX::eBitDepthNone:
        BOOST_THROW_EXCEPTION(exception::Unsupported()
                              << exception::user(
                                     "OIIO Writer: bit depth not supported"));
      }
      break;
    }
    ...
    case eTuttlePluginBitDepth16f: {
      ...
      case eTuttlePluginComponentsAuto: {
        switch (_plugin._clipSrc->getPixelComponents()) {
        case OFX::ePixelComponentAlpha:
          writeImage<gray16h_image_t>(this->_srcView, params._filepath,
                                      params._bitDepth);
          break;
        case OFX::ePixelComponentRGB:
          writeImage<rgb16h_image_t>(this->_srcView, params._filepath,
                                     params._bitDepth);
          break;
        case OFX::ePixelComponentRGBA:
          writeImage<rgba16h_image_t>(this->_srcView, params._filepath,
                                      params._bitDepth);
          break;
        default:
          BOOST_THROW_EXCEPTION(
    ...
    case eTuttlePluginBitDepth32: {
      ...
      case eTuttlePluginComponentsAuto: {
        switch (_plugin._clipSrc->getPixelComponents()) {
        case OFX::ePixelComponentAlpha:
          writeImage<gray32_image_t>(this->_srcView, params._filepath,
                                     params._bitDepth);
          break;
        case OFX::ePixelComponentRGB:
          writeImage<rgb32_image_t>(this->_srcView, params._filepath,
                                    params._bitDepth);
          break;
        case OFX::ePixelComponentRGBA:
          writeImage<rgba32_image_t>(this->_srcView, params._filepath,
                                     params._bitDepth);
          break;
        default:
          BOOST_THROW_EXCEPTION(
...
void OpenImageIOWriterProcess<View>::writeImage(
    View &src, const std::string &filepath,
  ...
  if (out.get() == NULL) {
    BOOST_THROW_EXCEPTION(
        exception::Bug()
        << exception::user(
               "OIIO Writer: error from writer while opening file."));
  }
  ...
  case eTuttlePluginBitDepthAuto:
    BOOST_THROW_EXCEPTION(exception::Bug()
                          << exception::user(
                                 "OIIO Writer: undefined output bit depth."));
  case eTuttlePluginBitDepth8:
  ...
  ImageSpec spec(src.width(), src.height(), gil::num_channels<WImage>::value,
                 oiioBitDepth);
  ...
  const stride_t xstride = gil::is_planar<WImage>::value
                               ? sizeOfChannel
                               : vw.num_channels() * sizeOfChannel;
  ...
  out->write_image(oiioBitDepth, &((*vw.begin())[0]), // get the adress of the
                                                      // first channel value
