...
template <class View>
OpenImageIOReaderProcess<View>::OpenImageIOReaderProcess(
    OpenImageIOReaderPlugin &instance)
 ...
 */
template <class View>
void OpenImageIOReaderProcess<View>::multiThreadProcessImages(
  ...
  boost::scoped_ptr<OpenImageIO::ImageInput> img(
      OpenImageIO::ImageInput::create(filename));
  ...
  if (!img->open(filename, spec)) {
    BOOST_THROW_EXCEPTION(exception::Unknown()
                          << exception::user("OIIO Reader: " + img->geterror())
...
View &OpenImageIOReaderProcess<View>::readImage(
    View &dst, boost::scoped_ptr<OpenImageIO::ImageInput> &img, int pixelSize) {
  ...
  img->read_image(TypeDesc::UNKNOWN, // it's to not convert into OpenImageIO,
                                     // convert with GIL
                  &((*tmpView.begin())[0]), // get the adress of the first
                                            // channel value from the first
                                            // pixel
