...
ImageMagickReaderProcessParams
ImageMagickReaderPlugin::getProcessParams(const OfxTime time) {
...
void ImageMagickReaderPlugin::render(const OFX::RenderArguments &args) {
  ...
  case OFX::ePixelComponentRGBA: {
    doGilRender<ImageMagickReaderProcess, false, boost::gil::rgba_layout_t>(
        *this, args, bitDepth);
    return;
  }
  case OFX::ePixelComponentRGB: {
    doGilRender<ImageMagickReaderProcess, false, boost::gil::rgb_layout_t>(
        *this, args, bitDepth);
    return;
  }
  case OFX::ePixelComponentAlpha: {
    doGilRender<ImageMagickReaderProcess, false, boost::gil::gray_layout_t>(
        *this, args, bitDepth);
    return;
  }
...
void ImageMagickReaderPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                                           const std::string &paramName) {
  ReaderPlugin::changedParam(args, paramName);
}

bool ImageMagickReaderPlugin::getRegionOfDefinition(
  ...
  if (!image) {
    ...
    BOOST_THROW_EXCEPTION(
        exception::FileInSequenceNotExist()
...
void ImageMagickReaderPlugin::getClipPreferences(
  ...
  //	Image* image = PingImage( imageInfo, exceptionsInfo );
  Image *image =
      ReadImage(imageInfo, exceptionsInfo); // necessary to obtain QuantumType

  if (!image)
    BOOST_THROW_EXCEPTION(exception::File() << exception::user("ImageMagick: ")
                                            << exception::filename(filename));

  if (getExplicitBitDepthConversion() == eParamReaderBitDepthAuto) {
    clipPreferences.setClipBitDepth(*this->_clipDst,
                                    OFX::eBitDepthFloat); // by default
    unsigned long bitDepth =
        GetImageDepth(image, exceptionsInfo); // if image information use it
    if (bitDepth <= 8) {
  ...
  if (getExplicitChannelConversion() == eParamReaderChannelAuto) {
    switch (colorType) {
    case RGBQuantum: {
      if (OFX::getImageEffectHostDescription()->supportsPixelComponent(
  ...
  clipPreferences.setPixelAspectRatio(*this->_clipDst,
                                      1.0); /// @todo tuttle: retrieve info
