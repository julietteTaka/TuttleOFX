...
BOOST_STATIC_ASSERT(MAGICKCORE_QUANTUM_DEPTH != 8 ||
...
typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 0,
    MAGICKCORE_QUANTUM_DEPTH, true> rgba_quantum_packed_channel0_t;
typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 1,
    MAGICKCORE_QUANTUM_DEPTH, true> rgba_quantum_packed_channel1_t;
typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 2,
    MAGICKCORE_QUANTUM_DEPTH, true> rgba_quantum_packed_channel2_t;
typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 3,
    MAGICKCORE_QUANTUM_DEPTH, true> rgba_quantum_packed_channel3_t;

typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 0,
    MAGICKCORE_QUANTUM_DEPTH, true> rgb_quantum_packed_channel0_t;
typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 1,
    MAGICKCORE_QUANTUM_DEPTH, true> rgb_quantum_packed_channel1_t;
typedef const packed_channel_reference<
    MAGICKCORE_QUANTUM_SUPPORT, MAGICKCORE_QUANTUM_DEPTH * 2,
    MAGICKCORE_QUANTUM_DEPTH, true> rgb_quantum_packed_channel2_t;

// RGBA
typedef mpl::vector4<
    rgba_quantum_packed_channel0_t, rgba_quantum_packed_channel1_t,
    rgba_quantum_packed_channel2_t,
    rgba_quantum_packed_channel3_t> rgba_quantum_packed_channels_t;
typedef packed_pixel<MAGICKCORE_QUANTUM_SUPPORT, rgba_quantum_packed_channels_t,
                     bgra_layout_t> bgra_quantum_packed_pixel_t;
typedef view_type_from_pixel<bgra_quantum_packed_pixel_t>::type
bgra_quantum_packed_view_t;
typedef image<bgra_quantum_packed_pixel_t, false> bgra_quantum_packed_image_t;

// RGB
typedef mpl::vector3<
    rgb_quantum_packed_channel0_t, rgb_quantum_packed_channel1_t,
    rgb_quantum_packed_channel2_t> rgb_quantum_packed_channels_t;
typedef packed_pixel<MAGICKCORE_QUANTUM_SUPPORT, rgb_quantum_packed_channels_t,
                     bgr_layout_t> bgr_quantum_packed_pixel_t;
typedef view_type_from_pixel<bgr_quantum_packed_pixel_t>::type
...
template <class View>
ImageMagickReaderProcess<View>::ImageMagickReaderProcess(
    ImageMagickReaderPlugin &instance)
 ...
 */
template <class View>
void ImageMagickReaderProcess<View>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  // no tiles and no multithreading supported
  BOOST_ASSERT(procWindowRoW == this->_dstPixelRod);
  readGilImage(this->_dstView,
               _plugin.getProcessParams(this->_renderArgs.time)._filepath);
}
...
void copy_and_convert_from_buffer(Image *image, DView &dst) {
  ...
  PixelPacket *buffer =
      GetAuthenticPixels(image, 0, 0, dst.width(), dst.height(), &exception);

  SView bufferView = interleaved_view(
      dst.width(), dst.height(), (typename SView::value_type *)(buffer),
...
template <class View>
View &
ImageMagickReaderProcess<View>::readGilImage(View &dst,
  ...
  case RGBQuantum: {
    ...
    copy_and_convert_from_buffer<bgr_quantum_packed_view_t, rgb8_view_t>(image,
                                                                         tmpVw);
#elif(MAGICKCORE_QUANTUM_DEPTH == 16)
    rgb16_image_t tmpImg(dst.width(), dst.height());
    rgb16_view_t tmpVw = view(tmpImg);

    copy_and_convert_from_buffer<bgr_quantum_packed_view_t, rgb16_view_t>(
        image, tmpVw);
#elif(MAGICKCORE_QUANTUM_DEPTH == 32)
    rgb32_image_t tmpImg(dst.width(), dst.height());
    rgb32_view_t tmpVw = view(tmpImg);

    copy_and_convert_from_buffer<bgr_quantum_packed_view_t, rgb32_view_t>(
        image, tmpVw);
#endif
  ...
  case GrayQuantum: {
    ...
    copy_and_convert_from_buffer<bgra_quantum_packed_view_t, rgba8_view_t>(
        image, tmpVw);
#elif(MAGICKCORE_QUANTUM_DEPTH == 16)
    rgba16_image_t tmpImg(dst.width(), dst.height());
    rgba16_view_t tmpVw = view(tmpImg);

    copy_and_convert_from_buffer<bgra_quantum_packed_view_t, rgba16_view_t>(
        image, tmpVw);
#elif(MAGICKCORE_QUANTUM_DEPTH == 32)
    rgba32_image_t tmpImg(dst.width(), dst.height());
    rgba32_view_t tmpVw = view(tmpImg);

    copy_and_convert_from_buffer<bgra_quantum_packed_view_t, rgba32_view_t>(
        image, tmpVw);
#endif
  ...
  default: {
    BOOST_THROW_EXCEPTION(exception::Unsupported()
                          << exception::user(
                                 "Unknown color type (" +
                                 boost::lexical_cast<std::string>(colorType) +
                                 ")"));
    break;
  }
  }
