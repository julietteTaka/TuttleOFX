...
OutputBufferPlugin::OutputBufferPlugin(OfxImageEffectHandle handle)
    : OFX::ImageEffect(handle), _tempStoreCustomDataPtr(NULL) {
  ...
  _paramCallbackDestroyCustomData =
      fetchStringParam(kParamOutputCallbackDestroyCustomData);
}
...
void OutputBufferPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamOutputCustomData) {
    ...
    _tempStoreCustomDataPtr = static_cast<CustomDataPtr>(
        stringToPointer(_paramCustomData->getValue()));
  }
}

OutputBufferProcessParams OutputBufferPlugin::getProcessParams() const {
  OutputBufferProcessParams params;
  params._callbackPtr = reinterpret_cast<CallbackOutputImagePtr>(
      stringToPointer(_paramCallbackOutputPointer->getValue()));
  params._customDataPtr =
      static_cast<CustomDataPtr>(stringToPointer(_paramCustomData->getValue()));
  params._callbackDestroyPtr = reinterpret_cast<CallbackDestroyCustomDataPtr>(
      stringToPointer(_paramCallbackDestroyCustomData->getValue()));
  return params;
}
...
void OutputBufferPlugin::render(const OFX::RenderArguments &args) {
  ...
  if (params._callbackPtr != NULL) {
    params._callbackPtr(args.time, params._customDataPtr, rawImagePtrLink,
                        bounds.x2 - bounds.x1, bounds.y2 - bounds.y1,
                        rowBytesToCopy, depth, components, field);
  }
}
}
}
