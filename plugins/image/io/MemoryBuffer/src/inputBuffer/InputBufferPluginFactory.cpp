 ...
 */
void
InputBufferPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *inputMode =
      desc.defineChoiceParam(kParamInputMode);
  ...
  OFX::StringParamDescriptor *inputBuffer =
      desc.defineStringParam(kParamInputBufferPointer);
  ...
  inputBuffer->setHint(
      ...
      " - If you want to animate this parameter to put different image buffer, "
      "be careful to disable interpolation.\n");
  inputBuffer->setCacheInvalidation(OFX::eCacheInvalidateValueAll);
  // This is a pointer to a dynamic memory allocation, so there no sense to
  // store this value!
  // Need to be setted at each load.
  inputBuffer->setIsPersistant(false);
  inputBuffer->setDefault("");

  OFX::StringParamDescriptor *callbackPointer =
      desc.defineStringParam(kParamInputCallbackPointer);
  callbackPointer->setLabel("Callback Pointer");
  callbackPointer->setHint(
      "This parameter represent a pointer to a function of type \"void*(*)(int "
      "time, void* inputCustomData)\" .\n"
      "WARNING:\n"
      " - Your application will crash if you set an invalid value here.\n");
  ...
  OFX::StringParamDescriptor *customData =
      desc.defineStringParam(kParamInputCustomData);
  customData->setLabel("Callback Custom Data Pointer");
  customData->setHint(
      "This parameter represent a pointer to a custom data which is given to "
  ...
  OFX::IntParamDescriptor *rowBytesSize =
      desc.defineIntParam(kParamRowBytesSize);
  ...
  OFX::DoubleParamDescriptor *par =
      desc.defineDoubleParam(kParamPixelAspectRatio);
  par->setLabel("Pixel Aspect Ratio");
  par->setCacheInvalidation(OFX::eCacheInvalidateValueAll);
  par->setAnimates(false);
  par->setDefault(1.0);

  OFX::DoubleParamDescriptor *framerate =
      desc.defineDoubleParam(kParamFramerate);
  ...
  //	OFX::ImageEffectHostDescription* hostDesc =
  //OFX::getImageEffectHostDescription();

  OFX::ChoiceParamDescriptor *components =
      desc.defineChoiceParam(kParamPixelComponents);
  ...
  OFX::ChoiceParamDescriptor *orientation =
      desc.defineChoiceParam(kParamOrientation);
  ...
  OFX::Double2DParamDescriptor *timeDomain =
      desc.defineDouble2DParam(kParamTimeDomain);
  ...
  OFX::StringParamDescriptor *callbackDestroyCustomData =
      desc.defineStringParam(kParamInputCallbackDestroyCustomData);
  ...
  callbackDestroyCustomData->setHint(
      ...
      "WARNING:\n"
      " - Your application could crash if you set an invalid value here.\n");
  callbackDestroyCustomData->setCacheInvalidation(
      OFX::eCacheInvalidateValueAll);
 ...
 */
OFX::ImageEffect *
InputBufferPluginFactory::createInstance(OfxImageEffectHandle handle,
                                         OFX::EContext context) {
