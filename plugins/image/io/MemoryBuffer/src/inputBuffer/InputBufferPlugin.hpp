...
private:
  ...
  /**
   * @brief We call this function each time we need to know something about the
   * image,
   *        the size, the buffer, etc. And this function ensures to get valid
   * values,
   *        and is responsible to call the callback only once for a given input
   * time.
   */
  void callbackMode_updateImage(const OfxTime time,
