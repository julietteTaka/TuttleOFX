...
inline std::size_t numberOfComponents(const OFX::EPixelComponent c) {
  ...
  case OFX::ePixelComponentCustom:
    BOOST_THROW_EXCEPTION(exception::Value()
                          << exception::user() + "Can't retrieve the number of "
                                                 "values inside a custom pixel "
                                                 "component.");
  }
  BOOST_ASSERT(false);
  return 0;
...
InputBufferPlugin::InputBufferPlugin(OfxImageEffectHandle handle)
  ...
  _paramInputCallbackDestroyCustomData =
      fetchStringParam(kParamInputCallbackDestroyCustomData);
...
InputBufferProcessParams
InputBufferPlugin::getProcessParams(const OfxTime time) const {
  InputBufferProcessParams params;
  params._mode = static_cast<EParamInputMode>(_paramInputMode->getValue());

  params._inputBuffer = static_cast<unsigned char *>(
      stringToPointer(_paramInputBufferPointer->getValueAtTime(time)));
  params._callbackPtr = reinterpret_cast<CallbackInputImagePtr>(
      stringToPointer(_paramInputCallbackPointer->getValue()));
  params._customDataPtr = static_cast<CustomDataPtr>(
      stringToPointer(_paramInputCallbackCustomData->getValue()));
  params._callbackDestroyPtr = reinterpret_cast<CallbackDestroyCustomDataPtr>(
      stringToPointer(_paramInputCallbackDestroyCustomData->getValue()));
  ...
  switch (
      static_cast<EParamPixelComponent>(_paramPixelComponents->getValue())) {
  ...
  params._orientation =
      static_cast<EParamOrientation>(_paramOrientation->getValue());

  return params;
}

void InputBufferPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                                     const std::string &paramName) {
  if (paramName == kParamInputMode) {
    const EParamInputMode mode =
        static_cast<EParamInputMode>(_paramInputMode->getValue());

    _paramInputBufferPointer->setIsSecretAndDisabled(
        (mode != eParamInputModeBufferPointer));
    _paramInputCallbackPointer->setIsSecretAndDisabled(
        (mode != eParamInputModeCallbackPointer));
    _paramInputCallbackCustomData->setIsSecretAndDisabled(
        (mode != eParamInputModeCallbackPointer));
    ...
    _tempStoreCustomDataPtr = static_cast<CustomDataPtr>(
        stringToPointer(_paramInputCallbackCustomData->getValue()));
  }
}
...
void InputBufferPlugin::getClipPreferences(
  ...
  clipPreferences.setOutputFrameVarying(params._mode ==
...
bool InputBufferPlugin::getRegionOfDefinition(
...
void InputBufferPlugin::callbackMode_updateImage(
    const OfxTime time, const InputBufferProcessParams &params) {
  ...
  params._callbackPtr(time, params._customDataPtr,
                      (void **)&(_callbackMode_imgPointer),
                      &_callbackMode_imgSize.x, &_callbackMode_imgSize.y,
...
void InputBufferPlugin::render(const OFX::RenderArguments &args) {
  ...
  //	else
  {
    ...
    const std::size_t nbComponents =
        numberOfComponents(params._pixelComponents);
    ...
    case eParamOrientationFromBottomToTop: {
      for (int y = 0; y < dstPixelRodSize.y; ++y) {
        memcpy(dst->getPixelAddress(0, y),
               inputImageBufferPtr + y * rowBytesDistanceSize, widthBytesSize);
      }
      break;
    }
    case eParamOrientationFromTopToBottom: {
      for (int y = 0; y < dstPixelRodSize.y; ++y) {
        memcpy(dst->getPixelAddress(0, y),
               inputImageBufferPtr +
                   (dstPixelRodSize.y - y) * rowBytesDistanceSize,
               widthBytesSize);
      }
      break;
    }
