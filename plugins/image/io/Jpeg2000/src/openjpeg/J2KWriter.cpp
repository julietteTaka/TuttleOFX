...
J2KWriter::J2KWriter()
    : _width(0), _height(0), _components(0), _precision(0), _cinemaMode(OFF),
      _cio(NULL) {
  memset(&_openjpeg, 0, sizeof(OpenJpegStuffs));
}

J2KWriter::~J2KWriter() { close(); }

void J2KWriter::open(const std::string &filename, const std::size_t w,
                     const std::size_t h, const std::size_t nc,
  ...
  if (_openjpeg.parameters.cp_comment == NULL) {
    ...
    _openjpeg.parameters.cp_comment =
        (char *)malloc(clen + strlen(version) + 1);
  ...
  _openjpeg.image->x1 =
      !_openjpeg.image->x0 ? (w - 1) * subsampling_dx + 1
                           : _openjpeg.image->x0 + (w - 1) * subsampling_dx + 1;
  _openjpeg.image->y1 =
      !_openjpeg.image->y0 ? (h - 1) * subsampling_dy + 1
                           : _openjpeg.image->y0 + (h - 1) * subsampling_dy + 1;
...
void J2KWriter::encode(const uint8_t *data, const std::size_t sprecision) {
  ...
  default: {
    BOOST_THROW_EXCEPTION(exception::Value()
                          << exception::user() +
                                 "Unrecognized output J2K format \"" +
...
void J2KWriter::cinemaSetupEncoder() {
  ...
  case CINEMA2K_48: {
    ...
    if (_openjpeg.image->comps[0].w != 2048 &&
        _openjpeg.image->comps[0].h != 1080) {
      _openjpeg.parameters.cp_rsiz = STD_RSIZ;
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user() + "Image dimensions " +
                                   _openjpeg.image->comps[0].w + " x " +
                                   _openjpeg.image->comps[0].h +
                                   " is not 2K compliant.\n" +
                                   "JPEG Digital Cinema Profile-3 (2K profile) "
                                   "compliance requires that at least one of "
  ...
  case CINEMA4K_24: {
    ...
    if (_openjpeg.image->comps[0].w != 4096 &&
        _openjpeg.image->comps[0].h != 2160) {
      _openjpeg.parameters.cp_rsiz = STD_RSIZ;
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user() + "Image dimensions " +
                                   _openjpeg.image->comps[0].w + " x " +
                                   _openjpeg.image->comps[0].h +
                                   " is not 4K compliant.\n" +
                                   "JPEG Digital Cinema Profile-3 (4K profile) "
                                   "compliance requires that at least one of "
                                   "coordinates match 4096 x 2160");
    }
    _openjpeg.parameters.numpocs = initialize4Kpocs(
        _openjpeg.parameters.POC, _openjpeg.parameters.numresolution);
    break;
  }
  default:
    break;
  ...
  case CINEMA4K_24: {
    for (int i = 0; i < _openjpeg.parameters.tcp_numlayers; ++i) {
      _openjpeg.parameters.tcp_rates[0] =
          ((float)(_openjpeg.image->numcomps * _openjpeg.image->comps[0].w *
                   _openjpeg.image->comps[0].h *
                   _openjpeg.image->comps[0].prec)) /
          (CINEMA_24_CS * 8 * _openjpeg.image->comps[0].dx *
           _openjpeg.image->comps[0].dy);
    }
    ...
    for (int i = 0; i < _openjpeg.parameters.tcp_numlayers; ++i) {
      _openjpeg.parameters.tcp_rates[0] =
          ((float)(_openjpeg.image->numcomps * _openjpeg.image->comps[0].w *
                   _openjpeg.image->comps[0].h *
                   _openjpeg.image->comps[0].prec)) /
          (CINEMA_48_CS * 8 * _openjpeg.image->comps[0].dx *
           _openjpeg.image->comps[0].dy);
    }
