...
public:
  J2KWriter();
  virtual ~J2KWriter();
  void open(const std::string &filename, const size_t w, const size_t h,
            const size_t nc, const size_t dprecision);
...
inline void J2KWriter::setLossless(bool lossless) { _lossless = lossless; }

inline void J2KWriter::setPrecision(const size_t prec) { _precision = prec; }
