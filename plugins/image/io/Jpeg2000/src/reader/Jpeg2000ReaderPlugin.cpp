...
Jpeg2000ReaderPlugin::~Jpeg2000ReaderPlugin() { _reader.close(); }

Jpeg2000ReaderProcessParams
Jpeg2000ReaderPlugin::getProcessParams(const OfxTime time) {
...
void Jpeg2000ReaderPlugin::changedParam(const OFX::InstanceChangedArgs &args,
...
bool Jpeg2000ReaderPlugin::getRegionOfDefinition(
...
void Jpeg2000ReaderPlugin::getClipPreferences(
  ...
  if (fileInfo._failed) {
    BOOST_THROW_EXCEPTION(exception::Failed()
                          << exception::user(
                                 "Jpeg2000: Unable to read file infos."));
  }
...
void Jpeg2000ReaderPlugin::render(const OFX::RenderArguments &args) {
  ...
  if (dstComponents == OFX::ePixelComponentRGBA) {
    ...
    case OFX::eBitDepthCustom: {
      BOOST_THROW_EXCEPTION(exception::BitDepthMismatch()
                            << exception::user(
                                   "Jpeg2000: BitDepthCustom not recognize"));
    }
    }
...
Jpeg2000ReaderPlugin::FileInfo
Jpeg2000ReaderPlugin::retrieveFileInfo(const OfxTime time) {
  ...
  if (!_reader.componentsConform()) {
    ...
    BOOST_THROW_EXCEPTION(exception::ImageFormat()
                          << exception::user(
                                 "Jpeg2000 image components aren't conform.")
                          << exception::filename(filename)
                          << exception::time(time));
  }
  ...
  default:
    ...
    BOOST_THROW_EXCEPTION(exception::ImageFormat()
                          << exception::user() +
                                 "Jpeg2000: Bit depth not handled ! (" +
                                 _reader.precision() + ")"
                          << exception::filename(filename)
                          << exception::time(time));
  }
