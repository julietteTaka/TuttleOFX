...
Jpeg2000ReaderProcess<View>::Jpeg2000ReaderProcess(
    Jpeg2000ReaderPlugin &instance)
    : ImageGilProcessor<View>(instance, eImageOrientationFromTopToBottom),
      _plugin(instance) {
  this->setNoMultiThreading();
}

template <class View> Jpeg2000ReaderProcess<View>::~Jpeg2000ReaderProcess() {
 ...
 */
template <class View>
void Jpeg2000ReaderProcess<View>::multiThreadProcessImages(
...
void Jpeg2000ReaderProcess<View>::switchLayoutCopy(const View &dstView) {
  ...
  default:
    BOOST_THROW_EXCEPTION(exception::ImageFormat()
                          << exception::user("Unhandled image precision!"));
    break;
  }
}
