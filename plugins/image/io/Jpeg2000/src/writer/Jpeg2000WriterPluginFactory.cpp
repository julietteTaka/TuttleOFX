...
void Jpeg2000WriterPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  desc.setLabels("TuttleJpeg2000Writer", "Jpeg2000Writer",
                 "Jpeg 2000 image writer");
  desc.setPluginGrouping("tuttle/image/io");

  desc.setDescription("Jpeg2000 writer\n"
                      "plugin is used to output jpeg 2000 files.\n"
                      "In the filename pattern, put @ where you want your "
                      "incrementation to be.");
 ...
 */
void
Jpeg2000WriterPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *channel =
      static_cast<OFX::ChoiceParamDescriptor *>(
          desc.getParamDescriptor(kTuttlePluginChannel));
  ...
  OFX::BooleanParamDescriptor *lossless =
      desc.defineBooleanParam(kParamLossless);
  lossless->setLabel("lossless");
  lossless->setHint(
      "When no cinema profile is selected, set compression to lossless.");
  lossless->setDefault(false);

  OFX::ChoiceParamDescriptor *cineProfil =
      desc.defineChoiceParam(kParamCinemaProfil);
 ...
 */
OFX::ImageEffect *
Jpeg2000WriterPluginFactory::createInstance(OfxImageEffectHandle handle,
                                            OFX::EContext context) {
