...
Jpeg2000WriterProcess<View>::Jpeg2000WriterProcess(
    Jpeg2000WriterPlugin &instance)
    : ImageGilFilterProcessor<View>(instance, eImageOrientationFromTopToBottom),
      _plugin(instance) {
  this->setNoMultiThreading();
}

template <class View> Jpeg2000WriterProcess<View>::~Jpeg2000WriterProcess() {
  _writer.close();
}
 ...
 */
template <class View>
void Jpeg2000WriterProcess<View>::multiThreadProcessImages(
  ...
  case eTuttlePluginBitDepthAuto: {
    ...
    case OFX::eBitDepthNone:
      BOOST_THROW_EXCEPTION(exception::Unsupported()
                            << exception::user(
                                   "Jpeg2000 Writer: bit depth not supported"));
    }
    break;
  }
...
void Jpeg2000WriterProcess<View>::writeImage(const View &srcView,
                                             const int &bitDepth) {
  using namespace terry;
  ...
  _writer.open(_params._filepath, srcView.width(), srcView.height(),
               num_channels<typename SImg::view_t::value_type>::type::value,
               bitDepth);
