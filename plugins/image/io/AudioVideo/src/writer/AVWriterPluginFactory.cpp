...
void AVWriterPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  desc.setLabels("TuttleAVWriter", "AVWriter", "Audio Video writer");
  ...
  std::vector<std::string> supportedExtensions;
  {
    ...
    while (oFormat != NULL) {
      if (oFormat->extensions != NULL) {
        ...
        for (std::vector<std::string>::iterator it = exts.begin();
             it != exts.end();) {
          if (it->size() == 0)
            it = exts.erase(it);
          else
        ...
        supportedExtensions.insert(supportedExtensions.end(), exts.begin(),
                                   exts.end());
      }
...
void AVWriterPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *bitDepth =
      static_cast<OFX::ChoiceParamDescriptor *>(
          desc.getParamDescriptor(kTuttlePluginBitDepth));
  ...
  OFX::BooleanParamDescriptor *premult =
      static_cast<OFX::BooleanParamDescriptor *>(
          desc.getParamDescriptor(kParamPremultiplied));
  premult->setDefault(true);

  /// MAIN PRESET SELECTOR
  OFX::ChoiceParamDescriptor *mainPreset =
      desc.defineChoiceParam(kParamMainPreset);
  ...
  // Groups
  OFX::GroupParamDescriptor *formatGroup =
      desc.defineGroupParam(kParamFormatGroup);
  OFX::GroupParamDescriptor *videoGroup =
      desc.defineGroupParam(kParamVideoGroup);
  OFX::GroupParamDescriptor *audioGroup =
      desc.defineGroupParam(kParamAudioGroup);
  ...
  /// FORMAT PARAMETERS
  /// format preset
  OFX::ChoiceParamDescriptor *formatPreset =
      desc.defineChoiceParam(kParamFormatPreset);
  ...
  addOptionsFromAVOption(desc, formatGroup, (void *)avFormatContext,
                         AV_OPT_FLAG_ENCODING_PARAM, 0);
  avformat_free_context(avFormatContext);

  /// format parameters
  OFX::GroupParamDescriptor *formatDetailledGroup =
      desc.defineGroupParam(kParamFormatDetailledGroup);
  formatDetailledGroup->setLabel("Detailled");
  formatDetailledGroup->setAsTab();
  formatDetailledGroup->setParent(formatGroup);

  addOptionsFromAVOption(desc, formatDetailledGroup,
                         writer.getFormatPrivOpts());

  // fps parameters
  OFX::BooleanParamDescriptor *useCustomFps =
      desc.defineBooleanParam(kParamUseCustomFps);
  useCustomFps->setLabel("Override Fps");
  useCustomFps->setDefault(false);
  useCustomFps->setHint(
      "Override the input Fps (Frames Per Second) with a custom Fps value.");

  OFX::DoubleParamDescriptor *customFps =
      desc.defineDoubleParam(kParamCustomFps);
  customFps->setLabel("Custom Fps");
  customFps->setDefault(1.0);
  customFps->setHint(
      "Choose a custom value to override the Fps (Frames Per Second).");

  /// VIDEO PARAMETERS
  /// video codec preset
  OFX::ChoiceParamDescriptor *videoPreset =
      desc.defineChoiceParam(kParamVideoPreset);
  ...
  /// video codec list
  int default_codec = 0;
  OFX::ChoiceParamDescriptor *videoCodec =
      desc.defineChoiceParam(kParamVideoCodec);
  for (std::vector<std::string>::const_iterator
           itShort = writer.getVideoCodecsShort().begin(),
  ...
  OFX::ChoiceParamDescriptor *videoCodecPixelFmt =
      desc.defineChoiceParam(kParamVideoCodecPixelFmt);
...
// avCodecContext = avcodec_alloc_context2( AVMEDIA_TYPE_UNKNOWN );
  ...
  addOptionsFromAVOption(desc, videoGroup, (void *)avCodecContext,
                         AV_OPT_FLAG_ENCODING_PARAM | AV_OPT_FLAG_VIDEO_PARAM,
                         0);

  OFX::GroupParamDescriptor *videoDetailledGroup =
      desc.defineGroupParam(kParamVideoDetailledGroup);
  videoDetailledGroup->setLabel("Detailled");
  videoDetailledGroup->setAsTab();
  videoDetailledGroup->setParent(videoGroup);

  addOptionsFromAVOption(desc, videoDetailledGroup,
                         writer.getVideoCodecPrivOpts());

  /// AUDIO PARAMETERS
  /// audio codec preset
  OFX::ChoiceParamDescriptor *audioPreset =
      desc.defineChoiceParam(kParamAudioPreset);
  ...
  OFX::ChoiceParamDescriptor *audioCodec =
      desc.defineChoiceParam(kParamAudioCodec);
  for (std::vector<std::string>::const_iterator
           itShort = writer.getAudioCodecsShort().begin(),
  ...
  addOptionsFromAVOption(desc, audioGroup, (void *)avCodecContext,
                         AV_OPT_FLAG_ENCODING_PARAM | AV_OPT_FLAG_AUDIO_PARAM,
                         AV_OPT_FLAG_VIDEO_PARAM);

  OFX::GroupParamDescriptor *audioDetailledGroup =
      desc.defineGroupParam(kParamAudioDetailledGroup);
  audioDetailledGroup->setLabel("Detailled");
  audioDetailledGroup->setAsTab();
  audioDetailledGroup->setParent(audioGroup);

  addOptionsFromAVOption(desc, audioDetailledGroup,
                         writer.getAudioCodecPrivOpts());
  ...
  hints[kParamMetaLanguage] = "main language in which the work is performed, "
                              "preferably in ISO 639-2 format. Multiple "
                              "languages can be specified by separating them "
                              "with commas.";
  hints[kParamMetaPerformer] =
      "artist who performed the work, if different from artist. ( E.g for "
      "'Also sprach Zarathustra', artist would be 'Richard Strauss' and "
      "performer 'London Philharmonic Orchestra'.";
  hints[kParamMetaPublisher] = "name of the label/publisher.";
  hints[kParamMetaServiceName] =
      "name of the service in broadcasting (channel name).";
  hints[kParamMetaServiceProvider] =
      "name of the service provider in broadcasting.";
  hints[kParamMetaTitle] = "name of the work.";
  hints[kParamMetaTrack] =
      "number of this work in the set, can be in form current/total.";
  hints[kParamMetaVariantBitrate] = "the total bitrate of the bitrate variant "
                                    "that the current stream is part of";

  std::map<std::string, std::string>::iterator ritKeys;
  std::map<std::string, std::string>::iterator ritHint;

  for (ritKeys = keys.begin(), ritHint = hints.begin(); ritKeys != keys.end();
       ++ritKeys, ++ritHint) {
    OFX::StringParamDescriptor *metaParam =
        desc.defineStringParam(ritKeys->first);
  ...
  OFX::GroupParamDescriptor *metaDetailledGroup =
      desc.defineGroupParam(kParamMetaDetailledGroup);
 ...
 */
OFX::ImageEffect *
AVWriterPluginFactory::createInstance(OfxImageEffectHandle handle,
                                      OFX::EContext context) {
