...
SwscaleProcessParams
SwscalePlugin::getProcessParams(const OfxPointD &renderScale) const {
...
void SwscalePlugin::changedParam(const OFX::InstanceChangedArgs &args,
...
bool SwscalePlugin::getRegionOfDefinition(
...
// void SwscalePlugin::getRegionsOfInterest( const
...
bool SwscalePlugin::isIdentity(const OFX::RenderArguments &args,
...
void SwscalePlugin::render(const OFX::RenderArguments &args) {
  ...
  if (desc->_supportedPixelDepths.size() == 1 &&
      OFX::eBitDepthFloat == desc->_supportedPixelDepths.at(0)) {
    BOOST_THROW_EXCEPTION(exception::BitDepthMismatch()
                          << exception::user(
                                 "SwScale: unsupported plugin on this host."));
  }
