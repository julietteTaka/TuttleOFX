...
PixelFormat
ofxPixelComponentToSwsPixelFormat(const OFX::EPixelComponent component,
...
void SwscaleProcess::multiThreadProcessImages(const OfxRectI &procWindow) {
  ...
  _context = sws_getCachedContext(
      _context, this->_src->getBoundsSize().x, this->_src->getBoundsSize().y,
      pixFmt, this->_dst->getBoundsSize().x, this->_dst->getBoundsSize().y,
      pixFmt, _params._sws_filter, NULL, NULL, NULL);
  if (!_context) {
    BOOST_THROW_EXCEPTION(exception::Failed()
                          << exception::user(
                                 "swscale: Could not get swscale context."));
  }
  ...
  if (!srcPtr || !dstPtr) {
    BOOST_THROW_EXCEPTION(exception::Failed()
                          << exception::user(
                                 "swscale: Pixel data pointer invalid."));
  }

  // "this->_src->getRowDistanceBytes()" is not the same than
  // "this->_src->getBoundsSize().x * pixelBytes"
...
void SwscaleProcess::postProcess() { sws_freeContext(_context); }
}
}
}
