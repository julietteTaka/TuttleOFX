 ...
 */
void SwscalePluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  scale->setRange(0.01, 0.01, std::numeric_limits<double>::max(),
                  std::numeric_limits<double>::max());
  scale->setDisplayRange(0.1, 0.1, 2.5, 2.5);
  scale->setHint("Scale the input image [0, 0, width*scale, height*scale].");

  OFX::BooleanParamDescriptor *keepRatio =
      desc.defineBooleanParam(kParamSizeKeepRatio);
  ...
  OFX::ChoiceParamDescriptor *direction =
      desc.defineChoiceParam(kParamSizeOrientation);
 ...
 */
OFX::ImageEffect *
SwscalePluginFactory::createInstance(OfxImageEffectHandle handle,
                                     OFX::EContext context) {
