...
PresetParameters LibAVPresetFileParser::parseFile(const std::string &filepath) {
  ...
  if (!inPreset.is_open()) {
    ...
    BOOST_THROW_EXCEPTION(exception::File()
                          << exception::user() +
                                 "AV Preset: Unable to open preset file."
  ...
  try {
    ...
    for (; iter != iterEnd; ++iter) {
      // TUTTLE_LOG_INFO( iter->first << " " <<
      // iter->second.get_value<std::string>() );
      std::vector<std::string> values;
      std::string value = iter->second.get_value<std::string>();

      if (value.length() == 0) {
        BOOST_FOREACH(boost::property_tree::ptree::value_type & v,
                      pt.get_child(iter->first)) {
    ...
    TUTTLE_LOG_WARNING("Error parsing: " << je.filename()
                                         << " on line: " << je.line());
...
void LibAVPresetFileParser::print() const {
  ...
  switch (presetType) {
  case eMainPresetType:
    name += "(main)";
    break;
  case eFormatPresetType:
    name += "(format)";
    break;
  case eVideoPresetType:
    name += "(video)";
    break;
  case eAudioPresetType:
    name += "(audio)";
    break;
  case eUnknownPresetType:
    name += "(unknown)";
    break;
  }
