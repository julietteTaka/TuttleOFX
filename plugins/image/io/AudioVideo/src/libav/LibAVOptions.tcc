...
template <typename IOPlugin>
int AVOptionPlugin<IOPlugin>::convertIntWithOptionalUnit(
    const std::string &param, const std::string &stringValue) {
  int intValue = 0;
  if (std::isdigit(stringValue[stringValue.length() - 1])) {
    try {
      ...
      TUTTLE_LOG_ERROR("avwriter: parameter "
                       << param << " can't convert the value: " << stringValue);
    }
  } else {
    ...
    case 'G':
      ...
      break;
    default:
      TUTTLE_LOG_ERROR("avwriter: unknown unit " << unit << " for parameter "
                                                 << param);
      break;
    }
  }
...
void AVOptionPlugin<IOPlugin>::setParameters(LibAVVideoRW &videoRW,
                                             const EAVParamType &type,
                                             void *av_class, int req_flags,
                                             int rej_flags) {
    ...
    case AV_OPT_TYPE_CONST: {
      for (size_t i = 0; i < unit.size(); i++) {
        if (opt->unit == unit.at(i)) {
          // TUTTLE_LOG_INFO( "options " << opt->name << " to " <<
          // choices.at(i)->getName() );

          if (choiceListWithCountOption.find(choices.at(i)->getName()) ==
              choiceListWithCountOption.end()) {
            choiceListWithCountOption[choices.at(i)->getName()] = 0;
          } else {
            choiceListWithCountOption[choices.at(i)->getName()] += 1;
          }

          if (choiceListWithCountOption[choices.at(i)->getName()] ==
              choiceValue.at(i)) {
            int value = opt->default_val.i64;
            // TUTTLE_LOG_INFO( "options "  << choices.at(i)->getName() << " = "
            // << choiceValue.at(i) << " - " << opt->name << " (" << value <<
            // ")" );
...
template <typename IOPlugin>
template <typename LibAVVideoRW>
void AVOptionPlugin<IOPlugin>::setParameters(
    LibAVVideoRW &videoRW, const EAVParamType &type,
...
template <typename IOPlugin>
void
AVOptionPlugin<IOPlugin>::setParameters(const PresetParameters &parameters) {
    ...
    if (IOPlugin::paramExists(param.first)) {
      ...
      case OFX::eInt2DParam: {
        ...
        if (param.second.size() == 2) {
          ...
          TUTTLE_LOG_WARNING("avwriter: unable to set an 2D Int value with "
                             << param.second.size() << " parameters.");
        }
        break;
      }
      case OFX::eInt3DParam: {
        ...
        if (param.second.size() == 3) {
          ...
          TUTTLE_LOG_WARNING("avwriter: unable to set an 3D Int value with "
                             << param.second.size() << " parameters.");
        }
        break;
      }
      case OFX::eDoubleParam: {
        ...
        try {
          ...
          TUTTLE_LOG_WARNING("avwriter: parameter "
                             << param.first
                             << " can't convert the value: " << value);
        }
        break;
      }
      case OFX::eDouble2DParam: {
        OFX::Double2DParam *fetchParam =
            IOPlugin::fetchDouble2DParam(param.first);
        if (param.second.size() == 2) {
          try {
            ...
            TUTTLE_LOG_WARNING("avwriter: parameter "
                               << param.first << " can't convert the value: ["
                               << param.second.at(0) << ", "
                               << param.second.at(1) << "]");
          }
        } else {
          TUTTLE_LOG_WARNING("avwriter: unable to set an 2D Double value with "
                             << param.second.size() << " parameters.");
        }
        break;
      }
      case OFX::eDouble3DParam: {
        OFX::Double3DParam *fetchParam =
            IOPlugin::fetchDouble3DParam(param.first);
        if (param.second.size() == 3) {
          try {
            double x = boost::lexical_cast<double>(param.second.at(0));
            double y = boost::lexical_cast<double>(param.second.at(1));
          ...
          TUTTLE_LOG_WARNING("avwriter: unable to set an 3D Double value with "
                             << param.second.size() << " parameters.");
        }
        break;
      }
      case OFX::eRGBParam: {
        ...
        if (param.second.size() == 3) {
          ...
          TUTTLE_LOG_WARNING("avwriter: unable to set an RGB value with "
                             << param.second.size() << " parameters.");
        }
        break;
      }
      case OFX::eRGBAParam: {
        ...
        if (param.second.size() == 4) {
          ...
          TUTTLE_LOG_WARNING("avwriter: unable to set an RGBA value with "
                             << param.second.size() << " parameters.");
        }
        break;
      }
      case OFX::eBooleanParam: {
        OFX::BooleanParam *fetchParam =
            IOPlugin::fetchBooleanParam(param.first);
        bool boolValue =
            (value == "1" || boost::iequals(value, "y") ||
             boost::iequals(value, "yes") || boost::iequals(value, "true"));
      ...
      case OFX::eChoiceParam: {
        ...
        int index = -1;
        for (int i = 0; i < fetchParam->getProps().propGetDimension(
                                "OfxParamPropChoiceOption");
             i++) {
          if (fetchParam->getProps().propGetString("OfxParamPropChoiceOption",
                                                   i) == value) {
            index = i;
            break;
          }
        ...
        if (index > -1)
          ...
          TUTTLE_LOG_WARNING("avwriter: unable to find value "
                             << value << " for parameter " << param.first);
        break;
      }
      ...
      TUTTLE_LOG_WARNING("avwriter: parameter " << param.first
                                                << " not exist.");
    }
  }
}
...
template <typename IOPlugin>
template <typename LibAVVideoRW>
void AVOptionPlugin<IOPlugin>::optionSet(LibAVVideoRW &videoRW,
                                         const EAVParamType &type,
                                         const AVOption &opt, bool &value) {
  int error = 0;
  switch (type) {
  case eAVParamFormat: {
    error = av_opt_set_int(videoRW._avFormatOptions, opt.name, value,
                           AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamVideo: {
    error = av_opt_set_int(videoRW._stream->codec, opt.name, value,
                           AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamAudio: {
  ...
  if (error)
    TUTTLE_LOG_WARNING("avwriter: " << LibAV::libavError_toString(error)
                                    << " : " << opt.name << " ( "
                                    << (value ? "True" : "False") << " )");
}
...
void AVOptionPlugin<IOPlugin>::optionSet(LibAVVideoRW &videoRW,
                                         const EAVParamType &type,
                                         const AVOption &opt, double &value) {
  int error = 0;
  switch (type) {
  case eAVParamFormat: {
    error = av_opt_set_double(videoRW._avFormatOptions, opt.name, value,
                              AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamVideo: {
    error = av_opt_set_double(videoRW._stream->codec, opt.name, value,
                              AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamAudio: {
...
void AVOptionPlugin<IOPlugin>::optionSet(LibAVVideoRW &videoRW,
                                         const EAVParamType &type,
                                         const AVOption &opt, int &valueNum,
                                         int &valueDen) {
  int error = 0;
  AVRational q;
  q.num = valueNum;
  ...
  switch (type) {
  case eAVParamFormat: {
    error = av_opt_set_q(videoRW._avFormatOptions, opt.name, q,
                         AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamVideo: {
    error = av_opt_set_q(videoRW._stream->codec, opt.name, q,
                         AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamAudio: {
...
void AVOptionPlugin<IOPlugin>::optionSet(LibAVVideoRW &videoRW,
  ...
  switch (type) {
  case eAVParamFormat: {
    error = av_opt_set(videoRW._avFormatOptions, opt.name, value.c_str(),
                       AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamVideo: {
    error = av_opt_set(videoRW._stream->codec, opt.name, value.c_str(),
                       AV_OPT_SEARCH_CHILDREN);
    break;
  }
  case eAVParamAudio: {
