...
public:
  ...
  bool finishInit();
  int execute(boost::uint8_t *const in_buffer, const int in_width,
              const int height, const PixelFormat in_fmt = PIX_FMT_RGB24);
  void finish();
...
public:
  ...
  void setAspectRatio(const double aspectRatio) { _aspectRatio = aspectRatio; }
  ...
  void setPixelFormat(const PixelFormat pxlFmt) { _out_pixelFormat = pxlFmt; }

  PixelFormat getPixelFormat() const { return _out_pixelFormat; }

  const std::string &getFormat() const { return _formatName; }
  ...
  void setFormat(const std::string &format) { _formatName = format; }

  const std::string &getVideoCodec() const { return _videoCodecName; }
  ...
  const std::string &getAudioCodec() const { return _audioCodecName; }
  ...
  void setVideoCodec(const std::string &codec) { _videoCodecName = codec; }

  void setAudioCodec(const unsigned int id) {
    _audioCodecName = _audioCodecsShortNames[id];
  }

  void setAudioCodec(const std::string &codec) { _audioCodecName = codec; }
