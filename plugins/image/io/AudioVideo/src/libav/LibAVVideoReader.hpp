#ifndef _TUTTLE_PLUGIN_LIBAV_LIBAVVIDEOREADER_HPP_
...
class LibAVVideoReader : public LibAV {
public:
  explicit LibAVVideoReader();
  ~LibAVVideoReader();
...
private:
  ...
  bool hasVideo() const { return !_videoIdx.empty(); }

  AVStream *getVideoStream() {
    return _avFormatOptions && _currVideoIdx >= 0
               ? _avFormatOptions->streams[_videoIdx[_currVideoIdx]]
               : NULL;
  }
...
public:
  ...
  double aspectRatio() const { return _aspect; }

  EIntrelacment interlacment() { return _interlacment; }
  ...
  uint64_t nbFrames() const { return _nbFrames; }

  int frame() const { return _lastDecodedPos; }

  bool isOpen() const { return _isOpen; }

  unsigned char *data() { return &_data[0]; }

  std::string codecName() const {
    if (!_videoCodec)
      return "";
    return (_videoCodec->long_name) + std::string(" (") +
           std::string(_videoCodec->name) + std::string(", ") +
           boost::lexical_cast<std::string>(static_cast<int>(_videoCodec->id)) +
           std::string(")");
  }
  ...
  std::string formatName() const {
    if (!_format)
      return "";
    return (_format->long_name) + std::string(" (") +
           std::string(_format->name) + std::string(")");
  }
  ...
  //	{
  //		if( !_videoCodec || !_avctxOptions ||_videoCodec->type <= 0 ||
  //!_avctxOptions[_videoCodec->type] )
  //			return PIX_FMT_NONE;
