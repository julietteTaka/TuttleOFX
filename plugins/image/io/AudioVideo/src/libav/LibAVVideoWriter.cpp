...
LibAVVideoWriter::LibAVVideoWriter()
    : LibAV(), _avFormatOptions(NULL), _stream(NULL), _avVideoOptions(NULL),
      _avAudioOptions(NULL), _sws_context(NULL), _videoCodec(NULL),
      _audioCodec(NULL), _ofmt(NULL), _statusCode(eWriterStatusIgnoreFinish),
      _filename(""), _width(0), _height(0), _aspectRatio(1),
      _out_pixelFormat(PIX_FMT_YUV420P), _fps(25.0f), _formatName(""),
      _videoCodecName(""), _audioCodecName("") {
    ...
    if (fmt->video_codec != AV_CODEC_ID_NONE) {
      if (fmt->long_name) {
        _formatsLongNames.push_back(std::string(fmt->long_name) +
                                    std::string(" (") + std::string(fmt->name) +
                                    std::string(")"));
...
int LibAVVideoWriter::start(const std::map<std::string, std::string> &metas) {
  if (!_avFormatOptions) {
    ...
    if (!_formatName.empty()) {
      ...
      if (!_ofmt) {
        TUTTLE_LOG_ERROR("avWriter: Requested output format "
                         << _formatName << " is not a suitable output format");
        return false;
      }
    } else {
      ...
      if (!_ofmt) {
        TUTTLE_LOG_ERROR(
            "avWriter: Unable to find a suitable output format for "
            << filename);
        return false;
      }
    }
    ...
    if (_avFormatOptions->oformat->priv_data_size > 0) {
      _avFormatOptions->priv_data =
          av_mallocz(_avFormatOptions->oformat->priv_data_size);
      if (!_avFormatOptions->priv_data) {
        TUTTLE_LOG_ERROR("avWriter: format context allocation failed");
        return false;
      }
      if (_avFormatOptions->oformat->priv_class) {
        *(const AVClass **)_avFormatOptions->priv_data =
            _avFormatOptions->oformat->priv_class;
    ...
    if (filename)
      av_strlcpy(_avFormatOptions->filename, filename,
    ...
    BOOST_FOREACH(const metaPair meta, metas) {
      av_dict_set(&_avFormatOptions->metadata, meta.first.c_str(),
                  meta.second.c_str(), 0);
    }

    TUTTLE_LOG_TRACE("avWriter: " << std::string(_ofmt->name)
                                  << " format selected");
  }
  ...
  if (!_stream) {
    ...
    TUTTLE_LOG_TRACE("avWriter: " << std::string(_videoCodec->name)
                                  << " codec selected");
    ...
    // TUTTLE_LOG_VAR2( TUTTLE_INFO, _stream->codec->time_base.num,
...
bool LibAVVideoWriter::finishInit() {
  ...
  if (!pixfmt_allowed) {
    ...
    TUTTLE_LOG_ERROR("avWriter: pixel format "
                     << av_get_pix_fmt_name(_out_pixelFormat)
                     << " not available in codec");
    _out_pixelFormat = _videoCodec->pix_fmts[0];
    TUTTLE_LOG_ERROR("avWriter: auto-selecting "
                     << av_get_pix_fmt_name(_out_pixelFormat));
  }
  ...
  if (error) {
    ...
    BOOST_THROW_EXCEPTION(exception::Format()
                          << exception::user(
                                 "avWriter: unable to write header."));
  }
  return true;
}
...
int LibAVVideoWriter::execute(boost::uint8_t *const in_buffer,
                              const int in_width, const int in_height,
                              const PixelFormat in_pixelFormat) {
  ...
  avpicture_fill((AVPicture *)in_frame, in_buffer, in_pixelFormat, in_width,
                 in_height);

  AVFrame *out_frame = avcodec_alloc_frame();
  avcodec_get_frame_defaults(out_frame);
  int out_picSize =
      avpicture_get_size(_out_pixelFormat, getWidth(), getHeight());
  boost::uint8_t *out_buffer = (boost::uint8_t *)av_malloc(out_picSize);
  avpicture_fill((AVPicture *)out_frame, out_buffer, _out_pixelFormat,
                 getWidth(), getHeight());

  _sws_context = sws_getCachedContext(
      _sws_context, in_width, in_height, in_pixelFormat, getWidth(),
      getHeight(), _out_pixelFormat, SWS_BICUBIC, NULL, NULL, NULL);

  TUTTLE_TLOG(TUTTLE_TRACE, "avWriter: input format: "
                                << av_get_pix_fmt_name(in_pixelFormat)
                                << " -> output format: "
                                << av_get_pix_fmt_name(_out_pixelFormat));

  if (!_sws_context) {
    BOOST_THROW_EXCEPTION(exception::Failed()
                          << exception::user() + "libav-conversion failed (" +
                                 in_pixelFormat + "->" + _out_pixelFormat +
                                 ").");
  }
  const int error =
      sws_scale(_sws_context, in_frame->data, in_frame->linesize, 0,
                getHeight(), out_frame->data, out_frame->linesize);
  if (error < 0) {
    BOOST_THROW_EXCEPTION(exception::Failed()
                          << exception::user() + "libav-conversion failed (" +
                                 in_pixelFormat + "->" + _out_pixelFormat +
  ...
  if ((_avFormatOptions->oformat->flags & AVFMT_RAWPICTURE) &&
    ...
    pkt.pts = av_rescale_q(out_frame->pts, _stream->codec->time_base,
                           _stream->time_base);
    ...
    if (ret < 0) {
      BOOST_THROW_EXCEPTION(exception::File()
                            << exception::user(
                                   "avWriter: error writing packet to file"));
    }
  } else {
    AVPacket pkt;
    _hasFrame = 0;
    ...
    if ((_stream->codec->coded_frame) &&
        (_stream->codec->coded_frame->pts !=
         static_cast<boost::int64_t>(AV_NOPTS_VALUE))) {
      pkt.pts = av_rescale_q(_stream->codec->coded_frame->pts,
  ...
  if (ret < 0) {
    BOOST_THROW_EXCEPTION(exception::File()
                          << exception::user(
                                 "avWriter: error writing frame to file."));
  }
