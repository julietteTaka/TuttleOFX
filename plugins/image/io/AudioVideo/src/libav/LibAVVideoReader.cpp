...
LibAVVideoReader::LibAVVideoReader()
    : _avFormatOptions(NULL), _stream(NULL), _format(NULL), _avFrame(NULL),
      _videoCodec(NULL), _sws_context(NULL), _fpsNum(0), _fpsDen(0),
      _currVideoIdx(-1), _nbFrames(0), _width(0), _height(0), _aspect(1.0),
      _offsetTime(true), _lastSearchPos(-1), _lastDecodedPos(-1),
      _lastDecodedFrame(-1), _isOpen(false) {
...
bool LibAVVideoReader::open(const std::string &filename) {
  ...
  int error =
      av_open_input_file(&_avFormatOptions, filename.c_str(), _format, 0, 0);
#else
  int error =
      avformat_open_input(&_avFormatOptions, filename.c_str(), NULL, NULL);
#endif
  if (error < 0) {
  ...
  if (!strcmp(codecContext->codec->name, "mjpeg") ||
      !strcmp(codecContext->codec->name, "dvvideo")) {
    std::cerr << "avReader: [WARNING] codec is using a specific offsetTime"
              << codecContext->codec->name << std::endl;
    _offsetTime = false;
  }
  ...
  /*
      ...
      std::cout << "MetaData::FILE_CREATION_TIME: " <<
     double(_context->timestamp ) << std::endl;
      std::cout << "libav/num_streams: " << _context->nb_streams << std::endl;
      std::cout << "MetaData::FRAME_RATE: " << fps( ) << std::endl;
      std::cout << "libav/codec/codecName: " << codecContext->codec->name <<
     std::endl;
...
bool LibAVVideoReader::read(const int frame) {
  ...
  if (frameNumber != frame) {
    std::cerr << "Read outside the video range (time:" << frame
              << ", video size:" << _nbFrames << std::endl;
  }
  ...
  while (error >= 0 && !hasPicture) {
    ...
    if (_videoIdx.size() && _currVideoIdx != -1 &&
        _pkt.stream_index == _videoIdx[_currVideoIdx]) {
...
bool LibAVVideoReader::setupStreamInfo() {
  ...
  for (std::size_t i = 0; i < _avFormatOptions->nb_streams; ++i) {
    ...
    if (codecContext->codec_id == AV_CODEC_ID_NONE) {
      std::cerr
          << "avReader: Can't find decoder codec_id: CODEC_ID_NONE codecType:"
          << codecType_toString(codecContext->codec_type) << std::endl;
      continue;
    }
    if (avcodec_find_decoder(codecContext->codec_id) == NULL) {
      std::cerr << "avReader: Can't find decoder codec_id: "
                << codecContext->codec_id
                << " codecType:" << codecType_toString(codecContext->codec_type)
                << std::endl;
      continue;
    }
  ...
  if (stream->codec->coded_frame &&
      stream->codec->coded_frame->interlaced_frame) {
  ...
  if (stream->avg_frame_rate.num != 0 && stream->avg_frame_rate.den != 0) {
    TUTTLE_TLOG(TUTTLE_INFO, "fps " << stream->avg_frame_rate.num << " / "
  ...
  if (_avFormatOptions->duration > 0) {
    _nbFrames = boost::numeric_cast<boost::uint64_t>(
        (fps() * (double)_avFormatOptions->duration / (double)AV_TIME_BASE));
  } else {
    _nbFrames = 1 << 29;
  }
  ...
  if (!_nbFrames) {
    ...
    while (stream && av_read_frame(_avFormatOptions, &_pkt) >= 0) {
      boost::uint64_t currPts = boost::numeric_cast<boost::uint64_t>(
          av_q2d(getVideoStream()->time_base) * (_pkt.pts - firstPts) * fps());
      if (currPts > maxPts)
        maxPts = currPts;
    }
  ...
  if (stream->nb_frames) {
    if ((int64_t)_nbFrames != stream->nb_frames) {
      std::cerr << "Warning: calculated number of frames (" << _nbFrames
                << ") does not match container information ("
...
void LibAVVideoReader::openVideoCodec() {
  ...
  if (stream) {
    ...
    if (_videoCodec == NULL ||
        avcodec_open2(codecContext, _videoCodec, NULL) < 0) {
      _currVideoIdx = -1;
    }
  } else {
    _currVideoIdx = -1;
...
boost::int64_t LibAVVideoReader::getTimeStamp(int pos) const {
  boost::int64_t timestamp =
      boost::numeric_cast<boost::int64_t>(((double)pos / fps()) * AV_TIME_BASE);
...
bool LibAVVideoReader::decodeImage(const int frame) {
...
#else
    avcodec_decode_video(codecContext, _avFrame, &hasPicture, _pkt.data,
                         _pkt.size);
#endif
  } else if (_offsetTime) {
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(52, 21, 0)
...
#else
    avcodec_decode_video(codecContext, _avFrame, &curSearch, _pkt.data,
                         _pkt.size);
#endif
  }
  ...
  _sws_context = sws_getCachedContext(
      _sws_context, width(), height(), in_pixelFormat, width(), height(),
      out_pixelFormat, SWS_BICUBIC, NULL, NULL, NULL);

  if (!_sws_context) {
    std::cerr << "avReader: libav-conversion failed (" << in_pixelFormat << "->"
              << out_pixelFormat << ")" << std::endl;
    return false;
  }
  int error = sws_scale(_sws_context, _avFrame->data, _avFrame->linesize, 0,
