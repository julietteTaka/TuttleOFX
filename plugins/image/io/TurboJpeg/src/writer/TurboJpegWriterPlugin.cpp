...
TurboJpegWriterProcessParams
TurboJpegWriterPlugin::getProcessParams(const OfxTime time) {
  ...
  params.optimization =
      static_cast<ETurboJpegOptimization>(_paramOptimization->getValue());
  params.subsampling =
      static_cast<ETurboJpegSubsampling>(_paramSubsampling->getValue());
  return params;
}
