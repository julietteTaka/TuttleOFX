...
template <class View>
TurboJpegWriterProcess<View>::TurboJpegWriterProcess(
    TurboJpegWriterPlugin &effect)
 ...
 */
template <class View>
void TurboJpegWriterProcess<View>::multiThreadProcessImages(
...
template <class View> void TurboJpegWriterProcess<View>::writeImage(View &src) {
  using namespace boost::gil;
  ...
  unsigned char *data =
      (unsigned char *)boost::gil::interleaved_view_get_raw_data(tmpVw);

  // int ret =
  tjCompress2(jpeghandle, data, width, pitch, height, ps, &jpegBuf, &jpegSize,
              subsampling, _params.quality, flags);
