...
template <class View>
TurboJpegReaderProcess<View>::TurboJpegReaderProcess(
    TurboJpegReaderPlugin &effect)
 ...
 */
template <class View>
void TurboJpegReaderProcess<View>::multiThreadProcessImages(
    const OfxRectI &procWindowRoW) {
  BOOST_ASSERT(procWindowRoW == this->_dstPixelRod);
  readImage(this->_dstView);
}

template <class View> void TurboJpegReaderProcess<View>::readImage(View &dst) {
  FILE *file = NULL;
  ...
  ret = tjDecompressHeader2(jpeghandle, jpegbuf, jpgbufsize, &width, &height,
                            &jpegsubsamp);
  if (ret != 0) {
  ...
  ret = tjDecompress2(jpeghandle, jpegbuf, jpgbufsize, rgbbuf, width, 0, height,
                      ps, flags);
  if (ret != 0) {
  ...
  rgb8_view_t bufferView = interleaved_view(
      width, height, (typename rgb8_view_t::value_type *)(rgbbuf),
