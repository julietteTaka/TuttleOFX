...
TurboJpegReaderProcessParams
TurboJpegReaderPlugin::getProcessParams(const OfxTime time) const {
  TurboJpegReaderProcessParams params;
  params.filepath = getAbsoluteFilenameAt(time);
  params.optimization =
      static_cast<ETurboJpegOptimization>(_optimization->getValue());
  params.fastUpsampling = _fastUpsampling->getValue();
  return params;
}

void TurboJpegReaderPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                                         const std::string &paramName) {
  ReaderPlugin::changedParam(args, paramName);
}

bool TurboJpegReaderPlugin::getRegionOfDefinition(
  ...
  try {
    ...
    if (file == NULL) {
      BOOST_THROW_EXCEPTION(exception::File()
                            ...
                            << exception::filename(
                                   getAbsoluteFilenameAt(args.time)));
    }
    ...
    int ret = tjDecompressHeader2(jpeghandle, jpegbuf, jpgbufsize, &width,
                                  &height, &jpegsubsamp);
...
void TurboJpegReaderPlugin::getClipPreferences(
