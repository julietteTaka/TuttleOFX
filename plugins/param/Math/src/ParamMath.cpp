...
template <class ParamType> class ParamMathPlugin : public OFX::ParamEffect {
  enum EOperation {
    eMultiply,
    eDivide,
    eAddition,
...
void ParamMathPluginFactory::describe(OFX::ImageEffectDescriptor &desc) {
  desc.setLabels("TuttleMathParam", "MathParam", "Math Param");
...
template <class ParamType>
OFX::ParamEffect *
ParamMathPluginFactory::createInstance(OfxImageEffectHandle handle) {
...
void getPluginIDs(OFX::PluginFactoryArray &ids) {
  static tuttle::ParamMathPluginFactory<DoubleParam> d(
      "tuttle.parammath.double", 0, 0);
  static tuttle::ParamMathPluginFactory<IntParam> i("tuttle.parammath.int", 0,
                                                    0);
  ids.push_back(&d);
  ids.push_back(&i);
}
