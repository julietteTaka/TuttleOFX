...
struct without_alpha_channel_t
{
        ...
        typedef details::without_alpha_channel_impl_t<Colorspace, ChannelType>
Do;
...
template <typename Pixel>
Pixel pixel_standard_deviation(const Pixel &v_sum, const Pixel &v_sum_p2,
                               const std::size_t nb) {
 ...
 */
template <typename T>
T kurtosis(const T v_mean, const T v_standard_deviation, const T v_sum,
           const T v_sum_p2, const T v_sum_p3, const T v_sum_p4,
           const std::size_t nb) {
  using namespace boost::units;
  return ((v_sum_p4 - 4.0 * v_mean * v_sum_p3 +
           6.0 * pow<2>(v_mean) * v_sum_p2) /
              nb -
          3.0 * pow<4>(v_mean)) /
...
template <typename Pixel>
Pixel pixel_kurtosis(const Pixel &v_mean, const Pixel &v_standard_deviation,
                     const Pixel &v_sum, const Pixel &v_sum_p2,
                     const Pixel &v_sum_p3, const Pixel &v_sum_p4,
                     const std::size_t nb) {
  using namespace boost::gil;
  Pixel res;
  for (int i = 0; i < num_channels<Pixel>::type::value; ++i) {
    res[i] = kurtosis(v_mean[i], v_standard_deviation[i], v_sum[i], v_sum_p2[i],
                      v_sum_p3[i], v_sum_p4[i], nb);
  }
  return res;
}
 ...
 */
template <typename T>
T skewness(const T v_mean, const T v_standard_deviation, const T v_sum,
           const T v_sum_p2, const T v_sum_p3, const std::size_t nb) {
  using namespace boost::units;
  return ((v_sum_p3 - 3.0 * v_mean * v_sum_p2) / nb + 2.0 * pow<3>(v_mean)) /
         pow<3>(v_standard_deviation);
}

template <typename Pixel>
Pixel pixel_skewness(const Pixel &v_mean, const Pixel &v_standard_deviation,
                     const Pixel &v_sum, const Pixel &v_sum_p2,
                     const Pixel &v_sum_p3, const std::size_t nb) {
  using namespace boost::gil;
  Pixel res;
  for (int i = 0; i < num_channels<Pixel>::type::value; ++i) {
    res[i] = skewness(v_mean[i], v_standard_deviation[i], v_sum[i], v_sum_p2[i],
                      v_sum_p3[i], nb);
  }
  return res;
}

template <class Pixel> struct OutputParams {
  OutputParams() {
...
struct ComputeOutputParams {
  ...
  typedef boost::gil::pixel<typename boost::gil::channel_type<View>::type,
                            boost::gil::layout<boost::gil::gray_t> >
  ...
  Output operator()(const View &image, ImageStatisticsPlugin &plugin) {
    ...
    for (int y = 0; y < image.height(); ++y) {
      ...
      for (int x = 0; x < image.width(); ++x, ++src_it) {
        ...
        pixel_assigns_t<CPixel, CPixel>()(pixel_pow_t<CPixel, 2>()(pix),
                                          pix_p2); // pix_p2 = pow<2>( pix );
        pixel_assigns_t<CPixel, CPixel>()(
            pixel_multiplies_t<CPixel, CPixel, CPixel>()(pix, pix_p2),
            pix_p3); // pix_p3 = pix * pix_p2;
        pixel_assigns_t<CPixel, CPixel>()(
            pixel_multiplies_t<CPixel, CPixel, CPixel>()(pix_p2, pix_p2),
            pix_p4); // pix_p4 = pix_p2 * pix_p2;

        pixel_plus_assign_t<CPixel, CPixel>()(pix, sum); // sum += pix;
        pixel_plus_assign_t<CPixel, CPixel>()(pix_p2,
                                              sum_p2); // sum_p2 += pix_p2;
        pixel_plus_assign_t<CPixel, CPixel>()(pix_p3,
                                              sum_p3); // sum_p3 += pix_p3;
        pixel_plus_assign_t<CPixel, CPixel>()(pix_p4,
                                              sum_p4); // sum_p4 += pix_p4;
        ...
        if (get_color(grayCurrentPixel, gray_color_t()) <
            get_color(luminosityMinGray, gray_color_t())) {
          luminosityMin = *src_it;
          luminosityMinGray = grayCurrentPixel;
        }
        // search max luminosity
        if (get_color(grayCurrentPixel, gray_color_t()) >
            get_color(luminosityMaxGray, gray_color_t())) {
    ...
    for (int y = 0; y < image.height(); ++y) {
      ...
      for (int x = 0; x < image.width(); ++x, ++src_it) {
        ...
        pix_diff = pixel_minus_t<CPixel, CPixel, CPixel>()(
            pix, output._average); // pix_diff = (pix - mean)
        pix_diff2 = pixel_multiplies_t<CPixel, CPixel, CPixel>()(
            pix_diff, pix_diff); // pix_diff2 = (x - mean)*(x - mean)

        pixel_plus_assign_t<CPixel, CPixel>()(
            pix_diff2, varianceSum); // varianceSum += pix_diff2;
      }
    }

    CPixel varianceSquare =
        pixel_divides_scalar_t<CPixel, double>()(varianceSum, nbPixels);
    output._variance = pixel_sqrt_t<CPixel, Pixel>()(varianceSquare);
    output._kurtosis = pixel_kurtosis(output._average, stdDeriv, sum, sum_p2,
                                      sum_p3, sum_p4, nbPixels);
    output._skewness = pixel_skewness(output._average, stdDeriv, sum, sum_p2,
                                      sum_p3, nbPixels);
...
void setOutputParams(const OutputParamsRGBA &outputParamsRGBA,
                     const OutputParamsHSL &outputParamsHSL, const OfxTime time,
                     ImageStatisticsPlugin &plugin) {
  setRGBAParamValuesAtTime(*plugin._paramOutputAverage, time,
                           outputParamsRGBA._average);
  setRGBAParamValuesAtTime(*plugin._paramOutputVariance, time,
                           outputParamsRGBA._variance);
  //	TUTTLE_LOG_VAR4( TUTTLE_INFO, outputParamsRGBA._average[0],
  // outputParamsRGBA._average[1], outputParamsRGBA._average[2],
  // outputParamsRGBA._average[3] );
  setRGBAParamValuesAtTime(*plugin._paramOutputChannelMin, time,
                           outputParamsRGBA._channelMin);
  //	TUTTLE_LOG_VAR4( TUTTLE_INFO, outputParamsRGBA._channelMin[0],
  // outputParamsRGBA._channelMin[1], outputParamsRGBA._channelMin[2],
  // outputParamsRGBA._channelMin[3] );
  setRGBAParamValuesAtTime(*plugin._paramOutputChannelMax, time,
                           outputParamsRGBA._channelMax);
  setRGBAParamValuesAtTime(*plugin._paramOutputLuminosityMin, time,
                           outputParamsRGBA._luminosityMin);
  setRGBAParamValuesAtTime(*plugin._paramOutputLuminosityMax, time,
                           outputParamsRGBA._luminosityMax);
  setRGBAParamValuesAtTime(*plugin._paramOutputKurtosis, time,
                           outputParamsRGBA._kurtosis);
  setRGBAParamValuesAtTime(*plugin._paramOutputSkewness, time,
                           outputParamsRGBA._skewness);

  set012ParamValuesAtTime(plugin._paramOutputAverageHSL, time,
                          outputParamsHSL._average);
  //	TUTTLE_LOG_VAR4( TUTTLE_INFO, outputParamsHSL._average[0],
  // outputParamsHSL._average[1], outputParamsHSL._average[2],
  // outputParamsHSL._average[3] );
  set012ParamValuesAtTime(*plugin._paramOutputChannelMinHSL, time,
                          outputParamsHSL._channelMin);
  //	TUTTLE_LOG_VAR4( TUTTLE_INFO, outputParamsHSL._channelMin[0],
  // outputParamsHSL._channelMin[1], outputParamsHSL._channelMin[2],
  // outputParamsHSL._channelMin[3] );
  set012ParamValuesAtTime(*plugin._paramOutputChannelMaxHSL, time,
                          outputParamsHSL._channelMax);
  set012ParamValuesAtTime(*plugin._paramOutputLuminosityMinHSL, time,
                          outputParamsHSL._luminosityMin);
  set012ParamValuesAtTime(*plugin._paramOutputLuminosityMaxHSL, time,
                          outputParamsHSL._luminosityMax);
  set012ParamValuesAtTime(*plugin._paramOutputKurtosisHSL, time,
                          outputParamsHSL._kurtosis);
  set012ParamValuesAtTime(*plugin._paramOutputSkewnessHSL, time,
                          outputParamsHSL._skewness);
}

template <class View>
ImageStatisticsProcess<View>::ImageStatisticsProcess(
    ImageStatisticsPlugin &instance)
...
void ImageStatisticsProcess<View>::setup(const OFX::RenderArguments &args) {
  ...
  View image = subimage_view(this->_srcView, _processParams._rect.x1,
  ...
  typedef ComputeOutputParams<typename HSLConverter::type, boost::gil::bits64f>
  ComputeHSL;
  typename ComputeHSL::Output outputHSL =
      ComputeHSL()(color_converted_view<HSLPixel>(image), this->_plugin);
 ...
 */
template <class View>
void ImageStatisticsProcess<View>::multiThreadProcessImages(
  ...
  if (_processParams._chooseOutput == eParamChooseOutputSource) {
    for (int y = procWindowOutput.y1; y < procWindowOutput.y2; ++y) {
      typename View::x_iterator src_it =
          this->_srcView.x_at(procWindowOutput.x1, y);
      typename View::x_iterator dst_it =
          this->_dstView.x_at(procWindowOutput.x1, y);
      for (int x = procWindowOutput.x1; x < procWindowOutput.x2;
           ++x, ++src_it, ++dst_it) {
        (*dst_it) = (*src_it);
      }
