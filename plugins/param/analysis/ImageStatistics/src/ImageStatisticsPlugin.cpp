...
ImageStatisticsPlugin::ImageStatisticsPlugin(OfxImageEffectHandle handle)
  ...
  _paramOutputLuminosityMinHSL =
      fetchDouble3DParam(kParamOutputLuminosityMinHSL);
  _paramOutputLuminosityMaxHSL =
      fetchDouble3DParam(kParamOutputLuminosityMaxHSL);
...
ImageStatisticsProcessParams
ImageStatisticsPlugin::getProcessParams(const OfxRectI &srcRod) const {
  ...
  params._rect.x2 =
      boost::numeric_cast<int>(std::ceil(rectCenter.x + rectSize.x));
  params._rect.y2 =
      boost::numeric_cast<int>(std::ceil(rectCenter.y + rectSize.y));
  params._rect = rectanglesIntersection(params._rect, srcRod);
  params._chooseOutput =
      static_cast<EParamChooseOutput>(_paramChooseOutput->getValue());

  return params;
}

void ImageStatisticsPlugin::getRegionsOfInterest(
    const OFX::RegionsOfInterestArguments &args,
    OFX::RegionOfInterestSetter &rois) {
  OfxRectI srcRealRoi = getProcessParams(_clipSrc->getPixelRod(args.time))
                            ._rect; // we need the selected rectangle
...
void ImageStatisticsPlugin::render(const OFX::RenderArguments &args) {
  ...
  case OFX::ePixelComponentRGBA: {
    ...
    case OFX::eBitDepthFloat: {
      doGilRender<ImageStatisticsProcess, false, boost::gil::rgba_layout_t,
                  boost::gil::bits32f>(*this, args);
      return;
    }
...
void ImageStatisticsPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamCoordinateSystem) {
    ...
    default:
      BOOST_THROW_EXCEPTION(exception::Value()
                            << exception::user(
                                   "Unrecognized coordinate system option."));
    }
