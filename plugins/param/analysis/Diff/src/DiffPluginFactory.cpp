...
void DiffPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *diffFunction =
      desc.defineChoiceParam(kMeasureFunction);
  assert(diffFunction);
  ...
  OFX::RGBAParamDescriptor *outputQualityMesure =
      desc.defineRGBAParam(kOutputQualityMesure);
