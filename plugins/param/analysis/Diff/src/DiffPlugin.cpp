...
DiffPlugin::DiffPlugin(OfxImageEffectHandle handle) : OFX::ImageEffect(handle) {
...
DiffProcessParams DiffPlugin::getProcessParams() const {
  ...
  params.measureFunction =
      static_cast<EMeasureFunction>(_measureFunction->getValue());

  return params;
}

void DiffPlugin::changedParam(const OFX::InstanceChangedArgs &args,
                              const std::string &paramName) {}

bool
DiffPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args,
                                  OfxRectD &rod) {
  const OfxRectD irod =
      rectanglesIntersection(_clipSrcA->getCanonicalRod(args.time),
