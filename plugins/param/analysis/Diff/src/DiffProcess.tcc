...
template <class View>
void
DiffProcess<View>::multiThreadProcessImages(const OfxRectI &procWindowRoW) {
  using namespace boost::gil;
  OfxPointI procWindowSize = { procWindowRoW.x2 - procWindowRoW.x1,
                               procWindowRoW.y2 - procWindowRoW.y1 };
  View srcViewA = subimage_view(
      this->_srcViewA, procWindowRoW.x1 - _srcPixelRodA.x1,
      procWindowRoW.y1 - _srcPixelRodA.y1, procWindowSize.x, procWindowSize.y);
  View srcViewB = subimage_view(
      this->_srcViewB, procWindowRoW.x1 - _srcPixelRodB.x1,
      procWindowRoW.y1 - _srcPixelRodB.y1, procWindowSize.x, procWindowSize.y);
  View dstView =
      subimage_view(this->_dstView, procWindowRoW.x1 - this->_dstPixelRod.x1,
                    procWindowRoW.y1 - this->_dstPixelRod.y1, procWindowSize.x,
                    procWindowSize.y);
  ...
  _plugin._qualityMesure->setValueAtTime(
      this->_renderArgs.time, get_color(paramRgbaValue, red_t()),
...
DiffProcess<View>::psnr(const SView &v1, const SView &v2, const SView &dst) {
  ...
  typedef typename boost::gil::channel_type<typename SView::value_type>::type
  SValueType;
...
DiffProcess<View>::mse(const SView &v1, const SView &v2, const SView &dst,
  ...
  for (typename SView::y_coord_t y = 0; y < h; ++y) {
    ...
    for (typename SView::x_coord_t x = 0; x < w; ++x) {
      for (int i = 0; i < boost::gil::num_channels<Pixel32F>::type::value;
           ++i) {
