...
/*
template<class View>
View getFullView( View tileView, const Rect<std::ssize_t>& bounds, const
Rect<std::ssize_t>& rod )
{
        ...
        return subimage_view( tileView, rod.x1 - bounds.x1, rod.y1 - bounds.y1,
...
struct image_from_view {
  ...
  typedef typename boost::gil::image<
      value_type, boost::gil::is_planar<View>::value, Alloc> type;
};
// typedef typename view_type_from_pixel<OutPixelType,
// boost::gil::is_planar<View>::value >::type OutView;

template <class View> struct layout_type {
...
struct floating_channel_type_t {
  typedef typename boost::mpl::if_<boost::is_floating_point<T>, T, F>::type
  type;
};

template <class View> struct floating_pixel_from_view {
  ...
  typedef boost::gil::pixel<ChannelFloat, typename layout_type<View>::type>
  type;
};

template <class View> inline float max_value() {
  using namespace boost::gil;
  return channel_traits<typename channel_type<View>::type>::max_value();
}

template <class View> inline float domain_max_value() {
  using namespace boost::gil;
