...
template <typename T>
T Rect<T>::*const Rect<T>::mem_array[Rect<T>::num_dimensions] = {
  &Rect<T>::x1, &Rect<T>::y1, &Rect<T>::x2, &Rect<T>::y2
};
 ...
 */
template <typename T, class View> Rect<T> getBounds(const View &v) {
...
template <class Rect>
inline Rect translateRegion(const Rect &windowRoW, const std::ptrdiff_t x,
