...
void copy_channel_if_exist(const View &src, const View &dst) {
  using namespace boost::gil;
  typedef typename contains_color<typename View::value_type, Channel>::type
  hasChannel;
  detail_copy_channel::copy_channel_if_exist<Channel, View>(src, dst,
                                                            hasChannel());
}
...
void copy_pixel_channel_if_exist(const Pixel &src, const Pixel &dst) {
  ...
  detail_copy_channel::copy_pixel_channel_if_exist<Channel, Pixel>(
      src, dst, hasChannel());
}
}
