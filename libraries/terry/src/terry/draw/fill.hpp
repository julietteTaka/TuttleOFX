...
template <class View>
GIL_FORCEINLINE void fill_pixels(View &dstView,
...
GIL_FORCEINLINE void fill_pixels(View &dstView,
  ...
  View dst = subimage_view(dstView, window.x1, window.y1, window.x2 - window.x1,
                           window.y2 - window.y1);
  boost::gil::fill_pixels(dst, pixelValue);
}

/**
 * @brief fill a range of pixels (on the same line or with an 1d_traversable
 *image).
 *
 * @param[out] dstBegin Begining of the region to fill. The content is filled
 *but the original iterator is not modified (not a reference parameter).
 * @param[in] dstEnd End of the region to fill.
 * @param[in] pixelValue Pixel value used to fill.
 */
template <class DIterator, class DPixel>
GIL_FORCEINLINE void fill_pixels_range(DIterator dstBegin,
