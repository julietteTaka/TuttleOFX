...
struct make_kerning {
  ...
  template <typename glyph_t> int operator()(const glyph_t &glyph) {
    ...
    FT_Vector delta;
    FT_Get_Kerning(glyph.face, left_glyph, right_glyph, FT_KERNING_DEFAULT,
                   &delta);
...
template <typename view_t> class render_glyph {
public:
...
public:
  render_glyph(const view_t &outView, const Pixel &color,
               const double letterSpacing)
      : _outView(outView), _color(color), _letterSpacing(letterSpacing),
        _roi(0, 0, outView.width(), outView.height()), _x(0) {}

  render_glyph(const view_t &outView, const Pixel &color,
               const double letterSpacing, const Rect<std::ptrdiff_t> roi)
      : _outView(outView), _color(color), _letterSpacing(letterSpacing),
        _roi(roi), _x(0) {}
  ...
  void operator()(const glyph_t &glyph, int kerning = 0) {
    ...
    const int y =
        _outView.height() - (glyph.face->glyph->metrics.horiBearingY >> 6);
    ...
    if (glyphRegionSize.x != 0 && glyphRegionSize.y != 0) {
      const rect_t glyphLocalRoi =
          translateRegion(glyphRoi, -glyphRod.x1, -glyphRod.y1);
      ...
      gray8c_view_t glyphView = interleaved_view(
          width, height, reinterpret_cast<gray8_pixel_t *>(slot->bitmap.buffer),
      ...
      copy_and_convert_alpha_blended_pixels(
          color_converted_view<gray32f_pixel_t>(glyphViewRoi), _color,
          outViewRoi);
    }
