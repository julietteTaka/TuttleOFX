...
struct make_special_interval {
  ...
  make_special_interval(int width, int size)
      : size(size), r((width - 1) % size), adj((width - 1) / size), pos(0) {}
...
struct make_alpha_blend {
  ...
  template <typename T> void operator()(T &dst, const T src) {
...
inline void copy_and_convert_alpha_blended_pixels(
    const GlyphView &glyphView, const typename View::value_type &glyphColor,
    const View &dstView) {
  using namespace boost::gil;
  // typedef typename GlyphView::value_type GlyphPixel;
  typedef typename View::value_type Pixel;

  // typedef pixel<bits32f, layout<typename color_space_type<GlyphView>::type> >
  // GlyphPixel32f;
  // typedef pixel<typename channel_type<view_t>::type, layout<gray_t> >
  // PixelGray;

  BOOST_STATIC_ASSERT((boost::is_same<
      typename color_space_type<GlyphView>::type, gray_t>::value));
  // BOOST_STATIC_ASSERT(( boost::is_same<typename
  // channel_type<GlyphView>::type, bits32f>::value ));
...
inline void wuline(const view_t &view, const value_type &pixel, int X0, int Y0,
                   int X1, int Y1, int NumLevels, int IntensityBits) {
...
template <typename view_t> struct draw_line {
  ...
  draw_line(const view_t &view, const value_type &pixel, int NumLevels = 256,
            int IntensityBits = 8)
      : view(view), pixel(pixel), NumLevels(NumLevels),
        IntensityBits(IntensityBits) {}

  template <typename point_t> void operator()(point_t pt0, point_t pt1) {
