...
template <typename ChannelValue> struct channel_base_type {
  typedef ChannelValue type;
};

template <typename ChannelValue, typename MinV, typename MaxV>
struct channel_base_type<
    boost::gil::scoped_channel_value<ChannelValue, MinV, MaxV> > {
  typedef ChannelValue type;
};

template <typename ChannelValue> struct is_scoped_channel {
...
template <class Pixel, class Channel,
          class HasChannel =
              typename boost::gil::contains_color<Pixel, Channel>::type>
