...
static const bool lutthin1[512] = {
...
static const bool lutthin2[512] = {
  false, false, false, false, false, false, false, false, false, false, false,
  false, false, false, false, false, true,  true,  true,  true,  true,  true,
...
template <class SView, class DView = SView> struct pixel_locator_thinning_t {
  ...
  pixel_locator_thinning_t(const SView &src, const bool *lut)
      : _lut(lut), _loc_ref(src.xy_at(0, 0)),
        LT(_loc_ref.cache_location(-1, -1)), CT(_loc_ref.cache_location(0, -1)),
        RT(_loc_ref.cache_location(1, -1)), LC(_loc_ref.cache_location(-1, 0)),
        RC(_loc_ref.cache_location(1, 0)), LB(_loc_ref.cache_location(-1, 1)),
        CB(_loc_ref.cache_location(0, 1)), RB(_loc_ref.cache_location(1, 1)) {
  ...
  DPixel operator()(const SLocator &src) const {
    ...
    std::size_t id = (src[LT] == _sWhite) | ((src[LC] == _sWhite) << 1) |
                     ((src[LB] == _sWhite) << 2) | ((src[CT] == _sWhite) << 3) |
                     ((*src == _sWhite) << 4) | ((src[CB] == _sWhite) << 5) |
                     ((src[RT] == _sWhite) << 6) | ((src[RC] == _sWhite) << 7) |
                     ((src[RB] == _sWhite) << 8);
...
void applyThinning(const SView &srcView, DView &tmpView, DView &dstView) {
  ...
  algorithm::transform_pixels_locator(
      srcView, srcRod, tmpView, getBounds<std::ptrdiff_t>(tmpView), proc1,
      terry::filter::thinning::pixel_locator_thinning_t<SView, DView>(
          srcView, terry::filter::thinning::lutthin1));

  // todo: only fill borders !!
  fill_pixels(dstView, pixelZero);

  algorithm::transform_pixels_locator(
      tmpView, getBounds<std::ptrdiff_t>(tmpView), dstView,
      getBounds<std::ptrdiff_t>(dstView), proc2,
      terry::filter::thinning::pixel_locator_thinning_t<DView, DView>(
          tmpView, terry::filter::thinning::lutthin2));
}
}
}
