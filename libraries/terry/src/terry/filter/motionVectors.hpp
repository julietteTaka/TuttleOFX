...
bool modifyVectors(const View &xVecView, const View &yVecView,
                   const double angle, const double intensity, Progress &p) {
  ...
  typedef typename terry::channel_base_type<
      typename boost::gil::channel_type<View>::type>::type VecChannel;
...
template <typename GView, typename View, typename Point, typename Scalar,
          typename Progress>
bool correlateMotionVectors(GView &xGradientView, GView &yGradientView,
                            View &img, const Point &topleft,
  ...
  correlate_rows<GPixel>(color_converted_view<GPixel>(img), kernel,
                         xGradientView, topleft, boundary_option);
  if (p.progressForward(xGradientView.size()))
    return true;
  correlate_cols<GPixel>(color_converted_view<GPixel>(img), kernel,
                         yGradientView, topleft, boundary_option);
  if (p.progressForward(yGradientView.size()))
    return true;
  return false;
}

template <template <typename> class Alloc, typename GView, typename View,
          typename Point, typename Scalar, typename Progress>
bool correlateMotionVectors(
    GView &xGradientView, GView &yGradientView, View &img, const Point &topleft,
    const terry::filter::kernel_1d<Scalar> &kernel,
    const terry::filter::kernel_1d<Scalar> &kernelSecondary,
    const convolve_boundary_option boundary_option, Progress &p) {
  typedef typename GView::value_type GPixel;
  using namespace boost::gil;
  correlate_rows_cols<GPixel, Alloc>(color_converted_view<GPixel>(img), kernel,
                                     kernelSecondary, xGradientView, topleft,
                                     boundary_option);
  if (p.progressForward(xGradientView.size()))
    return true;
  correlate_rows_cols<GPixel, Alloc>(color_converted_view<GPixel>(img),
                                     kernelSecondary, kernel, yGradientView,
                                     topleft, boundary_option);
  if (p.progressForward(yGradientView.size()))
    return true;
  return false;
}
...
/**
 * @brief Moves the pixels based on the variation of the mask (the derivative:
 * [-1 0 1] kernel)
 */
...
bool motionvectors_resample_pixels(
    const SrcView &srcView, const Rect<std::ssize_t> &srcRod,
    const VecView &xVecView, const VecView &yVecView,
    const Rect<std::ssize_t> &vecRod, const DstView &dstView,
  ...
  typedef typename boost::gil::channel_type<VecView>::type::base_channel_t
  VecChannel;
  typedef typename boost::gil::point2<VecChannel> VecPoint2;
  ...
  for (Coord y = procWindowRoW.y1; y < procWindowRoW.y2; ++y) {
    ...
    typename DstView::x_iterator xit_dst =
        dstView.x_at(shiftProcWinDstRod.x1, yDst);
    typename VecView::x_iterator xit_xVec =
        xVecView.x_at(shiftProcWinVecRod.x1, yVec);
    typename VecView::x_iterator xit_yVec =
        yVecView.x_at(shiftProcWinVecRod.x1, yVec);
    for (Coord x = procWindowRoW.x1; x < procWindowRoW.x2;
      ...
      if (!terry::sampler::sample(sampler, srcView, pos + motion, *xit_dst,
                                  outOfImageProcess)) {
