 ...
 */
template <template <typename> class Alloc, class SView, class TRGBView,
          class TGrayView, class DView>
void canny(const SView &srcView, const TRGBView &tmpSobelView,
           const TGrayView &tmpGrayView, const DView &cannyView,
  ...
  // boost::gil::png_write_view( "data/terry/output_in_terry.png",
  // color_converted_view<rgb8_pixel_t>( srcView ) );

  // boost::timer t;
  sobel<Alloc>(srcView, kth_channel_view<0>(tmpSobelView), // sobel X
               kth_channel_view<1>(tmpSobelView),          // sobel Y
               sobelSize, sobelBoundaryOption);
  ...
  // boost::gil::png_write_view( "data/terry/output_sobel_terry.png",
  // color_converted_view<rgb8_pixel_t>( tmpSobelView ) );

  // t.restart();
  applyLocalMaxima(tmpSobelView, tmpGrayView);
  // std::cout << "localMaxima time: " << t.elapsed() << std::endl;

  // boost::gil::png_write_view( "data/terry/output_localMaxima_terry.png",
  // color_converted_view<rgb8_pixel_t>( tmpGrayView ) );

  // t.restart();
  applyFloodFill<Alloc>(tmpGrayView, cannyView, cannyThresLow, cannyThresUpper);
  // std::cout << "floodFill time: " << t.elapsed() << std::endl;

  // boost::gil::png_write_view( "data/terry/output_floodFill_terry.png",
  // color_converted_view<rgb8_pixel_t>( cannyView ) );
