...
/**
 * @brief fill all pixels respecting the @p condition in a range of pixels (on
 * the same line or with an 1d_traversable image).
 */
template <class SIterator, class DIterator, class DPixel, class Test>
GIL_FORCEINLINE void
fill_pixels_range_if(SIterator srcBegin, const SIterator &srcEnd,
...
template <typename T> struct IsUpper {
  const T _threshold;
  IsUpper(const T &threshold) : _threshold(threshold) {}
  GIL_FORCEINLINE
 ...
 */
template <class SView, class DView> struct FloodElement {
  ...
  SLocator _srcBegin; //< locator on the first pixel of the source image of the
                      //range to propagate
  SLocator _srcEnd; //< like end() in the stl, this is the pixel after the last
                    //pixel of the range
  DLocator _dstBegin;   //< locator on the first pixel of the dest image
  std::ssize_t _xBegin; //< x coordinate of the first pixel
  std::ssize_t _xEnd; //< x coordinate of the pixel after the last pixel of the
                      //range
 ...
 * Fill all pixels respecting the soft condition if connected with a pixel
 *respecting the strong condition.
 *
 ...
 *
 * @remark Implementation is based on standard filling algorithms. So we use
 *ranges by line (x axis),
 * and check connections between these x ranges and possible x ranges in the
 *above or bellow lines.
 */
template <class Connexity, class StrongTest, class SoftTest, class SView,
          class DView, template <class> class Allocator>
void flood_fill(const SView &srcView, const Rect<std::ssize_t> &srcRod,
  ...
  SLocator src_loc =
      srcView.xy_at(procWindow.x1 - srcRod.x1, procWindow.y1 - srcRod.y1);
  DLocator dst_loc =
      dstView.xy_at(procWindow.x1 - dstRod.x1, procWindow.y1 - dstRod.y1);
  ...
  boost::gil::point2<std::ptrdiff_t> endToBegin(
      -(procWindow.x2 - procWindow.x1), 1);
  ...
  for (std::ssize_t y = procWindow.y1; y < procWindow.y2; ++y) {
    {
      ...
      for (std::ssize_t x = procWindow.x1; x < procWindow.x2;
        ...
        if ((*dst_loc)[0] == white[0]) {
          ...
          if (/*inside &&*/ containsStrong) {
            ...
            if (y < rod.y2) {
              //						fill_range_if(
              //srcLastBegin[sLB], src_loc[sRB], dstLastBegin[dLB], white,
              //softTest );
              // fill line bellow for current range if respect softTest
              fill_pixels_range_if(srcLastBegin.x_at(-Connexity::x, 1),
                                   src_loc.x_at(Connexity::x, 1),
                                   dstLastBegin.x_at(-Connexity::x, 1), white,
                                   softTest);
            }
          }
          // re-init values
    ...
    while (!propagation.empty()) {
      ...
      // current line to apply the propagation
      ...
      //
      // example 2:
      // [sssssssssssssss].....[sssss]...[sss]..[ssssssssssssssssssssss] //
      // search in the same direction
      // [fffffffffffffff].....[fffff]...[fff]..[ffffffffffffffffffffff] //
      // current line to apply the propagation
      // [sssssssssssss].[iiiiiiiiiiiiiiiiiiiiiiiiiiii].[ssssssssssssss] //
      // search in the opposite direction
      //                              ^
      ...
      //
      // example 3:
      // .....................[sssss]....[sss]..[ssssssssssssssssssssss] //
      // search in the same direction
      // .....................[fffff]....[fff]..[ffffffffffffffffffffff] //
      // current line to apply the propagation
      // ................[iiiiiiiiiiiiiiiiiiiiiiiiiiii].[ssssssssssssss] //
      // search in the opposite direction
      //                              ^
      //                              |  direction: above
      ...
      // new sub-ranges
      {
        ...
        if (!localPropagation.empty()) {
          ...
          if (localPropagation.front()._xBegin - 1 + Connexity::x <
              iElem._xBegin) {
          ...
          if (localPropagation.back()._xEnd + 1 - Connexity::x > iElem._xEnd) {
            ...
            std::size_t shift =
                rightElem._xBegin - localPropagation.back()._xBegin;
            rightElem._srcBegin =
                localPropagation.back()._srcBegin.xy_at(shift, 0);
            rightElem._dstBegin =
                localPropagation.back()._dstBegin.xy_at(shift, 0);
            rightElem._xEnd = localPropagation.back()._xEnd;
            rightElem._srcEnd = rightElem._srcBegin.xy_at(
                rightElem._xEnd - rightElem._xBegin, 0);
            propagation.push_back(rightElem);
          }

          propagation.insert(propagation.end(), localPropagation.begin(),
                             localPropagation.end());
        }
      }
    }
  }
...
void
flood_fill_bruteForce(const SView &srcView, const Rect<std::ssize_t> &srcRod,
                      DView &dstView, const Rect<std::ssize_t> &dstRod,
                      const Rect<std::ssize_t> &procWindow,
                      const StrongTest &strongTest, const SoftTest &softTest) {
  typedef typename SView::value_type SPixel;
  ...
  boost::gil::point2<int> procWindowSize(procWindow.x2 - procWindow.x1,
                                         procWindow.y2 - procWindow.y1);
  Rect<int> rectLimit = rectangleReduce(procWindow, 1);

  const Point2 nextLine(-procWindowSize.x, 1);
  typename DView::xy_locator dst_loc =
      dstView.xy_at(procWindowOutput.x1, procWindowOutput.y1);
  typename DView::xy_locator src_loc =
      srcView.xy_at(procWindow.x1 - srcRod.x1, procWindow.y1 - srcRod.y1);

  for (int y = procWindow.y1; y < procWindow.y2; ++y) {
    for (int x = procWindow.x1; x < procWindow.x2;
         ++x, ++dst_loc.x(), ++src_loc.x()) {
          ...
          while (!fifo.empty()) {
            ...
            for (int dy = -1; dy < 2; ++dy) {
              for (int dx = -1; dx < 2; ++dx) {
                DIterator pix =
                    dstView.at(p.x + dx - dstRod.x1, p.y + dy - dstRod.y1);
                SIterator spix =
                    srcView.at(p.x + dx - srcRod.x1, p.y + dy - srcRod.y1);
                if (softTest((*spix)[gradMax]) &&
                    (*pix)[flooding] !=
                        boost::gil::channel_traits<DChannel>::max_value()) {
                  (*pix)[flooding] =
                      boost::gil::channel_traits<DChannel>::max_value();
  ...
  //	{
  DView tmp_dst =
      subimage_view(dstView, procWindowOutput.x1, procWindowOutput.y1,
                    procWindowSize.x, procWindowSize.y);
  boost::gil::copy_pixels(boost::gil::kth_channel_view<flooding>(tmp_dst),
                          boost::gil::kth_channel_view<lower>(tmp_dst));
  boost::gil::copy_pixels(boost::gil::kth_channel_view<flooding>(tmp_dst),
                          boost::gil::kth_channel_view<upper>(tmp_dst));
  boost::gil::copy_pixels(boost::gil::kth_channel_view<flooding>(tmp_dst),
                          boost::gil::kth_channel_view<3>(tmp_dst));
  //	}
}
}

template <template <class> class Allocator, class SView, class DView>
void applyFloodFill(const SView &srcView, DView &dstView,
  ...
  pixel_minmax_by_channel_t<typename LocalView::type::value_type> minmax(
      localView(0, 0));

  terry::algorithm::transform_pixels(localView, minmax);

  const bool isConstantImage = (minmax.max[0] == minmax.min[0]);
  const double lowerThresR =
      (lowerThres * (minmax.max[0] - minmax.min[0])) + minmax.min[0];
  const double upperThresR =
      (upperThres * (minmax.max[0] - minmax.min[0])) + minmax.min[0];
  ...
  floodFill::flood_fill<floodFill::Connexity4, floodFill::IsUpper<Scalar>,
                        floodFill::IsUpper<Scalar>, SView, DView, Allocator>(
      srcView, getBounds<std::ptrdiff_t>(srcView), dstView,
