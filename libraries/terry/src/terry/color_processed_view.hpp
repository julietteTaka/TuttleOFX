...
// the difference with color_convert is on same pixels, only return pixel
// without process it.
namespace detail {

template <typename SrcConstRefP, typename DstP, typename CC> // const_reference
                                                             // to the source
                                                             // pixel and
                                                             // destination
                                                             // pixel value
class color_process_deref_fn
    : public deref_base<color_process_deref_fn<SrcConstRefP, DstP, CC>, DstP,
...
template <typename CC, typename View> struct _color_processed_view_type {

private:
  typedef typename View::value_type Pixel;
  typedef color_process_deref_fn<typename View::const_t::reference, Pixel, CC>
  deref_t;
...
struct color_processed_view_type
    : public detail::_color_processed_view_type<CC, View> {
  GIL_CLASS_REQUIRE(typename View::value_type, boost::gil,
                    MutablePixelConcept) // why does it have to be mutable?
};
}

template <typename CC, typename View>
inline typename detail::color_processed_view_type<CC, View>::type
