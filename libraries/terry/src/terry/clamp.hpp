...
template <typename Pixel> struct clamp_converter {
...
struct clamp_view_type_resolver {
  typedef typename detail::color_processed_view_type<
      clamp_converter<typename View::value_type>, View>::type Result;
 ...
 *        range.
 */
template <class View> struct clamp_view_type {
  typedef typename channel_type<View>::type Channel;

  typedef clamp_view_type_resolver<
      View, typename is_scoped_channel<Channel>::type> Resolver;
...
inline typename detail::color_processed_view_type<
    clamp_converter<typename View::value_type>, View>::type
