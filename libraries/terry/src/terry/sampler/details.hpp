...
void getPixelsPointers(const xy_locator &loc, const point2<std::ptrdiff_t> &p0,
                       const int &windowWidth, const int &imageWidth,
  ...
  if ((p0.x < 0) || (p0.x > imageWidth - 1)) {
    ...
    case eParamFilterOutCopy: {
      src.at(middlePosition) =
          loc.x()[(p0.x < 0) ? (-p0.x) : -1 - p0.x + imageWidth];
      break;
    }
...
struct process1Dresampling {
  void operator()(const std::vector<SrcP> &src,
                  const std::vector<Weight> &weight, DstP &dst) const {
    DstP mp(0);
    for (std::size_t i = 0; i < src.size(); i++)
      details::add_dst_mul_src<SrcP, Weight, DstP>()(src.at(i), weight.at(i),
                                                     mp);
    dst = mp;
  }
};
...
// struct process1Dresampling
//{
//	void operator()( const std::vector<rgba32f_t> src, const std::vector<F>
//weight, rgba32f_t & dst ) const
//	{
//		//...
//	}
//};
...
bool sample(Sampler &sampler, const SrcView &src, const point2<F> &p,
            DstP &result, const EParamFilterOutOfImage outOfImageProcess) {
  ...
  // xWeights and yWeights are weights for in relation of the distance to each
  // point
  ...
  if ((pTL.y < 0.0) || (pTL.y > (int)(src.height() - 1.0))) {
    if (pTL.y < 0.0) // under the image
    {
      ...
      case eParamFilterOutCopy: {
        loc.y() -= pTL.y;
        details::getPixelsPointers(loc, pTL, sampler._windowSize, src.width(),
                                   outOfImageProcess, ptr);
        details::process1Dresampling<SrcP, Weight, SrcC>()(
            ptr, xWeights, xProcessed.at(middlePosition));
        loc.y() += pTL.y;
        break;
      }
      case eParamFilterOutMirror: {
  ...
  for (int i = middlePosition - 1; i > -1; i--) {
    if ((int)(pTL.y - (middlePosition - i)) < (int)src.height()) {
      if ((int)(pTL.y - (middlePosition - i)) < 0) {
        ...
        details::getPixelsPointers(loc, pTL, sampler._windowSize, src.width(),
                                   outOfImageProcess, ptr);
        details::process1Dresampling<SrcP, Weight, SrcC>()(ptr, xWeights,
                                                           xProcessed.at(i));
        loc.y() += (middlePosition - i);
      }
    } else {
