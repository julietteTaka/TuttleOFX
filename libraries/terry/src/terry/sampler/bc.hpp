...
struct bc_sampler {
  ...
  bc_sampler(RESAMPLING_CORE_TYPE valB, RESAMPLING_CORE_TYPE valC)
      : _windowSize(4.0), // 4 pixels:    A B C D
        _valB(valB), _valC(valC) {}

  /**
   * @brief Get weight for a specific distance, for all BC-cubic resampling
  *(bicubic, catmul-rom, ...).
   *
   ...
   *   <a
  *href="http://www.cs.utexas.edu/users/fussell/courses/cs384g/lectures/mitchell/Mitchell.pdf">online
  *paper</a>
   * ]:
   *
   * \f[ W(x) =
  ...
  void operator()(const RESAMPLING_CORE_TYPE &distance, Weight &weight) {
    ...
    if (absDistance <= 1.0) {
      ...
      if (absDistance < 2.0) {
        ...
        weight =
            (((T * absDistance + U) * absDistance + V) * absDistance + W) / 6.0;
        return;
      }
      weight = 0.0;
    }
...
struct cubic_sampler : bc_sampler {
  cubic_sampler() : bc_sampler(0.0, 0.0) {}
  cubic_sampler(RESAMPLING_CORE_TYPE a) : bc_sampler(0.0, -a) {}
};
...
struct mitchell_sampler : bc_sampler {
  mitchell_sampler() : bc_sampler(1.0 / 3.0, 1.0 / 3.0) {}
};

struct parzen_sampler : bc_sampler {
  parzen_sampler() : bc_sampler(1.0, 0.0) {}
};
}
}
