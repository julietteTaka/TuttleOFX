...
template <typename Sampler, // Models SamplerConcept
          ...
          typename MapFn, typename Progress>
// Models MappingFunctionConcept
void resample_pixels_progress(const SrcView &src_view, const DstView &dst_view,
                              const MapFn &dst_to_src,
                              const terry::Rect<std::ssize_t> &procWindow,
                              const EParamFilterOutOfImage &outOfImageProcess,
                              Progress &p, Sampler sampler = Sampler()) {
  ...
  for (dst_p.y = procWindow.y1; dst_p.y < procWindow.y2; ++dst_p.y) {
    ...
    for (dst_p.x = procWindow.x1; dst_p.x < procWindow.x2; ++dst_p.x) {
      ...
      if (!sample(sampler, src_view, transform(dst_to_src, dst_p), xit[dst_p.x],
                  outOfImageProcess)) {
