...
struct alpha_max_filler {
  template <class P> inline P operator()(const P &p) const {
    using namespace boost::gil;
    gil_function_requires<ColorSpacesCompatibleConcept<
        typename color_space_type<P>::type, rgba_t> >();
    P p2;
 ...
 */
template <class Pixel> static inline const Pixel get_black() {
  using namespace boost::gil;
  Pixel black;
  /// @todo tuttle: to rewrite !!!
...
template <class View> static inline typename View::value_type get_white() {
  using namespace boost::gil;
...
template <class View> void fill_alpha_max(const View &v) {
  using namespace boost::gil;
  transform_pixels(v, v, alpha_max_filler());
}

/**
 * @brief Fill an image in black, all channels to 0.0 value and alpha channel to
 * 1.0 (if exists)
 * @todo tuttle: to rewrite !!!
 */
template <class View> void fill_black(View &v) {
  using namespace boost::gil;
