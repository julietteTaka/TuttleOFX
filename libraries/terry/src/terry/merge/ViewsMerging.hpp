...
namespace detail {
template <class OPERATES> struct merger {};

template <> struct merger<merge_per_pixel> {
  template <class Pixel, class Functor>
  GIL_FORCEINLINE void operator()(const Pixel &A, const Pixel &B, Pixel &d,
                                  Functor &fun) const {
    fun(A, B, d);
  }
};

template <> struct merger<merge_per_channel_with_alpha> {
  template <class Pixel, class Functor>
  GIL_FORCEINLINE void operator()(const Pixel &A, const Pixel &B, Pixel &d,
                                  Functor &fun) const {
...
template <> struct merger<merge_per_channel> {
  template <class Pixel, class Functor>
  GIL_FORCEINLINE void operator()(const Pixel &A, const Pixel &B, Pixel &d,
                                  Functor &fun) const {
    static_for_each(A, B, d, fun);
...
void merge_views(const View &srcA, const View &srcB, View &dst, F fun) {
  ...
  typedef typename contains_color<typename View::value_type, alpha_t>::type
  has_alpha_t;
  //	BOOST_STATIC_ASSERT(( boost::is_same<typename F::operating_mode_t,
  //merge_per_channel_with_alpha>::value ? has_alpha_t::value : true ));
