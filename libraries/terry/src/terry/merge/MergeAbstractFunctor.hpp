...
}; ///< standard per channel functor with the alpha values stored inside the
   ///functor
 ...
 */
template <typename Pixel, typename OPERATES> struct merge_functor {
 ...
 */
template <typename Pixel> struct merge_functor<Pixel, merge_per_pixel> {
 ...
 */
template <typename Pixel> struct merge_functor<Pixel, merge_per_channel> {
