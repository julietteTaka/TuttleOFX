...
Pixel clearChannel(const Pixel &in, bool red, bool green, bool blue, bool alpha,
                   const boost::mpl::true_, const boost::mpl::true_) {
  Pixel out = in;
  if (red)
...
template <class Pixel>
Pixel clearChannel(const Pixel &in, bool red, bool green, bool blue, bool alpha,
                   const boost::mpl::true_, const boost::mpl::false_) {
  Pixel out = in;
  if (red)
    get_color(out, red_t()) = 0.0;
  if (green)
...
template <class Pixel>
Pixel clearChannel(const Pixel &in, bool red, bool green, bool blue, bool alpha,
                   const boost::mpl::false_, const boost::mpl::true_) {
  return in;
}

template <class Pixel>
Pixel clearChannel(const Pixel &in, bool red, bool green, bool blue, bool alpha,
                   const boost::mpl::false_, const boost::mpl::false_) {
  return in;
}
...
struct RampFunctor {
  ...
  RampFunctor() {}
  RampFunctor(const point_t &tileSize, const value_type &start_color,
              const value_type &end_color, bool horizontal = true,
              bool _multiColor = false)
      : _tile_size(tileSize), _start_color(start_color), _end_color(end_color),
        _horizontal(horizontal), _multiColor(_multiColor) {}
  ...
  Pixel operator()(const point_t &p) const {
    ...
    Pixel pixelStart = numeric::pixel_multiplies_scalar_t<Pixel, float>()(
        _start_color, 1.0 - level);
    Pixel pixelEnd =
        numeric::pixel_multiplies_scalar_t<Pixel, float>()(_end_color, level);
    ...
    if (_horizontal) {
      if (p.y < 0.25 * _tile_size.y) {
        pixel = clearChannel(pixel, false, true, true, false, hasRGBChannel(),
                             hasAlphaChannel());
      } else {
        if (p.y < 0.5 * _tile_size.y) {
          pixel = clearChannel(pixel, true, false, true, false, hasRGBChannel(),
                               hasAlphaChannel());
        } else {
          if (p.y < 0.75 * _tile_size.y) {
            pixel = clearChannel(pixel, true, true, false, false,
                                 hasRGBChannel(), hasAlphaChannel());
          }
        }
      }
    } else {
