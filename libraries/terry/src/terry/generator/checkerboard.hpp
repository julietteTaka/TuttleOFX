...
struct CheckerboardFunctor {
  ...
  CheckerboardFunctor() {}
  CheckerboardFunctor(const point_t &tileSize, const value_type &in_color,
                      const value_type &out_color)
      : _in_color(in_color), _out_color(out_color), _tile_size(tileSize),
  ...
  result_type operator()(const point_t &p) const {
    ...
    TUTTLE_LOG_TRACE( "_tile_size.x: " << _tile_size.x << " _tile_size.y: " <<
    _tile_size.y );*/
