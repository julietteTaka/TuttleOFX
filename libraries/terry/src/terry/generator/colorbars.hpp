...
struct ColorBarsFunctor {
  ...
  ColorBarsFunctor(int width, bool is75percents)
      : width1(1.0 / 7 * width), width2(2.0 / 7 * width),
        width3(3.0 / 7 * width), width4(4.0 / 7 * width),
        width5(5.0 / 7 * width), width6(6.0 / 7 * width),
  ...
  Pixel operator()(const point_t &p) const {
    ...
    if (p.x < width1 || (p.x >= width2 && p.x < width3) ||
        (p.x >= width4 && p.x < width5) || (p.x >= width6)) {
      blue = level;
    }
