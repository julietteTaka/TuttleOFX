...
struct ConstantFunctor {
  ...
  ConstantFunctor() {}
  ConstantFunctor(const value_type &color) : _color(color) {}

  result_type operator()(const point_t &) const { return _color; }
};

template <typename Pixel> struct ConstantColorViewFactory {
  ...
  static ConstantVirtualView getView(const Pixel &color) {
    ...
    return ConstantVirtualView(Point(1, 1), Locator(Point(0, 0), Point(1, 1),
                                                    ConstantFunctorT(color)));
  }
};

template <typename Pixel>
typename ConstantColorViewFactory<Pixel>::ConstantVirtualView
