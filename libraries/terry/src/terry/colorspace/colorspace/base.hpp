...
#define TERRY_DEFINE_GIL_INTERNALS_4(CS)                                       \
  ...
  inline typename type_from_x_iterator<                                        \
      planar_pixel_iterator<IC, color::CS##_colorspace_t> >::view_t            \
  planar_##CS##_view(std::size_t width, std::size_t height, IC a, IC b, IC c,  \
                     IC d, std::ptrdiff_t rowsize_in_bytes) {                  \
    typedef typename type_from_x_iterator<                                     \
        planar_pixel_iterator<IC, color::CS##_colorspace_t> >::view_t RView;   \
    return RView(                                                              \
        width, height,                                                         \
        typename RView::locator(                                               \
            planar_pixel_iterator<IC, color::CS##_colorspace_t>(a, b, c, d),   \
            rowsize_in_bytes));                                                \
  }
...
#define TERRY_DEFINE_GIL_INTERNALS_3(CS)                                       \
  ...
  planar_##CS##_view(std::size_t width, std::size_t height, IC a, IC b, IC c,  \
    ...
    return RView(                                                              \
        width, height,                                                         \
        typename RView::locator(                                               \
            planar_pixel_iterator<IC, color::CS##_colorspace_t>(a, b, c),      \
            rowsize_in_bytes));                                                \
  }
...
#define TERRY_GIL_DEFINE_BASE_TYPEDEFS_INTERNAL(T, CS, LAYOUT)                 \
  ...
  typedef memory_based_2d_locator<                                             \
      memory_based_step_iterator<CS##T##_step_ptr_t> > CS##T##_step_loc_t;     \
  typedef memory_based_2d_locator<                                             \
      memory_based_step_iterator<CS##T##c_step_ptr_t> > CS##T##c_step_loc_t;   \
...
#define TERRY_GIL_DEFINE_ALL_TYPEDEFS_INTERNAL(T, CS, CS_FULL, LAYOUT)         \
  TERRY_GIL_DEFINE_BASE_TYPEDEFS_INTERNAL(                                     \
      T, CS, LAYOUT) typedef planar_pixel_reference<bits##T &, CS_FULL>        \
  CS##T##_planar_ref_t;                                                        \
  ...
  typedef memory_based_2d_locator<                                             \
      memory_based_step_iterator<CS##T##_planar_ptr_t> > CS##T##_planar_loc_t; \
  typedef memory_based_2d_locator<memory_based_step_iterator<                  \
      CS##T##c_planar_ptr_t> > CS##T##c_planar_loc_t;                          \
  typedef memory_based_2d_locator<memory_based_step_iterator<                  \
      CS##T##_planar_step_ptr_t> > CS##T##_planar_step_loc_t;                  \
  typedef memory_based_2d_locator<memory_based_step_iterator<                  \
      CS##T##c_planar_step_ptr_t> > CS##T##c_planar_step_loc_t;                \
...
#define TERRY_DEFINE_ALL_TYPEDEFS(T, CS)                                       \
  TERRY_GIL_DEFINE_ALL_TYPEDEFS_INTERNAL(T, CS, color::CS##_colorspace_t,      \
                                         color::CS##_layout_t)

#define TERRY_DEFINE_COLORSPACE_STANDARD_TYPEDEFS(CS)                          \
  TERRY_DEFINE_ALL_TYPEDEFS(8, CS) TERRY_DEFINE_ALL_TYPEDEFS(8s, CS)           \
      TERRY_DEFINE_ALL_TYPEDEFS(16, CS) TERRY_DEFINE_ALL_TYPEDEFS(16s, CS)     \
      TERRY_DEFINE_ALL_TYPEDEFS(32, CS) TERRY_DEFINE_ALL_TYPEDEFS(32s, CS)     \
