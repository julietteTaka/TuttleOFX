...
typedef ::boost::mpl::vector3<hsl_colorspace::hue_t,
                              hsl_colorspace::saturation_t,
                              hsl_colorspace::lightness_t> hsl_colorspace_t;
...
void color_transformation_step(
    const HSLParams &params,
...
void color_transformation_step(
  ...
  ChannelFloat temp_red =
      channel_convert<ChannelFloat>(get_color(src, red_t()));
  ChannelFloat temp_green =
      channel_convert<ChannelFloat>(get_color(src, green_t()));
  ChannelFloat temp_blue =
      channel_convert<ChannelFloat>(get_color(src, blue_t()));
  ...
  if (diff == 0.0) {
    ...
    get_color(dst, hsl_colorspace::lightness_t()) =
        channel_convert<DChannelType>(temp_red);
    return;
  }
  ...
  get_color(dst, hsl_colorspace::saturation_t()) =
      channel_convert<DChannelType>(saturation);
  get_color(dst, hsl_colorspace::lightness_t()) =
      channel_convert<DChannelType>(lightness);
}

BOOST_MPL_ASSERT((::boost::mpl::equal< ::boost::mpl::vector<XYZ, RGB, HSL>,
                                       color_dependencies<HSL>::from_root>));

BOOST_MPL_ASSERT((::boost::mpl::equal< ::boost::mpl::vector<HSL, RGB, XYZ>,
                                       color_dependencies<HSL>::to_root>));

BOOST_MPL_ASSERT((::boost::mpl::equal<
    ::boost::mpl::vector<XYZ, RGB>,
    ::boost::mpl::at_c<color_dependencies<HSL>::color_steps_from_root,
                       0>::type>));
BOOST_MPL_ASSERT((::boost::mpl::equal<
