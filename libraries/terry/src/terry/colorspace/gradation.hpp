...
struct Rec709 {};
struct Cineon {
  Cineon() : _blackPoint(95.0), _whitePoint(685.0), _gammaSensito(0.6) {}
  Cineon(const double blackPoint, const double whitePoint,
         const double gammaSensito)
      : _blackPoint(blackPoint), _whitePoint(whitePoint),
...
struct Gamma {
  Gamma() : _gamma(1.0) {}
  Gamma(const double gamma) : _gamma(gamma) {}
  double _gamma;
};
struct Panalog {};
struct REDLog {};
...
struct channel_color_gradation_t
  ...
  // * with TIN != TOUT
  BOOST_STATIC_ASSERT((
      !boost::is_same<TIN, TOUT>::value)); // Must use
                                           // channel_color_gradation_t<Channel,
                                           // INOUT> !
  // * TIN and TOUT must be other gradation mode than Linear
  //   For each gradation mode, you have to specialize: GradationMode -> Linear
  // and Linear -> GradationMode
  BOOST_STATIC_ASSERT(
      (!boost::is_same<TIN, gradation::Linear>::value)); // The conversion IN to
                                                         // Linear is not
                                                         // implemented !
  BOOST_STATIC_ASSERT(
      (!boost::is_same<TOUT, gradation::Linear>::value)); // The conversion
                                                          // Linear to OUT is
                                                          // not implemented !
  ...
  ChannelRef operator()(ChannelConstRef src, ChannelRef dst) const {
    Channel inter;
    // IN -> Linear
    channel_color_gradation_t<Channel, TIN, gradation::Linear>(
        _in, gradation::Linear())(src, inter);
    // Linear -> OUT
...
template <typename Channel>
struct channel_color_gradation_t<
    Channel, gradation::Linear,
    gradation::sRGB> : public std::binary_function<Channel, Channel, Channel> {
  typedef typename floating_channel_type_t<Channel>::type T;
  typedef typename channel_base_type<Channel>::type TBase;
  typedef typename channel_traits<Channel>::const_reference ChannelConstRef;
  typedef typename channel_traits<Channel>::reference ChannelRef;
...
struct channel_color_gradation_t<
  ...
  ChannelRef operator()(ChannelConstRef src, ChannelRef dst) const {
    ...
    double offset = std::pow(10, (_in._blackPoint - _in._whitePoint) * 0.002 /
                                     _in._gammaSensito);
    double gain = 1.0 / (1 - offset);
    T fDst = gain * (std::pow(10.0, (1023 * fSrc - _in._whitePoint) * 0.002 /
                                        _in._gammaSensito) -
                     offset);
    return dst = channel_convert<Channel>(fDst);
  }
...
struct channel_color_gradation_t<
  ...
  ChannelRef operator()(ChannelConstRef src, ChannelRef dst) const {
    const T fSrc = channel_convert<T>(src);
    double offset = std::pow(10, (_out._blackPoint - _out._whitePoint) * 0.002 /
                                     _out._gammaSensito);
    double gain = 1.0 / (1 - offset);
    T fDst =
        (std::log10((fSrc + offset) / gain) / (0.002 / _out._gammaSensito) +
         _out._whitePoint) /
        1023.0;
...
struct channel_color_gradation_t<
    Channel, gradation::Linear,
    gradation::Gamma> : public std::binary_function<Channel, Channel, Channel> {
  typedef typename floating_channel_type_t<Channel>::type T;
  typedef typename channel_base_type<Channel>::type TBase;
  typedef typename channel_traits<Channel>::const_reference ChannelConstRef;
  typedef typename channel_traits<Channel>::reference ChannelRef;
...
struct channel_color_gradation_t<
  ...
  ChannelRef operator()(ChannelConstRef src, ChannelRef dst) const {
    const T fSrc = channel_convert<T>(src);
    T fDst =
        (444.0 * std::log10(0.0408 + (1.0 - 0.0408) * fSrc) + 681.0) / 1023.0;
    return dst = channel_convert<Channel>(fDst);
  }
};
...
template <class TIN, class TOUT> struct transform_pixel_color_gradation_t {
  const TIN &_in;
  const TOUT &_out;
...
/**
 * @example gradation_convert_view( srcView, dstView, gradation::sRGB(),
 * gradation::Gamma(5.0) );
 */
template <class GradationIN, class GradationOUT, class View>
void gradation_convert_view(const View &src, View &dst,
                            const GradationIN &gradationIn = GradationIN(),
                            const GradationOUT &gradationOut = GradationOUT()) {
  boost::gil::transform_pixels(
      src, dst, transform_pixel_color_gradation_t<GradationIN, GradationOUT>(
                    gradationIn, gradationOut));
}

/**
 * @example gradation_convert_pixel( srcPix, dstPix, gradation::sRGB(),
 * gradation::Gamma(5.0) );
 */
template <class GradationIN, class GradationOUT, class Pixel>
void gradation_convert_pixel(const Pixel &src, Pixel &dst,
                             const GradationIN &gradationIn = GradationIN(),
                             const GradationOUT &gradationOut =
                                 GradationOUT()) {
  pixel_color_gradation_t<Pixel, GradationIN, GradationOUT>(
      gradationIn, gradationOut)(src, dst);
}
}
}
