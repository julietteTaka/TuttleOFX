...
template <typename Pixel, class IN, class OUT> struct pixel_color_layout_t {
  typedef typename channel_type<Pixel>::type Channel;
  const IN &_in;
  const OUT &_out;

  pixel_color_layout_t(const IN &in, const OUT &out) : _in(in), _out(out) {}
...
template <class IN, class OUT> struct transform_pixel_color_layout_t {
  const IN &_in;
  const OUT &_out;
...
void layout_convert_view(const View &src, View &dst,
                         const LayoutIN &layoutIn = LayoutIN(),
                         const LayoutOUT &layoutOut = LayoutOUT()) {
  boost::gil::transform_pixels(
      src, dst,
      transform_pixel_color_layout_t<LayoutIN, LayoutOUT>(layoutIn, layoutOut));
}
...
void layout_convert_pixel(const Pixel &src, Pixel &dst,
                          const LayoutIN &layoutIn = LayoutIN(),
                          const LayoutOUT &layoutOut = LayoutOUT()) {
  pixel_color_layout_t<Pixel, LayoutIN, LayoutOUT>(layoutIn, layoutOut)(src,
                                                                        dst);
}
}
}
