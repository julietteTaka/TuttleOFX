 ...
 */
template <class color> struct color_dependencies {
  /// list of color types dependencies from root (eg. HSL: [XYZ,RGB,HSL])
  typedef typename ::boost::mpl::push_back<
      typename color_dependencies<typename color::reference>::from_root,
      color>::type from_root;
  /// list of color types dependencies to root (eg. HSL: [HSL,RGB,XYZ])
  typedef typename ::boost::mpl::push_front<
      typename color_dependencies<typename color::reference>::to_root,
      color>::type to_root;
  ...
  typedef typename ::boost::mpl::zip_view< ::boost::mpl::vector<
      typename details::from_root_A, typename details::from_root_B> >::type
  color_steps_from_root;
  /// a vector of colors pairs (eg. HSL: [(HSL,RGB),(RGB,XYZ)])
  typedef typename ::boost::mpl::zip_view< ::boost::mpl::vector<
      typename details::to_root_A, typename details::to_root_B> >::type
  color_steps_to_root;
};
template <> struct color_dependencies<IsRootReference> {
  typedef ::boost::mpl::vector<> from_root;
  typedef ::boost::mpl::vector<> to_root;
};

/**
 * @brief Mpl functor to convert a list of color types into a list of color
 * params types
 * @example HSL>RGB>XYZ to HSLParams>RGBParams>XYZParams
 */
template <typename Color> struct color_to_colorparams_mplfunc {
 ...
 */
template <class Color> struct FullColorParams {
  ...
  typedef typename ::boost::mpl::transform<
      from_root, color_to_colorparams_mplfunc< ::boost::mpl::_1> >::type
  full_params; ///< all params from root to Color
  typedef typename ::boost::fusion::result_of::as_vector<full_params>::type
  full_params_v;
...
template <typename Color, typename ChannelType> struct color_to_pixel {
  typedef boost::gil::pixel<ChannelType, typename Color::layout> type;
};

/**
 * @brief Utility class to compute all internal types needed by
 *color_transformation function.
 *
 ...
 * @param ChannelType type used from pixels (defined the bitdepth used: bits8,
 *bits32f, ...)
 * @param fromRoot direction of the color conversion
 */
template <typename ColorsFromRoot, std::size_t sizeLeft, class ChannelType,
          bool fromRoot>
struct color_transformation_types_impl {
  static const std::size_t size = ::boost::mpl::size<ColorsFromRoot>::value;
  static const std::size_t indexMax =
      size - 1; // only to get valid typedefs to compile...
  static const std::size_t indexDirFromRoot =
      size - sizeLeft; // direction from root, index from root
  static const std::size_t indexDirToRoot =
      sizeLeft - 1; // direction to root, index from root

  // with sizeLeft = 2
  //
  // [ XYZ,  RGB, CMYK, PLOP, PLIP ] : colors_from_color created from PLIP
  // colorspace
  //
  //    | > > | > > | > > ^     |    : direction fromRoot
  ...
  static const std::size_t sIndex =
      fromRoot ? indexDirFromRoot : indexDirToRoot;
  static const std::size_t dIndex = ::boost::static_unsigned_min < fromRoot
                                        ? sIndex + 1
                                        : sIndex - 1,
                           indexMax > ::value; // incrementation depends from
                                               // the direction
  static const std::size_t nextIndex = ::boost::static_unsigned_min < fromRoot
                                           ? dIndex + 1
                                           : dIndex - 1,
                           indexMax > ::value;
  ...
  typedef typename ::boost::mpl::at_c<ColorsFromRoot, sIndex>::type
  SColor; // current color index
  typedef typename ::boost::mpl::at_c<ColorsFromRoot, dIndex>::type
  DColor; // dest color index (destination for current
          // color_transformation_step)
  typedef typename ::boost::mpl::at_c<ColorsFromRoot, nextIndex>::type
  NextColor; // next color index
  typedef typename ::boost::mpl::back<ColorsFromRoot>::type
  Color; // color from which the ColorsFromRoot mpl::vector is build
  typedef typename ::boost::mpl::if_c<
      fromRoot, typename ::boost::mpl::back<ColorsFromRoot>::type,
...
/**
 */
template <class ColorsFromRoot, std::size_t sizeLeft, class ChannelType,
          bool fromRoot>
struct color_transformation_static_for_impl_rec {
  typedef color_transformation_types_impl<ColorsFromRoot, sizeLeft, ChannelType,
                                          fromRoot> T;

  void operator()(const typename T::FullParams &params,
                  const typename T::SPixel &src, typename T::DPixel &dst,
                  typename T::FPixel &finalDst) const {
    BOOST_STATIC_ASSERT(sizeLeft > 3);

    color_transformation_step(boost::fusion::at_c<T::index>(params._params),
                              src, dst);

    typename T::NextPixel pixNext;

    color_transformation_static_for_impl_rec<ColorsFromRoot, sizeLeft - 1,
                                             ChannelType, fromRoot>()(
        params, dst, pixNext, finalDst);
  }
};
...
struct color_transformation_static_for_impl_rec<ColorsFromRoot, 3, ChannelType,
                                                fromRoot> {
...
/**
 */
template <class ColorsFromRoot, std::size_t colorsSize, class ChannelType,
          bool fromRoot>
struct color_transformation_static_for_impl {
  typedef color_transformation_types_impl<ColorsFromRoot, colorsSize,
                                          ChannelType, fromRoot> T;
  ...
  void operator()(const typename T::FullParams &params,
                  ...
                  typename T::FPixel &dst) const {
    typename T::DPixel pixNext;

    // multiple steps in a static foreach color_transformation_step
    color_transformation_static_for_impl_rec<
        ColorsFromRoot, colorsSize, ChannelType, fromRoot>()(params, src,
                                                             pixNext, dst);
  }
};
// specialized for a size left == 2
template <class ColorsFromRoot, class ChannelType, bool fromRoot>
struct color_transformation_static_for_impl<ColorsFromRoot, 2, ChannelType,
                                            fromRoot> {
  ...
  void operator()(const typename T::FullParams &params,
    ...
    // only one step
    color_transformation_step(
        boost::fusion::at_c<T::colorParamIndex>(params._params), src, dst);
  }
};
template <class ColorsFromRoot, class ChannelType, bool fromRoot>
struct color_transformation_static_for_impl<ColorsFromRoot, 1, ChannelType,
                                            fromRoot> {
...
struct color_transformation_static_for {
  ...
  void operator()(const typename T::FullParams &params,
                  const typename T::SPixel &src,
                  typename T::FPixel &dst) const {
    color_transformation_static_for_impl<ColorsFromRoot, sizeLeft, ChannelType,
                                         fromRoot>()(params, src, dst);
  }
};
...
void color_transformation_to_root(
  ...
  color_transformation_static_for<typename FullColorParams<Color>::from_root,
                                  ChannelType, false>()(params, src, dst);
}
...
void color_transformation_from_root(
  ...
  color_transformation_static_for<typename FullColorParams<Color>::from_root,
                                  ChannelType, true>()(params, src, dst);
}
