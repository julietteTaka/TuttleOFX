...
struct channel_converter_unsigned<
    bits16h, DstChannelV> : public std::unary_function<bits16h, DstChannelV> {
  DstChannelV operator()(bits16h h) const {
    return DstChannelV(
        half(h) * float(channel_traits<DstChannelV>::max_value()) + 0.5f);
  }
};
...
struct channel_converter_unsigned<
    SrcChannelV, bits16h> : public std::unary_function<SrcChannelV, bits16h> {
  bits16h operator()(SrcChannelV x) const {
    return bits16h(float(x) / float(channel_traits<SrcChannelV>::max_value()));
  }
};
...
template <>
struct channel_converter_unsigned<
    bits16h, bits16h> : public std::unary_function<bits16h, bits16h> {
...
template <>
struct channel_converter_unsigned<
    bits32f, bits16h> : public std::unary_function<bits32f, bits16h> {
...
template <>
struct channel_converter_unsigned<
    bits16h, bits32f> : public std::unary_function<bits16h, bits32f> {
  bits32f operator()(bits16h h) const { return bits32f(float(half(h))); }
};

///////////////////////////////////////////////////////////////////////////////
template <>
struct channel_multiplier_unsigned<bits16h> : public std::binary_function<
                                                  bits16h, bits16h, bits16h> {
