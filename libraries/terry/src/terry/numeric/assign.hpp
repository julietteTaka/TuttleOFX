...
struct pixel_assigns_t {
  ...
  PixelRefR &operator()(const PixelRef &src, PixelRefR &dst) const {
    static_for_each(
        src, dst, channel_assigns_t<typename channel_type<PixelRef>::type,
...
struct pixel_assigns_color_t {
  ...
  pixel_assigns_color_t(const PixelRef &color) : _color(color) {}
...
struct pixel_assigns_scalar_t {
  ...
  PixelR &operator()(const Scalar s, PixelR &dst) const {
    static_for_each(
        dst,
        std::bind1st(
            channel_assigns_t<Scalar, typename channel_type<PixelR>::type>(),
            s));
    return dst;
  }
};
...
assign_pixels(SrcIterator src, SrcIterator src_end, DstIterator dst) {
  algorithm::for_each(
      src, src_end, dst,
      pixel_assigns_t<
