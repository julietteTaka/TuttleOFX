...
struct pixel_plus_assign_t {
  ...
  PixelDst &operator()(const PixelSrc &p1, PixelDst &p2) const {
    static_for_each(
        p1, p2, channel_plus_assign_t<typename channel_type<PixelSrc>::type,
...
struct channel_plus_scalar_assign_t
    : public std::binary_function<Scalar, ChannelDst, ChannelDst> {
  GIL_FORCEINLINE
  typename channel_traits<ChannelDst>::reference
...
struct pixel_multiplies_scalar_assign_t {
  ...
  PixelDst &operator()(const Scalar s, PixelDst &p) const {
    static_for_each(
        p, std::bind1st(channel_multiplies_scalar_assign_t<
                            Scalar, typename channel_type<PixelDst>::type>(),
                        s));
    return p;
  }
};
...
struct pixel_divides_scalar_assign_t {
  ...
  PixelDst &operator()(const Scalar s, PixelDst &p) const {
    static_for_each(
        p, std::bind1st(channel_divides_scalar_assign_t<
                            Scalar, typename channel_type<PixelDst>::type>(),
                        s));
    return p;
  }
