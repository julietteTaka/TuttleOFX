...
struct pixel_assign_min_t {
  ...
  PixelDst &operator()(const PixelSrc &p1, PixelDst &p2) const {
    static_for_each(
        p1, p2, channel_assign_min_t<typename channel_type<PixelSrc>::type,
...
struct pixel_assign_max_t {
  ...
  PixelDst &operator()(const PixelSrc &p1, PixelDst &p2) const {
    static_for_each(
        p1, p2, channel_assign_max_t<typename channel_type<PixelSrc>::type,
                                     typename channel_type<PixelDst>::type>());
    return p2;
  }
};

template <typename CPixel> struct pixel_minmax_by_channel_t {
  typedef typename channel_type<CPixel>::type Channel;

  CPixel min;
  CPixel max;

  pixel_minmax_by_channel_t(const CPixel &v) : min(v), max(v) {}

  template <typename Pixel> GIL_FORCEINLINE void operator()(const Pixel &v) {
