...
struct channel_pow_t : public std::unary_function<Channel, ChannelR> {
  GIL_FORCEINLINE
  ChannelR
  operator()(typename channel_traits<Channel>::const_reference ch) const {
...
template <typename PixelRef, int n, typename PixelR = PixelRef> // models pixel
                                                                // concept
struct pixel_pow_t {
  GIL_FORCEINLINE
  PixelR operator()(const PixelRef &p) const {
    ...
    static_transform(p, result,
                     channel_pow_t<typename channel_type<PixelRef>::type, n,
...
struct pixel_scalar_pow_t {
  ...
  PixelR operator()(const PixelRef &p, const Scalar &s) const {
    ...
    static_transform(
        p, result,
        std::bind2nd(
            channel_scalar_pow_t<typename channel_type<PixelRef>::type, Scalar,
                                 typename channel_type<PixelR>::type>(),
            s));
    return result;
  }
