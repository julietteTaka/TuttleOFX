...
template <typename T> struct matrices_rgb_to_lab_t {
  typedef boost::numeric::ublas::bounded_matrix<T, 3, 3> Matrix33;
  Matrix33 _RGB_2_LMS;
  Matrix33 _LMS_2_LAB;

  matrices_rgb_to_lab_t() : _RGB_2_LMS(3, 3), _LMS_2_LAB(3, 3) {
...
template <typename T> struct matrices_lab_to_rgb_t {
  typedef boost::numeric::ublas::bounded_matrix<T, 3, 3> Matrix33;
  Matrix33 _LAB_2_LMS;
  Matrix33 _LMS_2_RGB;

  matrices_lab_to_rgb_t() : _LAB_2_LMS(3, 3), _LMS_2_RGB(3, 3) {
    // LAB -> LMS
...
struct pixel_rgb_to_lms_t {
  ...
  PixelR operator()(const PixelRef &rgb) const {
    ...
    // RGB to LMS
    return pixel_matrix33_multiply_t<PixelRef, Matrix, PixelR>(
        _matrices._RGB_2_LMS)(rgb);
  }
};
...
struct pixel_rgb_to_lab_t {
  ...
  PixelR operator()(const PixelRef &rgb) const {
    ...
    static const ChannelR channelThresold =
        ChannelR(thresold); // channel_convert<ChannelR, T>( thresold );
    // RGB to LMS
    PixelR lms = pixel_matrix33_multiply_t<PixelRef, Matrix, PixelR>(
        _matrices._RGB_2_LMS)(rgb);
    // log(v)
    PixelR lms_log = pixel_log10_t<PixelR, PixelR>()(
        // log10(x) is not defined for x <= 0, so we need to clamp
        pixel_clamp_lower_than_t<PixelR>(channelThresold,
                                         channelThresold)(lms));
    // LMS to LAB (lambda alpha beta)
    PixelR lab = pixel_matrix33_multiply_t<PixelR, Matrix, PixelR>(
        _matrices._LMS_2_LAB)(lms_log);
    return lab;
  }
};
...
struct pixel_lms_to_rgb_t {
  ...
  PixelR operator()(const PixelRef &lms) const {
    using namespace terry::numeric;
    // LMS to RGB
    return pixel_matrix33_multiply_t<PixelR, Matrix, PixelR>(
        _matrices._LMS_2_RGB)(lms);
  }
};
...
struct pixel_lab_to_rgb_t {
  ...
  PixelR operator()(const PixelRef &lab) const {
    ...
    PixelR lms_log = pixel_matrix33_multiply_t<PixelRef, Matrix, PixelR>(
        _matrices._LAB_2_LMS)(lab);
    // 10^v
    PixelR lms = pixel_scalar_pow_t<PixelR, T, PixelR>()(lms_log, 10.0);
    // LMS to RGB
    PixelR rgb = pixel_matrix33_multiply_t<PixelR, Matrix, PixelR>(
        _matrices._LMS_2_RGB)(lms);
    return rgb;
  }
};
