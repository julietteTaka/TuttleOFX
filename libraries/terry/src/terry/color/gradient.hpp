 ...
 */
template <typename Channel> struct channel_gradientDirection_t {
  GIL_FORCEINLINE
 ...
 */
template <typename Channel> struct channel_gradientDirectionAbs_t {
  GIL_FORCEINLINE
  void operator()(const Channel &x, const Channel &y, Channel &res) const {
    channel_gradientDirection_t<Channel>()(x, y, res);
    if (res < 0)
      res += boost::math::constants::pi<
          typename channel_base_type<Channel>::type>();
  }
};
}
}
