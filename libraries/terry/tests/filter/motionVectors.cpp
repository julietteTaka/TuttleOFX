...
BOOST_AUTO_TEST_CASE(motionVector) {
  ...
  filter::motionvectors_resample_pixels<sampler::bilinear_sampler>(
      inView, getBounds<std::ssize_t>(inView), xVecView, yVecView,
      getBounds<std::ssize_t>(xVecView), outView,
      getBounds<std::ssize_t>(outView), getBounds<std::ssize_t>(outView),
      sampler::eParamFilterOutCopy, progress);
}
