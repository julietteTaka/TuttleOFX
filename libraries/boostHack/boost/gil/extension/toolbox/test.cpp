  ...
  for (int y = 0; y < view.height(); ++y) {
    ...
    for (int x = 0; x < view.width(); ++x) {
      ...
      for (int i = 0; i < num_channels<PIXEL>::type::value; ++i) {
        if (dynamic_at_c(p, i) < dynamic_at_c(min, i)) /// @todo: use ( p[i] <
                                                       /// min[i] ), more
                                                       /// readable...
        {
...
inline PIXEL min_channel_values_(const VIEW &view, const size_t percent = 1) {
  ...
  typedef kth_semantic_element_type<PIXEL, 0>::type
  channel_t; // channel_0 = semantic_at_c<0>( PIXEL() );
...
template <class PIXEL, class VIEW>
inline boost::fusion::vector<PIXEL, PIXEL>
...
void down_sample(const SRC &src_view, const DST &dst_view) {
  ...
  boost::fusion::vector<src_pixel_t, src_pixel_t> minmax =
      minmax_channel_values<src_pixel_t>(src_view);
      ...
      for (int i = 0; i < num_channels<src_pixel_t>::type::value; ++i) {
        if (dynamic_at_c(diff, i) == 0) {
          ...
          dynamic_at_c(dst, i) = static_cast<dst_channel_t>(
              dst_max *
              (static_cast<float>(dynamic_at_c(src, i) - dynamic_at_c(min, i)) /
...
template <typename DST_MAX> struct foo {
  foo(const DST_MAX &dst_max) : _dst_max(dst_max) {}

  template <typename DST_CHANNEL, typename SRC_CHANNEL>
  void operator()(DST_CHANNEL &dst, SRC_CHANNEL &src, SRC_CHANNEL &min,
                  SRC_CHANNEL &diff) const {
    if (diff == 0) {
      dst = 0;
...
// @todo need better name
template <typename SRC_VIEW, typename SRC_PIXEL, typename DST_VIEW,
          typename DST_MAX>
struct do_it {
  do_it(const DST_VIEW &dst_view, const DST_MAX &dst_max, const SRC_PIXEL &min,
        const SRC_PIXEL &diff)
      : _dst_view(dst_view), _min(min), _diff(diff), _op(dst_max) {
...
int main(int argc, char *argv[]) {
...

  {
    ...
    my_virt_view_t mandel(dims,
                          locator_t(point_t(0, 0), point_t(1, 1),
                                    deref_t(dims, src_pixel_t(max_value, 0, 0),
