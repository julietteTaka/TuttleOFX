...
inline void wuline(const view_t &view, const value_type_t &pixel, int X0,
                   int Y0, int X1, int Y1, int NumLevels = 256,
                   int IntensityBits = 8) {
  ...
  while (--DeltaX) {
    ...
    dst = pixel;
    static_for_each(dst, view(X0, Y0 + 1), alpha24_blend(Weighting));
    view(X0, Y0 + 1) = dst;
  }
