...
template <typename view_t> struct render_glyphs {
  int x;
  const view_t &view;
  ...
  void operator()(glyph &glyph) {
    ...
    gray8c_view_t gray = interleaved_view(
        width, height, (gray8_pixel_t *)glyphslot->bitmap.buffer,
        glyphslot->bitmap.width);

    copy<alpha24_blend>(glyph.pixel, gray,
                        subimage_view(view, x, y, width, height));
    x += xadvance;
  }
};

static inline FT_Error face_requester(FTC_FaceID id, FT_Library library,
                                      void *data, FT_Face *face) {
  char **paths = (char **)data;
  char *path = paths[(long)id];
  return FT_New_Face(library, path, 0, face);
}

template <typename view_t> struct text {
  enum {
    left = (0x1 << 0),
    center = (0x1 << 1),
    right = (0x1 << 2),
  ...
  text(FTC_Manager manager, std::string str, color_t color, int id, int size,
       int align = center | middle, int options = 0, char special = '#')
      : manager(manager), str(str), id(id), size(size), align(align),
        color(color), options(options), special(special) {}

  text(FTC_Manager manager, color_t color, int id, int size,
       int align = center | middle, int options = 0, char special = '.')
      : manager(manager), id(id), size(size), align(align), color(color),
        options(options), special(special) {}
  ...
  void operator()(view_t &view) {
    ...
    std::transform(glyphs.begin(), glyphs.end(), std::back_inserter(m),
                   make_metric());
    ...
    if (options & clipspecial) {
      ...
      if (pos >= 1 && pos < glyphs.size()) {
        ...
        std::copy(glyphs.begin(), glyphs.begin() + pos,
...
template <typename view_t> struct elipsed_layer {
  typedef typename view_t::value_type color_t;
  ...
  elipsed_layer(FTC_Manager manager, std::string sfront, std::string sback,
                color_t color, int id, int size)
      : manager(manager), sfront(sfront), sback(sback), id(id), size(size),
        color(color) {}
  ...
  void operator()(view_t &view) {
    ...
    text_t ldots(manager, color, id, size, text_t::center | text_t::bottom,
                 text_t::fill, '.');
