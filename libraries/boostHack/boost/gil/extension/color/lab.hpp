...
typedef mpl::vector3<lab_color_space::luminance_t,
                     ...
                     lab_color_space::b_color_opponent_t> lab_t;
...
template <> struct default_color_converter_impl<rgb_t, lab_t> {
  ...
  void operator()(const P1 &src, P2 &dst) const {
    ...
    bits32f x, y, z;
    x = normalized_r * 0.4124f + normalized_g * 0.3576f +
        normalized_b * 0.1805f;
    y = normalized_r * 0.2126f + normalized_g * 0.7152f +
        normalized_b * 0.0722f;
    z = normalized_r * 0.0193f + normalized_g * 0.1192f +
        normalized_b * 0.9505f;
...
template <> struct default_color_converter_impl<lab_t, rgb_t> {
  ...
  void operator()(const P1 &src, P2 &dst) const {
    ...
    bits32f result_r = normalized_x * 3.2406f + normalized_y * -1.5372f +
                       normalized_z * -0.4986f;
    bits32f result_g = normalized_x * -0.9689f + normalized_y * 1.8758f +
                       normalized_z * 0.0415f;
    bits32f result_b = normalized_x * 0.0557f + normalized_y * -0.2040f +
                       normalized_z * 1.0570f;
