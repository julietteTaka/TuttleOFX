...
typedef mpl::vector3<hsl_color_space::hue_t, hsl_color_space::saturation_t,
                     hsl_color_space::lightness_t> hsl_t;
...
/// \ingroup ColorConvert
///  \brief Converting HSL to any pixel type. Note: Supports homogeneous pixels
/// only.
///
/// Done by an intermediate RGB conversion
template <typename C2> struct default_color_converter_impl<hsl_t, C2> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
    typedef hsl_t C1;
...
/// \ingroup ColorConvert
/// \brief Unfortunately HSL to HSL must be explicitly provided - otherwise we
/// get ambiguous specialization error.
template <> struct default_color_converter_impl<hsl_t, hsl_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
