...
template <typename RGB_VIEW> void negative(const RGB_VIEW &view) {
...
template <typename VALUE> struct calc_brightness_ {
  VALUE _number;
...
template <typename RGB_VIEW>
void difference(const RGB_VIEW &src_1, const RGB_VIEW &src_2,
                const RGB_VIEW &dst) {
  if (src_1.dimensions() != src_2.dimensions()) {
    BOOST_THROW_EXCEPTION(
        std::runtime_error("Source image have different dimensions."));
  }

  if (src_1.dimensions() != dst.dimensions()) {
    BOOST_THROW_EXCEPTION(std::runtime_error(
        "Destination image must have equal dimensions as the source images."));
  }
