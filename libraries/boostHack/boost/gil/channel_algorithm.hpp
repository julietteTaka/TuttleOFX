...
//warning (using -1 to get the max value of an integral)
//#endif
...
template <>
struct unsigned_integral_max_value<uint8_t> : public mpl::integral_c<uint32_t,
                                                                     0xFF> {};
template <>
struct unsigned_integral_max_value<uint16_t> : public mpl::integral_c<uint32_t,
                                                                      0xFFFF> {
};
template <>
struct unsigned_integral_max_value<uint32_t> : public mpl::integral_c<
                                                   uintmax_t, 0xFFFFFFFF> {};

template <int K>
struct unsigned_integral_max_value<packed_channel_value<
    K> > : public mpl::integral_c<typename packed_channel_value<K>::integer_t,
                                  (1 << K) - 1> {};
...
template <int K>
struct unsigned_integral_num_bits<packed_channel_value<K> > : public mpl::int_<
                                                                  K> {};
...
One implication of this is that the value 0 of signed channels may not be
preserved!

When creating new channel models, it is often a good idea to provide
specializations for the channel conversion algorithms, for
example, for performance optimizations. If the new model is an integral type
that can be signed, it is easier to define the conversion
only for the unsigned type (\p channel_converter_unsigned) and provide
specializations of \p detail::channel_convert_to_unsigned
and \p detail::channel_convert_from_unsigned to convert between the signed and
unsigned type.
...
template <typename SrcChannelV, typename DstChannelV> // Model
                                                      // ChannelValueConcept
...
struct channel_converter_unsigned_impl<
    ...
    true> : public channel_converter_unsigned_integral<SrcChannelV, DstChannelV,
                                                       mpl::less<
...
struct channel_converter_unsigned_integral<
    ...
    true> : public channel_converter_unsigned_integral_impl<SrcChannelV,
                                                            DstChannelV, true,
                                                            !(unsigned_integral_max_value<
                                                                  DstChannelV>::
                                                                  value %
...
struct channel_converter_unsigned_integral<
    SrcChannelV, DstChannelV,
    false> : public channel_converter_unsigned_integral_impl<SrcChannelV,
                                                             DstChannelV, false,
                                                             !(unsigned_integral_max_value<
                                                                   SrcChannelV>::
                                                                   value %
...
struct channel_converter_unsigned_integral_impl<SrcChannelV, DstChannelV, true,
                                                true> {
  DstChannelV operator()(SrcChannelV src) const {
    typedef typename unsigned_integral_max_value<DstChannelV>::value_type
    integer_t;
...
struct channel_converter_unsigned_integral_impl<SrcChannelV, DstChannelV, false,
                                                true> {
  DstChannelV operator()(SrcChannelV src) const {
    typedef typename unsigned_integral_max_value<SrcChannelV>::value_type
    integer_t;
...
struct channel_converter_unsigned_integral_impl<
    ...
    false> : public channel_converter_unsigned_integral_nondivisible<SrcChannelV,
                                                                     DstChannelV,
                                                                     SrcLessThanDst,
                                                                     mpl::greater<
                                                                         mpl::plus<
...
template <typename DstChannelV>
struct channel_converter_unsigned<
    bits32f, DstChannelV> : public std::unary_function<bits32f, DstChannelV> {
  DstChannelV operator()(bits32f x) const {
    typedef typename detail::unsigned_integral_max_value<
        DstChannelV>::value_type dst_integer_t;

    const bits32f convertedValue =
        x * channel_traits<DstChannelV>::max_value() + 0.5f;
    const bits32f clampedValue =
        std::min((bits32f)channel_traits<DstChannelV>::max_value(),
                 std::max((bits32f)channel_traits<DstChannelV>::min_value(),
                          convertedValue));
...
struct channel_converter_unsigned<
    SrcChannelV, bits32f> : public std::unary_function<SrcChannelV, bits32f> {
...
template <>
struct channel_converter_unsigned<
    bits32f, bits32f> : public std::unary_function<bits32f, bits32f> {
  bits32f operator()(bits32f x) const { return x; }
};

/// \brief 32 bit <-> float channel conversion
template <>
struct channel_converter_unsigned<bits32, bits32f> : public std::unary_function<
                                                         bits32, bits32f> {
...
/// \brief 32 bit <-> float channel conversion
template <>
struct channel_converter_unsigned<bits32f, bits32> : public std::unary_function<
                                                         bits32f, bits32> {
...
template <>
struct channel_convert_to_unsigned<bits8s> : public std::unary_function<bits8s,
                                                                        bits8> {
  typedef bits8 type;
...
template <>
struct channel_convert_to_unsigned<bits16s> : public std::unary_function<
                                                  bits16s, bits16> {
  typedef bits16 type;
...
template <>
struct channel_convert_to_unsigned<bits32s> : public std::unary_function<
                                                  bits32s, bits32> {
  typedef bits32 type;
...
template <>
struct channel_convert_from_unsigned<bits8s> : public std::unary_function<
                                                   bits8, bits8s> {
  typedef bits8s type;
...
template <>
struct channel_convert_from_unsigned<bits16s> : public std::unary_function<
                                                    bits16, bits16s> {
...
template <>
struct channel_convert_from_unsigned<bits32s> : public std::unary_function<
                                                    bits32, bits32s> {
...
/// This is important when invoking it
...
template <>
struct channel_multiplier_unsigned<bits8> : public std::binary_function<
                                                bits8, bits8, bits8> {
...
template <>
struct channel_multiplier_unsigned<bits16> : public std::binary_function<
...
template <>
struct channel_multiplier_unsigned<bits32f> : public std::binary_function<
