...
template <typename T> class TimeValue {
public:
  OfxTime time;
  T value;
...
/* An Interpolator determines the shape of the curve between key frames in an
 * animation */
...
public:
  ...
  /* Given two key frames and a time between them, get the appropriate value for
   * that time */
  virtual OfxStatus getValue(const TimeValueT &prev, const TimeValueT &next,
                             const OfxTime t, T &v) = 0;
};

template <typename T> Interpolator<T>::~Interpolator() {}

/* Reduces interpolation to a value between 0 and 1 */

/* Extend with a custom interpolate() function to provide different behaviors.*/
template <typename T> class GenericInterpolator : public Interpolator<T> {
  typedef TimeValue<T> TimeValueT;

public:
  virtual ~GenericInterpolator() = 0;

  OfxStatus getValue(const TimeValueT &prev, const TimeValueT &next,
                     const OfxTime t, T &v) {

    /* If the time is before the first key frame or both key frames are the
       same,
    ...
    if (t <= prev.time || prev.time == next.time) {
      ...
      v = (T)(prev.value + (next.value - prev.value) *
                               interpolate((double)(t - prev.time) /
                                           (double)(next.time - prev.time)));
    }
    return kOfxStatOK;
  }
...
template <typename T> GenericInterpolator<T>::~GenericInterpolator() {}

/* No smoothing */
template <typename T> class LinearInterpolator : public GenericInterpolator<T> {
public:
...
template <typename T> class SmoothInterpolator : public GenericInterpolator<T> {
public:
...
/* Ease out */
template <typename T> class FastInterpolator : public GenericInterpolator<T> {
public:
...
/* Ease in */
template <typename T> class SlowInterpolator : public GenericInterpolator<T> {
public:
