...
pythonObjectFromExpression(const std::string &expression) {
...
inline Type extractValueFromPythonObject(const boost::python::object &result) {
  ...
  try {
    ...
    BOOST_THROW_EXCEPTION(
        exception::Value()
        << exception::user() + "Can't extract value from expression."
        << exception::dev() +
...
inline Type extractValueFromExpression(const std::string &expression) {
  ...
  try {
    python::object result =
        expression_details::pythonObjectFromExpression(expression);
...
template <>
inline std::string
extractValueFromExpression<std::string>(const std::string &expression) {
  using namespace boost;
