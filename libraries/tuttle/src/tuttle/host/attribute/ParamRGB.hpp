...
public:
  ParamRGB(INode &effect, const std::string &name,
  ...
  void getValueAtTime(const OfxTime time, double &r, double &g, double &b) const
      OFX_EXCEPTION_SPEC;
  void setValue(const double &r, const double &g, const double &b,
                const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC;
  void setValueAtTime(const OfxTime time, const double &r, const double &g,
                      const double &b,
                      const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC;

  void setValue(const int &r, const int &g, const int &b,
                const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    setValue((double)r, (double)g, (double)b, change);
  }
  void setValueAtTime(const OfxTime time, const int &r, const int &g,
                      const int &b,
                      const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
