...
public:
  ParamDouble(INode &effect, const std::string &name,
              const ofx::attribute::OfxhParamDescriptor &descriptor,
              const std::size_t index = 0);

  double getDefault() const;

  /// @brief We reimplement some functions of AnimatedParamDouble, because
  /// without them,
  ...
  inline void
  setValueAtIndex(const std::size_t index, const double &v,
                  const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    AnimatedParamDouble::setValueAtIndex(index, v, change);
  }

  inline void setValueAtTimeAndIndex(
      const OfxTime time, const std::size_t index, const double &v,
      const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    AnimatedParamDouble::setValueAtTimeAndIndex(time, index, v, change);
  }
  /// @}

  /// @brief Re-implementation for Int, to allows an implicit conversion from
  /// Int to Double.
  /// @{
  ...
  inline void
  setValueAtIndex(const std::size_t index, const int &v,
                  const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    AnimatedParamDouble::setValueAtIndex(index, (double)v, change);
  }

  inline void setValueAtTimeAndIndex(
      const OfxTime time, const std::size_t index, const int &v,
      const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
