...
void ParamDouble2D::getValueAtTime(const OfxTime time, double &x,
                                   double &y) const OFX_EXCEPTION_SPEC {
  _controls.at<0>().getValueAtTime(time, x);
  _controls.at<1>().getValueAtTime(time, y);
}

void ParamDouble2D::setValue(const double &x, const double &y,
...
void ParamDouble2D::setValueAtTime(
    const OfxTime time, const double &x, const double &y,
