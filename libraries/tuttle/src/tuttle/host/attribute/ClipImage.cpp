...
ClipImage::ClipImage(INode &effect,
                     const ofx::attribute::OfxhClipImageDescriptor &desc)
    : Attribute(effect), tuttle::host::ofx::attribute::OfxhClipImage(desc),
      _isConnected(false), _continuousSamples(false),
      _memoryCache(core().getMemoryCache()) {
  getEditableProperties().addProperty(
      new ofx::property::String("TuttleFullName", 1, 1, getFullName().c_str()));
  getEditableProperties().addProperty(
      new ofx::property::String("TuttleIdentifier", 1, 1, ""));
}

ClipImage::ClipImage(const ClipImage &other)
    : Attribute(other), ofx::attribute::OfxhClipImage(other),
      _memoryCache(core().getMemoryCache()) {
  _name = other._name;
...
OfxTime ClipImage::getRemappedTime(const OfxTime time) const {
  ...
  /// Maybe we are not inside a compute (eg. overlay)... so datas are not
  /// initialized, etc.
  if (!getNode().hasData(time))
    return time; // throw an error?

  const OfxTime remappedTime = getNode()
                                   .getData(time)
                                   .getInputEdgeByClipName(getName(), time)
                                   .getOutTime();
  return remappedTime;
}
  ...
  if (!isOutput()) {
    if (!isConnected()) {
      BOOST_THROW_EXCEPTION(exception::Bug()
                            << exception::dev("fetchRegionOfDefinition on an "
                                              "unconnected input clip ! "
                                              "(clip: " +
                                              getFullName() + ")."));
    }
  ...
  default:
    BOOST_THROW_EXCEPTION(
        ...
        << exception::dev("fetchRegionOfDefinition unsupported on " +
                          mapNodeTypeEnumToString(getNode().getNodeType()) +
                          " node."));
  }
}
...
void ClipImage::setFrameRange(const double startFrame, const double endFrame) {
  getEditableProperties().setDoubleProperty(kOfxImageEffectPropFrameRange,
                                            startFrame, 0);
  getEditableProperties().setDoubleProperty(kOfxImageEffectPropFrameRange,
                                            endFrame, 1);
}
...
//
//  The unmaped frame range over which an output clip has images.
// this is applicable only to hosts and plugins that allow a plugin to change
// frame rates
void ClipImage::setUnmappedFrameRange(const double unmappedStartFrame,
                                      const double unmappedEndFrame) {
  getEditableProperties().setDoubleProperty(
      kOfxImageEffectPropUnmappedFrameRange, unmappedStartFrame, 0);
  getEditableProperties().setDoubleProperty(
      kOfxImageEffectPropUnmappedFrameRange, unmappedEndFrame, 1);
}
...
ClipImage::getImage(const OfxTime time, const OfxRectD *optionalBounds) {
  OfxRectD bounds;
  ...
  if (optionalBounds) {
    ...
    //	<< exception::dev("Uses optionalBounds not supported yet.") ); ///<
    //@todo tuttle: needs to be supported !
    // TUTTLE_TLOG( TUTTLE_TRACE, "on clip: " << getFullName() << "
  ...
  // TUTTLE_TLOG( TUTTLE_TRACE, "--> getImage <" << getFullName() << ">
  // connected on <" << getConnectedClipFullName() << "> with connection <" <<
  // isConnected() << "> isOutput <" << isOutput() << ">" << " bounds: " <<
  // bounds );
  boost::shared_ptr<Image> image =
      _memoryCache.get(getClipIdentifier(), realTime);
