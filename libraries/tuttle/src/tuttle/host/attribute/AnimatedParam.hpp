...
public:
  ...
  typedef typename std::vector<TimeValue<T> >::const_iterator
  TimeValueTConstIterator;

  AnimatedParam(INode &effect, const std::string &name,
                const ofx::attribute::OfxhParamDescriptor &descriptor,
                const std::size_t index)
      : Param(effect), OFX_PARAM(descriptor, name, effect.getParamSet(), index),
        _interpolator(new LinearInterpolator<T>()) {}

  AnimatedParam(const AnimatedParam<T, OFX_PARAM> &other)
      : Param(other), OFX_PARAM(other),
        _interpolator(other._interpolator->clone()) {}

  ~AnimatedParam() {}

  void setInterpolator(const ofx::attribute::EInterpolatorType interpolatorType)
      OFX_EXCEPTION_SPEC {
  ...
  void setValueAtTime(const OfxTime time, const T &v,
                      const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    TimeValue<T> new_tv;
  ...
  void integrate(const OfxTime time1, const OfxTime time2, T &) const
      OFX_EXCEPTION_SPEC {
    BOOST_THROW_EXCEPTION(ofx::OfxhException(kOfxStatErrMissingHostFeature));
  }
  ...
  void copy(const ofx::attribute::OfxhParam &p) OFX_EXCEPTION_SPEC {
    const AnimatedParam<T, OFX_PARAM> &param =
        dynamic_cast<const AnimatedParam<T, OFX_PARAM> &>(p);
  ...
  void getKeyIndex(const OfxTime time, const int direction, int &outIndex) const
      OFX_EXCEPTION_SPEC {
    TimeValue<T> temp;
    TimeValueTConstIterator it;
    ...
    if (direction == 0) {
      ...
      // Find the key frame directly before the match. If the match is the first
      // element, fail.
...
class AnimatedParamDouble
    : public AnimatedParam<double, ofx::attribute::OfxhParamDouble> {
  typedef AnimatedParam<double, ofx::attribute::OfxhParamDouble> Parent;

public:
  AnimatedParamDouble(INode &effect, const std::string &name,
                      const ofx::attribute::OfxhParamDescriptor &descriptor,
                      const std::size_t index)
      : Parent(effect, name, descriptor, index) {}

  inline void setValue(const double &v, const ofx::attribute::EChange change)
      OFX_EXCEPTION_SPEC {
    Parent::setValue(v, change);
  }

  inline void
  setValueAtTime(const OfxTime time, const double &v,
                 const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    Parent::setValueAtTime(time, v, change);
  }

  inline void setValue(const int &v, const ofx::attribute::EChange change)
      OFX_EXCEPTION_SPEC {
    Parent::setValue((double)v, change);
  }

  inline void
  setValueAtTime(const OfxTime time, const int &v,
                 const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    Parent::setValueAtTime(time, (double)v, change);
  }
};

typedef AnimatedParam<int, ofx::attribute::OfxhParamInteger>
AnimatedParamInteger;
}
}
}
