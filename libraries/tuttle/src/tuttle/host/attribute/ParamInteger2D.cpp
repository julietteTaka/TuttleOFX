...
ParamInteger2D::ParamInteger2D(
    INode &effect, const std::string &name,
    const ofx::attribute::OfxhParamDescriptor &descriptor)
    : Param(effect), ofx::attribute::OfxhMultiDimParam<ParamInteger, 2>(
                         descriptor, name, effect.getParamSet()) {
...
void ParamInteger2D::getValueAtTime(const OfxTime time, int &x, int &y) const
    OFX_EXCEPTION_SPEC {
  _controls.at<0>().getValueAtTime(time, x);
  _controls.at<1>().getValueAtTime(time, y);
}

void ParamInteger2D::setValue(const int &x, const int &y,
...
void ParamInteger2D::setValueAtTime(
    const OfxTime time, const int &x, const int &y,
