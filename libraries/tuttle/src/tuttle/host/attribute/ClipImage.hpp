...
protected:
  ...
  const ClipImage *_connectedClip; ///< @warning HACK ! to keep the connection
                                   ///@todo remove this !!!!
...
public:
  ...
  const std::string &getName() const {
    return ofx::attribute::OfxhAttributeAccessor::getName();
  }
  ...
  void setConnectedClip(const ClipImage &other) {
    if (isOutput()) {
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user(
                                   "You can't connect an output Clip !"));
    }
    if (!other.isOutput()) {
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user(
                                   "You can't connect to an input Clip !"));
    }
    ...
    getEditableProperties().setStringProperty("TuttleIdentifier",
                                              getClipIdentifier());
  }
  ...
  std::string getConnectedClipFullName() const {
    if (isOutput() || !isConnected() ||
        _connectedClip->getFullName().size() == 0) {
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user("Input clip " + getFullName() +
                                               " is not connected !"));
    }
   ...
   */
  void setUnmappedFrameRange(const double unmappedStartFrame,
  ...
  getImage(const OfxTime time, const OfxRectD *optionalBounds = NULL);
