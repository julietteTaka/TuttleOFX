...
ParamCustom::ParamCustom(INode &effect, const std::string &name,
...
void ParamCustom::getValueAtTime(const OfxTime time, std::string &v) const
    OFX_EXCEPTION_SPEC {
  v = _value; ///< @todo: in time !
}

void
ParamCustom::setValue(const std::string &v,
...
void ParamCustom::setValueFromExpression(const std::string &value,
                                         const ofx::attribute::EChange change)
    OFX_EXCEPTION_SPEC {
