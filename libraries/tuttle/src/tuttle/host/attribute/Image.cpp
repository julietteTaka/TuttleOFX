...
Image::Image(ClipImage &clip, const OfxTime time, const OfxRectD &bounds,
             const EImageOrientation orientation, const int rowDistanceBytes)
    : ofx::imageEffect::OfxhImage(
          clip, time) ///< this ctor will set basic props on the image
      ,
      _memorySize(0), _pixelBytes(clip.getPixelMemorySize()),
      _rowAbsDistanceBytes(0), _orientation(orientation),
  ...
  const OfxPointI dimensions = { _bounds.x2 - _bounds.x1,
                                 _bounds.y2 - _bounds.y1 };
  ...
  // row bytes
  _rowAbsDistanceBytes =
      rowDistanceBytes != 0 ? rowDistanceBytes : automaticRowSize;
  setIntProperty(kOfxImagePropRowBytes,
                 getOrientedRowDistanceBytes(eImageOrientationFromBottomToTop));
}
...
boost::uint8_t *
Image::getOrientedPixelData(const EImageOrientation orientation) {
  if (_orientation == orientation) {
    return getPixelData();
  } else {
    const std::ssize_t distance =
        getRowAbsDistanceBytes() * (_bounds.y2 - _bounds.y1 - 1);
...
boost::uint8_t *Image::pixel(const int x, const int y) {
  ...
  if ((x >= bounds.x1) && (x < bounds.x2) && (y >= bounds.y1) &&
      (y < bounds.y2)) {
    const int yOffset = (_orientation == eImageOrientationFromTopToBottom)
                            ? (y - bounds.y1)
                            : (y - bounds.y2);
    const int offset =
        yOffset * getRowAbsDistanceBytes() + (x - bounds.x1) * _pixelBytes;
...
void Image::copy(D_VIEW &dst, S_VIEW &src, const OfxPointI &dstCorner,
  ...
  if (src.width() >= (count.x - srcCorner.x) &&
    ...
    S_VIEW subSrc =
        subimage_view(src, srcCorner.x, srcCorner.y, count.x, count.y);
    D_VIEW subDst =
        subimage_view(dst, dstCorner.x, dstCorner.y, count.x, count.y);
