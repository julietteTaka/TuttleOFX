...
ParamChoice::ParamChoice(INode &effect, const std::string &name,
...
void ParamChoice::getValue(int &v) const OFX_EXCEPTION_SPEC { v = _value; }

void ParamChoice::getValueAtTime(const OfxTime time, int &v) const
    OFX_EXCEPTION_SPEC {
  v = _value; ///< @todo: in time !
}

void ParamChoice::setValue(const int &v, const ofx::attribute::EChange change)
    OFX_EXCEPTION_SPEC {
  const std::size_t nbKeys = this->getChoiceKeys().size();
  if (v < 0 || static_cast<std::size_t>(v) > nbKeys) {
    BOOST_THROW_EXCEPTION(exception::Value()
                          << exception::user() +
                                 "Choice index is out of range. Index is " + v +
                                 " for " + nbKeys + " values.");
  }
  _value = v;
  paramChanged(change);
...
void ParamChoice::setValueAtTime(const OfxTime time, const int &v,
...
void ParamChoice::setValueFromExpression(const std::string &value,
                                         const ofx::attribute::EChange change)
    OFX_EXCEPTION_SPEC {
  try {
