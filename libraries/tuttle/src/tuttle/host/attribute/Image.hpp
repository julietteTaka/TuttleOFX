...
public:
  enum EImageOrientation {
    eImageOrientationFromTopToBottom, //< Use image orientation from top to
                                      //bottom
    eImageOrientationFromBottomToTop  //< Use image orientation from bottom to
                                      //top
  };
...
protected:
  ...
  int _rowAbsDistanceBytes; ///< positive memory size for the distance between
                            ///rows
  OfxRectI _bounds;
...
public:
  Image(ClipImage &clip, const OfxTime time, const OfxRectD &bounds,
  ...
  void setPoolData(const memory::IPoolDataPtr &pData) {
    ...
    setPointerProperty(
        kOfxImagePropData,
        getOrientedPixelData(eImageOrientationFromBottomToTop)); // OpenFX
                                                                 // standard use
                                                                 // BottomToTop
  }
#endif
  ...
  std::size_t getNbComponents() const {
    return ofx::imageEffect::numberOfComponents(getComponentsType());
  }
  ...
  void getImage(boost::uint8_t *&outData, int &outWidth, int &outHeight,
                int &outRowSizeBytes, ofx::imageEffect::EBitDepth &outBitDepth,
  ...
  template <class VIEW_T> VIEW_T getGilView();
...
VIEW_T Image::getGilView(const EImageOrientation orientation) {
  ...
  return boost::gil::interleaved_view(
      std::abs(bounds.x2 - bounds.x1), std::abs(bounds.y2 - bounds.y1),
      (Pixel *)(this->getOrientedPixelData(orientation)),
      this->getOrientedRowDistanceBytes(orientation));
}

template <class VIEW_T> VIEW_T Image::getGilView() {
  // const OfxRectI rod = this->getROD();
  const OfxRectI bounds = this->getBounds();
