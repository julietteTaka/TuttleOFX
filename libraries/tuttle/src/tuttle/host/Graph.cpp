...
std::vector<INode *>
Graph::addConnectedNodes(const std::vector<NodeInit> &nodes) {
...
void Graph::renameNode(Graph::Node &node, const std::string &newUniqueName) {
  ...
  TUTTLE_TLOG(TUTTLE_INFO,
              "Graph::renameNode: from: " << node.getName()
                                          << " -> to: " << newUniqueName);
  {
    ...
    if (itNew != _nodes.end()) {
      BOOST_THROW_EXCEPTION(exception::Value()
                            << exception::user() + "New node name " +
                                   quotes(newUniqueName) + " already exists.");
    }
  }
  NodeMap::iterator it = _nodes.find(node.getName());
  if (it == _nodes.end()) {
    BOOST_THROW_EXCEPTION(exception::Value() << exception::user() + "Node " +
                                                    quotes(node.getName()) +
                                                    " is not in the graph.");
  }
...
void Graph::removeFromInternalGraph(Node &node) {
  // TUTTLE_TLOG( TUTTLE_INFO, "Graph::removeFromInternalGraph: " <<
  // node.getName() );
...
void Graph::connect(const std::string &outNode, const std::string &inNode,
...
void Graph::connect(const Node &outNode, const Attribute &inAttr) {
  _graph.connect(outNode.getName(), inAttr.getNode().getName(),
                 inAttr.getName());
}

void Graph::connect(const Attribute &outAttr, const Attribute &inAttr) {
  _graph.connect(outAttr.getNode().getName(), inAttr.getNode().getName(),
                 inAttr.getName());
}

void Graph::unconnect(const Attribute &outAttr, const Attribute &inAttr) {
  _graph.unconnect(outAttr.getNode().getName(), inAttr.getNode().getName(),
                   inAttr.getName());
}

namespace {
template <class TGraph> inline void graphConnectClips(TGraph &graph) {
  BOOST_FOREACH(typename TGraph::edge_descriptor ed, graph.getEdges()) {
    ...
    if (!vertexDest.isFake() && !vertexSource.isFake()) {
      ...
      targetNode.connect(sourceNode,
                         targetNode.getAttribute(edge.getInAttrName()));
    }
  }
}
}

void Graph::init() { graphConnectClips<InternalGraphImpl>(_graph); }
...
void Graph::replaceNodeConnections(const Node &fromNode, const Node &toNode) {
  BOOST_FOREACH(edge_descriptor e, _graph.getInEdges(_graph.getVertexDescriptor(
                                       fromNode.getName()))) {
#ifdef DEBUG
    // The current node should have the declared input attribute.
    _graph.targetInstance(e).getProcessNode().getAttribute(
        _graph.instance(e).getInAttrName());
#endif
    // Check that the new node has all needed attributes, before to start to
    // modify the graph.
  ...
  BOOST_FOREACH(edge_descriptor e, _graph.getInEdges(_graph.getVertexDescriptor(
    ...
    connect(_graph.sourceInstance(e).getProcessNode(),
            toNode.getAttribute(_graph.instance(e).getInAttrName()));
  }
  BOOST_FOREACH(
      edge_descriptor e,
      _graph.getOutEdges(_graph.getVertexDescriptor(fromNode.getName()))) {
    // fromNode == sourceInstance
    // So replace sourceInstance with toNode
    connect(toNode, _graph.targetInstance(e).getProcessNode().getAttribute(
                        _graph.instance(e).getInAttrName()));
  }
  unconnect(fromNode);
}
...
void Graph::computeGlobalHashAtTime(NodeHashContainer &outNodesHash,
                                    const OfxTime time,
                                    const NodeListArg &outputNodes) {
  const ComputeOptions options;
  graph::ProcessGraph procGraph(options, *this, outputNodes.getNodes());
...
bool Graph::compute(memory::MemoryCache &memoryCache,
                    const ComputeOptions &options) {
  return compute(memoryCache, NodeListArg(), options);
}

bool Graph::compute(memory::MemoryCache &memoryCache, const NodeListArg &nodes,
                    const ComputeOptions &options) {
...
Graph::getNodesByContext(const std::string &context) {
...
std::vector<Graph::Node *>
Graph::getNodesByPlugin(const std::string &pluginId) {
  std::vector<Node *> selectedNodes;
  for (NodeMap::iterator it = getNodes().begin(), itEnd = getNodes().end();
       it != itEnd; ++it) {
    try {
...
void Graph::exportDot(const std::string &filename,
                      const EDotExportLevel level) const {
  switch (level) {
