...
struct TimeRange {
  TimeRange()
      : _begin(std::numeric_limits<int>::min()),
        _end(std::numeric_limits<int>::max()), _step(1) {}
  TimeRange(const int frame) : _begin(frame), _end(frame), _step(1) {}
...
public:
  ...
  ComputeOptions()
      : _begin(std::numeric_limits<int>::min()),
        _end(std::numeric_limits<int>::max()), _abort(false) {
    init();
  }
...
public:
   ...
   * If an image file inside an image sequence failed to be loaded, we continue
   * to process other images of the sequence.
   */
  ...
  This &setVerboseLevel(const EVerboseLevel level) {
    tuttle::common::formatters::Formatter::get()->setLogLevel(
  ...
  This &setColorEnable(const bool enable = true) {
    enable ? tuttle::common::Color::get()->enable()
  ...
  /**
   * @brief For debug puposes only, you could force to call the process on all
   * identity nodes.
  ...
  bool getForceIdentityNodesProcess() const {
    return _forceIdentityNodesProcess;
  }

  /**
   * @brief The application would like to abort the process (from another
   * thread).
   */
