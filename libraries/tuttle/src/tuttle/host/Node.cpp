...
INode *createNode(const std::string &pluginName) {
  ofx::imageEffect::OfxhImageEffectPlugin *plug =
      core().getImageEffectPluginById(pluginName);

  if (!plug) {
    BOOST_THROW_EXCEPTION(
        exception::Logic() << exception::user("Plugin not found.")
  ...
  if (plug->supportsContext(kOfxImageEffectContextReader)) {
    ...
    BOOST_THROW_EXCEPTION(
        exception::Logic()
        << exception::user("Plugin contexts not supported by the host. (" +
                           pluginName + ")"));
  }

  if (!plugInst) {
    BOOST_THROW_EXCEPTION(exception::Logic()
                          << exception::user("Plugin not found. plugInst (" +
                                             pluginName + ")"));
  }
...
NodeInit::NodeInit(INode &node) { setNode(node); }
...
NodeInit &NodeInit::setParamExp(const std::string &paramName,
                                const std::string &paramExpValue) {
  _node->getParam(paramName)
      .setValueFromExpression(paramExpValue, ofx::attribute::eChangeUserEdited);
  return *this;
}
}
}
