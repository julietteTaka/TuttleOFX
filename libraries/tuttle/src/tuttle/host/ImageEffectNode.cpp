...
ImageEffectNode::ImageEffectNode(
    ...
    : tuttle::host::ofx::imageEffect::OfxhImageEffectNode(plugin, desc, context,
                                                          false) {
  populate();
...
ImageEffectNode::ImageEffectNode(const ImageEffectNode &other)
    : INode(other), tuttle::host::ofx::imageEffect::OfxhImageEffectNode(other) {
  populate();
  copyAttributesValues(
      other); // values need to be setted before the createInstanceAction !
  createInstanceAction();
}
...
bool ImageEffectNode::operator==(const INode &other) const {
  const ImageEffectNode *other_ptr =
      dynamic_cast<const ImageEffectNode *>(&other);
  if (other_ptr == NULL)
    return false;
...
void ImageEffectNode::connect(const INode &sourceEffect,
                              attribute::Attribute &attr) {
  const attribute::ClipImage &outputClip =
      dynamic_cast<const attribute::ClipImage &>(
          sourceEffect.getClip(kOfxImageEffectOutputClipName));
  attribute::ClipImage &inputClip = dynamic_cast<attribute::ClipImage &>(
      attr); // throw an exception if not a ClipImage attribute
...
attribute::Attribute &ImageEffectNode::getSingleInputAttribute() {
  ...
  if (clips.size() == 1) {
    ...
    const ofx::attribute::OfxhClipImageSet::ClipImageMap::iterator it(
        clipsMap.find(kOfxSimpleSourceAttributeName));
    if (it != clipsMap.end()) {
      inAttr = it->second;
    } else {
      inAttr = &clips[0];
...
tuttle::host::ofx::attribute::OfxhClipImage *ImageEffectNode::newClipImage(
    const tuttle::host::ofx::attribute::OfxhClipImageDescriptor &descriptor) {
...
const std::string &ImageEffectNode::getDefaultOutputFielding() const {
  /// our clip is pretending to be progressive PAL SD, so return
  /// kOfxImageFieldNone
 ...
 */
int ImageEffectNode::abort() { return 0; }
...
// vmessage
void ImageEffectNode::vmessage(const char *type, const char *id,
...
double ImageEffectNode::getProjectPixelAspectRatio() const { return 1.0; }

// we are only 25 frames
double ImageEffectNode::getEffectDuration() const { return 99999.0; }

/// This is called whenever a param is changed by the plugin so that
/// the recursive instanceChangedAction will be fed the correct frame
double ImageEffectNode::getFrameRecursive() const { return 0.0; }
 ...
 * @todo tuttle: to remove in the future.... size, pixelType, BitDepth, etc...
...
ofx::attribute::OfxhParam *ImageEffectNode::newParam(
    const ofx::attribute::OfxhParamDescriptor &descriptor) OFX_EXCEPTION_SPEC {
  ...
  try {
    if (descriptor.getParamType() == kOfxParamTypeString)
      ...
      BOOST_THROW_EXCEPTION(
          exception::Failed()
          << exception::user() + "Can't create param " + quotes(name) +
    ...
    BOOST_THROW_EXCEPTION(
        ofx::OfxhException(*boost::get_error_info<exception::ofxStatus>(e),
                           boost::diagnostic_information(e)));
  }
  catch (...) {
    BOOST_THROW_EXCEPTION(ofx::OfxhException(
        kOfxStatErrUnknown, boost::current_exception_diagnostic_information()));
  }
  return param;
}
...
void ImageEffectNode::progressStart(const std::string &message) {
  ...
  if (!(getContext() == kOfxImageEffectContextReader) &&
      !(getContext() == kOfxImageEffectContextWriter))
    TUTTLE_LOG_INFO(std::left << "       " << common::Color::get()->_green
                              << std::setw(TUTTLE_LOG_PLUGIN_NAME_WIDTH)
...
bool ImageEffectNode::progressUpdate(const double progress) {
  /*
  if( ( getContext() == kOfxImageEffectContextReader ) || ( getContext() ==
  kOfxImageEffectContextWriter ) )
          TUTTLE_LOG_INFO( "\r" << common::Color::get()->_std << "[" <<
  std::right << std::setw(3) << int(progress * 100) << "%] " << " " << std::left
  << std::flush );
  else
          TUTTLE_LOG_INFO( "\r" << common::Color::get()->_std << "[" <<
  std::right << std::setw(3) << int(progress * 100) << "%] " << std::left <<
  common::Color::get()->_green << getName() << common::Color::get()->_std <<
  std::flush );
          */
  return false;
}
...
double ImageEffectNode::timelineGetTime() { return 0; }
...
void ImageEffectNode::beginSequenceRenderAction(
    OfxTime startFrame, OfxTime endFrame, OfxTime step, bool interactive,
    OfxPointD renderScale) OFX_EXCEPTION_SPEC {
  OfxhImageEffectNode::beginSequenceRenderAction(startFrame, endFrame, step,
                                                 interactive, renderScale);
}
...
void ImageEffectNode::checkClipsConnections() const {
  for (ClipImageMap::const_iterator it = _clipImages.begin();
    ...
    const attribute::ClipImage &clip =
        dynamic_cast<const attribute::ClipImage &>(*(it->second));
    if (!clip.isOutput() && !clip.isConnected() &&
        !clip.isOptional()) // one non optional input clip is unconnected
    {
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user(
                                   "A non optional clip is unconnected ! (" +
                                   clip.getFullName() + ")"));
    }
  }
}
...
void ImageEffectNode::initComponents() {
  attribute::ClipImage &outputClip =
      dynamic_cast<attribute::ClipImage &>(getOutputClip());
  ...
  for (ClipImageMap::iterator it = _clipImages.begin(); it != _clipImages.end();
       ++it) {
    attribute::ClipImage &clip =
        dynamic_cast<attribute::ClipImage &>(*(it->second));
    if (!clip.isOutput() && clip.isConnected()) {
      // inputClipsFound = true;
      const attribute::ClipImage &linkClip = clip.getConnectedClip();
      mostChromaticComponents = findMostChromaticComponents(
          linkClip.getComponentsString(), mostChromaticComponents);
    }
  }
  // components
  for (ClipImageMap::iterator it = _clipImages.begin(); it != _clipImages.end();
    ...
    if (!clip.isOutput() && clip.isConnected()) {
      ...
      if (clip.isSupportedComponent(mostChromaticComponents))
        clip.setComponentsStringIfNotModifiedByPlugin(
            linkClip.getComponentsString());
    }
  }
  if (outputClip.isSupportedComponent(mostChromaticComponents))
    outputClip.setComponentsStringIfNotModifiedByPlugin(
...
void ImageEffectNode::initInputClipsPixelAspectRatio() {
  ...
  //	if( supportsMultipleClipPARs() )
  {
    for (ClipImageMap::iterator it = _clipImages.begin();
      ...
      if (!clip.isOutput() && clip.isConnected()) {
        ...
        TUTTLE_TLOG(TUTTLE_INFO, "[Clip] " << linkClip.getName()
  ...
  //	else
  //	{
  //		// @todo The plugin doesn't support PAR, the host should do the
  //conversions!
  //		//
  //http://openfx.sourceforge.net/Documentation/1.3/ofxProgrammingReference.html#ImageEffectsPixelAspectRatios
  //		// If a plugin does not accept clips of differing PARs, then the
  //host must resample all images fed to that effect to agree with the output's
  //PAR.
  //		// If a plugin does accept clips of differing PARs, it will need to
  //specify the output clip's PAR in the kOfxImageEffectActionGetClipPreferences
  //action.
  //
  //		// Convert images here ? Or introduce convert nodes into the
  //ProcessGraph?
  //		BOOST_ASSERT(false);
  //	}
  ...
  TUTTLE_TLOG(TUTTLE_INFO, "[Clip] support Multiple clip PAR = "
                               << supportsMultipleClipPARs());
  TUTTLE_TLOG(TUTTLE_INFO, "[Clip] number of clips = " << getNbClips());
  BOOST_ASSERT(inputPARs.size() <= 1 || supportsMultipleClipPARs() ||
               getNbClips() <= 2);
}
...
void ImageEffectNode::initPixelAspectRatio() {
  attribute::ClipImage &outputClip =
      dynamic_cast<attribute::ClipImage &>(getOutputClip());
  outputClip.setPixelAspectRatio(getOutputPixelAspectRatio(),
                                 ofx::property::eModifiedByHost);
}
...
void ImageEffectNode::maximizeBitDepthFromReadsToWrites() {
  ...
  for (ClipImageMap::iterator it = _clipImages.begin(); it != _clipImages.end();
    ...
    attribute::ClipImage &clip =
        dynamic_cast<attribute::ClipImage &>(*(it->second));
    if (!clip.isOutput() &&
        clip.isConnected()) // filter for clip.isMask() and clip.isOptional() ?
    {
      inputClipsFound = true;
      const attribute::ClipImage &linkClip = clip.getConnectedClip();
      biggestBitDepth = ofx::imageEffect::findDeepestBitDepth(
          linkClip.getBitDepthString(), biggestBitDepth);
    }
  }
  const std::string validBitDepth =
      this->bestSupportedBitDepth(biggestBitDepth);
  ...
  } else // multiple clip depth not supported (standard case)
  {
    if (inputClipsFound && // if we have an input clip
        validBitDepth ==
            kOfxBitDepthNone) // if we didn't found a valid bit depth value
    {
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user(
                                   "Pixel depth " + biggestBitDepth +
...
void ImageEffectNode::maximizeBitDepthFromWritesToReads() {
  // TUTTLE_TLOG( TUTTLE_INFO, "maximizeBitDepthFromWritesToReads: " <<
  // getName() );
  if (!supportsMultipleClipDepths()) {
    attribute::ClipImage &outputClip =
        dynamic_cast<attribute::ClipImage &>(getOutputClip());
    ...
    for (ClipImageMap::iterator it = _clipImages.begin();
      ...
      if (!clip.isOutput() && clip.isConnected()) {
        ...
        // TUTTLE_TLOG( TUTTLE_INFO, clip.getFullName() << "(" <<
        // clip.getBitDepth() << ")" << "-->" << linkClip.getFullName() << "("
        // << linkClip.getBitDepth() << ")" );
        if (linkClip.getNode().getNodeType() == INode::eNodeTypeImageEffect &&
            linkClip.getNode().asImageEffectNode().isSupportedBitDepth(
                outputClipBitDepthStr)) // need to be supported by the other
                                        // node
        {
          if (linkClip.getNode()
                  .asImageEffectNode()
                  .supportsMultipleClipDepths()) /// @todo tuttle: is this test
                                                 /// correct in all cases?
          {
            linkClip.setBitDepthStringIfUpper(outputClipBitDepthStr);
          } else {
            linkClip.setBitDepthStringIfUpperAndNotModifiedByPlugin(
                outputClipBitDepthStr);
          }
        }
        // TUTTLE_TLOG( TUTTLE_INFO, clip.getFullName() << "(" <<
        // clip.getBitDepth() << ")" << "-->" << linkClip.getFullName() << "("
      ...
      //{
      //	TUTTLE_TLOG( TUTTLE_INFO, clip.getFullName() << "(" <<
      //clip.getBitDepth() << ")" << ", unconnected ? " << clip.isConnected() <<
      //", output ? " << clip.isOutput() );
      //}
    }
  }
}
...
void ImageEffectNode::coutBitDepthConnections() const {
  ...
  for (ClipImageMap::const_iterator it = _clipImages.begin();
    ...
    // clip.getProperties().fetchStringProperty( kOfxImageEffectPropPixelDepth
    // );
    // const ofx::property::String& propComponent        =
    // clip.getProperties().fetchStringProperty( kOfxImageEffectPropComponents
    // );
    // const ofx::property::Double& propPixelAspectRatio =
    // clip.getProperties().fetchDoubleProperty( kOfxImagePropPixelAspectRatio
    // );
    /*
    TUTTLE_TLOG( TUTTLE_INFO, "-- " << "clip: " << " = " << clip.getFullName()
    );
    TUTTLE_TLOG( TUTTLE_INFO, "-- " << kOfxImageEffectPropPixelDepth << " = " <<
    propPixelDepth.getValue()
                                               << " : " << (
    propPixelDepth.getModifiedBy() == ofx::property::eModifiedByPlugin ?
    "(plugin)" : "(host)" ) );
    TUTTLE_TLOG( TUTTLE_INFO, "-- " << kOfxImageEffectPropComponents << " = " <<
    propComponent.getValue()
                                               << " : " << (
    propComponent.getModifiedBy() == ofx::property::eModifiedByPlugin ?
    "(plugin)" : "(host)" ) );
    TUTTLE_TLOG( TUTTLE_INFO, "-- " << kOfxImagePropPixelAspectRatio << " = " <<
    propPixelAspectRatio.getValue()
                                               << " : " << (
    propPixelAspectRatio.getModifiedBy() == ofx::property::eModifiedByPlugin ?
    "(plugin)" : "(host)" ) );
    */
    if (!clip.isOutput() && clip.isConnected()) {
      const attribute::ClipImage &linkClip = clip.getConnectedClip();
      TUTTLE_TLOG(TUTTLE_INFO, "[Bit Depth Connection] Connection between "
                                   << clip.getFullName() << " ("
                                   << clip.getBitDepth() << " bytes)"
                                   << " => " << linkClip.getFullName() << " ("
                                   << linkClip.getBitDepth() << " bytes).");
    }
  }
#endif
}
...
OfxRangeD ImageEffectNode::getDefaultTimeDomain() const {
  // TUTTLE_TLOG( TUTTLE_INFO, "- ImageEffectNode::getDefaultTimeDomain: " <<
  // getName() );
  OfxRangeD range;
...
OfxRangeD ImageEffectNode::computeTimeDomain() {
  ...
  TUTTLE_TLOG(TUTTLE_INFO, "[Time domain] getTimeDomain "
                               << quotes(getName())
  ...
  dynamic_cast<attribute::ClipImage *>(
      _clipImages[kOfxImageEffectOutputClipName])
      ->setFrameRange(range.min, range.max);
  dynamic_cast<attribute::ClipImage *>(
      _clipImages[kOfxImageEffectOutputClipName])
      ->setUnmappedFrameRange(range.min, range.max);
  return range;
}
...
void ImageEffectNode::setup2_reverse() { maximizeBitDepthFromWritesToReads(); }
...
void ImageEffectNode::beginSequence(graph::ProcessVertexData &vData) {
  ...
  beginSequenceRenderAction(vData._renderTimeRange.min,
                            vData._renderTimeRange.max, vData._step,
                            vData._interactive, vData._renderScale);
}

void ImageEffectNode::preProcess1(graph::ProcessVertexAtTimeData &vData) {
  TUTTLE_TLOG(TUTTLE_INFO, "[Pre Process 1] " << getName()
                                              << " at time: " << vData._time);
  ...
  TUTTLE_TLOG(TUTTLE_INFO, "[Pre Process 1] rod: x1:"
                               << rod.x1 << " y1:" << rod.y1 << " x2:" << rod.x2
                               << " y2:" << rod.y2);
}

void
ImageEffectNode::preProcess2_reverse(graph::ProcessVertexAtTimeData &vData) {
  //	TUTTLE_TLOG( TUTTLE_INFO, "preProcess2_finish: " << getName() << " at
  //time: " << vData._time );

  getRegionOfInterestAction(vData._time, vData._nodeData->_renderScale,
...
bool ImageEffectNode::isIdentity(const graph::ProcessVertexAtTimeData &vData,
                                 std::string &clip, OfxTime &time) const {
  time = vData._time;
  double par = this->getOutputClip().getPixelAspectRatio();
  if (par == 0.0)
    par = 1.0;
  OfxRectI renderWindow;
  renderWindow.x1 = boost::numeric_cast<int>(
      std::floor(vData._apiImageEffect._renderRoI.x1 / par));
  renderWindow.x2 = boost::numeric_cast<int>(
      std::ceil(vData._apiImageEffect._renderRoI.x2 / par));
  renderWindow.y1 =
  ...
  return isIdentityAction(time, vData._apiImageEffect._field, renderWindow,
                          vData._nodeData->_renderScale, clip);
}

void ImageEffectNode::preProcess_infos(
    const graph::ProcessVertexAtTimeData &vData, const OfxTime time,
  ...
  const std::size_t bitDepth =
      this->getOutputClip().getBitDepth(); // value in bytes
  const std::size_t nbComponents = getOutputClip().getNbComponents();
  nodeInfos._memory = std::ceil((rod.x2 - rod.x1) * (rod.y2 - rod.y1) *
                                nbComponents * bitDepth);
}
...
void ImageEffectNode::process(graph::ProcessVertexAtTimeData &vData) {
  ...
  */
  ...
  BOOST_FOREACH(const graph::ProcessVertexAtTimeData::
                        ProcessEdgeAtTimeByClipName::value_type &
                    inEdgePair,
                vData._inEdges) {
    ...
    TUTTLE_TLOG(TUTTLE_INFO, "[Node Process] out: " << inEdge->getOut()
                                                    << " -> in "
                                                    << inEdge->getIn());
    memory::CACHE_ELEMENT imageCache(
        memoryCache.get(clip.getClipIdentifier(), outTime));
    if (imageCache.get() == NULL) {
      BOOST_THROW_EXCEPTION(exception::Memory()
                            << exception::dev() + "Input attribute " +
                                   quotes(clip.getFullName()) + " at time " +
                                   vData._time +
  ...
  BOOST_FOREACH(ClipImageMap::value_type & i, _clipImages) {
    attribute::ClipImage &clip =
        dynamic_cast<attribute::ClipImage &>(*(i.second));
    if (clip.isOutput()) {
      TUTTLE_TLOG(TUTTLE_INFO, "[Node Process] "
                                   << vData._apiImageEffect._renderRoI);
      memory::CACHE_ELEMENT imageCache(new attribute::Image(
          clip, vData._time, vData._apiImageEffect._renderRoI,
          attribute::Image::eImageOrientationFromBottomToTop, 0));
      imageCache->setPoolData(
          core().getMemoryPool().allocate(imageCache->getMemorySize()));
    ...
    //			const INode::TimesSet& timesSet = timesSetMap[clip.getName()]; ///
    //@todo tuttle: for each edge use edge._outTime
    //			if( timesSet.size() == 0 )
    ...
    //)
    //			{
    //				memory::CACHE_ELEMENT imageCache( memoryCache.get(
    //clip.getClipIdentifier(), inTime ) );
    //				if( imageCache.get() == NULL )
    //				{
    //					BOOST_THROW_EXCEPTION(
    //exception::Memory()
    //						<< exception::dev() + "Input attribute " + quotes(
    //clip.getFullName() ) + " at time " + vData._time + " not in memory cache
    //(identifier:" + quotes( clip.getClipIdentifier() ) + ")." );
    //				}
    //				allNeededDatas.push_back( imageCache );
    //			}
    //		}
  ...
  renderAction(vData._time, vData._apiImageEffect._field, renderWindow,
  ...
  BOOST_FOREACH(const graph::ProcessVertexAtTimeData::
    ...
    */
    ...
    if (imageCache.get() == NULL) {
      BOOST_THROW_EXCEPTION(exception::Memory()
                            << exception::dev() + "Clip " +
                                   quotes(clip.getFullName()) +
                                   " not in memory cache (identifier: " +
                                   quotes(clip.getClipIdentifier()) +
                                   ", time: " + outTime + ").");
    }
    // TUTTLE_TLOG_TRACE( ">>>>> - ImageEffectNode => releaseReference: " <<
    // imageCache->getFullName() );
    imageCache->releaseReference(
        ofx::imageEffect::OfxhImage::eReferenceOwnerHost);
  }

  // declare future usages of the output
  BOOST_FOREACH(ClipImageMap::value_type & item, _clipImages) {
    attribute::ClipImage &clip =
        dynamic_cast<attribute::ClipImage &>(*(item.second));
    if (!clip.isOutput() && !clip.isConnected())
      continue;

    if (clip.isOutput()) {
      memory::CACHE_ELEMENT imageCache =
          memoryCache.get(clip.getClipIdentifier(), vData._time);
      if (imageCache.get() == NULL) {
        BOOST_THROW_EXCEPTION(exception::Memory()
                              << exception::dev() + "Clip " +
                                     quotes(clip.getFullName()) +
      ...
      const std::size_t realOutDegree =
          vData._outDegree - vData._isFinalNode; // final nodes have a
                                                 // connection to the fake
                                                 // output node.
      TUTTLE_TLOG(TUTTLE_INFO, "[Node Process] Declare future usages: "
                                   << clip.getClipIdentifier()
                                   << ", add reference: " << realOutDegree);
      if (realOutDegree > 0) {
        // TUTTLE_TLOG_TRACE( ">>>>> + ImageEffectNode => addReference: " <<
        // imageCache->getFullName() );
        imageCache->addReference(
            ofx::imageEffect::OfxhImage::eReferenceOwnerHost,
            realOutDegree); // add a reference on this node for each future
                            // usages
      }
    }
    //		else
    //		{
    //			const INode::TimesSet& timesSet = timesSetMap[clip.getName()]; ///
    //@todo tuttle: use edge._timesNeeded
    //			if( timesSet.size() == 0 )
    //				continue; // the plugin don't use this input (is it allowed by the
    //standard?)
    //			BOOST_FOREACH( const INode::TimesSet::value_type& inTime, timesSet
    //)
    //			{
    //				//TUTTLE_TLOG_VAR2( TUTTLE_INFO, clip.getIdentifier(),
    //clip.getFullName() );
    //				memory::CACHE_ELEMENT imageCache = memoryCache.get(
    //clip.getClipIdentifier(), inTime );
    //				if( imageCache.get() == NULL )
    ...
    //					BOOST_THROW_EXCEPTION(
    //exception::Memory()
    //						<< exception::dev() + "Clip " + quotes( clip.getFullName() ) +
    //" not in memory cache (identifier:" + quotes( clip.getClipIdentifier() ) +
    //")." );
    //				}
    //				imageCache->releaseReference();
    //			}
...
std::ostream &ImageEffectNode::print(std::ostream &os) const {
  ...
  os << "Clips:" << std::endl;
  for (ImageEffectNode::ClipImageMap::const_iterator
           it = v._clipImages.begin(),
           itEnd = v._clipImages.end();
       it != itEnd; ++it) {
    os << "  * " << it->second->getName() << std::endl;
  }
  os << "Params:" << std::endl;
  for (ImageEffectNode::ParamVector::const_iterator
           it = v._paramVector.begin(),
           itEnd = v._paramVector.end();
       it != itEnd; ++it) {
    os << "  * " << it->getName() << " (" << it->getLabel()
       << "): " << it->displayValues(os) << std::endl;
  }
  os << "______________________________________________________________________"
        "__________" << std::endl;
  return os;
...
void ImageEffectNode::debugOutputImage(const OfxTime time) const {
  ...
  boost::shared_ptr<Image> image =
      memoryCache.get(this->getName() + "." kOfxOutputAttributeName, time);

  // big hack, for debug...
  image->debugSaveAsPng("data/debug/" + boost::lexical_cast<std::string>(time) +
                        "_" + this->getName() + ".png");
#endif
}
}
