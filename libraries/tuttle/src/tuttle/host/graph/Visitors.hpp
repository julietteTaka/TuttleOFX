...
public:
  CycleDetector(bool &hasCycle) : _hasCycle(hasCycle) { _hasCycle = false; }
...
template <class TGraph> class MarkUsed : public boost::default_dfs_visitor {
public:
  ...
  MarkUsed(TGraph &graph) : _graph(graph) {}
...
template <class TGraph> class Test_dfs : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;
  typedef typename TGraph::Edge ProcessEdge;

  Test_dfs(TGraph &graph) : _graph(graph) {
    TUTTLE_TLOG(TUTTLE_TRACE, "Test_dfs");
  }

  ~Test_dfs() { TUTTLE_TLOG(TUTTLE_TRACE, "~Test_dfs"); }
...
public:
  ...
  void discover_vertex(VertexDescriptor v, Graph &g) {
    TUTTLE_TLOG(TUTTLE_TRACE, "discover_vertex "
                                  << g[v] << " outedges: " << out_degree(v, g));
  }
