...
ProcessVertexAtTime::ProcessVertexAtTime(const ProcessVertex &v,
                                         const OfxTime t)
    : IVertex(v), _clipName(v._name), _data(v._data, t) {
  this->_name += "_at_" + boost::lexical_cast<std::string>(t);
}

ProcessVertexAtTime::ProcessVertexAtTime(const ProcessVertexAtTime &other)
    : IVertex(other), _clipName(other._clipName), _data(other._data) {}
...
std::ostream &ProcessVertexAtTime::exportDotDebug(std::ostream &os) const {
  ...
  if (!isFake()) {
    ...
    case INode::eNodeTypeImageEffect: {
      const ImageEffectNode *ieNode =
          dynamic_cast<const ImageEffectNode *>(&getProcessNode());
      s << subDotEntry("bitdepth", ieNode->getOutputClip().getBitDepthString());
      s << subDotEntry("component",
                       ieNode->getOutputClip().getComponentsString());
      {
      ...
      s << subDotEntry(
               "output RoD",
               ieNode->getOutputClip().fetchRegionOfDefinition(_data._time));
  ...
  s << subDotEntry("timeDomain", (boost::format("[%1%:%2%]") %
                                  _data._nodeData->_timeDomain.min %
                                  _data._nodeData->_timeDomain.max).str());
...
std::ostream &operator<<(std::ostream &os, const ProcessVertexAtTime &v) {
  // Lexical_cast is used, here, to prevent an error ("error C2593: 'operator
  // <<' is ambiguous") with ostream operator and OFXTime in msvc10-express (at
  // least).
