...
IVertex::IVertex(const std::string &name)
    : _name(name), _processNode(NULL), _fake(true), _used(true), _id(_count++) {
}

IVertex::IVertex(const std::string &name, INode &processNode)
    : _name(name), _processNode(&processNode), _fake(false), _used(true),
      _id(_count++) {}
...
std::ostream &IVertex::exportDotDebug(std::ostream &os) const {
  ...
  if (!isFake()) {
    ...
    s << subDotEntry("bitdepth",
                     static_cast<const ImageEffectNode &>(getProcessNode())
                         .getOutputClip()
                         .getBitDepthString());
  }
