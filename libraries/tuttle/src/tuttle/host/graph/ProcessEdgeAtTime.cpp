...
ProcessEdgeAtTime::ProcessEdgeAtTime(const ProcessVertexAtTime::Key &out,
                                     const ProcessVertexAtTime::Key &in,
                                     const std::string &inAttrName)
    : IEdge(out.getName(), in.getName(), inAttrName), _inTime(in.getTime()),
      _outTime(out.getTime()) {}

ProcessEdgeAtTime::ProcessEdgeAtTime(const ProcessEdge &e) : IEdge(e) {}

ProcessEdgeAtTime::ProcessEdgeAtTime(const ProcessEdgeAtTime &other)
    : IEdge(other), _inTime(other._inTime), _outTime(other._outTime) {}
