...
public:
  ProcessVertexAtTimeInfo() : _nodes(0), _memory(0) {}
...
public:
  ProcessVertexAtTimeData()
      : _nodeData(NULL), _time(0), _isFinalNode(false), _outDegree(0),
        _inDegree(0) {
    _localInfos._nodes =
        1; // local infos can contain only 1 node by definition...
  }

  ProcessVertexAtTimeData(const ProcessVertexData &nodeData, const OfxTime time)
      : _nodeData(&nodeData), _time(time), _isFinalNode(false), _outDegree(0),
        _inDegree(0) {
    _localInfos._nodes =
        1; // local infos can contain only 1 node by definition...
  }

  ProcessVertexAtTimeData(const This &other) : _nodeData(other._nodeData) {
    operator=(other);
  }
  ~ProcessVertexAtTimeData() {}
  ...
  inline const ProcessEdgeAtTime &
  getInputEdgeByClipName(const std::string &inputClipName,
                         const OfxTime time) const {
    Key k(inputClipName, time);
    ProcessEdgeAtTimeByClipName::const_iterator it = _inEdges.find(k);
    if (it == _inEdges.end())
      BOOST_THROW_EXCEPTION(exception::Bug()
                            << exception::dev() + "No input clip " +
                                   quotes(inputClipName) + " at time " + time +
                                   " inside input edges map.");
    return *(it->second);
  }
...
public:
  ...
  struct ImageEffect {
    ...
    typedef std::map<tuttle::host::ofx::attribute::OfxhClipImage *, OfxRectD>
    MapClipImageRod;
    MapClipImageRod _inputsRoI; ///<< in which the plugin set the RoI it needs
                                ///for each input clip
