...
std::ostream &operator<<(std::ostream &os,
  ...
  std::ostringstream s;
  s << "memory:" << std::setprecision(2) << infos._memory * 1e-6 << "Mo ("
    << infos._memory << ")" << std::endl;
  os << s.str();
  return os;
}

std::ostream &operator<<(std::ostream &os,
                         const ProcessVertexAtTimeData &vData) {
  ...
  case INode::eNodeTypeImageEffect:
    ...
    BOOST_FOREACH(const ProcessVertexAtTimeData::ImageEffect::MapClipImageRod::
                          value_type &
                      item,
