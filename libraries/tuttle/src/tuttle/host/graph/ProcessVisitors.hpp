...
template <class TGraph> inline void connectClips(TGraph &graph) {
  BOOST_FOREACH(typename TGraph::edge_descriptor ed, graph.getEdges()) {
    ...
    TUTTLE_TLOG(TUTTLE_TRACE, "[Connect Clips] " << vertexOutput << " -> "
                                                 << vertexInput);
    ...
    if (!vertexOutput.isFake() && !vertexInput.isFake()) {
      ...
      inputNode.connect(outputNode,
                        inputNode.getAttribute(edge.getInAttrName()));
    }
  }
}
...
template <class TGraph> class Setup1 : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  Setup1(TGraph &graph) : _graph(graph) {}
...
template <class TGraph> class Setup2 : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  Setup2(TGraph &graph) : _graph(graph) {}
...
template <class TGraph> class Setup3 : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  Setup3(TGraph &graph) : _graph(graph) {}
  template <class VertexDescriptor, class Graph>
  void finish_vertex(VertexDescriptor v, Graph &g) {
    Vertex &vertex = _graph.instance(v);
...
template <class TGraph> class TimeDomain : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  TimeDomain(TGraph &graph) : _graph(graph) {}
  ...
  void finish_vertex(VertexDescriptor vd, Graph &g) {
    ...
    vertex.getProcessData()._timeDomain =
        vertex.getProcessNode().computeTimeDomain();
    TUTTLE_TLOG(TUTTLE_TRACE, "[Time Domain] min: "
                                  << vertex.getProcessData()._timeDomain.min
                                  << ", max: "
                                  << vertex.getProcessData()._timeDomain.max);
  }
...
public:
  ...
  ComputeHashAtTime(TGraph &graph, NodeHashContainer &outNodesHash,
                    const OfxTime time)
      : _graph(graph), _outNodesHash(outNodesHash), _time(time) {
  ...
  void finish_vertex(VertexDescriptor vd, Graph &g) {
    Vertex &vertex = _graph.instance(vd);
    TUTTLE_TLOG(TUTTLE_TRACE, "[Compute Hash At Time] finish vertex "
                                  << vertex);

    if (vertex.isFake())
      return;

    const std::size_t localHash =
        vertex.getProcessNode().getLocalHashAtTime(_time);
    ...
    BOOST_FOREACH(const edge_descriptor & ed, _graph.getOutEdges(vd)) {
      ...
      const std::size_t inputGlobalHash =
          _outNodesHash.getHash(inputVertex.getKey());
    ...
    BOOST_FOREACH(const typename InputsHash::value_type & inputGlobalHash,
                  inputsGlobalHash) {
      // TUTTLE_TLOG_VAR2( TUTTLE_TRACE, inputGlobalHash.first,
      // inputGlobalHash.second );
      boost::hash_combine(seed, inputGlobalHash.first.getName()); // name of the
                                                                  // input clip
                                                                  // connected
 ...
 *
 * Compute kOfxImageEffectActionGetFramesNeeded for each node if
 *kOfxImageEffectPropTemporalClipAccess is true.
 */
template <class TGraph> class DeployTime : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;
  typedef typename TGraph::Edge Edge;
  ...
  DeployTime(TGraph &graph, const OfxTime time) : _graph(graph), _time(time) {
  ...
  void discover_vertex(VertexDescriptor v, Graph &g) {
    ...
    BOOST_FOREACH(const edge_descriptor & ed, _graph.getInEdges(v)) {
      ...
      BOOST_FOREACH(const typename Edge::TimeMap::value_type & timesNeeded,
                    edge._timesNeeded) {
        vertex._data._times.insert(timesNeeded.second.begin(),
                                   timesNeeded.second.end());
        //				std::cout << "--- insert vertex: ";
        //				std::copy(
        //(*timesNeeded).second.begin(),
        //				           (*timesNeeded).second.end(),
    ...
    BOOST_FOREACH(const OfxTime t, vertex._data._times) {
      TUTTLE_TLOG(TUTTLE_TRACE, "[Deploy Time] time: "
                                    << boost::lexical_cast<std::string>(t));
      INode::ClipTimesSetMap mapInputsTimes =
          vertex.getProcessNode().getTimesNeeded(t);
      //			BOOST_FOREACH( const
      //INode::InputsTimeMap::value_type& v, mapInputsTimes )
      //			{
      ...
      BOOST_FOREACH(const edge_descriptor & ed, _graph.getOutEdges(v)) {
        ...
        //				TUTTLE_TLOG( TUTTLE_INFO, "-- inEdge "<<t<<":
        //" << edge );
        //				const Vertex& input = _graph.targetInstance(
        //ed );
        ...
        //				           mapInputsTimes[input.getName()+"."
        //kOfxOutputAttributeName].end(),
        //						   std::ostream_iterator<OfxTime>(std::cout,
        //",") );
...
template <class TGraph> struct IdentityNodeConnection {
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;
...
public:
  RemoveIdentityNodes(TGraph &graph,
      ...
      : _graph(graph), _toRemove(toRemove) {}

  template <class VertexDescriptor, class Graph>
  void finish_vertex(VertexDescriptor vd, Graph &g) {
    Vertex &vertex = _graph.instance(vd);

    TUTTLE_TLOG(TUTTLE_TRACE, "[Remove identity nodes] finish vertex "
                                  << vertex);
    if (vertex.isFake())
      return;
    ...
    OfxTime atTime;
    if (vertex.getProcessNode().isIdentity(vertex.getProcessDataAtTime(),
                                           inputClip, atTime)) {
      try {
        ...
        if (!inputClip.empty()) {
          ...
          if (!clipFound) {
            ...
            BOOST_THROW_EXCEPTION(exception::Bug()
                                  << exception::dev() +
                                         "Plugin declares itself as identity, "
                                         "but gives a non existing input clip. "
                                         "(node: " +
                                         vertex.getName() + ", input clip:" +
                                         inputClip + ", existing clips: (" +
                                         existingClips.str() + "))");
          }
          if (!clipFoundAtTime) {
            ...
            BOOST_THROW_EXCEPTION(
                exception::Bug() << exception::dev() +
                                        "Plugin declares itself as identity, "
                                        "but gives a time on the input clip "
                                        "different from declared framesNeeded. "
                                        "(node: " +
                                        vertex.getName() + ", input clip:" +
                                        inputClip + ", time:" + atTime +
                                        ", framesNeeded:(" +
                                        framesNeeded.str() + "))");
          }
        ...
        TUTTLE_TLOG(TUTTLE_TRACE, "isIdentity => " << vertex.getName() << " - "
                                                   << inputClip << " at time "
                                                   << atTime);
        ...
        e << exception::user() + "Error on the node " +
                 quotes(vertex.getName()) +
                 ". It fails to declare itself as an identity node."
 ...
 */
template <class TGraph>
void removeIdentityNodes(
    TGraph &graph,
    const std::vector<IdentityNodeConnection<TGraph> > &nodesToRemove) {
  //	TUTTLE_IF_DEBUG(
  //		TUTTLE_LOG_VAR( TUTTLE_TRACE, toRemove.size() );
  //		BOOST_FOREACH( const IdentityNodeConnection<TGraph>& connection,
  //toRemove )
  //		{
  //			TUTTLE_LOG( TUTTLE_TRACE, connection._identityVertex );
  //			TUTTLE_LOG( TUTTLE_TRACE, "IN: "
  //				<< connection._identityVertex << "::" <<
  //connection._input._inputClip
  //				<< " <<-- "
  //				<< connection._input._srcNode << "::"
  //kOfxOutputAttributeName );
  //
  //			BOOST_FOREACH( const typename
  //IdentityNodeConnection<TGraph>::OutputClipConnection& outputClipConnection,
  //connection._outputs )
  //			{
  ...
  //outputClipConnection._dstNodeClip );
  //			}
  //		}
  //	);
  ...
  typedef boost::unordered_map<
      VertexKey, const IdentityNodeConnection<TGraph> *> IdentityMap;
  ...
  BOOST_FOREACH(const IdentityNodeConnection<TGraph> & connection,
    ...
    if (connection._input._inputClip.empty()) {
      ...
      TUTTLE_TLOG_TRACE(
          boost::lexical_cast<std::string>(connection._identityVertex));
      TUTTLE_TLOG_TRACE(
          "IN: " << boost::lexical_cast<std::string>(connection._identityVertex)
                 << "::" << boost::lexical_cast<std::string>(
                                connection._input._inputClip) << " <<-- "
                 << boost::lexical_cast<std::string>(connection._input._srcNode)
                 << "::" kOfxOutputAttributeName);
      const typename TGraph::VertexKey *searchIn =
          &(connection._input._srcNode);
      {
      ...
      const typename TGraph::vertex_descriptor descIn =
          graph.getVertexDescriptor(in);

      // replace all input/output connections of the identity node by one edge
      BOOST_FOREACH(
          const typename IdentityNodeConnection<TGraph>::OutputClipConnection &
        ...
        const typename TGraph::vertex_descriptor descOut =
            graph.getVertexDescriptor(out);

        const typename TGraph::Edge e(in, out, inAttr);
        graph.addEdge(descOut, descIn, e);
      }
    }
    // Warning: We don't remove the vertex itself to not invalidate
    // vertex_descriptors but only remove edges.
    //		graph.removeVertex(
    //graph.getVertexDescriptor(connection._identityVertex) );
  ...
  if (disabledBranch) {
    ...
    // These unconnected branches have been created by identity nodes without
    // source clip.
  }
}

template <class TGraph> class PreProcess1 : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  PreProcess1(TGraph &graph) : _graph(graph) {}
...
template <class TGraph> class PreProcess2 : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  PreProcess2(TGraph &graph) : _graph(graph) {}
...
public:
  ...
  OptimizeGraph(TGraph &graph) : _graph(graph) {}
  ...
  void finish_vertex(VertexDescriptor v, const Graph &g) {
    ...
    if (!vertex.isFake()) {
      ...
      vertex.getProcessNode().preProcess_infos(vertex.getProcessDataAtTime(),
                                               procOptions._time,
                                               procOptions._localInfos);
    }
    ...
    //		BOOST_FOREACH( const edge_descriptor& ie, in_edges( v,
    //_graph.getGraph() ) )
    //		{
...
template <class TGraph> class Process : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  Process(TGraph &graph, memory::IMemoryCache &cache)
      : _graph(graph), _cache(cache), _result(NULL) {}

  Process(TGraph &graph, memory::IMemoryCache &cache,
          memory::IMemoryCache &result)
      : _graph(graph), _cache(cache), _result(&result) {}

  /**
   * Set a MemoryCache object to accumulate output nodes buffers.
   */
  void setOutputMemoryCache(memory::IMemoryCache &result) { _result = &result; }
  ...
  void finish_vertex(VertexDescriptor v, Graph &g) {
    ...
    TUTTLE_TLOG(TUTTLE_TRACE, "[Process] "
                                  << quotes(vertex._name) << " "
                                  << vertex._data._time << " took: " << t2 - t1
                                  << " (cumul: " << _cumulativeTime << ")"
                                  << vertex);

    if (_result && vertex.getProcessDataAtTime()._isFinalNode) {
      memory::CACHE_ELEMENT img = _cache.get(
          vertex._clipName + "." kOfxOutputAttributeName, vertex._data._time);
      if (!img.get()) {
        BOOST_THROW_EXCEPTION(exception::Logic()
                              << exception::user() + "Output buffer not found "
                                                     "in memoryCache at the "
...
template <class TGraph> class PostProcess : public boost::default_dfs_visitor {
public:
  typedef typename TGraph::GraphContainer GraphContainer;
  typedef typename TGraph::Vertex Vertex;

  PostProcess(TGraph &graph) : _graph(graph) {}
