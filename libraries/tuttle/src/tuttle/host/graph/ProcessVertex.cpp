...
ProcessVertex::ProcessVertex(const std::string &name) : IVertex(name) {}

ProcessVertex::ProcessVertex(const UVertex &v)
    : IVertex(v), _data(v.getProcessNode().getNodeType()) {}

ProcessVertex::ProcessVertex(const ProcessVertex &v)
    : IVertex(v), _data(v._data) {}
...
std::ostream &ProcessVertex::exportDotDebug(std::ostream &os) const {
  ...
  s << subDotEntry("timeDomain",
                   (boost::format("[%1%:%2%]") % _data._timeDomain.min %
                    _data._timeDomain.max).str());
  s << subDotEntry("allTimes", _data._times.size());
  std::ostringstream times;
  std::copy(_data._times.begin(), _data._times.end(),
