...
namespace detail {
template <class T> struct DotEntry {
  DotEntry(const std::string &key, const T &value) : _key(key), _value(value) {}
  const std::string &_key;
  const T &_value;
  template <class TT>
  friend std::ostream &operator<<(std::ostream &os, const DotEntry<TT> &d);
};
template <class T> struct SubDotEntry : public DotEntry<T> {
