...
ProcessGraph::ProcessGraph(const ComputeOptions &options, Graph &userGraph,
                           const std::list<std::string> &outputNodes)
    : _instanceCount(userGraph.getInstanceCount()), _options(options) {
...
ProcessGraph::VertexAtTime::Key
ProcessGraph::getOutputKeyAtTime(const OfxTime time) {
  return VertexAtTime(Vertex(_outputId), time).getKey();
}
ProcessGraph::InternalGraphAtTimeImpl::vertex_descriptor
ProcessGraph::getOutputVertexAtTime(const OfxTime time) {
  return _renderGraphAtTime.getVertexDescriptor(getOutputKeyAtTime(time));
}

/**
 * @brief After copying Vertices, we need to duplicate Nodes and relink Vertices
 * with new Nodes.
 */
void ProcessGraph::relink() {
  _renderGraph.removeUnconnectedVertices(
      _renderGraph.getVertexDescriptor(_outputId));

  BOOST_FOREACH(InternalGraphImpl::vertex_descriptor vd,
                _renderGraph.getVertices()) {
    Vertex &v = _renderGraph.instance(vd);

    // fake node has no ProcessNode
    if (!v.isFake()) {
#ifdef PROCESSGRAPH_USE_LINK
      tuttle::host::INode &origNode =
          v.getProcessNode(); // pointer of the copied graph, we don't owns it !
#else
      const tuttle::host::INode &origNode =
          v.getProcessNode(); // pointer of the copied graph, we don't owns it !
#endif
      ...
      if (it != _nodes.end()) {
        ...
        newNode = &origNode;
        _nodes[key] =
            dynamic_cast<Node *>(newNode); // link to the original node
#else
        newNode = origNode.clone();
        /// @todo tuttle: no dynamic_cast here, _nodes must use
        /// tuttle::host::Node
        _nodes.insert(key,
                      dynamic_cast<Node *>(newNode)); // owns the new pointer
#endif
      }
...
/*
   void removeVertexAndReconnectTo( const VertexDescriptor& v, const
   VertexDescriptor& other )
   {
...
public:
        ...
        inline bool operator()( const edge_descriptor& ed1, const
edge_descriptor& ed2 ) const
        {
                const Vertex& v1 = _renderGraph.targetInstance( ed1 );
                const Vertex& v2 = _renderGraph.targetInstance( ed2 );

                bool res= v1.getProcessDataAtTime()._globalInfos._memory <
v2.getProcessDataAtTime()._globalInfos._memory;
//		TUTTLE_LOG_VAR2(v1.getName(),
v1.getProcessDataAtTime()._globalInfos._memory);
//		TUTTLE_LOG_VAR2(v2.getName(),
v2.getProcessDataAtTime()._globalInfos._memory);
//		TUTTLE_LOG_VAR(res);
                return res;
        }
...
void ProcessGraph::bakeGraphInformationToNodes(
    InternalGraphAtTimeImpl &_renderGraphAtTime) {
  BOOST_FOREACH(const InternalGraphAtTimeImpl::vertex_descriptor vd,
                _renderGraphAtTime.getVertices()) {
    VertexAtTime &v = _renderGraphAtTime.instance(vd);
    ProcessVertexAtTimeData &vData = v.getProcessDataAtTime();

    TUTTLE_TLOG(TUTTLE_INFO,
                "[bake graph information to nodes] node: " << v.getName());
    ...
    BOOST_FOREACH(const InternalGraphAtTimeImpl::edge_descriptor ed,
                  _renderGraphAtTime.getInEdges(vd)) {
      const ProcessEdgeAtTime *e = &_renderGraphAtTime.instance(ed);
      VertexAtTime &v = _renderGraphAtTime.sourceInstance(ed);

      TUTTLE_TLOG(TUTTLE_INFO, "[bake graph information to nodes] in edge "
                                   << e->getInAttrName() << ", at time "
                                   << e->getInTime());
      vData._outEdges.push_back(e);
    }
    vData._inEdges.clear();
    BOOST_FOREACH(const InternalGraphAtTimeImpl::edge_descriptor ed,
                  _renderGraphAtTime.getOutEdges(vd)) {
      const ProcessEdgeAtTime *e = &_renderGraphAtTime.instance(ed);
      TUTTLE_TLOG(TUTTLE_INFO, "[bake graph information to nodes] out edge "
                                   << e->getInAttrName() << ", at time "
                                   << e->getInTime());
...
void ProcessGraph::beginSequence(const TimeRange &timeRange) {
  ...
  for (NodeMap::iterator it = _nodes.begin(), itEnd = _nodes.end(); it != itEnd;
       ++it) {
...
void ProcessGraph::updateGraph(Graph &userGraph,
                               const std::list<std::string> &outputNodes) {
  ...
  if (outputNodes.size()) {
    ...
    std::vector<InternalGraphImpl::vertex_descriptor> rootVertices =
        _renderGraph.rootVertices();
    _renderGraph.addVertex(outputVertex);
...
void ProcessGraph::setup() {
  ...
  /// But maybe it could better to check that here independently from node
  /// types.
  //	graph::visitor::UnconnectedClips<InternalGraphImpl>
  //unconnectedClipsVisitor( _renderGraph );
  //	_renderGraph.depthFirstSearch( unconnectedClipsVisitor );
  //	if( unconnectedClipsVisitor.value )
  //	{
  //		exception::user userMsg("Some non optional clips are unconnected. We
  //can't do the process.\n");
...

  {
    ...
    graph::visitor::TimeDomain<InternalGraphImpl> timeDomainPropagationVisitor(
        _renderGraph);
    _renderGraph.depthFirstVisit(timeDomainPropagationVisitor,
                                 _renderGraph.getVertexDescriptor(_outputId));
  }
...
std::list<TimeRange> ProcessGraph::computeTimeRange() {
  ...
  if (timeRanges.empty()) {
    BOOST_FOREACH(InternalGraphImpl::edge_descriptor ed,
                  boost::out_edges(_renderGraph.getVertexDescriptor(_outputId),
                                   _renderGraph.getGraph())) {
      // TUTTLE_TLOG_INFOS;
      ...
      if (_options.getBegin() != std::numeric_limits<int>::min() &&
          timeDomain.min < _options.getBegin())
        timeDomain.min = _options.getBegin();
      if (_options.getEnd() != std::numeric_limits<int>::max() &&
      ...
      TUTTLE_TLOG_INFOS;
      TUTTLE_TLOG(TUTTLE_INFO, "Compute " << quotes(v.getName())
                                          << " full time domain: from "
                                          << timeDomain.min << " to "
                                          << timeDomain.max << ".");
    }
  }
...
void ProcessGraph::setupAtTime(const OfxTime time) {
  ...
  graph::visitor::DeployTime<InternalGraphImpl> deployTimeVisitor(_renderGraph,
                                                                  time);
  _renderGraph.depthFirstVisit(deployTimeVisitor,
                               _renderGraph.getVertexDescriptor(_outputId));
  ...
  {
    BOOST_FOREACH(InternalGraphAtTimeImpl::vertex_descriptor vd,
                  _renderGraph.getVertices()) {
      Vertex &v = _renderGraph.instance(vd);
      BOOST_FOREACH(const OfxTime t, v._data._times) {
        TUTTLE_TLOG(TUTTLE_INFO, "[Setup at time "
                                     << time << "] add connection from node: "
                                     << v << " for time: " << t);
        _renderGraphAtTime.addVertex(ProcessVertexAtTime(v, t));
      }
    }
    BOOST_FOREACH(const InternalGraphAtTimeImpl::edge_descriptor ed,
                  _renderGraph.getEdges()) {
      ...
      TUTTLE_TLOG(TUTTLE_INFO, "[Setup at time " << time << "] set connection "
                                                 << e);
  ...
  InternalGraphAtTimeImpl::vertex_descriptor outputAtTime =
      getOutputVertexAtTime(time);
  ...
  BOOST_FOREACH(const InternalGraphAtTimeImpl::edge_descriptor ed,
    ...
    v.getProcessDataAtTime()._isFinalNode =
        true; /// @todo: this is maybe better to move this into the ProcessData?
  ...
  BOOST_FOREACH(const InternalGraphAtTimeImpl::vertex_descriptor vd,
    ...
    VertexAtTime &v = _renderGraphAtTime.instance(vd);
    if (!v.isFake()) {
      // TUTTLE_TLOG( TUTTLE_INFO, "setProcessDataAtTime: " << v._name << " id:
      // " << v._id << " at time: " << v._data._time );
  ...
  if (!_options.getForceIdentityNodesProcess()) {
    TUTTLE_TLOG(TUTTLE_INFO, "[Setup at time " << time
                                               << "] remove identity nodes");
    // The "Remove identity nodes" step need to be done after preprocess steps,
    // because the RoI need to be computed.
    std::vector<graph::visitor::IdentityNodeConnection<
        InternalGraphAtTimeImpl> > toRemove;

    graph::visitor::RemoveIdentityNodes<InternalGraphAtTimeImpl> vis(
        _renderGraphAtTime, toRemove);
    _renderGraphAtTime.depthFirstVisit(vis, outputAtTime);
    TUTTLE_TLOG(TUTTLE_INFO, "[Setup at time " << time << "] removing "
                                               << toRemove.size() << " nodes");
    if (toRemove.size()) {
...

  {
    ...
    TUTTLE_TLOG_INFOS;
    graph::visitor::PreProcess1<InternalGraphAtTimeImpl> preProcess1Visitor(
        _renderGraphAtTime);
    TUTTLE_TLOG_INFOS;
...

  {
    ...
    graph::visitor::PreProcess2<InternalGraphAtTimeImpl> preProcess2Visitor(
        _renderGraphAtTime);
...
/*
TUTTLE_TLOG( TUTTLE_INFO, "---------------------------------------- optimize
graph" );
graph::visitor::OptimizeGraph<InternalGraphAtTimeImpl> optimizeGraphVisitor(
_renderGraphAtTime );
...
*/
  ...
  TUTTLE_TLOG( TUTTLE_INFO, "---------------------------------------- sorting
graph" );
  BOOST_FOREACH( InternalGraphImpl::vertex_descriptor vd,
_renderGraph.getVertices() )
  {
          std::vector<InternalGraphImpl::Edge> edges( boost::out_degree(vd,
_renderGraph.getGraph()) );

          BOOST_FOREACH( InternalGraphImpl::edge_descriptor ed,
boost::out_edges( vd, _renderGraph.getGraph() ) )
          {
...
tmp
                  TUTTLE_TLOG( TUTTLE_INFO, e.getName() << " - " <<
_renderGraph.targetInstance(ed).getProcessDataAtTime()._globalInfos._memory  );
          }
          std::sort( edges.begin(), edges.end(),
SortEdgeByMemorySize<InternalGraphImpl>(_renderGraph) );
...
_renderGraph.targetInstance(ed).getProcessDataAtTime()._globalInfos._memory );
          ...
          boost::tie( oe_it, oe_itEnd ) = boost::out_edges( vd,
_renderGraph.getGraph() );
...
void ProcessGraph::computeHashAtTime(NodeHashContainer &outNodesHash,
                                     const OfxTime time) {
#ifdef TUTTLE_EXPORT_WITH_TIMER
  boost::timer::cpu_timer timer;
#endif
  ...
  graph::visitor::ComputeHashAtTime<InternalGraphAtTimeImpl>
  computeHashAtTimeVisitor(_renderGraphAtTime, outNodesHash, time);
  InternalGraphAtTimeImpl::vertex_descriptor outputAtTime =
      getOutputVertexAtTime(time);
...
void ProcessGraph::processAtTime(memory::MemoryCache &outCache,
                                 const OfxTime time) {
  ...
  ///@todo callback
  TUTTLE_TLOG(TUTTLE_INFO, common::Color::get()->_blue
                               << "process at time " << time
                               << common::Color::get()->_std);
  TUTTLE_TLOG(TUTTLE_INFO, "[Process at time "
                               << time << "] output node : "
                               << _renderGraph.getVertex(_outputId).getName());
  ...
  // do the process
  graph::visitor::Process<InternalGraphAtTimeImpl> processVisitor(
      _renderGraphAtTime, core().getMemoryCache());
  ...
  graph::visitor::PostProcess<InternalGraphAtTimeImpl> postProcessVisitor(
      _renderGraphAtTime);
  ...
  TUTTLE_TLOG(TUTTLE_INFO, "[Process at time " << time
                                               << "] clear unused buffers");
  core().getMemoryCache().clearUnused();
  TUTTLE_TLOG(TUTTLE_INFO, "[Process at time "
                               << time << "] Memory cache size: "
                               << core().getMemoryCache().size());
  // TUTTLE_TLOG( TUTTLE_INFO, "[Process at time " << time << "] Out cache size:
  // " << outCache );
}
...
bool ProcessGraph::process(memory::MemoryCache &outCache) {
  ...
  BOOST_FOREACH(const TimeRange & timeRange, timeRanges) {
    TUTTLE_TLOG(TUTTLE_INFO, "[Process render] timeRange: ["
                                 << timeRange._begin << ", " << timeRange._end
                                 << ", " << timeRange._step << "]");

    beginSequence(timeRange);

    for (int time = timeRange._begin; time <= timeRange._end;
         time += timeRange._step) {
      ...
      try {
        ...
        setupAtTime(time);
#ifdef TUTTLE_EXPORT_WITH_TIMER
        TUTTLE_LOG_WARNING("[process timer] setup "
                           << boost::timer::format(setup_timer.elapsed()));
#endif
...
#ifdef TUTTLE_EXPORT_WITH_TIMER
        TUTTLE_LOG_WARNING("[process timer] took "
                           << boost::timer::format(
                                  processAtTime_timer.elapsed()));
#endif
      }
      catch (tuttle::exception::FileInSequenceNotExist &e) // @todo tuttle:
                                                           // change that.
      {
        if (_options.getContinueOnMissingFile() && !_options.getAbort()) {
          ...
          TUTTLE_TLOG(TUTTLE_ERROR, "[Process render] Undefined input at time "
                                        << time << ".");
          endSequence();
          core().getMemoryCache().clearUnused();
          throw;
...
#ifdef TUTTLE_EXPORT_WITH_TIMER
  TUTTLE_LOG_WARNING("[all process timer] "
                     << boost::timer::format(all_process_timer.elapsed()));
#endif
