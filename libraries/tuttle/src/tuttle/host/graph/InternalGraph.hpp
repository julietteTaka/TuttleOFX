...
template <class GraphIn, class GraphOut> class Copier {
public:
  Copier(const GraphIn &gIn, GraphOut &gOut) : _gIn(gIn), _gOut(gOut) {}
  template <class VIn, class VOut> void operator()(const VIn &vIn, VOut &vOut) {
    _gOut[vOut] = _gIn[vIn];
  }
...
template <typename VERTEX, typename EDGE,
          typename OutEdgeList = boost::multisetS,
          typename VertexList = boost::vecS, typename EdgeList = boost::listS>
class InternalGraph {
public:
  ...
  typedef boost::adjacency_list<
      ...
      Vertex, Edge, boost::no_property, // no GraphProperty
      EdgeList                          // EdgeList
      > GraphContainer;
  ...
  typedef typename boost::graph_traits<GraphContainer>::degree_size_type
  degree_t;
  ...
  This &operator=(const InternalGraph<V, E, OEL, VL, EL> &g) {
    detail::Copier<typename InternalGraph<V, E, OEL, VL, EL>::GraphContainer,
                   GraphContainer> copier(g.getGraph(), _graph);
    boost::copy_graph(g.getGraph(), _graph,
                      boost::vertex_copy(copier).edge_copy(copier));
    rebuildVertexDescriptorMap();
    return *this;
  }
  ...
  vertex_descriptor addVertex(const Vertex &prop) {
    ...
    // TUTTLE_TLOG( TUTTLE_INFO, "addVertex, vd: " << vd << ", prop.getKey(): "
    // << prop.getKey() );
  ...
  void removeVertex(const vertex_descriptor &vd) {
    ...
    clearVertex(vd);
    boost::remove_vertex(
        vd, _graph); // finally remove the vertex from the boost graph
    rebuildVertexDescriptorMap();
  }

  void connect(const VertexKey &out, const VertexKey &in,
    ...
    try {
      ...
      e << exception::user() + "Error while connecting " + out + " --> " + in +
               "." + inAttr;
      throw;
    }
  }

  void unconnect(const VertexKey &out, const VertexKey &in,
                 const std::string &inAttr) {
    try {
      ...
      e << exception::user() +
               "No such connection in the graph. Can't unconnect from \"" +
               getVertex(out) + "\" to \"" + getVertex(in) + "." + inAttr +
               "\"";
    }
  }

  bool hasEdge(const VertexKey &out, const VertexKey &in,
               const std::string &inAttr) const {
    return hasEdge(getVertexDescriptor(out), getVertexDescriptor(in), inAttr);
  }

  bool hasEdge(const vertex_descriptor &out, const vertex_descriptor &in,
               const std::string &inAttr) const {
  ...
  edge_descriptor getEdge(const VertexKey &out, const VertexKey &in,
                          const std::string &inAttr) const {
    const out_edge_range_t edges =
        getEdges(getVertexDescriptor(out), getVertexDescriptor(in));
    BOOST_FOREACH(const edge_descriptor ed, edges) {
      if (instance(ed).getInAttrName() == inAttr) {
        return ed;
      }
    ...
    BOOST_THROW_EXCEPTION(exception::Logic()
                          << exception::user() +
                                 "No connection in the graph from \"" +
                                 getVertex(out) + "\" to \"" + getVertex(in) +
                                 "/" + inAttr + "\"");
  }

  out_edge_range_t getEdges(const vertex_descriptor &v1,
                            const vertex_descriptor &v2) const {
    return boost::edge_range(v1, v2, _graph);
  }

  edge_descriptor addEdge(const vertex_descriptor &v1,
                          const vertex_descriptor &v2, const Edge &prop) {
    if (hasEdge(v1, v2, prop.getInAttrName())) {
      BOOST_THROW_EXCEPTION(
          exception::Logic()
          << exception::dev() +
                 "Can't add Edge. There is already a connection from \"" +
    ...
    const edge_descriptor addedEdge =
        boost::add_edge(v1, v2, prop, _graph).first;

    if (hasCycle()) {
      removeEdge(addedEdge);
      BOOST_THROW_EXCEPTION(exception::Logic()
                            << exception::user() + "Connection error: the "
                                                   "graph becomes cyclic while "
                                                   "connecting \"" +
                                   instance(v1) + "\" to \"" + instance(v2) +
                                   "\"");
    }
  ...
  getVertexDescriptor(const VertexKey &vertexKey) const {
  ...
  adjacency_vertex_range_t
  getAdjacentVertices(const vertex_descriptor &v) const {
  ...
  void depthFirstVisitReverse(Visitor &vis, const vertex_descriptor &vroot) {
    ...
    std::vector<boost::default_color_type> colormap(
        boost::num_vertices(revGraph), boost::white_color);
  ...
  template <class Visitor> void depthFirstSearch(Visitor &vis) {
    boost::depth_first_search(_graph, boost::visitor(vis));
  }

  template <class Visitor> void depthFirstSearchReverse(Visitor &vis) {
    boost::reverse_graph<GraphContainer> revGraph(_graph);
  ...
  template <class Visitor> void breadthFirstSearch(Visitor &vis) {
  ...
  void copyTransposed(const InternalGraph<V, E, OEL, VL, EL> &g) {
    detail::Copier<typename InternalGraph<V, E, OEL, VL, EL>::GraphContainer,
                   GraphContainer> copier(g.getGraph(), _graph);
    // make a transposed copy of g in _graph
    boost::transpose_graph(g.getGraph(), _graph,
                           boost::vertex_copy(copier).edge_copy(copier));
    rebuildVertexDescriptorMap();
  }
  ...
  /**
   * @brief Create a vector of leaf vertices (or external node), ie. all nodes
   * that has zero child node.
   */
