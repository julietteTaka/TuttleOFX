...
namespace detail {
template <class Name> class simple_node_writer {
public:
...
template <typename Vertex, typename ProcessEdge, typename OutEdgeList,
          typename VertexList, typename EdgeList>
inline void exportSimple(std::ostream &os,
                         const InternalGraph<Vertex, ProcessEdge, OutEdgeList,
                                             VertexList, EdgeList> &g) {
...
template <typename Vertex, typename ProcessEdge, typename OutEdgeList,
          typename VertexList, typename EdgeList>
inline void exportAsDOT(std::ostream &os,
                        const InternalGraph<Vertex, ProcessEdge, OutEdgeList,
                                            VertexList, EdgeList> &g) {
...
namespace detail {
template <class Graph> struct debug_node_writer {
  debug_node_writer(const Graph &graph) : _graph(graph) {}
  template <class VertexOrEdge>
...
template <typename Vertex, typename ProcessEdge, typename OutEdgeList,
          typename VertexList, typename EdgeList>
inline void
exportDebugAsDOT(std::ostream &os,
                 const InternalGraph<Vertex, ProcessEdge, OutEdgeList,
                                     VertexList, EdgeList> &g) {
  std::map<std::string, std::string> graph_attr, vertex_attr, edge_attr;
  graph_attr["size"] = "6,6";
  ...
  using namespace boost;
  boost::write_graphviz(
      os, g.getGraph(), make_debug_node_writer(g), make_debug_node_writer(g),
      boost::make_graph_attributes_writer(graph_attr, vertex_attr, edge_attr));
}
...
template <typename Vertex, typename ProcessEdge, typename OutEdgeList,
          typename VertexList, typename EdgeList>
inline void
exportDebugAsDOT(const std::string &filename,
                 const InternalGraph<Vertex, ProcessEdge, OutEdgeList,
                                     VertexList, EdgeList> &g) {
  std::ofstream ofs(filename.c_str());
