...
void launchThread(OfxThreadFunctionV1 func, unsigned int threadIndex,
                  unsigned int threadMax, void *customArg) {
  ptr.reset(new ThreadSpecificData(threadIndex));
  func(threadIndex, threadMax, customArg);
}

OfxStatus multiThread(OfxThreadFunctionV1 func, const unsigned int nThreads,
                      void *customArg) {
  if (nThreads == 0) {
    ...
    for (unsigned int i = 0; i < nThreads; ++i) {
      group.create_thread(
          boost::bind(launchThread, func, i, nThreads, customArg));
    }
    group.join_all();
  }
  return kOfxStatOK;
...
OfxStatus multiThreadNumCPUs(unsigned int *const nCPUs) {
  //	*nCPUs = 1; /// @todo tuttle: needs to have an option to disable
  //multithreading (force only one cpu).
...
OfxStatus multiThreadIndex(unsigned int *const threadIndex) {
  //	*threadIndex = boost::this_thread::get_id(); //	we don't want a
  //global thead id, but the thead index inside a node multithread process.
  if (ptr.get() != NULL) {
    *threadIndex = 0;
...
int multiThreadIsSpawnedThread(void) { return ptr.get() != NULL; }
...
struct OfxMultiThreadSuiteV1 gSingleThreadedSuite = {
  multiThread,                multiThreadNumCPUs, multiThreadIndex,
  multiThreadIsSpawnedThread, mutexCreate,        mutexDestroy,
