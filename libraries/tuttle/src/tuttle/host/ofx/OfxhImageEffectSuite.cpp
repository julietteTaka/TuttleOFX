...
OfxStatus getPropertySet(OfxImageEffectHandle h1, OfxPropertySetHandle *h2) {
  OfxhImageEffectNodeBase *effectBase =
      reinterpret_cast<OfxhImageEffectNodeBase *>(h1);
...
OfxStatus getParamSet(OfxImageEffectHandle h1, OfxParamSetHandle *h2) {
  imageEffect::OfxhImageEffectNodeBase *effectBase =
      reinterpret_cast<imageEffect::OfxhImageEffectNodeBase *>(h1);
  ...
  imageEffect::OfxhImageEffectNode *effectInstance =
      dynamic_cast<imageEffect::OfxhImageEffectNode *>(effectBase);
...
OfxStatus clipDefine(OfxImageEffectHandle h1, const char *name,
  ...
  if (effectDescriptor) {
    attribute::OfxhClipImageDescriptor *clip =
        effectDescriptor->defineClip(name);
...
OfxStatus clipGetPropertySet(OfxImageClipHandle clip,
  ...
  attribute::OfxhClipImage *clipInstance =
      reinterpret_cast<attribute::OfxhClipImage *>(clip);
...
OfxStatus clipGetImage(OfxImageClipHandle h1, OfxTime time, OfxRectD *h2,
                       OfxPropertySetHandle *h3) {
  attribute::OfxhClipImage *clipInstance =
      reinterpret_cast<attribute::OfxhClipImage *>(h1);
  ...
  *h3 = image->getPropHandle(); // a pointer to the base class cast into
...
OfxStatus clipGetHandle(OfxImageEffectHandle imageEffect, const char *name,
  ...
  imageEffect::OfxhImageEffectNodeBase *effectBase =
      reinterpret_cast<imageEffect::OfxhImageEffectNodeBase *>(imageEffect);
  ...
  imageEffect::OfxhImageEffectNode *effectInstance =
      reinterpret_cast<imageEffect::OfxhImageEffectNode *>(effectBase);
...
OfxStatus clipGetRegionOfDefinition(OfxImageClipHandle clip, OfxTime time,
                                    OfxRectD *bounds) {
  attribute::OfxhClipImage *clipInstance =
      reinterpret_cast<attribute::OfxhClipImage *>(clip);
...
struct OfxImageEffectSuiteV1 gImageEffectSuite = {
  ...
  clipReleaseImage, clipGetRegionOfDefinition, abort,
