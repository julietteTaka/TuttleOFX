...
/**
 * An image effect host, passed to the setHost function of all image effect
 * plugins
 */
...
public:
   ...
   *
   * param: clientData - the clientData passed into the
   *ImageEffectPlugin::createInstance (tuttle remove this parameter)
  ...
  newInstance(OfxhImageEffectPlugin &plugin,
  ...
  /**
   * Override this to filter out plugins which the host can't support for
   *whatever reason
   *
   *   @param plugin - the plugin to examine
   *   @param reason - set this to report the reason the plugin was not loaded
   */
  virtual bool pluginSupported(const OfxhImageEffectPlugin &plugin,
                               std::string &reason) const;

  /// Override this to create a descriptor, this makes the 'root' descriptor
  virtual OfxhImageEffectNodeDescriptor *
  makeDescriptor(OfxhImageEffectPlugin &plugin) const = 0;

  /// used to construct a context description, rootContext is the main context
  virtual OfxhImageEffectNodeDescriptor *
  makeDescriptor(const OfxhImageEffectNodeDescriptor &rootContext,
