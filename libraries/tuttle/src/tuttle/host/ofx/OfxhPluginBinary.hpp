...
protected:
  OfxhBinary _binary;      ///< our binary object, abstracted layer ontop of OS
                           ///calls, defined in OfxhBinary.hpp
  ...
  time_t _fileModificationTime; ///< used as a time stamp to check modification
                                ///times, used for caching
  size_t _fileSize;    ///< file size last time we check, used for caching
  bool _binaryChanged; ///< whether the timestamp/filesize in this cache is
                       ///different from that in the actual binary

private:
  explicit OfxhPluginBinary()
      : _fileModificationTime(0), _fileSize(0), _binaryChanged(false) {}

public:
  /// create one from the cache.  this will invoke the Binary() constructor
  /// which
  /// will stat() the file.

  explicit OfxhPluginBinary(const std::string &file,
                            const std::string &bundlePath, time_t mtime,
                            size_t size)
      : _binary(file), _filePath(file), _bundlePath(bundlePath),
        _fileModificationTime(mtime), _fileSize(size), _binaryChanged(false) {
    checkBinaryChanged();
  }

  /// constructor which will open a library file, call things inside it, and
  /// then
...
public:
  bool operator==(const This &other) const {
    if (_binary != other._binary || _filePath != other._filePath ||
        _bundlePath != other._bundlePath || _plugins != other._plugins ||
...
private:
  ...
  void serialize(Archive &ar, const unsigned int version) {
    ...
    if (typename Archive::is_loading()) {
      ...
      for (PluginVector::iterator it = getPlugins().begin(),
                                  itEnd = getPlugins().end();
           it != itEnd; ++it) {
        it->setBinary(*this);
      }
    }
