...
struct OfxhPluginIdent {
  ...
  OfxhPluginIdent(const std::string &identifier, const std::string &rawId,
                  int verMin, int verMax)
      : _identifier(identifier), _rawIdentifier(rawId), _versionMinor(verMin),
        _versionMajor(verMax) {}

  bool operator<(const OfxhPluginIdent &d2) const {
    return _identifier < d2._identifier || (_identifier == d2._identifier &&
                                            _versionMajor < d2._versionMajor) ||
           (_identifier == d2._identifier &&
            _versionMajor == d2._versionMajor &&
...
public:
  ...
  OfxhPluginDesc(const std::string &api, int apiVersion,
                 const std::string &identifier,
                 const std::string &rawIdentifier, int versionMajor,
                 int versionMinor);
