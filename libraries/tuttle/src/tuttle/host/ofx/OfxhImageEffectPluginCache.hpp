...
public:
  ...
  OfxhImageEffectPlugin *getPluginById(const std::string &id, int vermaj = -1,
                                       int vermin = -1);
  const OfxhImageEffectPlugin *
  getPluginById(const std::string &id, int vermaj = -1, int vermin = -1) const {
    return const_cast<This &>(*this).getPluginById(id, vermaj, vermin);
  }

  /// get the plugin by label.  vermaj and vermin can be specified.  if they are
  /// not it will
  /// pick the highest found version.
  OfxhImageEffectPlugin *getPluginByLabel(const std::string &label,
                                          int vermaj = -1, int vermin = -1);
  const OfxhImageEffectPlugin *getPluginByLabel(const std::string &label,
                                                int vermaj = -1,
                                                int vermin = -1) const {
  ...
  const std::map<OfxhMajorPlugin, OfxhImageEffectPlugin *> &
  getPluginsByIDMajor() const {
    return _pluginsByIDMajor;
  }

  /// handle the case where the info needs filling in from the file.  runs the
  /// "describe" action on the plugin.
  ...
  OfxhPlugin *newPlugin(OfxhPluginBinary &pb, int pi, const std::string &api,
                        int apiVersion, const std::string &pluginId,
                        const std::string &rawId, int pluginMajorVersion,
