...
OfxStatus paramDefine(OfxParamSetHandle paramSet, const char *paramType,
  ...
  try {
    OfxhParamSetDescriptor *paramSetDescriptor =
        reinterpret_cast<OfxhParamSetDescriptor *>(paramSet);
    if (!paramSetDescriptor)
      return kOfxStatErrBadHandle;

    OfxhParamDescriptor *desc =
        paramSetDescriptor->paramDefine(paramType, name);
    if (!desc)
...
OfxStatus paramGetHandle(OfxParamSetHandle paramSet, const char *name,
  ...
  try {
    OfxhParamSetAccessor *baseSet =
        reinterpret_cast<OfxhParamSetAccessor *>(paramSet);
    if (!baseSet)
      return kOfxStatErrBadHandle;

    OfxhParamSet *setInstance = dynamic_cast<OfxhParamSet *>(baseSet);
    if (setInstance) {
      const std::map<std::string, OfxhParam *> &params =
          setInstance->getParamsByName();
      std::map<std::string, OfxhParam *>::const_iterator it = params.find(name);

      // if we can't find it return an error...
      if (it == params.end()) {
        BOOST_THROW_EXCEPTION(OfxhException(
            kOfxStatErrUnknown, std::string("Can't find parameter : ") + name));
      }
    ...
    OfxhParamSetDescriptor *setDescriptor =
        dynamic_cast<OfxhParamSetDescriptor *>(baseSet);
    if (setDescriptor) {
      const std::map<std::string, OfxhParamDescriptor *> &params =
          setDescriptor->getParams();
      std::map<std::string, OfxhParamDescriptor *>::const_iterator it =
          params.find(name);
...
OfxStatus paramGetPropertySet(OfxParamHandle param,
                              OfxPropertySetHandle *propHandle) {
  try {
    attribute::OfxhParam *paramInstance =
        reinterpret_cast<attribute::OfxhParam *>(param);
...
OfxStatus paramGetNumKeys(OfxParamHandle paramHandle,
  ...
  try {
    attribute::OfxhParam *pInstance =
        reinterpret_cast<attribute::OfxhParam *>(paramHandle);
    if (!pInstance || !pInstance->verifyMagic())
      return kOfxStatErrBadHandle;

    OfxhKeyframeParam *paramInstance =
        dynamic_cast<OfxhKeyframeParam *>(pInstance);
    if (!paramInstance)
      return kOfxStatErrBadHandle;
