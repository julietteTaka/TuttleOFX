 ...
 */
OfxhParam::OfxhParam(const OfxhParamDescriptor &descriptor,
                     const std::string &name,
                     attribute::OfxhParamSet &setInstance)
    : attribute::OfxhAttribute(descriptor), _paramSetInstance(&setInstance),
      _parentInstance(NULL), _avoidRecursion(false) {
  // parameter has to be owned by paramSet
  // setInstance.referenceParam( name, this ); ///< @todo tuttle move this
  // outside
...
void OfxhParam::getV(va_list arg) const OFX_EXCEPTION_SPEC {
  BOOST_THROW_EXCEPTION(OfxhException(
      ...
      std::string("ParamInstance getV failed (paramName:") + getName() + ")"));
}
...
void OfxhParam::setV(va_list arg, const EChange change) OFX_EXCEPTION_SPEC {
  BOOST_THROW_EXCEPTION(OfxhException(
      kOfxStatErrUnsupported,
      std::string("ParamInstance setV failed (paramName:") + getName() + ")"));
}

/**
 * key a value, implemented by instances to deconstruct var args
 */
void OfxhParam::setV(const OfxTime time, va_list arg,
 ...
 */
void OfxhParam::deriveV(const OfxTime time, va_list arg) const
    OFX_EXCEPTION_SPEC {
 ...
 */
void OfxhParam::integrateV(const OfxTime time1, const OfxTime time2,
 ...
 */
void OfxhParam::notify(const std::string &name, bool single,
 ...
 */
void OfxhParam::copy(const OfxhParam &instance,
 ...
 */
void OfxhParam::copy(const OfxhParam &instance, OfxTime offset,
...
OfxhParam *OfxhParam::getParentInstance() { return _parentInstance; }
