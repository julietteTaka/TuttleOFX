 ...
 */
class OfxhClipImage : public attribute::OfxhClip,
...
public:
  ...
  virtual std::string getConnectedClipFullName() const = 0; ///< @todo tuttle:
                                                            ///remove this!
   ...
   */
  void setBitDepthString(const std::string &s,
                         const property::EModifiedBy modifiedBy =
                             property::eModifiedByHost) {
    property::String &prop = getEditableProperties().fetchLocalStringProperty(
  ...
  void setBitDepthStringIfUpper(const std::string &s) {
    ...
    if (ofx::imageEffect::mapBitDepthStringToEnum(s) >
        getBitDepth()) // we can increase the bit depth but not decrease
      prop.setValue(s);
  }
  ...
  void setBitDepthStringIfUpperAndNotModifiedByPlugin(const std::string &s) {
    ...
    if (prop.getModifiedBy() !=
            property::eModifiedByPlugin && // if not modified by plugin
        ofx::imageEffect::mapBitDepthStringToEnum(s) >
            getBitDepth()) // we can increase the bit depth but not decrease
      prop.setValue(s);
  }
  ...
  void setPixelAspectRatio(const double &s,
                           const property::EModifiedBy modifiedBy) {
    property::Double &prop = getEditableProperties().fetchLocalDoubleProperty(
   ...
   * called by clip preferences action
   */
  void setComponentsString(const std::string &s,
                           const property::EModifiedBy modifiedBy =
                               property::eModifiedByHost) {
    property::String &prop = getEditableProperties().fetchLocalStringProperty(
  ...
  void setComponents(const imageEffect::EPixelComponent &comp,
                     const property::EModifiedBy modifiedBy =
                         property::eModifiedByHost) {
    setComponentsString(imageEffect::mapPixelComponentEnumToString(comp),
                        modifiedBy);
  }
  ...
  {
    return getProperties().getStringProperty(
   ...
   *
   *  @returns
   *      - kOfxImageComponentNone (implying a clip is unconnected, not valid
   *for an image)
   *      - kOfxImageComponentRGBA
   *      - kOfxImageComponentAlpha
   */
  ...
  /** PreMultiplication -
   *
   *  kOfxImageOpaque - the image is opaque and so has no premultiplication
   *state
  ...
  {
    startFrame =
        getProperties().getDoubleProperty(kOfxImageEffectPropFrameRange, 0);
    endFrame =
        getProperties().getDoubleProperty(kOfxImageEffectPropFrameRange, 1);
  }
   ...
   *   - kOfxImageFieldLower - the clip material is fielded, with image rows
   * 0,2,4.... occuring first in a frame
   *   - kOfxImageFieldUpper - the clip material is fielded, with image rows
   * line 1,3,5.... occuring first in a frame
   */
  ...
  {
    return getProperties().getDoubleProperty(
  ...
  virtual void getUnmappedFrameRange(double &unmappedStartFrame,
                                     double &unmappedEndFrame) const //= 0;
  {
    unmappedStartFrame = getProperties().getDoubleProperty(
        kOfxImageEffectPropUnmappedFrameRange, 0);
    unmappedEndFrame = getProperties().getDoubleProperty(
        kOfxImageEffectPropUnmappedFrameRange, 1);
  }

  /** Continuous Samples -
   *
   *  0 if the images can only be sampled at discreet times (eg: the clip is a
   *sequence of frames),
   *  1 if the images can only be sampled continuously (eg: the clip is infact
   *an animating roto spline and can be rendered anywhen).
   */
   ...
   *  If bounds is not null, fetch the indicated section of the canonical image
   * plane.
   */
  virtual tuttle::host::ofx::imageEffect::OfxhImage *
  ...
  /** given the colour component, find the nearest set of supported colour
   * components
 ...
 */
inline OfxhClipImage *new_clone(const OfxhClipImage &a) { return a.clone(); }
...
struct is_virtual_base_of<
    ...
    tuttle::host::ofx::attribute::OfxhClipImage> : public mpl::true_ {};
}
#endif
