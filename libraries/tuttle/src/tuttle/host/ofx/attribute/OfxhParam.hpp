...
protected:                                                                     \
  inline virtual void getValue(TYPE &) const OFX_EXCEPTION_SPEC {              \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a " #NAME " parameter (" +          \
                                  this->getParamType() + ")."));               \
  }                                                                            \
  inline virtual void getValueAtTime(const OfxTime, TYPE &) const              \
      OFX_EXCEPTION_SPEC {                                                     \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a " #NAME " parameter (" +          \
  ...
  inline virtual void getValueAtIndex(const std::size_t, TYPE &) const         \
      OFX_EXCEPTION_SPEC {                                                     \
    BOOST_THROW_EXCEPTION(OfxhException(kOfxStatErrBadHandle,                  \
                                        "\"" + this->getName() + "\"" +        \
                                            " is not a " #NAME                 \
                                            " multidimentional parameter (" +  \
                                            this->getParamType() + ")."));     \
  }                                                                            \
  inline virtual void getValueAtTimeAndIndex(                                  \
      const OfxTime, const std::size_t, TYPE &) const OFX_EXCEPTION_SPEC {     \
    BOOST_THROW_EXCEPTION(OfxhException(kOfxStatErrBadHandle,                  \
                                        "\"" + this->getName() + "\"" +        \
                                            " is not a " #NAME                 \
                                            " multidimentional parameter (" +  \
                                            this->getParamType() + ")."));     \
                                                                               ...
                                                                               \
public:                                                                        \
  inline TYPE get##NAME##ValueAtIndex(const std::size_t index) const           \
      OFX_EXCEPTION_SPEC {                                                     \
    TYPE dst;                                                                  \
    getValueAtIndex(index, dst);                                               \
    return dst;                                                                \
  }                                                                            \
  inline TYPE get##NAME##ValueAtTimeAndIndex(                                  \
      const OfxTime time, const std::size_t index) const OFX_EXCEPTION_SPEC {  \
    TYPE dst;                                                                  \
  ...
  inline TYPE get##NAME##ValueAtTime(const OfxTime time) const                 \
      OFX_EXCEPTION_SPEC {                                                     \
    TYPE dst;                                                                  \
    getValueAtTime(time, dst);                                                 \
    return dst;                                                                \
  }                                                                            \
  inline virtual void setValue(const TYPE &value,                              \
  ...
  inline virtual void setValue(const TYPE &value) OFX_EXCEPTION_SPEC {         \
    setValue(value, eChangeUserEdited);                                        \
  }                                                                            \
  inline virtual void setValueAtTime(                                          \
      const OfxTime time, const TYPE &value,                                   \
      const EChange change) OFX_EXCEPTION_SPEC {                               \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a " #NAME " parameter (" +          \
  ...
  inline virtual void setValueAtTime(const OfxTime time,                       \
                                     const TYPE &value) OFX_EXCEPTION_SPEC {   \
    setValueAtTime(time, value, eChangeUserEdited);                            \
  }                                                                            \
  inline virtual void setValueAtIndex(                                         \
      const std::size_t index, const TYPE &value,                              \
      const EChange change) OFX_EXCEPTION_SPEC {                               \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a multi-" #NAME " parameter (" +    \
                                  this->getParamType() + ", " +                \
                                  mapChangeEnumToString(change) + ")."));      \
  }                                                                            \
  inline virtual void setValueAtIndex(const std::size_t index,                 \
                                      const TYPE &value) OFX_EXCEPTION_SPEC {  \
    setValueAtIndex(index, value, eChangeUserEdited);                          \
  }                                                                            \
  inline virtual void setValueAtTimeAndIndex(                                  \
      const OfxTime time, const std::size_t index, const TYPE &value,          \
      const EChange change) OFX_EXCEPTION_SPEC {                               \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a multi-" #NAME " parameter (" +    \
  ...
  inline virtual void setValueAtTimeAndIndex(                                  \
      const OfxTime time, const std::size_t index,                             \
      const TYPE &value) OFX_EXCEPTION_SPEC {                                  \
    setValueAtTimeAndIndex(time, index, value, eChangeUserEdited);             \
  }                                                                            \
                                                                               \
  inline virtual void setValue(const std::vector<TYPE> &values,                \
                               const EChange change) OFX_EXCEPTION_SPEC {      \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a multi-" #NAME " parameter (" +    \
  ...
  inline virtual void setValue(                                                \
      const std::vector<TYPE> &values) OFX_EXCEPTION_SPEC {                    \
    setValue(values, eChangeUserEdited);                                       \
  }                                                                            \
  inline virtual void setValueAtTime(                                          \
      const OfxTime time, const std::vector<TYPE> &values,                     \
      const EChange change) OFX_EXCEPTION_SPEC {                               \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a multi-" #NAME " parameter (" +    \
...
#define TUTTLE_DEFINE_OFXHPARAM_MULTIDIM_ACCESSORS(NAME, TYPE)                 \
  inline virtual void setValue(const TYPE &value1, const TYPE &value2,         \
                               const EChange change) OFX_EXCEPTION_SPEC {      \
    BOOST_THROW_EXCEPTION(                                                     \
        OfxhException(kOfxStatErrBadHandle,                                    \
                      "\"" + this->getName() + "\"" + " is not a multi-" #NAME \
                                                      " parameter (2) (" +     \
                          this->getParamType() + ", " +                        \
                          mapChangeEnumToString(change) + ")."));              \
  }                                                                            \
  inline virtual void setValue(const TYPE &value1,                             \
                               const TYPE &value2) OFX_EXCEPTION_SPEC {        \
    setValue(value1, value2, eChangeUserEdited);                               \
  }                                                                            \
  inline virtual void setValueAtTime(                                          \
      const OfxTime time, const TYPE &value1, const TYPE &value2,              \
      const EChange change) OFX_EXCEPTION_SPEC {                               \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        "\"" + this->getName() + "\"" + " is not a multi-" #NAME               \
                                        " parameter (2) (" +                   \
  ...
  inline virtual void setValueAtTime(const OfxTime time, const TYPE &value1,   \
                                     const TYPE &value2) OFX_EXCEPTION_SPEC {  \
  ...
  inline virtual void setValue(const TYPE &value1, const TYPE &value2,         \
                               const TYPE &value3) OFX_EXCEPTION_SPEC {        \
    setValue(value1, value2, value3, eChangeUserEdited);                       \
  }                                                                            \
  inline virtual void setValueAtTime(                                          \
      const OfxTime time, const TYPE &value1, const TYPE &value2,              \
      const TYPE &value3, const EChange change) OFX_EXCEPTION_SPEC {           \
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a " #NAME " parameter (3) (" +      \
                                  this->getParamType() + ", " +                \
                                  mapChangeEnumToString(change) + ")."));      \
  }                                                                            \
  inline virtual void setValueAtTime(const OfxTime time, const TYPE &value1,   \
    ...
    setValueAtTime(time, value1, value2, value3, eChangeUserEdited);           \
                                                                               ...
                                                                               \
  inline virtual void setValue(const TYPE &value1, const TYPE &value2,         \
                               const TYPE &value3, const TYPE &value4,         \
                               const EChange change) OFX_EXCEPTION_SPEC {      \
    BOOST_THROW_EXCEPTION(                                                     \
        OfxhException(kOfxStatErrBadHandle,                                    \
                      "\"" + this->getName() + "\"" + " is not a multi-" #NAME \
  ...
  inline virtual void setValueAtTime(                                          \
    ...
    BOOST_THROW_EXCEPTION(OfxhException(                                       \
        kOfxStatErrBadHandle, "\"" + this->getName() + "\"" +                  \
                                  " is not a " #NAME " parameter (4) (" +      \
                                  this->getParamType() + ", " +                \
                                  mapChangeEnumToString(change) + ")."));      \
  }                                                                            \
  inline virtual void setValueAtTime(const OfxTime time, const TYPE &value1,   \
                                     const TYPE &value2, const TYPE &value3,   \
                                     const TYPE &value4) OFX_EXCEPTION_SPEC {  \
...
protected:
  ...
  bool _avoidRecursion; ///< Avoid recursion when updating with
                        ///paramChangedAction

protected:
  OfxhParam(const OfxhParam &other)
      : OfxhAttribute(other), _paramSetInstance(other._paramSetInstance),
        _parentInstance(other._parentInstance), _avoidRecursion(false) {
    /// @todo tuttle : copy content, not pointer ?
  }

public:
  /// make a parameter, with the given type and name
  explicit OfxhParam(const OfxhParamDescriptor &descriptor,
                     const std::string &name, OfxhParamSet &setInstance);
  ...
  // copy one parameter to another
  virtual void copy(const OfxhParam &instance,
                    OfxTime offset) OFX_EXCEPTION_SPEC;

  // copy one parameter to another, with a range
  virtual void copy(const OfxhParam &instance, OfxTime offset,
                    OfxRangeD range) OFX_EXCEPTION_SPEC;
...
// deconstruct the var args lists
  ...
  virtual void setValueFromExpression(const std::string &value,
                                      const EChange change) OFX_EXCEPTION_SPEC {
    BOOST_THROW_EXCEPTION(OfxhException(
        kOfxStatErrMissingHostFeature,
        "\"" + this->getName() +
            "Can't set value from expression on parameter \"" +
            this->getName() + "\", it's not supported for " +
            this->getParamType() + " parameters (" + this->getParamType() +
            ", " + mapChangeEnumToString(change) + ")."));
  }
  inline void
  setValueFromExpression(const std::string &value) OFX_EXCEPTION_SPEC {
  ...
  inline void setValue(const char *value) OFX_EXCEPTION_SPEC {
    setValue(value, eChangeUserEdited);
  }
  inline void
  setValueAtTime(const OfxTime time, const char *value,
                 const attribute::EChange change) OFX_EXCEPTION_SPEC {
    setValueAtTime(time, std::string(value), change);
  }
  inline void setValueAtTime(const OfxTime time,
                             const char *value) OFX_EXCEPTION_SPEC {
    setValueAtTime(time, value, eChangeUserEdited);
  }
  ...
  /* TuttleOFX specific. This is not part of OFX.
     ...
     */
  virtual void
  setInterpolator(const enum EInterpolatorType etype) OFX_EXCEPTION_SPEC {
  ...
  virtual void setV(const OfxTime time, va_list arg,
                    const EChange change) OFX_EXCEPTION_SPEC;

  /// derive a value, implemented by instances to deconstruct var args
  virtual void deriveV(const OfxTime time, va_list arg) const
      OFX_EXCEPTION_SPEC;

  /// integrate a value, implemented by instances to deconstruct var args
  virtual void integrateV(const OfxTime time1, const OfxTime time2,
                          va_list arg) const OFX_EXCEPTION_SPEC;

  /// overridden from Property::NotifyHook
  virtual void notify(const std::string &name, bool single,
                      int num) OFX_EXCEPTION_SPEC;
#endif
};
 ...
 */
inline OfxhParam *new_clone(const OfxhParam &a) { return a.clone(); }
...
struct is_virtual_base_of<
    ...
    tuttle::host::ofx::attribute::OfxhParam> : public mpl::true_ {};
}
#endif
