...
OfxhClipImageAccessor::getSupportedComponents() const {
 ...
 */
bool
OfxhClipImageAccessor::isSupportedComponent(const std::string &comp) const {
  return getProperties().findStringPropValueIndex(
             kOfxImageEffectPropSupportedComponents, comp) != -1;
}

/** does the clip do random temporal access
 */
bool OfxhClipImageAccessor::temporalAccess() const {
  return getProperties().getIntProperty(
             kOfxImageEffectPropTemporalClipAccess) != 0;
}
