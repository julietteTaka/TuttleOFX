...
public:
  OfxhParamChoice(const OfxhParamDescriptor &descriptor,
  ...
  const std::vector<std::string> &getChoiceKeys() const {
    return this->getProperties()
        .fetchStringProperty(kOfxParamPropChoiceOption)
        .getValues();
  }
  ...
  void getValueAtTime(const OfxTime time, std::string &outKey) const
      OFX_EXCEPTION_SPEC;
  ...
  void setValueAtTime(const OfxTime time, const std::string &key,
                      const EChange change) OFX_EXCEPTION_SPEC {
