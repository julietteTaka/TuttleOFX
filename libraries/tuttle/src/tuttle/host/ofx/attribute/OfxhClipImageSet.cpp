...
OfxhClipImageSet::OfxhClipImageSet() : _clipPrefsDirty(true) {}
...
void OfxhClipImageSet::initMapFromList() {
  for (ClipImageVector::iterator it = _clipsByOrder.begin(),
                                 itEnd = _clipsByOrder.end();
       it != itEnd; ++it) {
...
void OfxhClipImageSet::copyClipsValues(const OfxhClipImageSet &other) {
  if (_clipsByOrder.size() != other._clipsByOrder.size()) {
    BOOST_THROW_EXCEPTION(
        exception::Bug() << exception::dev("You try to copy clips values, but "
                                           "the two lists are not identical."));
  }

  ClipImageVector::const_iterator oit = other._clipsByOrder.begin(),
                                  oitEnd = other._clipsByOrder.end();
  for (ClipImageVector::iterator it = _clipsByOrder.begin(),
                                 itEnd = _clipsByOrder.end();
    ...
    if (c.getName() != oc.getName()) {
      BOOST_THROW_EXCEPTION(exception::Bug()
                            << exception::dev("You try to copy clips values, "
                                              "but it is not the same clips in "
                                              "the two lists."));
    }
    c.copyValues(oc);
  }
...
void OfxhClipImageSet::populateClips(
    const imageEffect::OfxhImageEffectNodeDescriptor &descriptor)
    OFX_EXCEPTION_SPEC {
  const imageEffect::OfxhImageEffectNodeDescriptor::ClipImageDescriptorVector &
  clips = descriptor.getClipsByOrder();

  _clipsByOrder.reserve(clips.size());
  /// @todo tuttle don't manipulate clip here, delegate to ClipInstanceSet
  for (imageEffect::OfxhImageEffectNodeDescriptor::ClipImageDescriptorVector::
           const_iterator it = clips.begin(),
                          itEnd = clips.end();
       it != itEnd; ++it) {
...
OfxhClipImage &OfxhClipImageSet::getClip(const std::string &name,
  ...
  if (acceptPartialName) {
    ...
    if (matches.size() > 1) {
      BOOST_THROW_EXCEPTION(exception::Value()
                            << exception::user() +
                                   "Ambiguous partial clip name \"" + name +
                                   "\". Possible values are: " +
...
OfxhClipImage *OfxhClipImageSet::getClipPtr(const std::string &name,
                                            const bool acceptPartialName) {
  try {
...
const OfxhClipImage *
OfxhClipImageSet::getClipPtr(const std::string &name,
