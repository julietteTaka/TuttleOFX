...
public:
  ...
  OfxhParamDouble(const OfxhParamDescriptor &descriptor,
                  const std::string &name, OfxhParamSet &setInstance,
                  const std::size_t index = 0)
      : OfxhParam(descriptor, name, setInstance), _index(index) {
  ...
  virtual void getValueAtTime(const OfxTime time, double &) const
      OFX_EXCEPTION_SPEC = 0;
  virtual void setValue(const double &,
                        const EChange change) OFX_EXCEPTION_SPEC = 0;
  virtual void setValueAtTime(const OfxTime time, const double &,
                              const EChange change) OFX_EXCEPTION_SPEC = 0;
  ...
  virtual void setValueAtTime(const OfxTime time, const int &v,
                              const EChange change) OFX_EXCEPTION_SPEC {
    this->setValueAtTime(time, (const double)(v), change);
  }

  virtual void derive(const OfxTime time, double &) const
      OFX_EXCEPTION_SPEC = 0;
  virtual void integrate(const OfxTime time1, const OfxTime time2,
                         double &) const OFX_EXCEPTION_SPEC = 0;
