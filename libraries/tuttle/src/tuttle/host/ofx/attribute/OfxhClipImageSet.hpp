...
public:
  ...
  void
  populateClips(const imageEffect::OfxhImageEffectNodeDescriptor &descriptor)
      OFX_EXCEPTION_SPEC;
   ...
   * get the clip
   */
  OfxhClipImage &getClip(const std::string &name,
                         const bool acceptPartialName = false);

  const OfxhClipImage &getClip(const std::string &name,
                               const bool acceptPartialName = false) const {
    return const_cast<This *>(this)->getClip(name, acceptPartialName);
  }

  const OfxhClipImage *getClipPtr(const std::string &name,
                                  const bool acceptPartialName = false) const;

  OfxhClipImage *getClipPtr(const std::string &name,
                            const bool acceptPartialName = false);
  ...
  void addClip(const std::string &name,
               OfxhClipImage *instance) OFX_EXCEPTION_SPEC {
  ...
  newClipImage(const OfxhClipImageDescriptor &descriptor) = 0;
#endif
