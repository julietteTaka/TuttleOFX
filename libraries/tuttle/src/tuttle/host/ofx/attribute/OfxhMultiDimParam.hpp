...
class OfxhMultiDimParam : public OfxhParam, public OfxhKeyframeParam {
public:
  typedef T Type;
...
public:
  OfxhMultiDimParam(const OfxhParamDescriptor &descriptor,
                    const std::string &name, OfxhParamSet &setInstance)
      : OfxhParam(descriptor, name, setInstance) {}

  virtual ~OfxhMultiDimParam() {}

  inline std::size_t getSize() const { return _controls.size(); }

  void setValueFromExpression(const std::string &value,
                              const ofx::attribute::EChange change)
      OFX_EXCEPTION_SPEC {
    std::vector<std::string> allExp =
        boost::program_options::split_unix(value, ",");
    ...
    if (allExp.size() == 1) {
      ...
      if (getSize() != allExp.size()) {
        BOOST_THROW_EXCEPTION(
            exception::Value()
            << exception::user() + "Set " + allExp.size() +
                   " values on a multidimensional parameter of size " + DIM +
                   ".");
      }
      for (std::size_t i = 0; i < getSize(); ++i) {
...
protected:
  ...
  inline virtual void getValueAtIndex(const std::size_t index,
                                      BaseType &outDst) const
      OFX_EXCEPTION_SPEC {
    if (index >= getSize()) {
      BOOST_THROW_EXCEPTION(exception::BadIndex()
                            << exception::user() + "Get value at index " +
                                   index +
                                   " on a multidimensional parameter of size " +
                                   DIM + ".");
    }
    _controls[index].getValue(outDst);
  }

  inline virtual void getValueAtTimeAndIndex(const OfxTime time,
...
public:
  ...
  inline void
  setValueAtTime(const OfxTime time, const BaseType &value,
                 const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC {
    for (std::size_t i = 0; i < getSize(); ++i) {
  ...
  inline virtual void
  setValueAtTimeAndIndex(const OfxTime time, const std::size_t index,
                         const BaseType &value,
                         const EChange change) OFX_EXCEPTION_SPEC {
    if (index >= getSize()) {
      BOOST_THROW_EXCEPTION(exception::BadIndex()
  ...
  inline virtual void setValue(const std::vector<BaseType> &values,
                               const EChange change) OFX_EXCEPTION_SPEC {
    if (values.size() != getSize()) {
      BOOST_THROW_EXCEPTION(
          exception::BadIndex()
          << exception::user() + "Set " + values.size() +
                 " values on a multidimensional parameter of size " + DIM +
                 ".");
    }
    for (std::size_t i = 0; i < getSize(); ++i) {
  ...
  inline virtual void deriveAtIndex(const OfxTime time, const std::size_t index,
                                    BaseType &outDst) const OFX_EXCEPTION_SPEC {
    if (index >= getSize()) {
      BOOST_THROW_EXCEPTION(exception::BadIndex()
                            << exception::user() + "Derive value at index " +
                                   index +
                                   " on a multidimensional parameter of size " +
                                   DIM + ".");
    }
  ...
  inline virtual void integrateAtIndex(const OfxTime time1, const OfxTime time2,
                                       const std::size_t index,
                                       BaseType &outDst) const
      OFX_EXCEPTION_SPEC {
    if (index >= getSize()) {
      BOOST_THROW_EXCEPTION(exception::BadIndex()
                            << exception::user() + "Integrate value at index " +
                                   index +
                                   " on a multidimensional parameter of size " +
                                   DIM + ".");
    }
