...
class OfxhParamString : public OfxhParam, public OfxhKeyframeParam {
  mutable std::string _returnValue; ///< location to hold temporary return
                                    ///value. Should delegate this to
                                    ///implementation!!!

public:
  typedef std::string BaseType;
  OfxhParamString(const OfxhParamDescriptor &descriptor,
  ...
  virtual void getValueAtTime(const OfxTime time, std::string &) const
      OFX_EXCEPTION_SPEC = 0;
  virtual void setValue(const std::string &,
                        const EChange change) OFX_EXCEPTION_SPEC = 0;
  virtual void setValueAtTime(const OfxTime time, const std::string &,
                              const EChange change) OFX_EXCEPTION_SPEC = 0;
