...
/// A set of parameters
///
/// As we are the owning object we delete the params inside ourselves. It was
/// tempting
/// to make params autoref objects and have shared ownership with the client
/// code
...
public:
  ...
  bool operator==(const This &other) const {
    return _paramVector == other._paramVector;
  }
  ...
  const OfxhParam &getParam(const std::string &name) const {
    return const_cast<This *>(this)->getParam(name);
  }

  OfxhParam &getParamByScriptName(const std::string &scriptName,
                                  const bool acceptPartialName = false);
  const OfxhParam &getParamByScriptName(const std::string &name,
                                        const bool acceptPartialName =
                                            false) const {
    return const_cast<This *>(this)
        ->getParamByScriptName(name, acceptPartialName);
  }
  OfxhParam *getParamPtrByScriptName(const std::string &name,
                                     const bool acceptPartialName = false);
  const OfxhParam *getParamPtrByScriptName(const std::string &name,
                                           const bool acceptPartialName =
                                               false) const;
  ...
  const OfxhParam &getParam(const std::size_t index) const {
    return const_cast<This *>(this)->getParam(index);
  }
...
protected:
  /// reference a param
  //	virtual void referenceParam( const std::string& name, OfxhParam*
  ...
  /// Client host code needs to implement this
  virtual OfxhParam *
  newParam(const OfxhParamDescriptor &Descriptor) OFX_EXCEPTION_SPEC = 0;
