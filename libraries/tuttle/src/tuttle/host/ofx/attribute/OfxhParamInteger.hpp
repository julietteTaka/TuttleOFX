...
public:
  ...
  OfxhParamInteger(const OfxhParamDescriptor &descriptor,
  ...
  virtual void getValueAtTime(const OfxTime time, int &) const
      OFX_EXCEPTION_SPEC = 0;
  virtual void setValue(const int &,
                        const EChange change) OFX_EXCEPTION_SPEC = 0;
  virtual void setValueAtTime(const OfxTime time, const int &,
                              const EChange change) OFX_EXCEPTION_SPEC = 0;
  ...
  virtual void setValueAtTime(const OfxTime time, const bool &v,
                              const EChange change) OFX_EXCEPTION_SPEC {
    setValueAtTime(time, int(v), change);
  }

  // probably derived class does not need to implement, default is an
  // approximation
  virtual void derive(const OfxTime time, int &) const OFX_EXCEPTION_SPEC;
  virtual void integrate(const OfxTime time1, const OfxTime time2, int &) const
      OFX_EXCEPTION_SPEC;
