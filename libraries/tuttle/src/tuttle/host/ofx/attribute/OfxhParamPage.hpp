...
public:
  OfxhParamPage(const OfxhParamDescriptor &descriptor, const std::string &name,
                OfxhParamSet &setInstance)
      : OfxhParam(descriptor, name, setInstance) {}
  const std::map<int, OfxhParam *> &getChildren() const;

protected:
  mutable std::map<int, OfxhParam *> _children; // if set in a notify hook, this
                                                // need not be mutable
  ...
  virtual void setValueAtTime(const OfxTime time, const int &v,
                              const EChange change) OFX_EXCEPTION_SPEC{};
  virtual void
  setValueFromExpression(const std::string &value,
                         const EChange change) OFX_EXCEPTION_SPEC {}
  inline void
