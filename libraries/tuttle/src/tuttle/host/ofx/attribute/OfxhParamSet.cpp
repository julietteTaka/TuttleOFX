...
OfxhParamSet::OfxhParamSet(const OfxhParamSet &other) { operator=(other); }
...
void OfxhParamSet::copyParamsValues(const OfxhParamSet &other) {
  if (_paramVector.size() != other._paramVector.size()) {
    BOOST_THROW_EXCEPTION(exception::Bug()
                          << exception::dev("You try to copy parameters "
                                            "values, but the two lists are not "
                                            "identical."));
  }

  ParamVector::const_iterator oit = other._paramVector.begin(),
                              oitEnd = other._paramVector.end();
  for (ParamVector::iterator it = _paramVector.begin(),
                             itEnd = _paramVector.end();
...
// void OfxhParamSet::referenceParam( const std::string& name, OfxhParam*
// instance ) OFX_EXCEPTION_SPEC
//{
//	if( _allParams.find( name ) != _allParams.end() )
//	{
//		BOOST_THROW_EXCEPTION( OfxhException( kOfxStatErrExists, "Trying
//to reference a new parameter which already exists." ) );
//	}
...
OfxhParam &OfxhParamSet::getParam(const std::string &name) {
  ...
  if (it == _params.end()) {
    BOOST_THROW_EXCEPTION(exception::BadIndex() << exception::user() +
                                                       "Param name \"" + name +
                                                       "\" not found.");
  }
  return *it->second;
...
OfxhParam &OfxhParamSet::getParamByScriptName(const std::string &scriptName,
  ...
  if (acceptPartialName) {
    ...
    if (matches.size() > 1) {
      BOOST_THROW_EXCEPTION(exception::Value()
                            << exception::user() +
                                   "Ambiguous partial param script name \"" +
  ...
  if (matches.size() == 0) {
    BOOST_THROW_EXCEPTION(exception::Value()
                          << exception::user() + "Param script name \"" +
                                 scriptName + "\" not found.");
  }
  return *res;
}
OfxhParam *OfxhParamSet::getParamPtrByScriptName(const std::string &name,
                                                 const bool acceptPartialName) {
  try {
...
const OfxhParam *
OfxhParamSet::getParamPtrByScriptName(const std::string &name,
                                      const bool acceptPartialName) const {
  try {
    return &this->getParamByScriptName(name, acceptPartialName);
  }
  catch (...) {
...
OfxhParam &OfxhParamSet::getParam(const std::size_t index) {
  if (index > _paramVector.size())
    BOOST_THROW_EXCEPTION(
        exception::BadIndex()
        << exception::user() + "Param not found, index out of range. (index=" +
  ...
  if (_params.find(instance->getName()) != _params.end()) {
    BOOST_THROW_EXCEPTION(
        OfxhException(kOfxStatErrExists,
                      "Trying to add a new parameter which already exists."));
  }
