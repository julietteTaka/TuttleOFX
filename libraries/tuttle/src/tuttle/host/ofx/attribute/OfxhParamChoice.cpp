...
int OfxhParamChoice::getIndexFor(const std::string &key) const {
  ...
  if (key.empty()) {
    BOOST_THROW_EXCEPTION(
        OfxhException("Empty key value for choice param \"" + this->getName() +
                      "\".\n" + "Possible values are : [\n\"" +
                      boost::algorithm::join(values, "\",\n\"") + "\" ]"));
  }
  if (values.empty()) {
    BOOST_THROW_EXCEPTION(OfxhException(
        "The key \"" + key + "\" doesn't exist for choice param \"" +
  ...
  for (StringVector::const_iterator it = values.begin(), itEnd = values.end();
    ...
    } else if (it->size() > key.size()) // partial key, the user only set the
                                        // beginning of the choice value
    {
  ...
  if (partialMatch != values.end()) {
    ...
    return boost::numeric_cast<int>(
        std::distance(values.begin(), partialMatch));
  }
  BOOST_THROW_EXCEPTION(OfxhException(
      "The key \"" + key + "\" doesn't exist for choice param \"" +
      this->getName() + "\".\n" + "Possible values are : [\n\"" +
...
const std::string &OfxhParamChoice::getChoiceKeyAt(const int index) const {
  ...
  if (index < 0 || index >= keys.size()) {
    BOOST_THROW_EXCEPTION(
        OfxhException(kOfxStatErrBadIndex)
        << exception::user() + "The index value \"" + index +
               "\" is out of kays range.\n" + "Choice keys are : [\n\"" +
...
void OfxhParamChoice::getValueAtTime(const OfxTime time,
 ...
 * implementation of var args function
 */
void OfxhParamChoice::getV(const OfxTime time, va_list arg) const
    OFX_EXCEPTION_SPEC {
  int *value = va_arg(arg, int *);
 ...
 * implementation of var args function
 */
void OfxhParamChoice::setV(va_list arg,
 ...
 * implementation of var args function
 */
void OfxhParamChoice::setV(const OfxTime time, va_list arg,
                           const EChange change) OFX_EXCEPTION_SPEC {
  int value = va_arg(arg, int);
...
std::size_t OfxhParamChoice::getHashAtTime(const OfxTime time) const {
  std::string value; // don't use the index here. The plugin could change the
                     // order of choices values in a minor version change.
