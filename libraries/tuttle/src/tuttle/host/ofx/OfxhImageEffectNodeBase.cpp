...
OfxhImageEffectNodeBase::OfxhImageEffectNodeBase(
    const property::OfxhPropSpec *propSpec)
    : _properties(propSpec) {}

OfxhImageEffectNodeBase::~OfxhImageEffectNodeBase() {}

bool OfxhImageEffectNodeBase::
operator==(const OfxhImageEffectNodeBase &other) const {
...
bool OfxhImageEffectNodeBase::isContextSupported(const std::string &s) const {
  return _properties.findStringPropValueIndex(
             kOfxImageEffectPropSupportedContexts, s) != -1;
}
...
bool OfxhImageEffectNodeBase::isSingleInstance() const {
  return _properties.getIntProperty(kOfxImageEffectPluginPropSingleInstance) !=
         0;
}
...
bool OfxhImageEffectNodeBase::getHostFrameThreading() const {
  return _properties.getIntProperty(
             kOfxImageEffectPluginPropHostFrameThreading) != 0;
}

/// get the overlay interact main entry if it exists

OfxPluginEntryPoint *
OfxhImageEffectNodeBase::getOverlayInteractMainEntry() const {
  return (OfxPluginEntryPoint *)(_properties.getPointerProperty(
...
bool OfxhImageEffectNodeBase::isBitDepthSupported(const std::string &s) const {
  return _properties.findStringPropValueIndex(
             kOfxImageEffectPropSupportedPixelDepths, s) != -1;
}
...
bool OfxhImageEffectNodeBase::fieldRenderTwiceAlways() const {
  return _properties.getIntProperty(
             kOfxImageEffectPluginPropFieldRenderTwiceAlways) != 0;
}
...
bool OfxhImageEffectNodeBase::isClipPreferencesSlaveParam(const std::string &s)
    const {
  return _properties.findStringPropValueIndex(
             kOfxImageEffectPropClipPreferencesSlaveParam, s) != -1;
}
}
}
}
