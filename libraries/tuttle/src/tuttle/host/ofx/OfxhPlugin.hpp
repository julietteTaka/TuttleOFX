...
protected:
  ...
  bool _isSupported; ///< Indicates if the plugin can be loaded (actions: Load
                     ///and Describe)

public:
  OfxhPlugin() : _binary(NULL), _index(-1), _isSupported(false) {}

  /**
   * construct this based on the struct returned by the getNthPlugin() in the
   * binary
   */
  OfxhPlugin(OfxhPluginBinary &bin, int idx, OfxPlugin &o)
      : OfxhPluginDesc(o), _binary(&bin), _index(idx), _isSupported(false) {}
  ...
  OfxhPlugin(OfxhPluginBinary &bin, int idx, const std::string &api,
             ...
             const std::string &rawIdentifier, int majorVersion,
             int minorVersion)
      : OfxhPluginDesc(api, apiVersion, identifier, rawIdentifier, majorVersion,
                       minorVersion),
        _binary(&bin), _index(idx), _isSupported(false) {}
...
private:
  ...
  void serialize(Archive &ar, const unsigned int version) {
    ...
    //		ar & BOOST_SERIALIZATION_NVP(_binary); // just a link, don't
    //save
