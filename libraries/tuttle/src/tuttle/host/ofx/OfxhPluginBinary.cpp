...
void OfxhPluginBinary::loadPluginInfo(OfxhPluginCache *cache) {
  ...
  OfxPlugin *(*getPlug)(int) =
      (OfxPlugin * (*)(int))_binary.findSymbol("OfxGetPlugin");
  ...
  if (getNo == 0 || getPlug == 0) {
    ...
    for (int i = 0; i < pluginCount; ++i) {
      ...
      APICache::OfxhPluginAPICacheI *api =
          cache->findApiHandler(plug.pluginApi, plug.apiVersion);
      assert(api);
      ...
      if (newPlug == NULL) {
        BOOST_THROW_EXCEPTION(exception::Unknown()
                              << exception::dev(
                                     "Error creating a new OfxhPlugin.")
                              << exception::pluginIdentifier(
