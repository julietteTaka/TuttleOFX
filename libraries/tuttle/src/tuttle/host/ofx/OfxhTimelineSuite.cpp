...
OfxStatus TimelineGetTime(void *effectInstance, double *time) {
  try {
    OfxhITimeline *me = dynamic_cast<OfxhITimeline *>(
        reinterpret_cast<OfxhIObject *>(effectInstance));
    if (!me)
 ...
 * timeline suite function
 */
OfxStatus TimelineGetBounds(void *effectInstance, double *firstTime,
                            double *lastTime) {
  try {
    OfxhITimeline *me = dynamic_cast<OfxhITimeline *>(
        reinterpret_cast<OfxhIObject *>(effectInstance));
 ...
 * our progress suite
 */
struct OfxTimeLineSuiteV1 gTimelineSuite = { TimelineGetTime, TimelineGotoTime,
                                             TimelineGetBounds };
}
