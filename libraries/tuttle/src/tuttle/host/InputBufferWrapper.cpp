...
void InputBufferWrapper::setMode(const EMode mode) {
  static std::map<EMode, const char *> toString = map_list_of(
      eModeBuffer, "bufferPointer")(eModeCallback, "callbackPointer");

  getNode().getParam("mode").setValue(toString.find(mode)->second);
}

void InputBufferWrapper::setBuffer(void *rawBuffer) {
  getNode().getParam("bufferPointer").setValue(boost::lexical_cast<std::string>(
      reinterpret_cast<std::ptrdiff_t>(rawBuffer)));
}

void InputBufferWrapper::set2DArrayBuffer(void *rawBuffer, const int width,
                                          const int height) {
  setBuffer(rawBuffer);
...
void InputBufferWrapper::set3DArrayBuffer(void *rawBuffer, const int width,
                                          const int height,
                                          const int nbComponents) {
  TUTTLE_TLOG_INFOS;
  TUTTLE_TLOG(TUTTLE_INFO, "[Inpput buffer wrapper] width = "
                               << width << ", height = " << height
...
void InputBufferWrapper::setComponents(const EPixelComponent components) {
  static std::map<EPixelComponent, const char *> toString =
      map_list_of(ePixelComponentRGBA, "RGBA")(ePixelComponentRGB, "RGB")(
...
void InputBufferWrapper::setBitDepth(const EBitDepth bitDepth) {
  static std::map<EBitDepth, const char *> toString =
      map_list_of(eBitDepthFloat, "Float")(eBitDepthUShort,
                                           "UShort")(eBitDepthUByte, "UByte");
...
void InputBufferWrapper::setOrientation(const EImageOrientation orientation) {
  static std::map<EImageOrientation, const char *> toString =
      map_list_of(eImageOrientationFromBottomToTop, "bottomToTop")(
          eImageOrientationFromTopToBottom, "topToBottom");

  getNode().getParam("orientation").setValue(
      toString.find(orientation)->second);
}

void InputBufferWrapper::setRawImageBuffer(
    void *rawBuffer, const int width, const int height,
...
//{
//	core().getMemoryCache().put( _outputClip.getClipIdentifier(),
//vData._time, _imageCache );
//	if( vData._outDegree > 0 )
//	{
//		_imageCache->addReference(
//ofx::imageEffect::OfxhImage::eReferenceOwnerHost, vData._outDegree ); // add a
//reference on this getNode() for each future usages
//	}
//}

void InputBufferWrapper::setCallback(
    CallbackInputImagePtr callback, CustomDataPtr customData,
    CallbackDestroyCustomDataPtr destroyCustomData) {
  getNode().getParam("callbackPointer").setValue(
      boost::lexical_cast<std::string>(
          reinterpret_cast<std::ptrdiff_t>(callback)));
  getNode().getParam("customData").setValue(boost::lexical_cast<std::string>(
      reinterpret_cast<std::ptrdiff_t>(customData)));
  getNode().getParam("callbackDestroyCustomData").setValue(
      boost::lexical_cast<std::string>(
          reinterpret_cast<std::ptrdiff_t>(destroyCustomData)));
}
}
}
