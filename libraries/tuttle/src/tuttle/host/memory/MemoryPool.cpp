...
public:
  PoolData(IPool &pool, const std::size_t size)
      : _pool(pool), _id(_count++), _reservedSize(size), _size(size),
        _pData(new char[size]), _refCount(0) {}
...
void intrusive_ptr_add_ref(IPoolData *pData) { pData->addRef(); }

void intrusive_ptr_release(IPoolData *pData) { pData->release(); }
...
MemoryPool::~MemoryPool() {
  /*	if( !_dataUsed.empty() )
          {
                  TUTTLE_LOG_WARNING( "[Memory Pool] Error inside memory pool.
     Some data always mark used at the destruction (nb elements:" <<
     _dataUsed.size() << ")" );
          }
  */
}
...
struct DataFitSize : public std::unary_function<PoolData *, void> {
  DataFitSize(std::size_t size)
      : _size(size), _bestMatchDiff(ULONG_MAX), _pBestMatch(NULL) {}
...
boost::intrusive_ptr<IPoolData> MemoryPool::allocate(const std::size_t size) {
...

  {
    ...
    pData = std::for_each(_dataUnused.begin(), _dataUnused.end(),
                          DataFitSize(size)).bestMatch();
  }
  ...
  if (size > availableSize) {
    std::stringstream s;
    s << "[Memory Pool] can't allocate size:" << size
      << " because memory available is equal to " << availableSize << " bytes";
...
std::size_t MemoryPool::updateMemoryAuthorizedWithRAM() {
  ...
  TUTTLE_LOG_DEBUG(
      TUTTLE_TRACE,
      "[Memory Pool] update memory authorized with RAM: " << _memoryAuthorized);
  return _memoryAuthorized;
}

namespace {

std::size_t accumulateReservedSize(const std::size_t &sum,
                                   const IPoolData *pData) {
  return sum + pData->reservedSize();
}

std::size_t accumulateWastedSize(const std::size_t &sum,
                                 const IPoolData *pData) {
...
std::size_t MemoryPool::getUsedMemorySize() const {
  boost::mutex::scoped_lock locker(_mutex);
  return std::accumulate(_dataUsed.begin(), _dataUsed.end(), 0,
                         &accumulateReservedSize);
}

std::size_t MemoryPool::getAllocatedAndUnusedMemorySize() const {
  boost::mutex::scoped_lock locker(_mutex);
  return std::accumulate(_dataUnused.begin(), _dataUnused.end(), 0,
                         &accumulateReservedSize);
}
...
std::size_t MemoryPool::getMaxMemorySize() const { return _memoryAuthorized; }
...
std::size_t MemoryPool::getWastedMemorySize() const {
  boost::mutex::scoped_lock locker(_mutex);
  return std::accumulate(_dataUsed.begin(), _dataUsed.end(), 0,
                         std::ptr_fun(&accumulateWastedSize));
}

std::size_t MemoryPool::getDataUsedSize() const { return _dataUsed.size(); }

std::size_t MemoryPool::getDataUnusedSize() const { return _dataUnused.size(); }
...
{
        os << "[Memory Pool] Used data:             " <<
memoryPool._dataUsed.size() << " bytes";
        os << "[Memory Pool] Unused data:           " <<
memoryPool.getDataUnusedSize() << " bytes";
        os << "[Memory Pool] All datas:             " <<
memoryPool.getDataUsedSize() << " bytes";
        os << "[Memory Pool] Memory authorized:     " <<
memoryPool.updateMemoryAuthorizedWithRAM() << " bytes";
        os << "";
        os << "[Memory Pool] Used memory:           " <<
memoryPool.getUsedMemorySize() << " bytes";
        os << "[Memory Pool] Allocated memory:      " <<
memoryPool.getAllocatedMemorySize() << " bytes";
        os << "[Memory Pool] Max memory:            " <<
memoryPool.getMaxMemorySize() << " bytes";
        os << "[Memory Pool] Available memory size: " <<
memoryPool.getAvailableMemorySize() << " bytes";
        os << "[Memory Pool] Wasted memory:         " <<
memoryPool.getWastedMemorySize() << " bytes";
        return ostream;
}*/
}
}
