...
void MemoryCache::put(const std::string &identifier, const double time,
                      CACHE_ELEMENT pData) {
  boost::mutex::scoped_lock lockerMap(_mutexMap);
  _map[Key(identifier, time)] = pData;
}

CACHE_ELEMENT MemoryCache::get(const std::string &identifier,
...
template <typename T>
struct FindValuePredicate
    : public std::unary_function<typename T::value_type, bool> {
...
MemoryCache::MAP::const_iterator
MemoryCache::getIteratorForValue(const CACHE_ELEMENT &pData) const {
  return std::find_if(_map.begin(), _map.end(), FindValuePredicate<MAP>(pData));
}

MemoryCache::MAP::iterator
MemoryCache::getIteratorForValue(const CACHE_ELEMENT &pData) {
  return std::find_if(_map.begin(), _map.end(), FindValuePredicate<MAP>(pData));
}
...
const std::string &
MemoryCache::getPluginName(const CACHE_ELEMENT &pData) const {
  boost::mutex::scoped_lock lockerMap(_mutexMap);
  MAP::const_iterator itr = getIteratorForValue(pData);
...
void MemoryCache::clearUnused() {
  ...
  for (MAP::iterator it = _map.begin(); it != _map.end();) {
    if (it->second->getReferenceCount(
            ofx::imageEffect::OfxhImage::eReferenceOwnerHost) <= 1) {
      _map.erase(it++); // post-increment here, increments 'it' and returns a
                        // copy of the original 'it' to be used by erase()
    } else {
      ++it;
    }
  }
...
std::ostream &operator<<(std::ostream &os, const MemoryCache &v) {
  ...
  BOOST_FOREACH(const MemoryCache::MAP::value_type & i, v._map) {
    os << i.first << " id:" << i.second->getId()
       << " ref host:" << i.second->getReferenceCount(
                              ofx::imageEffect::OfxhImage::eReferenceOwnerHost)
       << " ref plugins:"
       << i.second->getReferenceCount(
              ofx::imageEffect::OfxhImage::eReferenceOwnerPlugin) << std::endl;
  }
  return os;
}
}
