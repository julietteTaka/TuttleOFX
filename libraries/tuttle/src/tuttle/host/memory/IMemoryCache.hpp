 ...
 */
typedef ::boost::shared_ptr<tuttle::host::attribute::Image>
CACHE_ELEMENT; ///< @todo temporary solution..

struct Key {
  typedef Key This;
  Key(const std::string &identifier, const double &time)
      : _identifier(identifier), _time(time) {}
...
public:
  ...
  virtual void put(const std::string &identifier, const double time,
                   CACHE_ELEMENT pData) = 0;
  virtual CACHE_ELEMENT get(const std::string &identifier,
