...
public:
  ...
  /**
   * @brief Temporary solution ! Prepare the user graph, so we can call
   * getTimeDomain (and maybe others functions) on nodes.
   */
  void init();
  ...
  void computeGlobalHashAtTime(NodeHashContainer &outNodesHash,
                               const OfxTime time,
                               const NodeListArg &nodes = NodeListArg());
   ...
   * @brief Shortcut
   */
  bool compute(const NodeListArg &nodes,
...
public:
  ...
  //	const Node&          getNode( const std::string& name ) const { return
  //getNodes()[name]; }
  inline const Node &getNode(const std::string &name) const {
    return _nodes.at(name);
  }
  inline Node &getNode(const std::string &name) { return getNodes().at(name); }
  inline const InstanceCountMap &getInstanceCount() const {
...
public:
  ...
  void exportDot(const std::string &filename,
                 const EDotExportLevel level = eDotExportLevelSimple) const;
...
private:
  ...
  NodeMap _nodes;
  InstanceCountMap _instanceCount; ///< used to assign a unique name to each
                                   ///node
