...
void INode::setProcessData(Data *data) { _data = data; }

void INode::setProcessDataAtTime(DataAtTime *dataAtTime) {
  TUTTLE_TLOG(TUTTLE_TRACE, "setProcessDataAtTime \"" << getName() << "\" at "
                                                      << dataAtTime->_time);
  _dataAtTime[dataAtTime->_time] = dataAtTime;
}

void INode::clearProcessDataAtTime() { _dataAtTime.clear(); }
...
const INode::DataAtTime &INode::getData(const OfxTime time) const {
  // TUTTLE_TLOG( TUTTLE_TRACE, "- INode::getData(" << time << ") of " <<
  // getName() );
  DataAtTimeMap::const_iterator it = _dataAtTime.find(time);
...
INode::DataAtTime &INode::getData(const OfxTime time) {
  return const_cast<DataAtTime &>(
      const_cast<const This *>(this)->getData(time));
}
...
INode::DataAtTime &INode::getFirstData() {
  return const_cast<DataAtTime &>(
      const_cast<const This *>(this)->getFirstData());
}
...
INode::DataAtTime &INode::getLastData() {
  return const_cast<DataAtTime &>(
      const_cast<const This *>(this)->getLastData());
}
