...
public:
  ...
  typedef void (*CallbackOutputImagePtr)(OfxTime time,
                                         CustomDataPtr outputCustomData,
                                         void *rawdata, int width, int height,
                                         int rowSizeBytes, EBitDepth bitDepth,
                                         EPixelComponent components,
                                         EField field);
  typedef void (*CallbackDestroyCustomDataPtr)(CustomDataPtr outputCustomData);

  OutputBufferWrapper(INode &node) : _node(&node) {}
  OutputBufferWrapper() : _node(NULL) {}
  ~OutputBufferWrapper() {}

  INode &getNode() { return *_node; }

  void setCallback(CallbackOutputImagePtr callback,
