...
public:
  ...
  virtual const ofx::attribute::OfxhParam &
  ...
  virtual ofx::attribute::OfxhParam &
  getParamByScriptName(const std::string &name,
  ...
  virtual attribute::ClipImage &
  getClip(const std::string &name, const bool acceptPartialName = false) = 0;
  virtual const attribute::ClipImage &
  getClip(const std::string &name,
          const bool acceptPartialName = false) const = 0;
  ...
  /**
   * @brief Initialization pass to propagate informations from inputs to
   * outputs.
   * @param[in] processData
   * @remark Called on each node in a depth first search order. So you have the
   * guarantee that it has been called on each input nodes before.
   */
  virtual void preProcess1(graph::ProcessVertexAtTimeData &processData) {}

  /**
   * @brief Initialization pass to propagate informations from outputs to
   * inputs.
   * @param[in] processData
   * @remark Called on each node in a REVERSE depth first search order. So you
   * have the guarantee that it has been called on each output nodes before.
   * Output nodes are those who used the result of the current node.
   */
  virtual void
  preProcess2_reverse(graph::ProcessVertexAtTimeData &processData) {}

  /**
   * @brief The node can declare to be an identity operation.
   * In this case, the node is not processed and the rendering engine direcly
   *use the indicated input clip at a particular time.
   *
   * @param[in] processData
   ...
   */
  virtual bool isIdentity(const graph::ProcessVertexAtTimeData &processData,
                          std::string &clip, OfxTime &time) const = 0;
   ...
   */
  virtual void
  preProcess_infos(const graph::ProcessVertexAtTimeData &processData,
                   const OfxTime time,
  ...
  /**
   * @brief The process of all nodes is done for one frame, now finalize this
   * node.
   * @param[in] processData
   * @remark Called on each node in a depth first search order. So you have the
   * guarantee that it has been called on each input nodes before.
 ...
 */
inline INode *new_clone(const INode &a) { return a.clone(); }
...
inline std::string mapNodeTypeEnumToString(const INode::ENodeType e) {
  ...
  BOOST_THROW_EXCEPTION(
      exception::Bug() << exception::dev() + "Unrecognized ENodeType enum. (" +
                              e + ").");
}
}
}
