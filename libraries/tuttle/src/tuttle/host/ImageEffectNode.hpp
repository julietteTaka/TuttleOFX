...
public:
  ...
  std::string getLabel() const {
    return ofx::imageEffect::OfxhImageEffectNodeBase::getLabel();
  }
  const std::string &getName() const {
    return ofx::imageEffect::OfxhImageEffectNodeBase::getName();
  }
  void setName(const std::string &name) {
    return ofx::imageEffect::OfxhImageEffectNodeBase::setName(name);
  }
  std::size_t getNbParams() const {
    return ofx::attribute::OfxhParamSet::getNbParams();
  }
  ...
  const ofx::attribute::OfxhParam &
  getParamByScriptName(const std::string &name,
                       const bool acceptPartialName = false) const {
    return ofx::attribute::OfxhParamSet::getParamByScriptName(
        name, acceptPartialName);
  }
  ofx::attribute::OfxhParam &getParamByScriptName(const std::string &name,
                                                  const bool acceptPartialName =
                                                      false) {
    return ofx::attribute::OfxhParamSet::getParamByScriptName(
        name, acceptPartialName);
  }
  ...
  attribute::ClipImage &getClip(const std::string &name,
    ...
    return dynamic_cast<attribute::ClipImage &>(
        ofx::attribute::OfxhClipImageSet::getClip(name, acceptPartialName));
  }
  const attribute::ClipImage &
  getClip(const std::string &name, const bool acceptPartialName = false) const {
    return dynamic_cast<const attribute::ClipImage &>(
        ofx::attribute::OfxhClipImageSet::getClip(name, acceptPartialName));
  }
  ...
  INode::ClipTimesSetMap getTimesNeeded(const OfxTime time) const {
    return OfxhImageEffectNode::getFramesNeeded(time);
  }

  void preProcess1(graph::ProcessVertexAtTimeData &vData);
  void preProcess2_reverse(graph::ProcessVertexAtTimeData &vData);

  bool isIdentity(const graph::ProcessVertexAtTimeData &vData,
                  std::string &clip, OfxTime &time) const;
  void preProcess_infos(const graph::ProcessVertexAtTimeData &vData,
                        const OfxTime time,
  ...
  /// make a clip
  ofx::attribute::OfxhClipImage *
  newClipImage(const ofx::attribute::OfxhClipImageDescriptor &descriptor);

#ifndef SWIG
  /// vmessage
  void vmessage(const char *type, const char *id, const char *format,
  ...
  ofx::attribute::OfxhParam *newParam(
  ...
  void beginSequenceRenderAction(OfxTime startFrame, OfxTime endFrame,
                                 OfxTime step, bool interactive,
