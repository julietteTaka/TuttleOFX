...
inline void setStringVecValue(StringVec &sv, const std::string &value,
                              size_t index = 0) {
  size_t size = sv.size();
...
inline OfxRectD clamp(const OfxRectD &v, const OfxRectD &bounds) {
  OfxRectD r;
...
/// clamp the rect in v to the given bounds
inline OfxRectI clamp(const OfxRectI &v, const OfxRectI &bounds) {
  OfxRectI r;
...
inline OfxRectD rectUnion(const OfxRectD &a, const OfxRectD &b) {
  OfxRectD r;
