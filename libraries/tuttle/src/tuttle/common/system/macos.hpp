 ...
 */
template <typename T> class CFTypeContainer {
public:
  inline CFTypeContainer(const T &t = 0) : _type(t) {}

  inline CFTypeContainer(const CFTypeContainer &helper) : _type(helper._type) {
    if (_type)
      CFRetain(_type);
  }
...
public:
  inline CFStringContainer(const std::string &str)
      : CFTypeContainer<CFStringRef>(0), _string(str) {}
