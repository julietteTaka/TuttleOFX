...
/// @{
/**
 * @brief If you catch an error at the top level, you can print this information
 * to the user.
 * @remark User information.
 */
typedef ::boost::error_info<struct tag_userMessage, ::boost::error_info_sstream>
user;
...
typedef ::boost::error_info<struct tag_devMessage, ::boost::error_info_sstream>
dev;
 ...
 * @remark Dev information.
 */
typedef ::boost::error_info<struct tag_backtraceMessage,
                            ::boost::error_info_sstream> backtrace;
 ...
 * Each plugin can be instanciated in differents contexts (depending on the
 * declaration of supported contexts).
 * @remark Dev information.
 */
 ...
 * @remark Dev information.
 */
typedef ::boost::error_info<struct tag_pluginIdentifier, ::std::string>
pluginIdentifier;
/**
 * @brief Plugin name.
 * @remark User information.
 ...
 * When you call a base level function (C API) which returns an ofxStatus, you
 * can use this exception and fill it with the returned value using ofxStatus
 * tag.
 */
 ...
 *
 * Only returned in the case where the plug-in or host cannot continue to
...
/** @brief Status error code for an operation on or request for an unknown
 * object */
...
/**
 * @brief Status error code returned by plug-ins when they are missing host
 *functionality, either an API or some optional functionality (eg: custom
 *params).
 *
 * Plug-Ins returning this should post an appropriate error message stating what
 *they are missing.
 */
...
/** @brief Status error code for an operation attempting to create something
 * that exists */
...
/** @brief Status error code indicating that something failed due to memory
 * shortage */
...
/** @brief Status error code indicating that a given index was invalid or
 * unavailable */
struct BadIndex : virtual public Common {
  BadIndex() { *this << ofxStatus(kOfxStatErrBadIndex); }
};

/** @brief Status error code indicating that something failed due an illegal
 * value */
 ...
 * errors presumably detectable before the program executes, such as violations
 * of logical preconditions (cf. std::logic_error).
 ...
 *        but we prefer to keep a runtime check even in release (for the
 * moment).
 ...
 * * plugin: The host launch a process, but the input clips are not filled (eg.
 * NULL buffer pointer).
