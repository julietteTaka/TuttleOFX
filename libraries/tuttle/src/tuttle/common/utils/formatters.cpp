...
boost::shared_ptr<Formatter> Formatter::get() { return _formatter; }
...
void Formatter::setLogLevel_int(const int level) {
  ...
  default:
    setLogLevel(boost::log::trivial::warning);
    TUTTLE_LOG_WARNING("Unrecognized log level "
                       << level << ", fallback to \"warning\" (3).");
    break;
  }
}

int logLevel_stringToInt(const std::string &level) {
  static const std::vector<std::string> m = boost::assign::list_of("trace")(
      "debug")("info")("warning")("error")("fatal");
  std::string lowerStrLevel = level;
  boost::algorithm::to_lower(lowerStrLevel);

  std::vector<std::string>::const_iterator v =
      std::find(m.begin(), m.end(), lowerStrLevel);

  if (v == m.end()) {
    TUTTLE_LOG_WARNING("Unrecognized log level "
                       << quotes(level) << ", fallback to \"warning\" (3).");
    return 3;
  }
