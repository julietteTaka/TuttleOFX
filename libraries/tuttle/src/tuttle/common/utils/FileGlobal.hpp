...
#endif
    if (!_flux) {
      std::cerr << "impossible de creer le fichier global : " << fileName
                << std::endl;
    }
  }
 ...
 * @brief Output into the file define by FILEGLOBAL_FILENAME (if not specified,
 *by default "out.log")
 **/
#define FILEGLOBAL(...)                                                        \
  if (!FileGlobal::instance()._flux) {                                         \
...
#define FILEGLOBAL_VAR2(a, b)                                                  \
  ...
  FileGlobal::instance()._flux << #a << ": " << a << ", " << #b << ": " << b   \
                               << std::endl;
