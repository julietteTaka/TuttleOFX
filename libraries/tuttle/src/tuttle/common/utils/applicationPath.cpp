/*
 * This code is rewrite from the Trolltech Qt code applicationFilePath /
 * applicationDirPath.
 */
...
/*!
        ...
        If the application is in a read only drive, i.e. ROM, then the private
   path
...
#if defined(__WINDOWS__)
  {
    // We do MAX_PATH + 2 here, and request with MAX_PATH + 1, so we can handle
    // all paths
    // up to, and including MAX_PATH size perfectly fine with string
    // termination, as well
    // as easily detect if the file path is indeed larger than MAX_PATH, in
    // which case we
    // need to use the heap instead. This is a work-around, since contrary to
    // what the
    // MSDN documentation states, GetModuleFileName sometimes doesn't set the
    // ERROR_INSUFFICIENT_BUFFER error number, and we thus cannot rely on this
    // value if
    // GetModuleFileName(0, buffer, MAX_PATH) == MAX_PATH.
    // GetModuleFileName(0, buffer, MAX_PATH + 1) == MAX_PATH just means we hit
    // the normal
    // file path limit, and we handle it normally, if the result is MAX_PATH +
    // 1, we use
    ...
    if (v > 0) {
      ...
      // MAX_PATH sized buffer wasn't large enough to contain the full path, use
      // heap
      char *b = NULL;
      int i = 1;
      size_t size;
...
#elif defined(__MACOS__)
  {
    CFTypeContainer<CFURLRef> bundleURL(
        CFBundleCopyExecutableURL(CFBundleGetMainBundle()));
    if (bundleURL) {
      CFStringContainer cfPath(
          CFURLCopyFileSystemPath(bundleURL, kCFURLPOSIXPathStyle));
      if (cfPath) {
        return cfPath.str();
      }
    }
