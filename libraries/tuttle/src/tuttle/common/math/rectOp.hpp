...
template <class P>
inline OfxRectD pointsBoundingBox(const P &a, const P &b, const P &c,
                                  const P &d) {
  OfxRectD res;
...
inline OfxRectD pointsBoundingBox(const std::vector<Point2> &points) {
  ...
  for (typename std::vector<Point2>::const_iterator it = points.begin(),
                                                    itEnd = points.end();
       it != itEnd; ++it) {
    if (it->x < bounds.x1)
      bounds.x1 = it->x;
...
inline Point2 pointsMinXY(const std::vector<Point2> &points) {
  ...
  for (typename std::vector<Point2>::const_iterator it = points.begin() + 1,
                                                    itEnd = points.end();
       it != itEnd; ++it) {
    if (it->x < p.x)
      p.x = it->x;
...
template <class R> inline R rectanglesBoundingBox(const R &a, const R &b) {
  R res;
...
template <class R> inline R rectanglesIntersection(const R &a, const R &b) {
  R res;
...
template <class R> inline bool rectanglesIntersect(const R &a, const R &b) {
...
template <class R> inline bool rectangleAContainsB(const R &a, const R &b) {
