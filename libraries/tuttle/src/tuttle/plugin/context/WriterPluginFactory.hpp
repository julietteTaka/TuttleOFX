...
void describeWriterParamsInContext(OFX::ImageEffectDescriptor &desc,
  ...
  OFX::ChoiceParamDescriptor *channel =
      desc.defineChoiceParam(kTuttlePluginChannel);
  ...
  OFX::BooleanParamDescriptor *premult =
      desc.defineBooleanParam(kParamPremultiplied);
  premult->setLabel("Premultiplied");
  premult->setDefault(false);

  OFX::ChoiceParamDescriptor *existingFile =
      desc.defineChoiceParam(kParamWriterExistingFile);
  ...
  // existingFile->appendOption( kParamWriterExistingFile_reader ); // TODO: not
  // implemented yet.
  ...
  renderAlways->setHint(
      "This is only useful as a workaround for GUI applications.");
  //	renderAlways->setDefault( false );
  renderAlways->setDefault(
      true); // because tuttle is not declared as a background renderer
