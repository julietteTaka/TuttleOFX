...
void SamplerPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamFilter) {
    ...
    if ((_paramFilter->getValue() == terry::sampler::eParamFilterLanczos) ||
        (_paramFilter->getValue() == terry::sampler::eParamFilterGaussian)) {
...
void SamplerPlugin::fillProcessParams(SamplerProcessParams &samplerprocessParam)
    const {
  samplerprocessParam._filter =
      (terry::sampler::EParamFilter)_paramFilter->getValue();
  ...
  samplerprocessParam._outOfImageProcess =
      (terry::sampler::EParamFilterOutOfImage)_paramOutOfImage->getValue();
}
}
}
