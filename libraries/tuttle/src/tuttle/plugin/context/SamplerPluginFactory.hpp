...
void describeSamplerParamsInContext(OFX::ImageEffectDescriptor &desc,
...
#ifndef TUTTLE_PRODUCTION
      "gaussian: Gaussian sampler with parametrable filter size and sigma "
      "value\n"
#endif
      );
  ...
  OFX::DoubleParamDescriptor *filterSigma =
      desc.defineDoubleParam(kParamFilterSigma);
  ...
  OFX::DoubleParamDescriptor *filterSharpen =
      desc.defineDoubleParam(kParamFilterSharpen);
  ...
  OFX::ChoiceParamDescriptor *outOfImage =
      desc.defineChoiceParam(kParamFilterOutOfImage);
