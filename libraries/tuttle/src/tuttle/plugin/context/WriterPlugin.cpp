...
WriterPlugin::WriterPlugin(OfxImageEffectHandle handle)
    : ImageEffectGilPlugin(handle), _oneRender(false), _oneRenderAtTime(0) {
...
void WriterPlugin::changedParam(const OFX::InstanceChangedArgs &args,
...
void
WriterPlugin::getClipPreferences(OFX::ClipPreferencesSetter &clipPreferences) {
  // If pattern detected (frame varying on time)
  clipPreferences.setOutputFrameVarying(varyOnTime());
}

bool WriterPlugin::isIdentity(const OFX::RenderArguments &args,
                              OFX::Clip *&identityClip, OfxTime &identityTime) {
  EParamWriterExistingFile existingFile =
      static_cast<EParamWriterExistingFile>(_paramExistingFile->getValue());
  if (existingFile != eParamWriterExistingFile_overwrite) {
    ...
    case eParamWriterExistingFile_skip: {
      if (fileExists) {
        // We declare an empty clip as identity to disable the process of this
        // node.
        // This is not in the OpenFX standard. So this option only exist on
        // TuttleOFX host.
        identityClip = NULL;
        identityTime = 0;
        TUTTLE_TLOG_TRACE("[Plugin Writer] Identity node: "
                          << this->getName() << " at time: " << args.time
...
void WriterPlugin::beginSequenceRender(
