...
void GeneratorPlugin::changedParam(const OFX::InstanceChangedArgs &args,
  ...
  if (paramName == kParamMode) {
    ...
    case eParamModeSize: {
      ...
      _paramSizeWidth->setIsSecretAndDisabled(
          !specificRatio || orientation != eParamSizeOrientationX);
      _paramSizeHeight->setIsSecretAndDisabled(
          !specificRatio || orientation != eParamSizeOrientationY);
  ...
  } else if (paramName == kParamSizeSpecificRatio &&
...
void GeneratorPlugin::getClipPreferences(
...
bool GeneratorPlugin::getRegionOfDefinition(
