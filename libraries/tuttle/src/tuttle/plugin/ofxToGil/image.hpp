 ...
 * @param rod normally we don't need this parameter because we can get it from
 * the image,
 *            but there is a bug in nuke (which return the bounds),
 *            so we need to use the rod of the clip and not from the image.
 */
template <class View>
View getGilView(OFX::Image *img, const OfxRectI &pixelRod,
  ...
  //	OfxRectI imgrod = img->getRegionOfDefinition(); // bug in nuke returns
  //bounds... not the clip rod with renderscale...
  ...
  const point2<int> tileSize =
      point2<int>(bounds.x2 - bounds.x1, bounds.y2 - bounds.y1);
  ...
  TUTTLE_TLOG(TUTTLE_INFO, "[OFX to Gil] Row distance from image "
                               << img->getRowDistanceBytes() << " bytes");
  TUTTLE_TLOG(TUTTLE_INFO, "[OFX to Gil] Tile view, row size = "
                               << tileView.pixels().row_size());

  View fullView;
  const bool isTile = (bounds.x1 != pixelRod.x1 || bounds.y1 != pixelRod.y1 ||
  ...
  if (!isTile) {
    ...
    fullView = subimage_view(tileView, pixelRod.x1 - bounds.x1,
                             pixelRod.y1 - bounds.y1, pixelRod.x2 - pixelRod.x1,
                             pixelRod.y2 - pixelRod.y1);
  }

  TUTTLE_TLOG(TUTTLE_INFO, "[OFX to Gil] Full view, row size = "
                               << fullView.pixels().row_size());
  ...
  case eImageOrientationIndependant: // use memory order
  {
    ...
    if (img->getRowDistanceBytes() <
        0) // if the host use buffer ordered from top to bottom
    {
  ...
  case eImageOrientationFromTopToBottom: {
    TUTTLE_TLOG(TUTTLE_INFO,
                "[OFX to Gil] Image orientation from top to bottom");
    BOOST_ASSERT(!isTile); // can't manage ordering with tiles currently (no RoW
                           // information in OpenFX)
  ...
  case eImageOrientationFromBottomToTop: {
    TUTTLE_TLOG(TUTTLE_INFO,
                "[OFX to Gil] Image orientation from bottom to top");
