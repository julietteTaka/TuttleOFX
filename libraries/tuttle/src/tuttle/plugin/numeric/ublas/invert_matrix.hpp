// FROM
// http://ljk.imag.fr/membres/Pierre.Saramito/rheolef/source_html/ublas-invert_8h-source.html
...
template <class Matrix> bool invert_3x3(const Matrix &A, Matrix &result) {
...
template <class Matrix> bool invert(const Matrix &input, Matrix &inverse) {
...
template <class Matrix> Matrix invert(const Matrix &m, bool &is_singular) {
...
double
determinant(boost::numeric::ublas::matrix_expression<Matrix> const &mat_r) {
  using namespace boost::numeric;
