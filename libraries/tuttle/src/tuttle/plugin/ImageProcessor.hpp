...
public:
  ...
  ImageProcessor(OFX::ImageEffect &effect,
      ...
      : OfxProgress(effect), _effect(effect),
        _imageOrientation(imageOrientation),
        _nbThreads(0) // auto, maximum allowable number of CPUs will be used
  {
    _renderArgs.renderWindow.x1 = _renderArgs.renderWindow.y1 =
        _renderArgs.renderWindow.x2 = _renderArgs.renderWindow.y2 = 0;
  ...
  virtual void preProcess() {
    progressBegin(_renderWindowSize.y * _renderWindowSize.x);
  }
  ...
  virtual void setupAndProcess(const OFX::RenderArguments &args) {
    _renderArgs = args;
    _renderWindowSize.x =
        (_renderArgs.renderWindow.x2 - _renderArgs.renderWindow.x1);
    _renderWindowSize.y =
        (_renderArgs.renderWindow.y2 - _renderArgs.renderWindow.y1);
    try {
      setup(args);
    }
  ...
  /** @brief overridden from OFX::MultiThread::Processor. This function is
   * called once on each SMP thread by the base class */
  void multiThreadFunction(const unsigned int threadId,
                           const unsigned int nThreads) {
    // slice the y range into the number of threads it has
    const int dy =
        std::abs(_renderArgs.renderWindow.y2 - _renderArgs.renderWindow.y1);
  ...
  /** @brief this is called by multiThreadFunction to actually process images,
   * override in derived classes */
  virtual void multiThreadProcessImages(const OfxRectI &windowRoW) = 0;

  // to output clip coordinates
  OfxRectI
  translateRoWToOutputClipCoordinates(const OfxRectI &windowRoW) const {
  ...
  virtual void process() {
    ...
    if (_renderArgs.renderWindow.x2 - _renderArgs.renderWindow.x1 == 0 ||
      ...
      BOOST_THROW_EXCEPTION(exception::ImageFormat()
                            << exception::user("RenderWindow empty !"));
    }
    // call the pre MP pass
    preProcess();

    // call the base multi threading code, should put a pre & post thread calls
    // in too
