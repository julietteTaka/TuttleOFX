...
template <typename T> class OfxAllocator {
  static int size_all;
...
public:
  ...
  template <class U> OfxAllocator(const OfxAllocator<U> &) {}
...
public:
  inline pointer allocate(const size_type n, const void * = 0) {
    ...
    T *t = static_cast<T *>(
        OFX::memory::allocate(n * sizeof(T) /*, ImageEffect* handle = 0*/));
    //		T* t = (T*) malloc( n * sizeof(T) );
    // TUTTLE_TLOG( TUTTLE_TRACE, "allocate done (address:" << t << ") (+" << n
    // << ") " << size_all );
    return t;
  }
  ...
  inline void deallocate(void *ptr, size_type) {
    --size_all;
    // TUTTLE_TLOG( TUTTLE_TRACE, "Use OfxAllocator to deallocate (address:" <<
    // ptr << ") (-)" << size_all );
    OFX::memory::free(ptr);
    // free( ptr );
  ...
  template <class U>
  inline OfxAllocator<T> &operator=(const OfxAllocator<U> &) {
    return *this;
  }
