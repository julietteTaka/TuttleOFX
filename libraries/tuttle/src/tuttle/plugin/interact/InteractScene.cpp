...
InteractScene::InteractScene(OFX::ParamSet &params, const InteractInfos &infos)
    : _params(params), _infos(infos), _mouseDown(false),
      _multiSelectionEnabled(true), _creatingSelection(false),
      _manipulator(NULL), _manipulatorColor(NULL) {}
...
bool InteractScene::draw(const OFX::DrawArgs &args) {
  ...
  for (InteractObjectsVector::iterator it = _objects.begin(),
                                       itEnd = _objects.end();
       it != itEnd; ++it, ++itActive, ++itColor) {
    if (itActive->active()) {
...
bool InteractScene::penMotion(const OFX::PenArgs &args) {
  ...
  case eMotionRotate: {
    if (_manipulator) {
      rotate(_manipulator->getPosition(), penPosition,
             penPosition - _beginPenPosition);
    }
    break;
  }
  case eMotionScale: {
...
bool InteractScene::penDown(const OFX::PenArgs &args) {
    ...
    for (InteractObjectsVector::iterator it = _objects.begin(),
      ...
      if (m._axis != eAxisNone) {
        ...
        if (_motionType._axis == eAxisNone) {
          ...
          _motionType = m;
        } else if (m._axis == eAxisXY) // if we already register an object X or
                                       // Y and we found an XY intersection
        {
          oneSelectedObj = SelectedObject(&(*it), it->getPosition());
          _motionType = m;
        }
  ...
  if (_hasSelection) {
    if (result) {
      ...
      for (SelectedObjectVector::iterator it = _selected.begin(),
                                          itEnd = _selected.end();
           it != itEnd; ++it) {
...
bool InteractScene::penUp(const OFX::PenArgs &args) {
  ...
  if (_creatingSelection) {
    ...
    _selected.clear();
    // TUTTLE_LOG_VAR4( TUTTLE_TRACE, _selectionRect.x1, _selectionRect.y1,
    // _selectionRect.x2, _selectionRect.y2 );
    IsActiveFunctorVector::iterator itActive = _isActive.begin();
    for (InteractObjectsVector::iterator it = _objects.begin(),
                                         itEnd = _objects.end();
         it != itEnd; ++it, ++itActive) {
...
void InteractScene::rotate(const Point2 &center, const Point2 &from,
                           const Point2 &vec) {
  for (SelectedObjectVector::iterator it = _selected.begin(),
                                      itEnd = _selected.end();
       it != itEnd; ++it) {
