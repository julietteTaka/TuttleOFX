...
public:
  ParamRectangleFromTwoCorners(const InteractInfos &infos,
...
public:
  ...
  Point2 getPoint() const {
    ...
    return pointNormalizedXXcToCanonicalXY(
               ofxToGil((_paramB->getValue() + _paramA->getValue()) * 0.5),
               rodSize) +
           Point2(rod.x1, rod.y1);
  }
...
ParamRectangleFromTwoCorners<TFrame, coord>::ParamRectangleFromTwoCorners(
    const InteractInfos &infos, OFX::Double2DParam *paramA,
    OFX::Double2DParam *paramB, const TFrame &relativeFrame)
    : PointInteract(infos), _paramA(paramA), _paramB(paramB),
...
template <class TFrame, ECoordinateSystem coord>
bool ParamRectangleFromTwoCorners<TFrame, coord>::draw(
    const OFX::DrawArgs &args) const {
  PointInteract::draw(args);
  OfxRectD rod = _relativeFrame.getFrame(this->getTime());
  Point2 rodSize(rod.x2 - rod.x1, rod.y2 - rod.y1);
  Point2 pA(
      pointNormalizedXXcToCanonicalXY(ofxToGil(_paramA->getValue()), rodSize) +
      Point2(rod.x1, rod.y1));
  Point2 pB(
      pointNormalizedXXcToCanonicalXY(ofxToGil(_paramB->getValue()), rodSize) +
      Point2(rod.x1, rod.y1));
...
template <class TFrame, ECoordinateSystem coord>
typename ParamRectangleFromTwoCorners<TFrame, coord>::ESelectType
ParamRectangleFromTwoCorners<TFrame, coord>::selectType(
...
template <class TFrame, ECoordinateSystem coord>
MotionType ParamRectangleFromTwoCorners<TFrame, coord>::intersect(
