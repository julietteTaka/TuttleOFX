...
class ParamPointRelativePoint : public ParamPoint<TFrame, coord> {
  static const ECoordinateSystem notCenteredCoord =
      CoordinateSystemAxisXY<NotCenteredCoodinate<coord>::value>::value;

public:
  ParamPointRelativePoint(const InteractInfos &infos, OFX::Double2DParam *param,
...
public:
  Point2 getPoint() const {
    ...
    Point2 point =
        pointConvertCoordinateSystem<notCenteredCoord, eCoordinateSystemXY>(
            paramPoint, rodSize);
  ...
  void setPoint(const Scalar x, const Scalar y) {
    ...
    if (this->_frame.isEnabled()) {
      ...
      Point2 rodSize(rod.x2 - rod.x1, rod.y2 - rod.y1);
      Point2 point =
          pointConvertCoordinateSystem<eCoordinateSystemXY, notCenteredCoord>(
              Point2(x, y), rodSize);
      Point2 relativePoint =
          pointConvertCoordinateSystem<eCoordinateSystemXY, notCenteredCoord>(
              _relativePoint.getPoint(), rodSize);
      this->_param.setValue(point.x - relativePoint.x,
