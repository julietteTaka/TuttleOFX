...
template <ECoordinateSystem coord> struct CoordinateSystemNotCentered {
  static const ECoordinateSystem value = coord;
};
template <> struct CoordinateSystemNotCentered<eCoordinateSystemXXcn> {
  static const ECoordinateSystem value = eCoordinateSystemXXn;
};
template <> struct CoordinateSystemNotCentered<eCoordinateSystemXYc> {
...
public:
  ParamRectangleFromCenterSize(const InteractInfos &infos,
...
private:
  ...
  static std::string mapESelectTypeToString(const ESelectType &s) {
    ...
    BOOST_THROW_EXCEPTION(std::invalid_argument(
        "ESelectType: " + boost::lexical_cast<std::string>(s)));
    return "";
  }
...
public:
  ...
  Point2 getPoint() const { return _center.getPoint(); }
...
ParamRectangleFromCenterSize<TFrame, coord>::ParamRectangleFromCenterSize(
    const InteractInfos &infos, OFX::Double2DParam *paramCenter,
    OFX::Double2DParam *paramSize, const TFrame &frame)
    : PointInteract(infos), _center(infos, paramCenter, frame),
      _size(infos, paramSize, frame), _frame(frame) {}

template <class TFrame, ECoordinateSystem coord>
ParamRectangleFromCenterSize<TFrame, coord>::~ParamRectangleFromCenterSize() {}

template <class TFrame, ECoordinateSystem coord>
bool ParamRectangleFromCenterSize<TFrame, coord>::draw(
    const OFX::DrawArgs &args) const {
  _center.draw(args);
...
template <class TFrame, ECoordinateSystem coord>
typename ParamRectangleFromCenterSize<TFrame, coord>::ESelectType
ParamRectangleFromCenterSize<TFrame, coord>::selectType(
    const OFX::PenArgs &args) const {
  ...
  } else if (std::abs(max.x - p.x) < margeCanonical) // right
  {
    if (std::abs(max.y - p.y) < margeCanonical) // bottom
      ...
      return eSelectTypeR;
  } else if (std::abs(min.y - p.y) < margeCanonical && p.x > min.x &&
             p.x < max.x)
    return eSelectTypeT;
...
template <class TFrame, ECoordinateSystem coord>
MotionType ParamRectangleFromCenterSize<TFrame, coord>::intersect(
    const OFX::PenArgs &args) {
  ...
  TUTTLE_TLOG(TUTTLE_TRACE,
              "_selectType : " << mapESelectTypeToString(_selectType));
...
bool ParamRectangleFromCenterSize<TFrame, coord>::isIn(const OfxRectD &rect) {
  ...
  if (rect.x1 <= min.x && rect.x2 >= max.x && rect.y1 <= min.y &&
      rect.y2 >= max.y) {
    return true;
  }
  return false;
