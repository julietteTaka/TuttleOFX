...
struct Color : public IColor {
  ...
  Color(const OfxRGBAColourD &color) : _color(color) {}

  Color() { _color.r = _color.g = _color.b = _color.a = 1.0; }

  inline OfxRGBAColourD getColor(const OfxTime time) const { return _color; }
};

struct ColorRGBAParam : public IColor {
  const OFX::RGBAParam &_param;
  ColorRGBAParam(const OFX::RGBAParam *param) : _param(*param) {}
...
struct ColorRGBParam : public IColor {
  ...
  ColorRGBParam(const OFX::RGBParam *param) : _param(*param) {}
