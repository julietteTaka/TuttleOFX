...
template <typename Point2>
inline void displayRect(const Point2 &a, const Point2 &b,
...
inline void drawCurve(const std::vector<Point2> &vec) {
  glBegin(GL_LINE_STRIP);
  for (typename std::vector<Point2>::const_iterator it = vec.begin(),
                                                    itEnd = vec.end();
       it != itEnd; ++it) {
...
inline void drawCurves(const std::vector<std::vector<Point2> > &vec) {
  for (typename std::vector<std::vector<Point2> >::const_iterator
           it = vec.begin(),
           itEnd = vec.end();
       it != itEnd; ++it) {
    drawCurve(*it);
