...
public:
  ParamPoint(const InteractInfos &infos, OFX::Double2DParam *param,
             const TFrame &frame)
      : PointInteract(infos), _param(*param), _frame(frame) {}
...
public:
  Point2 getPoint() const {
    if (_frame.isEnabled()) {
      ...
      Point2 p = pointConvertCoordinateSystem<coord, eCoordinateSystemXY>(
          ofxToGil(_param.getValue()), rodSize);
