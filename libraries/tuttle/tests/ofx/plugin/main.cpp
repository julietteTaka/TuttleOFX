...
BOOST_AUTO_TEST_CASE(imageeffectplugin_serialization) {
  ...
  OfxhImageEffectPlugin *plugin =
      core().getImageEffectPluginById("tuttle.invert");
  ...
  const std::string testfile =
      (core().getPreferences().buildTuttleTestPath() /
       "test_imageEffectPlugin_serialization.xml").string();
  ...
  //// The new plugin re-created from the cache file is the same than the
  ///original
  ...
  const std::string testfile2 =
      (core().getPreferences().buildTuttleTestPath() /
       "test_imageEffectPlugin_serialization2.xml").string();
