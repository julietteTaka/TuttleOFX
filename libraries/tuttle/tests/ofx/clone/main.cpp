...
BOOST_AUTO_TEST_CASE(ofx_imageEffect_clones) {
  ...
  core().getPluginCache().addDirectoryToPath(
      BOOST_PP_STRINGIZE(TUTTLE_PLUGIN_PATH));
  ...
  //	BOOST_CHECK_THROW( plugin->getProperties().fetchPointerProperty(
  //kOfxPropInstanceData ), exception::LogicError );

  boost::scoped_ptr<tuttle::host::ImageEffectNode> ofxinst(
      dynamic_cast<tuttle::host::ImageEffectNode *>(
          plugin->createInstance(kOfxImageEffectContextReader)));
  ofxinst->setName("pluginTest");
  BOOST_CHECK(ofxinst->getProperties()
                  .fetchPointerProperty(kOfxPropInstanceData)
                  .getValue() != NULL);

  boost::scoped_ptr<tuttle::host::ImageEffectNode> ofxinstclone(
      ofxinst->clone());
  ...
  BOOST_CHECK(ofxinst->getProperties()
                  .fetchPointerProperty(kOfxPropInstanceData)
                  .getValue() != NULL);
  BOOST_CHECK(ofxinstclone->getProperties()
                  .fetchPointerProperty(kOfxPropInstanceData)
                  .getValue() != NULL);
  BOOST_CHECK(ofxinst->getProperties()
                  .fetchPointerProperty(kOfxPropInstanceData)
                  .getValue() != ofxinstclone->getProperties()
                                     .fetchPointerProperty(kOfxPropInstanceData)
                                     .getValue());
}
