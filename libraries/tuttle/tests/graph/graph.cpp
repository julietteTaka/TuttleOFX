...
BOOST_AUTO_TEST_CASE(graph_NodeInit) {
  ...
  BOOST_CHECK(compute(list_of(
      NodeInit("tuttle.pngreader")
          .setParam("filename", "TuttleOFX-data/image/png/color-chart.png")
          .setParamExp("bitDepth", "32f"))(NodeInit(
      "tuttle.pngwriter").setParam("filename", ".tests/graph/output.png"))));
...
BOOST_AUTO_TEST_CASE(graph_unconnect) {
  ...
  Graph g;
  g.addConnectedNodes(list_of(
      NodeInit("tuttle.pngreader")
          .setParam("filename", "TuttleOFX-data/image/png/color-chart.png")
          .setParamExp("bitDepth", "32f"))(NodeInit("tuttle.invert"))(
      NodeInit("tuttle.invert"))(NodeInit("tuttle.invert"))(NodeInit(
      "tuttle.pngwriter").setParam("filename", ".tests/graph/output.png")));
...
BOOST_AUTO_TEST_CASE(graph_replaceNodeConnections) {
  ...
  g.addConnectedNodes(list_of(
      NodeInit("tuttle.pngreader")
          .setParam("filename", "TuttleOFX-data/image/png/color-chart.png")
          .setParamExp("bitDepth", "32f"))(NodeInit("tuttle.invert"))(NodeInit(
      "tuttle.pngwriter").setParam("filename", ".tests/graph/output.png")));

  Graph::Node &inv = *g.getNodesByPlugin("tuttle.invert").front();

  Graph::Node &blur =
      g.addNode(NodeInit("tuttle.blur").setParam("size", 0.01, 0.02));
...
BOOST_AUTO_TEST_CASE(create_processGraph) {
  ...
  //	BOOST_CHECK_NO_THROW( g.compute( outputs ) ); /// @todo Bug: error with
  //merge
...
BOOST_AUTO_TEST_CASE(graph_compute) {
  ...
  TUTTLE_LOG_INFO("--> PLUGINS CONFIGURATION");
  read1.getParam("filename")
      .setValue("TuttleOFX-data/image/png/RGB16Million.png");
  read1.getParam("bitDepth").setValue(3);

  read2.getParam("filename")
      .setValue("TuttleOFX-data/image/png/RGB16Million.png");
  read2.getParam("bitDepth").setValue(3);
