...
public:
  ...
  TestEdge(const std::string &in, const std::string &out)
      : _nameIn(in), _nameOut(out), _name(out + "->" + in) {}
...
template <class G> struct vertex_label_writer {
...
template <class G> struct edge_label_writer {
...
template <template <typename> class T, class G> T<G> make(const G &g) {
  return T<G>(g);
}

template <>
void exportAsDOT<TestVertex, TestEdge>(
    std::ostream &os, const InternalGraph<TestVertex, TestEdge> &g) {
  ...
  boost::write_graphviz(
      os, g.getGraph(), make<vertex_label_writer>(g.getGraph()),
...
BOOST_AUTO_TEST_CASE(test_export) {
  ...
  typedef graph::InternalGraph<graph::TestVertex, graph::TestEdge>
  InternalGraph;
  ...
  graph.addEdge(nodesDescriptor[n1], nodesDescriptor[n2],
                graph::TestEdge(n1, n2));
  graph.addEdge(nodesDescriptor[n2], nodesDescriptor[n3],
                graph::TestEdge(n2, n3));
  ...
  graph::exportAsDOT<graph::TestVertex, graph::TestEdge>("boostgraphtest.dot",
                                                         graph);
  graph::exportAsDOT<graph::TestVertex, graph::TestEdge>("boostgraphTtest.dot",
                                                         graphT);

  TUTTLE_TLOG(TUTTLE_TRACE,
              "__________________________________________________");
  TUTTLE_TLOG(TUTTLE_TRACE, "graph:");
  //	std::vector<boost::default_color_type >
  //colormap(boost::num_vertices(graph.getGraph()));
  graph::visitor::Test_dfs<InternalGraph> testVisitorA(graph);
  //	boost::depth_first_search( graph.getGraph(),
  //boost::root_vertex(nodesDescriptor[n1]), boost::visitor(testVisitorA) );//,
  //colormap );
  ...
  for (InternalGraph::vertex_iterator i = vertices(graphT.getGraph()).first,
                                      iEnd = vertices(graphT.getGraph()).second;
       i != iEnd; ++i) {
  ...
  //	boost::depth_first_search( graphT.getGraph(),
  //boost::visitor(testVisitorB) );
...
BOOST_AUTO_TEST_CASE(check_cycle) {
  ...
  BOOST_CHECK_THROW(graph.addEdge(nodesDescriptor[n3], nodesDescriptor[n2],
                                  graph::TestEdge(n3, n2)),
                    exception::Logic);
  BOOST_CHECK_THROW(graph.addEdge(nodesDescriptor[n3], nodesDescriptor[n1],
                                  graph::TestEdge(n3, n1)),
                    exception::Logic);
...
BOOST_AUTO_TEST_CASE(test_create_add_remove_simple) {
  ...
  boost::add_edge(graph.vertex_set()[1], graph.vertex_set()[2], graph);
...
BOOST_AUTO_TEST_CASE(test_create_add_remove) {
  ...
  typedef boost::adjacency_list<boost::listS, boost::vecS, boost::undirectedS,
                                graph::TestVertex, graph::TestEdge> TestGraph;
  TestGraph graph;
  ...
  for (std::size_t i = 5; i < 8; ++i) {
    TestGraph::edge_descriptor ed = boost::add_edge(
        graph.vertex_set()[i], graph.vertex_set()[i + 1], graph).first;
  }
  ...
  for (std::size_t i = 0; i < 5; ++i) {
    ...
    boost::remove_vertex(
        vd, graph); // finally remove the vertex from the boost graph
  }
  ...
  //	for( Graph::vertex_iterator i = vertices( graph ).first, iEnd =
