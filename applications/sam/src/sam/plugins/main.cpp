...
std::string getDefaultValues(const tth::ofx::property::OfxhProperty &prop) {
  std::string s;
  if (!(prop.getType() == 3)) // if Pointer, we don't have _value and
                              // _defaultValue properties
  {
    int n = 0;
...
void printProperties(const tth::ofx::property::OfxhSet properties,
  ...
  if (context.size() == 0) {
    TUTTLE_COUT(color->_red << "Number of properties : " << properties.getSize()
                            << color->_std);
  } else {
    TUTTLE_COUT(color->_red << "Number of properties for \"" << context
                            << "\": " << properties.getSize() << color->_std);
  }

  std::string propertyId = "Property Name";
  propertyId.resize(50, ' ');
  TUTTLE_COUT(color->_red << "RW" << color->_std << " " << color->_red
                          << "ModifiedBy" << color->_std << "\t" << color->_red
                          << "Type" << color->_std << "\t" << color->_red
                          << "Size" << color->_std << "\t" << color->_red
                          << propertyId << color->_std << "\t" << color->_red
                          << "Default Values" << color->_std);

  tth::ofx::property::PropertyMap propMap = properties.getMap();
  for (tth::ofx::property::PropertyMap::const_iterator itProperty =
           propMap.begin();
       itProperty != propMap.end(); ++itProperty) {
    ...
    TUTTLE_COUT(color->_green
                << (prop.getPluginReadOnly()
                        ? (color->_green + "r-" + color->_std + " ")
                        : (color->_green + "rw" + color->_std + " "))
                << (prop.getModifiedBy() == tth::ofx::property::eModifiedByHost
                        ? (color->_green + "host" + color->_std + "  \t")
                        : (color->_green + "plugin" + color->_std + "\t"))
                << color->_green
                << (tth::ofx::property::mapTypeEnumToString(prop.getType()))
                << color->_std << "\t" << color->_green << "["
                << prop.getDimension() << "]" << color->_std << "\t"
                << color->_blue << propertyLabel << color->_std << "\t"
                << color->_green << "{ " << getDefaultValues(prop) << " }"
                << color->_std);
  }
}
...
void getPluginProperties(const std::string &plugName) {
  using namespace tuttle::common;
  boost::shared_ptr<Color> color(Color::get());
  tth::ofx::imageEffect::OfxhImageEffectPlugin *plug =
      tth::core().getImageEffectPluginById(plugName);

  if (!plug) {
    TUTTLE_LOG_ERROR(color->_red << "no plugin match to: " << plugName
                                 << color->_std);
    return;
  }
  ...
  tth::ofx::imageEffect::OfxhImageEffectPlugin::ContextSet contexts =
      plug->getContexts();
  ...
  if (properties) {
    ...
    const tth::ofx::property::OfxhSet properties =
        plug->getDescriptorInContext(*itContext).getProperties();
  ...
  if (clips | parameters) {
    if (plug->supportsContext(kOfxImageEffectContextReader)) {
      ...
      TUTTLE_LOG_ERROR("Plugin contexts not supported by the host. (" +
                       plugName + ")");
      return;
    }
  }
  if (clips) {
    ...
    typedef std::map<std::string,
                     tth::ofx::attribute::OfxhClipImageDescriptor *> ContextMap;

    // get contexts
    ContextMap::const_iterator it =
        plugInst->getDescriptor().getClips().begin();
    ...
    TUTTLE_COUT(color->_green << "[ " << strClipContexts << " ]"
                              << color->_std);

    // get propeties in each context
    ContextMap::const_iterator it2 =
        plugInst->getDescriptor().getClips().begin();
  ...
  if (parameters) {
    ...
    typedef std::map<std::string, tth::ofx::attribute::OfxhParamDescriptor *>
    ParamDescriptorMap;

    // get contexts
    ParamDescriptorMap::const_iterator it =
        plugInst->getDescriptor().getParams().begin();
    ...
    TUTTLE_COUT(color->_green << "[ " << strParamsContexts << " ]"
                              << color->_std);

    // get propeties in each context
    ParamDescriptorMap::const_iterator it2 =
        plugInst->getDescriptor().getParams().begin();
    for (; it2 != plugInst->getDescriptor().getParams().end(); it2++) {
      printProperties((*it2).second->getProperties(), (*it2).first);
    }
    /*
    const tth::ofx::property::OfxhSet paramProperties =
    plugInst->getDescriptor().getParamList().front().getProperties();
...
int main(int argc, char **argv) {
  ...
  mainOptions.add_options()(kHelpOptionString, kHelpOptionMessage)(
      kAllOptionString, kAllOptionMessage)(
      kFilterOptionString, bpo::value<std::string>(),
      kFilterOptionMessage)(kColorOptionString, kColorOptionMessage)(
      kPropertiesOptionString, kPropertiesOptionMessage)(kClipsOptionString,
                                                         kClipsOptionMessage)(
      kParametersOptionString, kParametersOptionMessage)(kBriefOptionString,
                                                         kBriefOptionMessage);
  ...
  try {
    ...
    if (const char *env_plugins_options = std::getenv("SAM_PLUGINS_OPTIONS")) {
      const std::vector<std::string> vecOptions =
          bpo::split_unix(env_plugins_options, " ");
  ...
  if (vm.count(kHelpOptionLongName)) {
    ...
    TUTTLE_COUT(
        color->_green << "\tsam-plugins [options] [specific_OpenFX_plugin]"
                      << color->_std);
    TUTTLE_COUT("");
    TUTTLE_COUT(color->_blue << "DESCRIPTION" << color->_std);
    TUTTLE_COUT("List OpenFX in OFX_PLUGIN_PATH by default.");
    TUTTLE_COUT(
        "And could print properties, parameters and clips for each plugins");
    TUTTLE_COUT("");
  ...
  if (vm.count(kBriefOptionLongName)) {
    TUTTLE_COUT(color->_green << "show informations about OpenFX plugins"
                              << color->_std);
    return 0;
  }
  ...
  if (vm.count(kFilterOptionLongName)) {
    bal::split(filters, vm[kFilterOptionLongName].as<std::string>(),
               bal::is_any_of(","));
  }

  if (vm.count(kAllOptionLongName) | (plugins.size() == 0)) {
    tth::core().preload();
    const std::vector<tth::ofx::imageEffect::OfxhImageEffectPlugin *> plugs =
        tth::core().getImageEffectPluginCache().getPlugins();
  ...
  try {
    tth::core().preload();
    // get the plugins names for research partials names (rawreader need to make
    // reference to the plug tuttle.reader)
    const std::vector<tth::ofx::imageEffect::OfxhImageEffectPlugin *> plugs =
        tth::core().getImageEffectPluginCache().getPlugins();
    ...
    BOOST_FOREACH(const std::string & plugin, plugins) {
      ...
      for (std::size_t i = 0; i < plugs.size(); i++) {
        ...
        for (std::size_t t = 0; t < termsPlugin.size(); t++) {
          std::size_t found1 =
              plugs.at(i)->getRawIdentifier().find("." + termsPlugin.at(t));
          std::size_t found2 =
              plugs.at(i)->getRawIdentifier().find(termsPlugin.at(t) + ".");
    ...
    TUTTLE_LOG_ERROR(
        color->_red << boost::current_exception_diagnostic_information()
                    << color->_std);
  }
