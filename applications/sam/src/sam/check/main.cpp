 ...
 */
EImageStatus checkImageStatus(Graph::Node &read, Graph::Node &stat,
                              Graph &graph, const bfs::path &filename) {
...
EImageStatus checkFile(Graph::Node &read, Graph::Node &stat, Graph &graph,
...
void checkSequence(Graph::Node &read, Graph::Node &stat, Graph &graph,
                   const sequenceParser::Sequence &seq) {
  for (sequenceParser::Time t = seq.getFirstTime(); t <= seq.getLastTime();
       ++t) {
...
int main(int argc, char **argv) {
  ...
  desc.add_options()(kHelpOptionString, kHelpOptionMessage)(
      kReaderOptionString, bpo::value(&readerId) /*->required()*/,
      kReaderOptionMessage)(kInputOptionString,
                            bpo::value(&inputs) /*->required()*/,
                            kInputOptionMessage)(
      kRangeOptionString, bpo::value(&range)->multitoken(),
      kRangeOptionMessage)(kBriefOptionString, kBriefOptionMessage)(
  ...
  try {
    ...
    if (const char *env_check_options = std::getenv("SAM_CHECK_OPTIONS")) {
      const std::vector<std::string> vecOptions =
          bpo::split_unix(env_check_options, " ");
  ...
  if (vm.count(kHelpOptionLongName) || vm.count(kInputDirOptionLongName) == 0) {
    ...
    TUTTLE_COUT(
        color->_green << "\tsam-check - detect black images in sequences"
                      << color->_std);
    TUTTLE_COUT("");
    TUTTLE_COUT(color->_blue << "SYNOPSIS" << color->_std);
    TUTTLE_COUT(color->_green << "\tsam-check [reader] [input] [options]"
                              << color->_std);
    TUTTLE_COUT("");
    ...
    TUTTLE_COUT("This tools process the PSNR of an image, and if it's null, "
                "the image is considered black.");
  ...
  try {
    ...
    read.getParam("explicitConversion")
        .setValue(3); // force reader to use float image buffer
        ...
        if (bfs::is_directory(path)) {
          ...
          BOOST_FOREACH(sequenceParser::FileObject & fObj, fObjects) {
            ...
            case sequenceParser::eMaskTypeFile: {
              const sequenceParser::File fFile =
                  dynamic_cast<const sequenceParser::File &>(fObj);
