...
bool Dummy::isDummyReaderNode(const std::string &nodeName) {
  return ((nodeName == READER_DUMMY_FULLNAME) ||
          (nodeName == READER_DUMMY_NAME) ||
          (nodeName == READER_DUMMY_SHORTNAME));
}

bool Dummy::isDummyWriterNode(const std::string &nodeName) {
  return ((nodeName == WRITER_DUMMY_FULLNAME) ||
          (nodeName == WRITER_DUMMY_NAME) ||
          (nodeName == WRITER_DUMMY_SHORTNAME));
}
...
bpo::options_description Dummy::getInfoOptions() {
  ...
  infoOptions.add_options()(kHelpOptionString, kHelpOptionMessage)(
      ...
      kPluginsOptionString, kPluginsOptionMessage)(kFormatOptionString,
                                                   kFormatOptionMessage);
  return infoOptions;
}
...
bpo::options_description Dummy::getOpenFXOptions() {
  ...
  openFXOptions.add_options()(kAttributesOptionString,
      ...
      kPropertiesOptionString, kPropertiesOptionMessage)(kClipsOptionString,
                                                         kClipsOptionMessage)(
      kClipOptionString, bpo::value<std::string>(),
      kClipOptionMessage)(kParametersOptionString, kParametersOptionMessage)(
...
void Dummy::getCommandLineParameters(bpo::variables_map &node_vm,
...
void
Dummy::getParametersFromCommandLine(std::vector<std::string> &parameters,
                                    const std::vector<std::string> &nodeArgs) {
  ...
  if (node_vm.count(kParamValuesOptionLongName)) {
    parameters =
        node_vm[kParamValuesOptionLongName].as<std::vector<std::string> >();
  }
}

void Dummy::getPathsFromCommandLine(std::vector<std::string> &paths,
                                    const std::vector<std::string> &nodeArgs) {
...
void
Dummy::getExtensionsFromCommandLine(std::vector<std::string> &extensions,
                                    const std::vector<std::string> &nodeArgs) {
...
Dummy::getAllSupportedNodes(const std::string &context) {
  const NodeList &nodeList =
      ttl::core().getImageEffectPluginCache().getPlugins();
...
std::vector<std::string>
Dummy::getSupportedExtensions(const std::string &context) {
  const NodeList &nodeList =
      ttl::core().getImageEffectPluginCache().getPlugins();
  ...
  BOOST_FOREACH(ttl::ofx::imageEffect::OfxhImageEffectPlugin * node, nodeList) {
    try {
      ...
      if (node->supportsContext(context)) {
        ...
        if (node->getDescriptorInContext(context).getProperties().hasProperty(
                kTuttleOfxImageEffectPropSupportedExtensions)) {
          const tuttle::host::ofx::property::OfxhProperty &prop =
              node->getDescriptorInContext(context)
                  .getProperties()
                  .fetchProperty(kTuttleOfxImageEffectPropSupportedExtensions);
  ...
  listOfExtensions.erase(
      std::unique(listOfExtensions.begin(), listOfExtensions.end()),
      listOfExtensions.end());
...
void Dummy::displayHelp(const std::string &nodeFullName) {
  ...
  TUTTLE_LOG_INFO(color->_blue << "TuttleOFX project [" << kUrlTuttleofxProject
                               << "]" << color->_std);
  TUTTLE_LOG_INFO("");
  TUTTLE_LOG_INFO(color->_blue << "NODE" << color->_std);
  TUTTLE_LOG_INFO(color->_green << "\tsam do " << nodeFullName
                                << " - OpenFX node." << color->_std);
  TUTTLE_LOG_INFO("");
  TUTTLE_LOG_INFO(color->_blue << "DESCRIPTION" << color->_std);
  TUTTLE_LOG_INFO(color->_green << "\tnode type: dummy (specific to TuttleOFX)"
                                << color->_std);
  // internal node help
  TUTTLE_LOG_INFO("");
  TUTTLE_LOG_INFO("Dummy reader call specific reader detected by the extension "
  ...
  TUTTLE_LOG_INFO("");
  TUTTLE_LOG_INFO(color->_blue << "DISPLAY OPTIONS (override the process)"
                               << color->_std);
...
void Dummy::foundAssociateSpecificDummyNode(
    std::string &inputNode, const std::string &dummyNodeName,
    const NodeList &nodeList, const std::vector<std::string> &nodeArgs) {
  ...
  if (nodeArgs.size() == 0) {
    ...
    BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                          << tuttle::exception::user() +
                                 "specify and input name (file or sequence).");
  }
  ...
  boost::algorithm::to_lower(inputExtension);
  unsigned int numberOfSupportedExtension =
      std::numeric_limits<unsigned int>::max();
  BOOST_FOREACH(ttl::ofx::imageEffect::OfxhImageEffectPlugin * node, nodeList) {
    const std::string pluginName = node->getRawIdentifier();
    if (boost::algorithm::find_first(pluginName, dummyNodeName)) {
      tuttle::host::ofx::imageEffect::OfxhImageEffectPlugin *plugin =
          tuttle::host::core().getImageEffectPluginById(pluginName);
      plugin->loadAndDescribeActions();
      const tuttle::host::ofx::imageEffect::OfxhImageEffectPlugin::ContextSet
      contexts = plugin->getContexts();
      const tuttle::host::ofx::property::OfxhSet &properties =
          plugin->getDescriptorInContext(*contexts.begin()).getProperties();
      if (properties.hasProperty(
        ...
        for (std::size_t n = 0; n < prop.getDimension(); ++n) {
          // TUTTLE_TLOG_VAR2( TUTTLE_TRACE, prop.getStringValue( n ),
          // inputExtension );
          if (prop.getStringValue(n) == inputExtension) {
            // TUTTLE_TLOG( TUTTLE_TRACE, pluginName << " [" <<
            // prop.getDimension() << "] can read " << prop.getStringValue( n )
            // );
            if (prop.getDimension() < numberOfSupportedExtension) {
              // Arbitrary solution...
              // If we found multiple plugins that support the requested format,
              // we select the plugin which supports the lowest number of
              // formats.
  ...
  if (numberOfSupportedExtension == std::numeric_limits<unsigned int>::max()) {
    BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                          << tuttle::exception::user() +
                                 "Unsupported extension \"" + inputExtension +
                                 "\".");
  }
  TUTTLE_LOG_WARNING("Replace " << dummyNodeName << " with: " << inputNode);
}

void Dummy::foundAssociateDummyNode(
    std::string &inputNode,
    const std::vector<ttl::ofx::imageEffect::OfxhImageEffectPlugin *> &nodeList,
    const std::vector<std::string> &nodeArgs) {
  getFullName(inputNode);
  foundAssociateSpecificDummyNode(inputNode, READER_DUMMY_NAME, nodeList,
                                  nodeArgs);
  foundAssociateSpecificDummyNode(inputNode, WRITER_DUMMY_NAME, nodeList,
                                  nodeArgs);
}
}
}
