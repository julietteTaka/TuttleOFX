...
/**
 * @brief Decomposes command line arguments into a list of options and a list of
 *node command lines. Groups the arguments without insterpretation at this step.
 *
 ...
 * @param[out]	cl_commands	list of node command lines (list of strings
 *groups without insterpretation at this step)
 * @param[in]	pipe	pipe character
 */
void decomposeCommandLine(const int argc, char **const argv,
  ...
  // first and last nodes may only be flags to sam-do itself
  {
    ...
    while (fNodeIt != fNodeItEnd && (*fNodeIt)[0] == '-') {
      ++fNodeIt;
    }
