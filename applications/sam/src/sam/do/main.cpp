...
void displayHelp(bpo::options_description &infoOptions,
  ...
  TUTTLE_COUT(std::left << color->_blue << "TuttleOFX project ["
                        << kUrlTuttleofxProject << "]" << color->_std);
  TUTTLE_COUT("");
  TUTTLE_COUT(color->_blue << "NAME" << color->_std);
  TUTTLE_COUT(color->_green
              << "\tsam do - A command line to execute a list of OpenFX nodes."
              << color->_std << std::endl);
  TUTTLE_COUT("");
  TUTTLE_COUT(color->_blue << "SYNOPSIS" << color->_std);
  TUTTLE_COUT("\tsam do [options]... [// node [node-options]... "
              "[[param=]value]...]... [// [options]...]"
              << std::endl);
  TUTTLE_COUT("");
  TUTTLE_COUT(color->_blue << "DESCRIPTION" << color->_std);
  TUTTLE_COUT(
      color->_green << "\tA command line to execute a list of OpenFX nodes."
                    << color->_std);
  TUTTLE_COUT(
      color->_green << "\tUse the sperarator // to pipe images between nodes."
                    << color->_std << std::endl);
  TUTTLE_COUT("");
  ...
  SAM_EXAMPLE_LINE_COUT("Print: ",
                        "sam do reader in.@.dpx // print color=full16ansi");
  SAM_EXAMPLE_LINE_COUT("Constant generator: ",
                        "sam do constant                // viewer");
  SAM_EXAMPLE_LINE_COUT("White constant generator: ",
                        "sam do constant color=1,1,1,1  // viewer");
  SAM_EXAMPLE_LINE_COUT("HD constant generator: ",
                        "sam do constant size=1920,1080 // viewer");
  SAM_EXAMPLE_LINE_COUT("Checkerboard generator: ",
                        "sam do checkerboard            // viewer");
  SAM_EXAMPLE_LINE_COUT("Checkerboard generator: ",
                        "sam do checkerboard width=500  // viewer");
  SAM_EXAMPLE_LINE_COUT("Checkerboard generator: ",
                        "sam do checkerboard width=1920 ratio=2.35 // viewer");
  /*
   SAM_EXAMPLE_LINE_COUT ( "Colorgradient generator: "  , "sam do colorgradient
   point0=1190,424 color0=0.246,0.44,0.254,1 \\" );
   SAM_EXAMPLE_LINE_COUT ( " "                      , "
   point1=458,726  color1=0.396,0.193,0.444,1 format=HD // viewer" );
   */
  SAM_EXAMPLE_LINE_COUT(
      "Text writing: ",
      "sam do constant // text text=\"hello\" size=80 // viewer");

  SAM_EXAMPLE_TITLE_COUT("Image sequence conversion and creation");
  SAM_EXAMPLE_LINE_COUT("Convert Image: ",
                        "sam do reader in.dpx // writer out.jpg");
  SAM_EXAMPLE_LINE_COUT("Convert Sequence: ",
                        "sam do reader in.####.dpx // writer out.####.jpg");
  SAM_EXAMPLE_LINE_COUT(
      "Select a range: ",
      "sam do reader in.####.dpx // writer out.####.jpg // --range=10,100");
  ...
  SAM_EXAMPLE_LINE_COUT("Crop: ", "sam do reader in.####.dpx // crop x1=20 "
                                  "x2=1000 y1=10 y2=300 // writer out.jpg");
  SAM_EXAMPLE_LINE_COUT("Fill: ", "sam do reader in.####.dpx // crop y1=10 "
                                  "y2=1060 mode=fill color=0.43,0.67,0.50 // "
                                  "writer out.jpg");
  SAM_EXAMPLE_LINE_COUT("Resize: ", "sam do reader in.####.dpx // resize "
                                    "size=1920,1080 // writer out.####.jpg");
  SAM_EXAMPLE_LINE_COUT("Upscaling: ", "sam do reader in.####.dpx // resize "
                                       "size=1920,1080 filter=lanczos  // "
                                       "writer out.####.jpg");
  SAM_EXAMPLE_LINE_COUT("Downscaling: ", "sam do reader in.####.dpx // resize "
                                         "size=720,576   filter=mitchell // "
                                         "writer out.####.jpg");

  SAM_EXAMPLE_TITLE_COUT("Color processing during conversion");
  SAM_EXAMPLE_LINE_COUT(
      "Lut :",
      "sam do reader in.####.dpx // lut lutFile.3dl // writer out.jpg");
  SAM_EXAMPLE_LINE_COUT("CTL: ", "sam do reader in.####.dpx // ctl "
                                 "file=ctlCode.ctl // writer out.####.jpg");
  SAM_EXAMPLE_LINE_COUT(
      "Gamma: ",
      "sam do reader in.####.dpx // gamma master=2.2 // writer out.####.jpg");

  SAM_EXAMPLE_TITLE_COUT("Image Sequence Numbering");
  SAM_EXAMPLE_LINE_COUT("Frames with or without padding: ", "image.@.jpg");
  SAM_EXAMPLE_LINE_COUT("Frames 1 to 100 padding 4: ",
  ...
  SAM_EXAMPLE_LINE_COUT(
      "Range process: ",
      "sam do reader in.@.dpx // writer out.@.exr // --range 50,100");
  SAM_EXAMPLE_LINE_COUT(
      "Single process: ",
      "sam do reader in.@.dpx // writer out.@.exr // --range 59");
  SAM_EXAMPLE_LINE_COUT(
      "Multiple CPUs: ",
      "sam do reader in.@.dpx // writer out.@.exr // --nb-cores 4");
  SAM_EXAMPLE_LINE_COUT(
      "Continues whatever happens: ",
      "sam do reader in.@.dpx // writer out.@.exr // --continueOnError");

  TUTTLE_COUT("");
  TUTTLE_COUT(color->_blue << "DISPLAY OPTIONS (replace the process)"
                           << color->_std);
...
void displayNodeHelp(std::string &nodeFullName, ttl::Graph::Node &currentNode,
  ...
  TUTTLE_COUT(color->_green << "\tsam do " << nodeFullName << " - OpenFX node."
                            << color->_std);
  TUTTLE_COUT("");
  TUTTLE_COUT(color->_blue << "DESCRIPTION" << color->_std);
  TUTTLE_COUT(color->_green << "\tnode type: " << ttl::mapNodeTypeEnumToString(
                                                      currentNode.getNodeType())
                            << color->_std);
  // internal node help
  if (currentNode.getNodeType() == ttl::INode::eNodeTypeImageEffect) {
    if (currentNode.asImageEffectNode()
            .getDescriptor()
            .getProperties()
            .hasProperty(kOfxImageEffectPluginPropGrouping)) {
      TUTTLE_COUT("\t" << color->_green
                       << currentNode.asImageEffectNode().getPluginGrouping()
                       << color->_std);
    }
  }
  TUTTLE_COUT("");
  if (currentNode.getProperties().hasProperty(kOfxPropPluginDescription)) {
    TUTTLE_COUT(currentNode.getProperties()
                    .fetchStringProperty(kOfxPropPluginDescription)
                    .getValue(0));
  } else {
  ...
  TUTTLE_COUT("");
  TUTTLE_COUT(color->_blue << "DISPLAY OPTIONS (override the process)"
                           << color->_std);
  TUTTLE_COUT(infoOptions);
  TUTTLE_COUT(color->_blue << "CONFIGURE PROCESS" << color->_std);
  TUTTLE_COUT(confOptions);
...
int addListOfSequencesInListOfProcess(
    boost::ptr_vector<sequenceParser::FileObject> &inputList,
    boost::ptr_vector<sequenceParser::FileObject> &outputList,
    const std::vector<std::string> &extensions) {
  sam::samdo::Dummy dummy;
  std::vector<std::string> exts =
      dummy.getSupportedExtensions(kOfxImageEffectContextReader);
  int count = 0;
  ...
  BOOST_FOREACH(sequenceParser::FileObject & fo, inputList) {
    ...
    if (fo.getMaskType() == sequenceParser::eMaskTypeSequence) {
      filename = static_cast<const sequenceParser::Sequence &>(fo)
                     .getAbsoluteStandardPattern();
    }
    if (fo.getMaskType() == sequenceParser::eMaskTypeFile) {
      filename =
          static_cast<const sequenceParser::File &>(fo).getAbsoluteFilename();
    }
...
std::string getAbsoluteFilename(const sequenceParser::FileObject &fo) {
  if (fo.getMaskType() == sequenceParser::eMaskTypeSequence)
    return static_cast<const sequenceParser::Sequence &>(fo)
        .getAbsoluteStandardPattern();
  if (fo.getMaskType() == sequenceParser::eMaskTypeFile)
...
bool isContextSupported(const ttl::Graph::Node *node,
  ...
  const ttl::ofx::property::OfxhProperty &prop =
      node->getProperties().fetchProperty(kOfxImageEffectPropSupportedContexts);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const std::vector<ttl::ofx::imageEffect::OfxhImageEffectPlugin *> &
    allNodes = ttl::core().getImageEffectPluginCache().getPlugins();
    ...
    // Analyze each part of the command line
    {
      ...
      try {
        ...
        infoOptions.add_options()(kHelpOptionString, kHelpOptionMessage)(
            kVersionOptionString, kVersionOptionMessage)(kNodesOptionString,
                                                         kNodesOptionMessage)(
            kColorOptionString, kColorOptionMessage)(kScriptOptionString,
                                                     kScriptOptionMessage)(
            kBriefOptionString, kBriefOptionMessage)(kExpertOptionString,
                                                     kExpertOptionMessage);
        bpo::options_description confOptions;
        confOptions.add_options()(kContinueOnErrorOptionString,
                                  bpo::value<bool>(),
                                  kContinueOnErrorOptionMessage)(
            kStopOnMissingFileOptionString, bpo::value<bool>(),
            kStopOnMissingFileOptionMessage)(kDisableProcessOptionString,
                                             kDisableProcessOptionMessage)(
            kForceIdentityNodesProcessOptionString,
            kForceIdentityNodesProcessOptionMessage)(
            kRangeOptionString, bpo::value<std::string>(), kRangeOptionMessage)(
            kFirstImageOptionString, bpo::value<int>(),
            kFirstImageOptionMessage)(kLastImageOptionString, bpo::value<int>(),
                                      kLastImageOptionMessage)(
            kRenderScaleOptionString, bpo::value<std::string>(),
            kRenderScaleOptionMessage)(
            kVerboseOptionString, bpo::value<std::string>()->default_value(
                                      kVerboseOptionDefaultValue),
            kVerboseOptionMessage)(kQuietOptionString, kQuietOptionMessage)(
            kNbCoresOptionString, bpo::value<std::size_t>(),
            kNbCoresOptionMessage);

        // describe hidden options
        bpo::options_description hidden;
        hidden.add_options()(kEnableColorOptionString,
        ...
        bpo::store(
            bpo::command_line_parser(cl_options).options(all_options).run(),
            samdo_vm);
        if (const char *env_do_options = std::getenv("SAM_DO_OPTIONS")) {
          const std::vector<std::string> vecOptions =
              bpo::split_unix(env_do_options, " ");
          bpo::store(
              bpo::command_line_parser(vecOptions).options(all_options).run(),
              samdo_vm);
        }
        ...
        if (samdo_vm.count(kEnableColorOptionLongName) && !script) {
          const std::string str =
              samdo_vm[kEnableColorOptionLongName].as<std::string>();
        ...
        formatter->setLogLevel_string(
            samdo_vm[kVerboseOptionLongName].as<std::string>());
        ...
        if (!(samdo_vm.count(kNodesOptionLongName) ||
              samdo_vm.count(kNodesListOptionLongName)) &&
            cl_commands.size() == 0) {
        ...
        const std::string logFilename =
            (ttl::core().getPreferences().getTuttleHomePath() / "sam-do.log")
                .string();
        std::ofstream logFile(logFilename.c_str());
        std::streambuf *strm_buffer =
            std::cerr.rdbuf();            // save cerr's output buffer
        ...
        if (samdo_vm.count(kNodesOptionLongName) ||
          ...
          BOOST_FOREACH(const ttl::ofx::imageEffect::OfxhImageEffectPlugin *
                            node,
                        allNodes) {
          ...
          const std::string indent =
              samdo_vm.count(kNodesOptionLongName) ? "\t" : "";
...

        {
          if (samdo_vm.count(kRangeOptionLongName) &&
              (samdo_vm.count(kFirstImageOptionLongName) ||
               samdo_vm.count(kLastImageOptionLongName))) {
            TUTTLE_LOG_ERROR(
                color->_red << "sam do: could not use " << kRangeOptionLongName
                            << "and " << kFirstImageOptionLongName << " or "
                            << kLastImageOptionLongName << " option."
                            << color->_std << std::endl);
            exit(255);
          }
          ...
          if (samdo_vm.count(kRangeOptionLongName)) {
            const std::string rangeStr =
                samdo_vm[kRangeOptionLongName].as<std::string>();
            std::vector<std::string> rangeVStr =
                boost::program_options::split_unix(rangeStr, " ,");
            range.reserve(rangeVStr.size());
            TUTTLE_TLOG(TUTTLE_TRACE, rangeVStr.size());

            BOOST_FOREACH(const std::string & rStr, rangeVStr) {
              range.push_back(
                  tuttle::host::attribute::extractValueFromExpression<
                      std::ssize_t>(rStr));
            }
          }
          if (range.size() == 1) {
        ...
        }
        {
          if (samdo_vm.count(kRenderScaleOptionLongName)) {
            const std::string renderscaleStr =
                samdo_vm[kRenderScaleOptionLongName].as<std::string>();
            std::vector<std::string> renderscaleVStr =
                boost::program_options::split_unix(renderscaleStr, " ,");
            ...
            BOOST_FOREACH(const std::string & rStr, renderscaleVStr) {
              renderscale.push_back(
                  tuttle::host::attribute::extractValueFromExpression<double>(
                      rStr));
            }
          }
        ...
        if (samdo_vm.count(kStopOnMissingFileOptionLongName)) {
          stopOnMissingFile =
              samdo_vm[kStopOnMissingFileOptionLongName].as<bool>();
        }

        forceIdentityNodesProcess =
            samdo_vm.count(kForceIdentityNodesProcessOptionLongName);
      }
      ...
      // Analyse options for each node
      {
        ...
        bpo::options_description infoOptions;
        infoOptions.add_options()(kHelpOptionString, kHelpOptionMessage)(
            kVersionOptionString, kVersionOptionMessage)(kExpertOptionString,
                                                         kExpertOptionMessage);
        bpo::options_description confOptions;
        confOptions.add_options()(kVerboseOptionString, kVerboseOptionMessage)(
            kIdOptionString, bpo::value<std::string>(),
            kIdOptionMessage)(kNbCoresOptionString, bpo::value<std::size_t>(),
                              kNbCoresOptionMessage);
        // describe openFX options
        bpo::options_description openfxOptions;
        openfxOptions.add_options()(kAttributesOptionString,
                                    kAttributesOptionMessage)(
            kPropertiesOptionString,
            kPropertiesOptionMessage)(kClipsOptionString, kClipsOptionMessage)(
            kClipOptionString, bpo::value<std::string>(), kClipOptionMessage)(
            kParametersOptionString, kParametersOptionMessage)(
            kParamInfosOptionString, bpo::value<std::string>(),
            kParamInfosOptionMessage)(kParamValuesOptionString,
                                      bpo::value<std::vector<std::string> >(),
                                      kParamValuesOptionMessage)
            // for auto completion
            (kParametersReduxOptionString, kParametersReduxOptionMessage)(
                kParamTypeOptionString, bpo::value<std::string>(),
                kParamTypeOptionMessage)(kParamPossibleValuesOptionString,
                                         bpo::value<std::string>(),
                                         kParamPossibleValuesOptionMessage)(
                kParamDefaultOptionString, bpo::value<std::string>(),
                kParamDefaultOptionMessage)(kParamGroupOptionString,
                                            kParamGroupOptionMessage);
        ...
        BOOST_FOREACH(const std::vector<std::string> & command, cl_commands) {
          ...
          std::copy(command.begin() + 1, command.end(),
          ...
          try {
            ...
            bpo::store(bpo::command_line_parser(nodeArgs)
                           .options(all_options)
                           .positional(param_values)
                           .run(),
                       node_vm);
            ...
            }
            {
              ...
              envVarName += "SAM_DO_";
              envVarName +=
                  boost::algorithm::replace_all_copy(nodeFullName, ".", "_");
              envVarName += "_OPTIONS";
            ...
            if (node_vm.count(kHelpOptionLongName)) {
              if (dummy.isDummyNode(userNodeName))
                ...
                displayNodeHelp(nodeFullName, currentNode, infoOptions,
                                confOptions);
              exit(0);
            }
            ...
            if (node_vm.count(kClipOptionLongName)) {
              ...
              if (!script) {
                ...
                TUTTLE_COUT(color->_blue << "CLIP: " << color->_green
                                         << clipName << color->_std);
                // TUTTLE_COUT( "supported bit depth: " <<
                // boost::algorithm::join( bitDepths, ", " ) );
                TUTTLE_COUT("supported components: "
                            << boost::algorithm::join(components, ", "));
                TUTTLE_COUT(
                    "pixel aspect ratio: " << clip.getPixelAspectRatio());
            ...
            if (node_vm.count(kParamInfosOptionLongName)) {
              const std::string attributeName =
                  node_vm[kParamInfosOptionLongName].as<std::string>();
              TUTTLE_COUT("\tsam do " << nodeFullName);
              TUTTLE_COUT(color->_blue << "PARAM: " << color->_green
                                       << attributeName << color->_std);
              ttl::ofx::attribute::OfxhParam &param =
                  currentNode.getParamByScriptName(attributeName);
              TUTTLE_COUT("");
              TUTTLE_COUT("\t" << color->_red
                               << (param.getSecret() ? "SECRET -- " : "")
                               << param.getScriptName() << ": "
                               << param.getParamTypeName() << " x"
                               << param.getSize() << color->_std);
              TUTTLE_COUT("");
            ...
            if (node_vm.count(kParamTypeOptionLongName)) {
              const std::string attributeName =
                  node_vm[kParamTypeOptionLongName].as<std::string>();
              ttl::ofx::attribute::OfxhParam &param =
                  currentNode.getParamByScriptName(attributeName);
            ...
            if (node_vm.count(kParamPossibleValuesOptionLongName)) {
              const std::string attributeName =
                  node_vm[kParamPossibleValuesOptionLongName].as<std::string>();
              ttl::ofx::attribute::OfxhParam &param =
                  currentNode.getParamByScriptName(attributeName);
            ...
            if (node_vm.count(kParamDefaultOptionLongName)) {
              const std::string attributeName =
                  node_vm[kParamDefaultOptionLongName].as<std::string>();
              ttl::ofx::attribute::OfxhParam &param =
                  currentNode.getParamByScriptName(attributeName);
              TUTTLE_COUT(getFormattedStringValue(
                  param.getProperties().fetchProperty(kOfxParamPropDefault)));
              exit(0);
            }
            if (node_vm.count(kParamGroupOptionLongName)) {
              if (currentNode.getNodeType() ==
                  ttl::INode::eNodeTypeImageEffect) {
                if (currentNode.asImageEffectNode()
                        .getDescriptor()
                        .getProperties()
                        .hasProperty(kOfxImageEffectPluginPropGrouping)) {
            ...
            if (node_vm.count(kIdOptionLongName)) {
              const std::string nodeId =
                  node_vm[kIdOptionLongName].as<std::string>();
              idNames.push_back(nodeId);
              graph.renameNode(currentNode, nodeId);
            }

            // Analyse attributes: parameters / clips
            typedef std::pair<ttl::ofx::attribute::OfxhClipImage *, std::string>
            ClipAndConnection;
            std::vector<ClipAndConnection> clipsToConnect;

            if (node_vm.count(kParamValuesOptionLongName)) {
              const std::vector<std::string> params =
                  node_vm[kParamValuesOptionLongName]
                      .as<std::vector<std::string> >();
              bool orderedParams = true;
              setParametersForNode(params, currentNode, clipsToConnect,
                                   dummy.isDummyNode(userNodeName),
                                   orderedParams);
            }

            // if it's a dummy, keeping parameters in node
            if (dummy.isDummyNode(userNodeName)) {
              currentNode.getParamByScriptName("originalnode")
                  .setValue(command.at(0));
              currentNode.getParamByScriptName("expression")
                  .setValue(boost::algorithm::join(nodeArgs, " "));
...
#else
            TUTTLE_LOG_ERROR(
                "Debug: " << boost::current_exception_diagnostic_information());
#endif
            exit(254);
          }
...
#ifdef TUTTLE_PRODUCTION
            TUTTLE_LOG_ERROR(
                "Error: " << *boost::get_error_info<tuttle::exception::user>(
                                  e));
#else
            TUTTLE_LOG_ERROR(
                "Debug: " << boost::current_exception_diagnostic_information());
    ...
    TUTTLE_LOG_TRACE(color->_blue << "********** graph composition **********"
                                  << color->_std);
    ...
    boost::ptr_vector<boost::ptr_vector<sequenceParser::FileObject> >
    listOfSequencesPerReaderNode;
    ...
    if (facticesNodes.size()) {
      ...
      sequenceParser::EMaskType researchMask =
          sequenceParser::eMaskTypeSequence | sequenceParser::eMaskTypeFile;
      sequenceParser::EMaskOptions descriptionMask =
          sequenceParser::eMaskOptionsAbsolutePath |
          sequenceParser::eMaskOptionsRecursive;
      ...
      BOOST_FOREACH(ttl::Graph::Node * n, facticesNodes) {
        std::string name =
            n->getParamByScriptName("originalnode").getStringValue();
        ...
        if (dummy.isDummyReaderNode(name)) {
          ...
          boost::ptr_vector<sequenceParser::FileObject>
          listOfSequencesForThisNode;

          // browse all paths in parameters
          BOOST_FOREACH(std::string & inputPath, paths) {
            size_t count = 0;
            TUTTLE_LOG_TRACE("[sam-do] " << n->getName()
                                         << " browse: " << inputPath);

            // get filenames adn sequences
            boost::ptr_vector<sequenceParser::FileObject> listOfSequences =
                sequenceParser::fileObjectInDirectory(
                    inputPath, filters, researchMask, descriptionMask);
            count += addListOfSequencesInListOfProcess(
                listOfSequences, listOfSequencesForThisNode, extensions);

            bfs::path p(inputPath);
            if (!p.extension().string().size()) {
              for (bfs::recursive_directory_iterator end, dir(inputPath);
                   dir != end; ++dir) {
                if (bfs::is_directory(*dir)) {
                  bfs::path currentPath = (bfs::path) * dir;
                  boost::ptr_vector<sequenceParser::FileObject>
                  listOfSequences = sequenceParser::fileObjectInDirectory(
                      currentPath.string(), filters, researchMask,
                      descriptionMask);
                  count += addListOfSequencesInListOfProcess(
                      listOfSequences, listOfSequencesForThisNode, extensions);
                }
          ...
          numberOfLoop =
              std::min(numberOfLoop, listOfSequencesForThisNode.size());

          listOfSequencesPerReaderNode.push_back(
              new_clone(listOfSequencesForThisNode));
        }
        if (dummy.isDummyWriterNode(name)) {
          // in case of dummy writer, only add the first path, else throw an
          // exception
          if (paths.size() > 1)
            BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                                  << tuttle::exception::user() +
                                         "unable to set mutli path in writer.");

          if (paths.size()) {
            listOfSequencesPerWriterNode.push_back(
                paths.at(0)); // push only the first path
    ...
    if (!listOfSequencesPerReaderNode.size() || !facticesNodes.size() ||
        ((listOfSequencesPerReaderNode.size() ||
          listOfSequencesPerWriterNode.size()) &&
         !numberOfLoop)) {
      numberOfLoop = 1;
    }
    ...
    BOOST_FOREACH(ttl::Graph::Node * n, nodes) {
      if (isContextSupported(n, kOfxImageEffectContextReader)) {
        filenames.push_back(
            n->getParamByScriptName("filename").getStringValue());
        haveReader = true;
      }
    ...
    TUTTLE_LOG_TRACE(
        "[sam-do] writer have extension: " << writerHaveExtensionInParameter);
    TUTTLE_LOG_TRACE("[sam-do] graph have reader : " << haveReader);
    TUTTLE_LOG_TRACE("[sam-do] graph have writer : " << haveWriter);
    TUTTLE_LOG_TRACE("[sam-do] number of sequence per reader : "
                     << listOfSequencesPerReaderNode.size());
    TUTTLE_LOG_TRACE("[sam-do] number of sequence per writer : "
                     << listOfSequencesPerWriterNode.size());

    if ((writerHaveExtensionInParameter || haveWriter || haveReader) &&
        numberOfLoop > 1) {
      BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                            << tuttle::exception::user() +
                                   "several sequences can’t be transformed "
                                   "into a single sequence or a movie for the "
                                   "moment.");
    }
    ...
    for (size_t loop = 0; loop < numberOfLoop; ++loop) {
      TUTTLE_LOG_TRACE("[sam-do] " << color->_red << "graph processing " << loop
                                   << color->_std);
      ...
      for (size_t nNode = 0; nNode < facticesNodes.size(); ++nNode) {
        ...
        std::string name = readerToReplace->getParamByScriptName("originalnode")
                               .getStringValue();
        dummy.getFullName(name);
        ...
        if (dummy.isDummyReaderNode(name)) {
          std::string baseSrcPath =
              readerToReplace->getParam("expression").getStringValue();
          ...
          if (refPath.extension().string().size()) {
            ...
            sequenceParser::FileObject &fo =
                listOfSequencesPerReaderNode.at(countDummyReader).at(loop);
        ...
        } else // writer dummy
        {
          ...
          if (refPath.extension().string().size()) {
            ...
            if (!listOfSequencesPerReaderNode.size()) {
              if (!filenames.size())
                BOOST_THROW_EXCEPTION(
                    ...
                    << tuttle::exception::user() +
                           "no pattern was found to set the output filename.");
              baseSrcPath = filenames.at(0);
            } else {
              baseSrcPath =
                  facticesNodes.at(0)->getParam("expression").getStringValue();
            }
            std::vector<std::string> srcPaths;
            const std::vector<std::string> srcNodeArgs =
                bpo::split_unix(baseSrcPath, " ");
            dummy.getPathsFromCommandLine(srcPaths, srcNodeArgs);

            std::vector<std::string> extensions;
            const std::vector<std::string> dstNodeArgs =
                bpo::split_unix(baseDstPath, " ");

            dummy.getExtensionsFromCommandLine(extensions, dstNodeArgs);
            if (extensions.size() > 1)
              BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                                    << tuttle::exception::user() +
                                           "many ext parameter are specified.");

            if (!listOfSequencesPerReaderNode.size()) {
              filename = bfs::path(baseSrcPath).filename().string();
            } else {
              if (listOfSequencesPerReaderNode.size() > 1 &&
                  numberOfLoop == 1) {
                filename = readerToReplace->getParamByScriptName("expression")
                               .getStringValue();
              } else {
                sequenceParser::FileObject &fo =
                    listOfSequencesPerReaderNode.at(0).at(loop);
                filename = getAbsoluteFilename(fo);
              }
            ...
            TUTTLE_TLOG_VAR3(TUTTLE_TRACE, baseSrcPath, filename,
                             srcPaths.at(0));
            ...
            if (extensions.size() > 0) {
              ...
              ext = extensions.at(0);
              std::vector<std::string> exts =
                  dummy.getSupportedExtensions(kOfxImageEffectContextWriter);
        ...
        TUTTLE_LOG_TRACE(readerToReplace->getName() << " => " << name << " : "
                                                    << args.at(0));
        ...
        bool orderedParams = true;
        setParametersForNode(args, currentNode, clipsToConnect, false,
                             orderedParams);
...
#ifdef TUTTLE_PRODUCTION
    TUTTLE_LOG_ERROR(
        "[sam-do] Error: " << *boost::get_error_info<tuttle::exception::user>(
                                   e));
#else
    TUTTLE_LOG_ERROR("[sam-do] Debug: "
                     << boost::current_exception_diagnostic_information());
