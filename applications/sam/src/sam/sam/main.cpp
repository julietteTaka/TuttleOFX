...
bfs::path retrieveToolFullPath(const std::string &toolName,
...
std::vector<bfs::path>
retrieveAllSamCommands(const std::vector<bfs::path> &dirs) {
  ...
  BOOST_FOREACH(PathMap::const_reference p, allRes) { res.push_back(p.second); }
  return res;
}
}
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    infoOptions.add_options()(kHelpOptionString, kHelpOptionMessage)(
        ...
        (kCommandsOptionString, kCommandsOptionMessage)(kColorOptionString,
                                                        kColorOptionMessage);

    // describe hidden options
    bpo::options_description hidden;
    hidden.add_options()(kCommandsListOptionString, kCommandsListOptionMessage)(
        kBinariesListOptionString, kBinariesListOptionMessage)(
        kEnableColorOptionString, bpo::value<std::string>(),
    ...
    bpo::store(bpo::command_line_parser(cl_options).options(all_options).run(),
               sam_vm);
    ...
    if (const char *env_sam_options = std::getenv("SAM_OPTIONS")) {
      ...
      bpo::store(
          bpo::command_line_parser(envOptions).options(all_options).run(),
          sam_vm);
    }
    bpo::notify(sam_vm);
    ...
    if (sam_vm.count(kEnableColorOptionLongName)) {
      const std::string str =
          sam_vm[kEnableColorOptionLongName].as<std::string>();
    ...
    const std::vector<bfs::path> searchPaths =
        retrieveSearchPaths(samDirectory);
    ...
    if ((sam_vm.count(kHelpOptionLongName) || (argc == 1)) ||
      ...
      TUTTLE_COUT(color->_green << "\tsam - A set of command line tools."
                                << color->_std);
      TUTTLE_COUT("");
      ...
      BOOST_FOREACH(const bfs::path & c, cmds) {
        std::cout << std::left << "\t" << std::setw(10)
                  << c.filename().string().substr(4) << std::flush;
    ...
    if (sam_vm.count(kBinariesListOptionLongName)) {
      const std::vector<bfs::path> cmds = retrieveAllSamCommands(searchPaths);
      BOOST_FOREACH(const bfs::path & c, cmds) { TUTTLE_COUT(c.string()); }
      exit(0);
    }
    ...
    const std::string fullcmd(toolFullPath.string() + " " +
                              boost::algorithm::join(sam_cmd, " "));
    ...
    TUTTLE_LOG_ERROR("Error in command line: "
