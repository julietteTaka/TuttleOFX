 ...
 */
EImageStatus diffImageStatus(Graph::Node &read1, Graph::Node &read2,
                             Graph::Node &stat, Graph &graph,
  ...
  try {
    ...
    for (size_t i = 0; i < generatorOptions.size(); i++) {
      ...
      if (optvalue == 0) {
        ...
        switch (optList.size()) {
        case 1: { // not a number, set a string parameter
        ...
        default: {
          ...
          TUTTLE_LOG_ERROR("unable to process " << optList.size()
                                                << " arguments");
          break;
        }
        }
 ...
 */
EImageStatus diffFile(Graph::Node &read1, Graph::Node &read2, Graph::Node &stat,
                      Graph &graph, const bfs::path &filename1,
                      const bfs::path &filename2) {
  EImageStatus s =
      diffImageStatus(read1, read2, stat, graph, filename1, filename2);
...
/**
 * @brief Difference between 1 reader and 1 generator associated with 1 file and
 * vector of options for the generator
 */
EImageStatus diffFile(Graph::Node &read1, Graph::Node &read2, Graph::Node &stat,
                      Graph &graph, const bfs::path &filename1,
                      const std::vector<std::string> &generatorOptions) {
  EImageStatus s =
      diffImageStatus(read1, read2, stat, graph, filename1, generatorOptions);
...
void diffSequence(Graph::Node &read1, Graph::Node &read2, Graph::Node &stat,
                  Graph &graph, const sequenceParser::Sequence &seq1,
                  const sequenceParser::Sequence &seq2) {
  for (sequenceParser::Time t = seq1.getFirstTime(); t <= seq1.getLastTime();
       ++t) {
    diffFile(read1, read2, stat, graph, seq1.getAbsoluteFilenameAt(t),
             seq2.getAbsoluteFilenameAt(t));
  }
}
...
void displayHelp(bpo::options_description &desc) {
  ...
  TUTTLE_LOG_INFO(
      color->_green
      << "\tsam-diff - compute difference between 2 images/sequences"
      << color->_std);
  TUTTLE_LOG_INFO("");
  TUTTLE_LOG_INFO(color->_blue << "SYNOPSIS" << color->_std);
  TUTTLE_LOG_INFO(
      color->_green << "\tsam-diff [reader] [input] [reader] [input] [options]"
                    << color->_std);
  TUTTLE_LOG_INFO("");
  TUTTLE_LOG_INFO(color->_blue << "DESCRIPTION" << color->_std);
  TUTTLE_LOG_INFO(color->_green << "\tDiff if sequence have black images."
                                << color->_std);
  TUTTLE_LOG_INFO(color->_green << "\tThis tools process the PSNR of an image, "
                                   "and if it's null, the image is considered "
                                   "black." << color->_std);
  TUTTLE_LOG_INFO(color->_green << "\tOnly compare RGB layout, not Alpha."
                                << color->_std);
  TUTTLE_LOG_INFO("");
  ...
  SAM_EXAMPLE_LINE_COUT("", "sam-diff --reader tuttle.jpegreader --input "
                            "path/image.jpg --reader tuttle.jpegreader --input "
                            "anotherPath/image.jpg");
  SAM_EXAMPLE_TITLE_COUT("Compare two sequences: ");
  SAM_EXAMPLE_LINE_COUT("", "sam-diff --reader tuttle.jpegreader --input "
                            "path/seq.@.jpg --reader tuttle.jpegreader --input "
                            "anotherPath/seq.@.jpg --range 677836 677839");
  SAM_EXAMPLE_TITLE_COUT("Compare one sequence with one generator (generator "
                         "need to be every time the second node): ");
  SAM_EXAMPLE_LINE_COUT("", "sam-diff --reader tuttle.jpegreader --input "
                            "path/seq.@.jpg --reader tuttle.constant "
                            "--generator-args width=500 components=rgb --range "
                            "677836 677839");
  TUTTLE_LOG_INFO("");
}
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    desc.add_options()(kHelpOptionString, kHelpOptionMessage)(
        ...
        kRangeOptionString, bpo::value(&range)->multitoken(),
        kRangeOptionMessage)(kGeneratorArgsOptionString,
                             bpo::value(&generator)->multitoken(),
                             kGeneratorArgsOptionMessage)(
    ...
    hidden.add_options()(kEnableColorOptionString, bpo::value<std::string>(),
    ...
    try {
      ...
      if (const char *env_diff_options = std::getenv("SAM_DIFF_OPTIONS")) {
        const std::vector<std::string> vecOptions =
            bpo::split_unix(env_diff_options, " ");
    ...
    if (vm.count(kGeneratorArgsOptionLongName)) {
      generator =
          vm[kGeneratorArgsOptionLongName].as<std::vector<std::string> >();
    }
    ...
    case 2: {
      ...
      if (bfs::exists(path1)) {
        if (!bfs::exists(path2)) {
          TUTTLE_LOG_ERROR(
              "could not find file or directory 2 (first is not a sequence).");
          return 0;
        }
        /*
         ...
         {
         diffSequence( read1, read2, stat, graph, dynamic_cast<const Sequence&>(
         *fObj ) );
         break;
         }
         case eMaskTypeFile:
         ...
         diffFile( read1, read2, stat, graph, fFile.getAbsoluteFilename(),
         fFile.getAbsoluteFilename() );
         break;
         }
        ...
        try {
          ...
          TUTTLE_LOG_WARNING("Unrecognized pattern " << path1 << " or "
                                                     << path2);
          return eReturnCodeApplicationError;
        }
      }
    ...
    TUTTLE_LOG_ERROR(
        "sam-diff error " << boost::current_exception_diagnostic_information());
    return 255;
  }
