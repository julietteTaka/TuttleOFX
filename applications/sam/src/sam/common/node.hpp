...
inline std::string retrieveNodeFullname(const std::string &userId) {
  ...
  PluginMap;
  const PluginMap &pluginsById =
      tuttle::host::core().getImageEffectPluginCache().getPluginsByID();
  ...
  BOOST_FOREACH(const PluginMap::value_type & plugin, pluginsById) {
    ...
    boost::iterator_range<std::string::const_iterator> it =
        boost::algorithm::find_last(plugId, userIdLower);
    if (it) {
      if ((it.end() == plugId.end()) && (*(it.begin() - 1) == '.')) {
  ...
  if (results.size() > 1) {
    ambiguousResults.insert(ambiguousResults.begin(), results.begin(),
                            results.end());
  }
  ...
  if (ambiguousResults.size() > 1) {
    ...
    BOOST_THROW_EXCEPTION(tuttle::exception::Value() << userMsg);
  }

  BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                        << tuttle::exception::user() +
                               "Unrecognized node name \"" + userId + "\".");
}

typedef std::pair<ttl::ofx::attribute::OfxhClipImage *, std::string>
ClipAndConnection;

void setParametersForNode(const std::vector<std::string> parameters,
  ...
  BOOST_FOREACH(const std::string & paramStr, parameters) {
    ...
    if (!boost::regex_match(paramStr.c_str(), matches, re_param)) {
      BOOST_THROW_EXCEPTION(tuttle::exception::Value()
                            << tuttle::exception::user() +
                                   "Parameter can't be parsed \"" + paramStr +
                                   "\".");
    }
    if (matches.size() != 3) {
      // should never happen
    ...
    if (attributeName.size()) {
      ...
      BOOST_THROW_EXCEPTION(
          ...
          << tuttle::exception::user() +
                 "Non-keyword parameter after keyword parameter. \"" +
                 paramStr + "\".");
    }
    ...
    if (attributeName.size()) {
      ...
      if (param == NULL && clip == NULL) {
        ...
        // if( acceptPartialName ) // if sam-do accept partial names
        {
          ...
          BOOST_FOREACH(OfxhParamSet::ParamMap::value_type & p,
                        currentNode.getParamSet().getParamsByScriptName()) {
          ...
          BOOST_FOREACH(OfxhClipImageSet::ClipImageMap::value_type & c,
                        currentNode.getClipImageSet().getClipsByName()) {
          ...
          if (paramMatches.size() + clipMatches.size() > 1) {
            ...
            matches.insert(matches.begin(), paramMatches.begin(),
                           paramMatches.end());
            matches.insert(matches.end(), clipMatches.begin(),
                           clipMatches.end());
            BOOST_THROW_EXCEPTION(ttl::exception::Value()
                                  << ttl::exception::user() +
                                         "Ambiguous partial attribute name \"" +
                                         attributeName +
                                         "\". Possible values are: " +
      ...
      if (param != NULL) {
        ...
        BOOST_THROW_EXCEPTION(ttl::exception::Value()
                              << ttl::exception::user() +
                                     "Parameter or clip name " +
                                     tuttle::quotes(attributeName) +
                                     " not found.");
      }
    } else {
...
void connectClips(const std::vector<ttl::Graph::Node *> nodes,
                  const std::vector<ClipAndConnection> clipsToConnect,
                  ttl::Graph::Node &currentNode, ttl::Graph &graph,
  ...
  if (nodes.size() > 0) // if not the first node
  {
    if (clipsToConnect.size() == 0) {
      ...
      /// @todo We only check if we have more than one clip (assuming that it's
      /// the default "Output" clip...)
      ...
      BOOST_FOREACH(const ClipAndConnection & clip, clipsToConnect) {
        // TUTTLE_TCOUT_VAR3( clip.second, currentNode.getName(),
        // clip.first->getName() );

        if (clip.second.size() <= 1 &&
            (clip.second == " " || clip.second == "!" || clip.second == "/" ||
             clip.second == "-")) {
        ...
        try {
          ...
          const int relativeIndex =
              std::abs(boost::lexical_cast<int>(clip.second));
          ...
          if (absIndex < 0 || absIndex >= (int)nodes.size()) {
            ...
            BOOST_THROW_EXCEPTION(
                exception::Value()
                << exception::user() + "The relative index \"" +
                       -relativeIndex + "\" for the connection of the clip " +
                       quotes(clip.first->getName()) + " on node " +
                       quotes(currentNode.getName()) + " is not valid.");
          }
          graph.connect(*nodes[absIndex],
                        currentNode.getAttribute(clip.first->getName()));
        }
        catch (...) {
          // It's not an index so we assume, it's the name/id of the clip.
          // If the node doesn't exist it will throw an exception.
          try {
            graph.connect(graph.getNode(clip.second),
                          currentNode.getAttribute(clip.first->getName()));
          }
          catch (...) {
            using namespace ttl;
            ...
            BOOST_THROW_EXCEPTION(exception::Failed()
                                  << exception::user() +
                                         "unable to connect clip " +
                                         quotes(clip.first->getName()) +
                                         ", with the id " +
                                         quotes(clip.second) +
                                         ". Possible id's are: " +
                                         boost::algorithm::join(idNames, ", "));
          }
        }
      }
    }
