...
getStringValues(const tuttle::host::ofx::property::OfxhProperty &prop) {
 ...
 */
std::string
getFormattedStringValue(const tuttle::host::ofx::property::OfxhProperty &prop) {
...
void coutProperties(const ttl::Graph::Node &node) {
  ...
  BOOST_FOREACH(ttl::ofx::property::PropertyMap::const_reference clip,
                props.getMap()) {
    TUTTLE_COUT(
        "\t" << color->_green << clip.first << " " << color->_red
             << (clip.second->getDimension() > 1
                     ? (std::string(" x") + boost::lexical_cast<std::string>(
                                                clip.second->getDimension()))
                     : "") << color->_std);
  }
}
...
void coutClipsWithDetails(const ttl::Graph::Node &node) {
  ...
  BOOST_FOREACH(const ttl::ofx::attribute::OfxhClipImage & clip,
                clips.getClipsByOrder()) {
    std::stringstream stream;

    stream << "\t" << color->_green << std::left << std::setw(25)
           << clip.getName() << ": " << color->_std;
...
void coutParameterValues(std::ostream &os,
  ...
  if (param.getProperties().hasProperty(kOfxParamPropChoiceOption)) {
    const OfxhProperty &prop =
        param.getProperties().fetchProperty(kOfxParamPropChoiceOption);
    ...
    if (!param.getProperties().hasProperty(kOfxParamPropDisplayMin)) {
      ...
      //			const OfxhProperty& propMin =
      //param.getProperties().fetchProperty( kOfxParamPropMin );
      //			const std::string minValue =
      //getFormattedStringValue( propMin );
      const OfxhProperty &propDisplayMin =
          param.getProperties().fetchProperty(kOfxParamPropDisplayMin);
      const std::string displayMinValue =
          getFormattedStringValue(propDisplayMin);
      ...
      const OfxhProperty &propDisplayMax =
          param.getProperties().fetchProperty(kOfxParamPropDisplayMax);
      const std::string displayMaxValue =
          getFormattedStringValue(propDisplayMax);
      //			const OfxhProperty& propMax =
      //param.getProperties().fetchProperty( kOfxParamPropMax );
      //			const std::string maxValue =
      //getFormattedStringValue( propMax );
...
void coutParametersWithDetails( const ttl::Graph::Node& node )
{
        ...
        BOOST_FOREACH( const ttl::ofx::attribute::OfxhParam& param,
params.getParamVector() )
        {
        if( param.getSecret() )
        ...
        TUTTLE_LOG_INFO(
                ...
                color->_red <<
                param.getParamType() <<
                (param.getSize() > 1 ? (std::string(" x") +
boost::lexical_cast<std::string>(param.getSize())) : "") <<
                color->_std
                );
...
void coutParameterWithDetails(const ttl::ofx::attribute::OfxhParam &param) {
  ...
  if ((type == kOfxParamTypeGroup) || (type == kOfxParamTypePushButton) ||
  ...
  if (param.getProperties().hasProperty(kOfxParamPropDefault)) {
    ...
    if (prop.getType() == ePropTypeInt) {
      choiceDefaultIndexValue =
          param.getProperties().getIntProperty(kOfxParamPropDefault, 0);
    }
  }
  ...
  if (type == kOfxParamTypeChoice) {
    ...
    if (param.getProperties().hasProperty(kOfxParamPropChoiceLabelOption)) {
      const OfxhProperty &prop =
          param.getProperties().fetchProperty(kOfxParamPropChoiceLabelOption);
    ...
    if (choiceValues.size()) // if it's a choice, we take the nth argument
    {
      ...
      std::stringstream stream;
      stream << "\t" << color->_green << std::left << std::setw(25)
             << param.getScriptName() << ": " << color->_std << std::setw(15)
             << typeName << color->_std;
    ...
    std::stringstream stream;
    const std::string stringDefaultValue =
        boost::algorithm::join(defaultValues, ",");
    stream << "\t" << color->_green << std::left << std::setw(25)
           << param.getScriptName() << ": " << color->_std << std::setw(15)
           << typeName << color->_yellow;
    ...
    if (param.getProperties().hasProperty(kOfxParamPropDisplayMin)) {
      ...
      const OfxhProperty &propDisplayMax =
          param.getProperties().fetchProperty(kOfxParamPropDisplayMax);
      const std::string minDisplayValue =
          getFormattedStringValue(propDisplayMin);
      const std::string maxDisplayValue =
          getFormattedStringValue(propDisplayMax);
  ...
  if (param.getProperties().hasProperty(kOfxParamPropHint)) {
    const std::string &hint =
        param.getProperties().getStringProperty(kOfxParamPropHint);
    if (hint.size()) {
...
void coutParametersWithDetails(const ttl::Graph::Node &node) {
  ...
  BOOST_FOREACH(const ttl::ofx::attribute::OfxhParam & param,
    ...
    // TUTTLE_LOG_TRACE( color->_green << "[ " << strParamsContexts << " ]" <<
    // color->_std );
  }
}
