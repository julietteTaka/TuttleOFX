...
void copy_sequence(const sequenceParser::Sequence &s,
                   ...
                   const sequenceParser::Sequence &d, int offset = 0) {
  ...
  for (sequenceParser::Time t = begin; (offset > 0) ? (t >= end) : (t <= end);
       t += step) {
...
void copy_sequence(const sequenceParser::Sequence &s,
                   const sequenceParser::Time firstImage,
                   const sequenceParser::Time lastImage, const bfs::path &d,
                   const sequenceParser::Time offset = 0) {
...
void copy_sequence(const sequenceParser::Sequence &s, const bfs::path &d,
                   const sequenceParser::Time offset = 0) {
  copy_sequence(s, s.getFirstTime(), s.getLastTime(), d, offset);
}
...
int sammvcp(int argc, char **argv) {
  ...
  mainOptions.add_options()(kHelpOptionString, kHelpOptionMessage)(
      ...
      //		( "force,f"     , bpo::value<bool>( )        , "if a
      //destination file exists, replace it" )
      (kVerboseOptionString,
       ...
       kVerboseOptionMessage)(kQuietOptionString, kQuietOptionMessage)(
          kInputFirstOptionString, bpo::value<std::ssize_t>(),
          kInputFirstOptionMessage)(kInputLastOptionString,
                                    bpo::value<std::ssize_t>(),
                                    kInputLastOptionMessage)(
          kOutputFirstOptionString, bpo::value<std::ssize_t>(),
          kOutputFirstOptionMessage)(
          kOutputLastOptionString, bpo::value<std::ssize_t>(),
          kOutputLastOptionMessage)(kColorOptionString, kColorOptionMessage)(
          kBriefOptionString, kBriefOptionMessage);

  // describe hidden options
  bpo::options_description hidden;
  hidden.add_options()(
      kInputOptionString, bpo::value<std::vector<std::string> >(),
      kInputOptionMessage)(kEnableColorOptionString, bpo::value<std::string>(),
...
#ifndef SAM_MOVEFILES
    TUTTLE_COUT(color->_green << "copy sequence(s) in a directory"
                              << color->_std);
#else
    TUTTLE_COUT(color->_green << "move sequence(s) in a directory"
                              << color->_std);
#endif
    return 0;
  }
  ...
  if (vm.count(kHelpOptionLongName) || isPathSizeTooSmall) {
    ...
    TUTTLE_COUT(color->_green << "\tsam-cp - copy sequence(s) in a directory"
                              << color->_std);
    TUTTLE_COUT("");
...
#else
    TUTTLE_COUT(color->_blue << "NAME" << color->_std);
    TUTTLE_COUT(color->_green << "\tsam-mv - move sequence(s) in a directory"
                              << color->_std);
    TUTTLE_COUT("");
    TUTTLE_COUT(color->_blue << "SYNOPSIS" << color->_std);
    TUTTLE_COUT(
    ...
    TUTTLE_COUT("");
#ifndef SAM_MOVEFILES
    TUTTLE_COUT("Copy sequence of image files, and could remove trees (folder, "
                "files and sequences).");
#else
    TUTTLE_COUT("Move sequence of image files, and could remove trees (folder, "
                "files and sequences).");
#endif
    TUTTLE_COUT("");
    ...
    SAM_EXAMPLE_LINE_COUT(
        "", "sam-cp /path/to/sequence/seq.@.jpg  /path/to/sequence_copy/");
    SAM_EXAMPLE_LINE_COUT("", "sam-cp /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequences_copy/seq.@.jpg");
    SAM_EXAMPLE_TITLE_COUT("Copy and rename a sequence: ");
    SAM_EXAMPLE_LINE_COUT("", "sam-cp /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequence_copy/seq_copy.@.jpg ");
    SAM_EXAMPLE_TITLE_COUT("Copy a part of sequence: ");
    SAM_EXAMPLE_LINE_COUT("", "sam-cp /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequence_copy/ --input-first 677837 "
                              "--input-last 677838");
    SAM_EXAMPLE_TITLE_COUT("Renumber a sequence: ");
    SAM_EXAMPLE_LINE_COUT("", "sam-cp /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequence_copy/ --output-first 0");
#else
    SAM_EXAMPLE_TITLE_COUT("Move a sequence: ");
    SAM_EXAMPLE_LINE_COUT(
        "", "sam-mv /path/to/sequence/seq.@.jpg  /path/to/sequence_move/");
    SAM_EXAMPLE_LINE_COUT("", "sam-mv /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequences_move/seq.@.jpg");
    SAM_EXAMPLE_TITLE_COUT("Move and rename a sequence:");
    SAM_EXAMPLE_LINE_COUT("", "sam-mv /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequence_move/seq_move.@.jpg");
    SAM_EXAMPLE_TITLE_COUT("Move a part of sequence:");
    SAM_EXAMPLE_LINE_COUT("", "sam-mv /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequence_move/ --input-first 677837 "
                              "--input-last 677838");
    SAM_EXAMPLE_TITLE_COUT("Renumber a sequence:");
    SAM_EXAMPLE_LINE_COUT("", "sam-mv /path/to/sequence/seq.@.jpg  "
                              "/path/to/sequence_move/ --output-first 0");
  ...
  if (vm.count(kOutputFirstOptionLongName)) {
    ...
    if (offsetMode != eOffsetModeNotSet) {
      TUTTLE_LOG_ERROR(
          "You can't cumulate multiple options to modify the time.");
      return 255;
    }
  ...
  if (!bfs::is_directory(dstPath)) {
    ...
    if (!dstPath.empty() && !bfs::is_directory(dstPath)) {
      TUTTLE_LOG_ERROR("Your destination is not in a valid directory: "
                       << tuttle::quotes(dstPath.string()) << ".");
      return 255;
    }
  } else {
    if (paths.size() > 1) {
      TUTTLE_LOG_ERROR(
          "To copy multiple sequences, your destination must be a directory: "
          << tuttle::quotes(dstPath.string()) << ".");
      return 255;
    }
    sequencePattern = "";
  ...
  if (sequencePattern.size() > 0) {
    dstIsSeq = dstSeq.initFromPattern(dstPath, sequencePattern, 0, 0, 1,
                                      descriptionMask,
                                      sequenceParser::Sequence::ePatternAll);
    if (!dstIsSeq) // there is a pattern, but it's not valid.
    {
      TUTTLE_LOG_ERROR("Your destination "
                       << tuttle::quotes(sequencePattern)
                       << " is not a valid pattern. Your destination can be a "
  ...
  try {
    BOOST_FOREACH(const bfs::path & srcPath, paths) {
      ...
      const bool srcIsSeq = srcSeq.initFromDetection(
          srcPath.string(), sequenceParser::Sequence::ePatternDefault);
      if (!srcIsSeq) {
        TUTTLE_LOG_ERROR(color->_error << "Input is not a sequence: "
                                       << tuttle::quotes(srcPath.string())
                                       << "." << color->_std);
        return 255;
      }
      if (srcSeq.getNbFiles() == 0) {
        TUTTLE_LOG_ERROR(
            color->_error << "No existing file for the input sequence: "
                          << tuttle::quotes(srcPath.string()) << "."
                          << color->_std);
        return 255;
      }

      sequenceParser::Time first =
          hasInputFirst ? inputFirst : srcSeq.getFirstTime();
      sequenceParser::Time last =
          hasInputLast ? inputLast : srcSeq.getLastTime();
      switch (offsetMode) {
      ...
      if (dstIsSeq) {
        TUTTLE_LOG_TRACE(srcSeq.getAbsoluteStandardPattern()
                         << " -> " << dstSeq.getAbsoluteStandardPattern()
                         << " (" << srcSeq.getNbFiles() << ") ");
        copy_sequence(srcSeq, first, last, dstSeq, offset);
      } else {
        TUTTLE_LOG_TRACE(srcSeq.getAbsoluteStandardPattern()
                         << " -> " << dstPath / srcSeq.getStandardPattern()
                         << " (" << srcSeq.getNbFiles() << ")");
