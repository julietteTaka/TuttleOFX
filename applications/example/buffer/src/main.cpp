...
void sam_terminate(void) {
  TUTTLE_LOG_ERROR(
      "[buffer example] Sorry, Sam has encountered a fatal error.");
...
void sam_unexpected(void) {
  ...
  BOOST_THROW_EXCEPTION(
      std::runtime_error("Sorry, Sam has encountered an unexpected "
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    TUTTLE_LOG_VAR(TUTTLE_INFO,
                   bitdepth.getParam("outputBitDepth").getStringValue());
