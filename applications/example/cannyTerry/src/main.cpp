...
int main(int argc, char **argv) {
  ...
  try {
    ...
    TUTTLE_LOG_INFO("input - x:" << imgView.width()
                                 << ", y:" << imgView.height());
    ...
    terry::filter::canny<std::allocator>(
        imgView, view(tmpImgRgb), view(tmpImgGray), view(imgCanny),
        point2<double>(1, 1), terry::filter::convolve_option_extend_zero, 0.025,
        0.1);
    TUTTLE_LOG_INFO("full canny time: " << t.elapsed());

    boost::gil::png_write_view(
        "data/terry/output_canny_terry.png",
        color_converted_view<rgb8_pixel_t>(view(imgCanny)));
  }
  catch (...) {
